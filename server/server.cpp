...
#define IN_EVENTS_LOG(id) { fout < "\t\t\t> [" <= ID < ":"  <= (GLOBAL_CLOCK() >> 8) < ":" <=  frame < "]" < #id < "\n"; }
#define IN_EVENTS_LOG1(id,code) { fout < "\t\t\t> [" <= ID < ":"  <= (GLOBAL_CLOCK() >> 8) < ":" <=  frame < "]" < #id < ": " <= code < "\n"; }
#define OUT_EVENTS_LOG(id) { fout < "< [" <= ID < ":"  <= (GLOBAL_CLOCK() >> 8) < ":" <=  frame < "]" < #id < "\n"; }
#define OUT_EVENTS_LOG1(id,code) { fout < "< [" <= ID < ":"  <= (GLOBAL_CLOCK() >> 8) < ":" <=  frame < "]" < #id < ": " <= code < "\n"; }
#else
...
//		player -> server -> add_rating_data(player -> name,player -> password,data.GameType,player -> rating);
...
void Game::get_total_list_of_players_data(OutputEventBuffer& out_buffer)
{
	...
	Player* p = players.first();
	while(p){
		out_buffer < (unsigned char)(p -> ID) < (unsigned char)p -> status < (unsigned char)(p -> world ? p -> world -> ID : 0) < short(p -> x) < short(p -> y) < p -> name < char(0);
...
void Game::process_MECHOSOMA_ratings()
{
	...
	double total_account = 2.*(double)(data.Mechosoma.ProductQuantity1 + data.Mechosoma.ProductQuantity2)*counter;
...
void World::process_set_position(Player* player)
{
	...
	if(player -> y_half_size_of_screen_prev){	// not a first set_position
		...
		if(getDistY(player -> y,player -> y_prev) > 0){
			update_y0 = ((player -> y_prev + player -> y_half_size_of_screen_prev) & (V_SIZE - 1)) >> Y_SHIFT;
			update_y1 = ((player -> y + player -> y_half_size_of_screen) & (V_SIZE - 1)) >> Y_SHIFT;
			hide_y0 = ((player -> y_prev - player -> y_half_size_of_screen_prev) & (V_SIZE - 1)) >> Y_SHIFT;
			hide_y1 = ((player -> y - player -> y_half_size_of_screen) & (V_SIZE - 1)) >> Y_SHIFT;
			}
		else{
			update_y1 = ((player -> y_prev - player -> y_half_size_of_screen_prev) & (V_SIZE - 1)) >> Y_SHIFT;
			update_y0 = ((player -> y - player -> y_half_size_of_screen) & (V_SIZE - 1)) >> Y_SHIFT;
			hide_y1 = ((player -> y_prev + player -> y_half_size_of_screen_prev) & (V_SIZE - 1)) >> Y_SHIFT;
	...
	while((code = in_buffer.current_event()) != 0){
		...
		switch(code){
				...
				if(GET_OBJECT_TYPE(obj_ID) == NID_VANGER){
					...
					if(!PLAYERS_OBJECT(obj_ID))
						...
						if(NON_GLOBAL_OBJECT(obj_ID))
							...
							game -> process_create_globals(!(in_buffer.current_event() & ECHO_EVENT) ? this : 0, obj);
						}
					}
				...
				if(GET_OBJECT_TYPE(obj_ID) == NID_VANGER){
					...
					if(!PLAYERS_OBJECT(obj_ID))
						...
						if(NON_GLOBAL_OBJECT(obj_ID))
							...
							game -> process_update_globals(!(in_buffer.current_event() & ECHO_EVENT) ? this : 0, obj);
						}
					}
			...
			case ATTACH_TO_GAME: 
				...
				if((game = server -> games.search(in_buffer.get_int())) == 0 || (game -> data.GameType == UNCONFIGURED && game -> players.size()) || game -> used_players_IDs == 0x7fffffff)
				...
				out_buffer < game -> ID < char(game -> data.GameType != UNCONFIGURED ? 1 : 0) < (unsigned int)round((double)game -> birth_time*(256./1000.)) < (unsigned char)ID;
			...
			case SET_PLAYER_DATA: {
				...
				switch(game -> data.GameType){
					case VAN_WAR:
						if(body.kills != prev_body.kills || body.deaths != prev_body.deaths || body.rating != prev_body.rating){
		...
		if(!code_queue.empty()){
			...
			switch(code){
				...
				case PLAYERS_WORLD: 
					...
					out_buffer < (unsigned char)player -> ID < (unsigned char)(player -> world ? player -> world -> ID : 0);
					out_buffer.end_event();
...
Server::Server(int main_port,int broadcast_port,int time_to_live)
{
	...
	if(StatLogging)
	 ...
	 for(int i = 0;i < NUMBER_MP_GAMES;i++)
		n_games[i] = n_players_max[i] =	n_players_sum[i] = playing_time_max[i] = playing_time_sum[i] = 0;
}
	...
	if(StatLogging){
		...
		while(p){
			...
			if(IP)
				...
				< " min.; Rating: " <= round(p -> body.rating) < "\n";
			p = p -> next;
			}
		stat_log < "\n";
		}
...
void Server::get_games_list(OutputEventBuffer& out_buffer,int client_version)
{
	...
	Game* g = games.first();
	while(g){
		if(g -> data.GameType != UNCONFIGURED && g -> used_players_IDs != 0x7fffffff && g -> client_version == client_version)
			num++;
		g = g -> next;
		}
	...
	while(g){
		if(g -> data.GameType != UNCONFIGURED && g -> used_players_IDs != 0x7fffffff && g -> client_version == client_version){
			...
			#else
				 < (g -> data.GameType == VAN_WAR ? "‚" : (g -> data.GameType == MECHOSOMA ? "Œ" : ""));
			#endif
...
//		fout < p -> name < "\t" < p -> password  < "\t" <= p -> MP_game < "\t" <= p -> rating < "\n";
		p = p -> next;
		}
//	fout < "\n";
}
...
RatingData* Server::search_rating_data(char* player_name, char* player_password, int MP_game)
{
	RatingData* p = rating_list.first();
	while(p){
		if(!strcmp(player_name,p -> name) && !strcmp(player_password,p -> password) && MP_game == p -> MP_game)
			return p;
		p = p -> next;
		}
	return p;
...
void Server::add_rating_data(Player* player, int MP_game)
{
	...
	if(fabs(player -> body.rating - player -> prev_rating) < 0.01 && (p || player -> body.rating < rating_threshoulds[MP_game]))
		return;
