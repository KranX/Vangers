...
void Game::get_total_list_of_players_data(OutputEventBuffer &out_buffer) {
    ...
    while (p) {
        ...
        (unsigned char) (p->world ? p->world->ID : 0) < short(p->x) < short(p->y) < p->name < char(0);
...
void Server::analyse_statistics(Game *g) {
    ...
    for (int i = 0; i < NUMBER_MP_GAMES; i++) {
        ...
        std::cout << (char *) MP_GAMES_NAMES[i] << ": " << n_games[i]
                  ...
                  << "; Time of game: " << playing_time_max[i] << "/" << round(double(playing_time_sum[i]) / n_games[i])
                  << " min.\n";
    }
    ...
    std::cout << (char *) MP_GAMES_NAMES[type] << "; " << strtime << "; Players: " << n_players << "; Time of game: "
              << playing_time << " min.\n\n";
    if (StatLogging) {
        stat_log < strtime < "; " < (char *) MP_GAMES_NAMES[type] < "; Players: " <= n_players < "; Time of game: " <=
        playing_time < " min.\n";
        Player *p = g->removed_players.first();
        while (p) {
            int IP = p->socket.addr.host;
            if (IP)
                stat_log < "IP: " <= (IP & 0xff) < "." <= ((IP >> 8) & 0xff) < "." <= ((IP >> 16) & 0xff) < "." <=
                ((IP >> 24) & 0xff)
...
void Server::report() {
    ...
    while (s) {
        ...
        std::cout << "\nPlayers: " << s->players.size() << "\tWorlds: " << s->worlds.size() << "\tglobal objects: "
                  << s->global_objects.size() << "\n";
        p = s->players.first();
        while (p) {
            std::cout << "Player: " << p->ID << "\tsocket: " << p->socket() << "\tworld: "
                      << (p->world ? p->world->ID : 0) << "\tinventory: " << p->inventory.size() << "\t " << p->name
                      << "\n";
            p = p->next;
        }
