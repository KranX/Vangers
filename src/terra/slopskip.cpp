...
void SlopTurnSkip(
	...
	int y0, y1, y2, y3;
#define uv2y(u, v) \
	(((Auy * (u) + Avy * (v) + Awy * F) >> 8) * H / ((Auz * (u) + Avz * (v) + Awz * F) >> 8))
	...
	// #ifndef TURN_TEST
	//  #ifndef _VTEST_
	//  	vMap -> request(MIN(MIN(MIN(y0,y1),y2),y3) - MAX_RADIUS/2, MAX(MAX(MAX(y0,y1),y2),y3) +
	//  MAX_RADIUS/2, 0, 0); 	uchar** lt = vMap -> lineTcolor; #else 	uchar** lt = lineTcolor;
	//  #endif #else 	uchar** lt = TextureDataTable; 	uchar** ht = HeightDataTable; #endif
	int first_line = pre_calc_fy(0, XDstSize, YDstSize, Awy, Awz, Auy, Avy, Avz, F, H, cy);
	int last_line =
		pre_calc_fy(YDstSize - 1, XDstSize, YDstSize, Awy, Awz, Auy, Avy, Avz, F, H, cy);
	...
	Ymax += 30;
#endif
	vMap->request(Ymin - MAX_RADIUS / 2, Ymax + MAX_RADIUS / 2, 0, 0);
	...
	if (!z)
		z = 1;
	int ky =
		((ParaMapSkipFactor0 << 16) + abs(sinAlpha) * ParaMapSkipFactor) / 100 * CenterDistance / F;
