...
void RenderPrepare(void) {
	...
	for (ind = 0; ind < TERRAIN_MAX; ind++) {
		...
		if (!ind) {
			...
			for (j = 0; j < 256 - d; j++) {
				v = round(
					(double)j * 1.25 * (double)colnum / (255.0 - (double)d) - 0.25 * (double)colnum
				);
				if (v < 0)
					v = 0;
	...
	if (dh) {
		...
		for (i = r + 1, dd = 1.0 - d; i <= rad; i++, dd -= d) {
			...
			case 0:
				...
				for (s = ds, k = 0, j = locp % max; k < max;
					j = (j + 1 == max) ? 0 : j + 1, k++, s += ds)
					if (s >= 1.0) {
									...
									for (dy = -1; dy <= 1; dy++)
										for (dx = -1; dx <= 1; dx++) {
											...
											if (pa_) {
												...
												if (abs(dx) + abs(dy) == 2)
													v +=
														GET_DOWN_ALT(pa_ + H_SIZE, *pa_, pa0_, cx_);
											}
										}
									v >>= 2;
									break;
...
void pixSet(int x, int y, int delta, int surf) {
...
#ifdef _SURMAP_
	if (surf)
		if (!ActiveTerrain[GET_REAL_TERRAIN(pf, x)])
			return;
	if (h < MiniLevel || h > MaxiLevel)
		return;
	if (MosaicToggle && mosaicBMP.mode <= 1)
		delta = mosaicBMP.getDelta(x, y, delta);
#endif
	...
	if (IS_DOUBLE(*pf))
		if (x & 1) {
			h += delta;
			if ((int)*(pa - 1) +
					(((GET_DELTA(*(pf - 1)) << 2) + GET_DELTA(*pf) + 1) << DELTA_SHIFT) >=
				h) {
				*pf &= ~DOUBLE_LEVEL;
				*(pf - 1) &= ~DOUBLE_LEVEL;
#ifdef _SURMAP_
	...
	*pa = h;
	...
	if (MosaicToggle && mosaicBMP.mode == 2) {
		...
		if (t >= 0)
			SET_REAL_TERRAIN(pf, (t % TERRAIN_MAX) << TERRAIN_OFFSET, x);
	} else if (surf)
		if (CurrentTerrain != -1)
			SET_REAL_TERRAIN(pf, CurrentTerrain << TERRAIN_OFFSET, x);
#endif
	...
	if (h < FloodLvl[y >> WPART_POWER]) {
		...
		int xx = XCYCL(x - 1);
		if (GET_REAL_TERRAIN_TYPE(pfN + xx, xx) == WATER_TERRAIN) {
			SET_REAL_TERRAIN(pf, WATER_TERRAIN, x);
			return;
		}
		xx = XCYCL(x + 1);
		if (GET_REAL_TERRAIN_TYPE(pfN + xx, xx) == WATER_TERRAIN) {
		...
		if (pfN) {
			pfN += H_SIZE + x;
			if (GET_REAL_TERRAIN_TYPE(pfN, x) == WATER_TERRAIN) {
				SET_REAL_TERRAIN(pf, WATER_TERRAIN, x);
				return;
			}
		}
	...
	if (IS_DOUBLE(*pf)) {
		if (x & 1) {
			...
			if (rx_h - 3 >= h)
				*pf |= *(pf - 1) |= SHADOW_MASK;
			if (*pf & SHADOW_MASK) {
				*pc = *(pc - 1) = palCLR[types]
										[256 + ((lightCLR[types][255 - (c_h - rx_h)] -
													((255 - c_h) >> H_CORRECTION)) >>
												   1)];
			} else
				*pc = *(pc - 1) = palCLR[types]
										[256 + (lightCLR[types][255 - (c_h - rx_h)] -
			...
			if (*pf & SHADOW_MASK) {
				*pc = *(pc - 1) = palCLR[types]
										[256 + ((lightCLR[types][255 - (lx_h - c_h)] -
													((255 - lx_h) >> H_CORRECTION)) >>
												   1)];
			} else
				*pc = *(pc - 1) = palCLR[types]
										[256 + (lightCLR[types][255 - (lx_h - c_h)] -
		...
		if (*pf & SHADOW_MASK) {
			*pc =
				palCLR[types]
					  [256 +
						  ((lightCLR[types][255 - (lx_h - rx_h)] - ((255 - c_h) >> H_CORRECTION)) >>
							  1)];
		} else {
			*pc = palCLR[types]
						[256 +
		...
		if (*pf & SHADOW_MASK) {
			*pc = palCLR[types]
						[256 + ((lightCLR[types][255 - (llx_h - c_h)] -
									((255 - lx_h) >> H_CORRECTION)) >>
								   1)];
		} else
			*pc = palCLR[types]
...
void pixDownSet(int x, int y, int delta, int surf) {
...
#ifdef _SURMAP_
	if (surf)
		if (!ActiveTerrain[GET_REAL_DOWNTERRAIN(pf, x)])
			return;
	if (h < MiniLevel || h > MaxiLevel)
		return;
	if (MosaicToggle && mosaicBMP.mode <= 1)
	...
	*pa = h;
	...
	if (MosaicToggle && mosaicBMP.mode == 2) {
		int t = mosaicBMP.getType(x, y);
		if (t >= 0)
			SET_REAL_DOWNTERRAIN(pf, (t % TERRAIN_MAX) << TERRAIN_OFFSET, x);
	} else if (surf)
		if (CurrentTerrain != -1)
			SET_REAL_DOWNTERRAIN(pf, CurrentTerrain << TERRAIN_OFFSET, x);
#endif
#ifndef TERRAIN16
	if (h < FloodLvl[y >> WPART_POWER]) {
		uchar *pfN = pa0 + H_SIZE;
		int xx = XCYCL(x - 1);
		if (GET_REAL_DOWNTERRAIN_TYPE(pfN + xx, xx) == WATER_TERRAIN) {
			SET_REAL_DOWNTERRAIN(pf, WATER_TERRAIN, x);
			return;
		}
		xx = XCYCL(x + 1);
		if (GET_REAL_DOWNTERRAIN_TYPE(pfN + xx, xx) == WATER_TERRAIN) {
		...
		if (pfN) {
			pfN += H_SIZE + x;
			if (GET_REAL_DOWNTERRAIN_TYPE(pfN, x) == WATER_TERRAIN) {
				SET_REAL_DOWNTERRAIN(pf, WATER_TERRAIN, x);
				return;
			}
		}
