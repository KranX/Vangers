...
const uchar TERRAIN_MASK = (1 << TERRAIN_OFFSET) | (1 << (TERRAIN_OFFSET + 1)) |
						   (1 << (TERRAIN_OFFSET + 2)) | (1 << (TERRAIN_OFFSET + 3));
#else
const int TERRAIN_MAX = 8;
const uchar TERRAIN_OFFSET = 3;
const uchar TERRAIN_MASK =
	(1 << TERRAIN_OFFSET) | (1 << (TERRAIN_OFFSET + 1)) | (1 << (TERRAIN_OFFSET + 2));
#endif

#define GET_DELTA(a) ((a) & DELTA_MASK)

#define SET_DELTA(a, b)           \
	{                             \
		a &= ~DELTA_MASK, a += b; \
	}
#define SET_TERRAIN(a, b)           \
	{                               \
		a &= ~TERRAIN_MASK, a += b; \
	}
