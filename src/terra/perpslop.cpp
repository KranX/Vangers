...
int PerpSlopTurn(
	...
	// std::cout<<"PerpSlopTurn Turn:"<<Turn<<" Slop:"<<Slop<<" H:"<<H<<" cx:"<<cx<<" cy:"<<cy<<"
...
#define uv2y(u, v) \
	...
	// MAX_RADIUS/2,0,0); 	uchar** lt = vMap -> lineTcolor; #else 	uchar** lt = lineTcolor; #endif
	// #else
	...
	if (!z)
		z = 1;
	int ky =
		((PerpMapSkipFactor0 << 16) + abs(cosAlpha) * PerpMapSkipFactor) / 100 * CenterDistance / F;
#ifdef TURN_TEST
	::MapSkipFactor = ky;
#endif
