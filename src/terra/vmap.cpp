...
extern int *SI;
extern int *CO;
extern int RestoreLog, CGenLog, MobilityLog, InitLog, SSSLog, ROLog, EncodeLog, DirectLog, ShotLog,
	GlassLog, CutvmpLog, WHLog, ExclusiveLog;
...
void YSetup(void) {
	...
	// std::cout<<"YSetup - V_POWER:"<<V_POWER<<" V_SIZE/map_size_y:"<<V_SIZE<<"
	// map_size_x:"<<map_size_x<<std::endl;
...
vrtMap::vrtMap(void): fmap(0), kmap(0) {
	pFile = new PrmFile;
	cWorld = 0;
	inbuf = NULL;
	st_table = NULL;
...
void vrtMap::analyzeINI(const char *name) {
	...
	WPART_POWER =
		atoi(iniparser_getstring(dict_name, "Global Parameters:Section Size Power", NULL));
	MINSQUARE =
		1 << atoi(iniparser_getstring(dict_name, "Global Parameters:Minimal Square Power", NULL));

	if (strcmp(version, iniparser_getstring(dict_name, "Storage:Version", NULL)))
		ErrH.Abort("Incorrect Storage Version");
	isCompressed = atoi(iniparser_getstring(dict_name, "Storage:Compressed Format Using", NULL));
#ifdef _SURMAP_
	if (ForcedCompressed)
		isCompressed = 1;
	if (ForcedUncompressed)
		isCompressed = 0;
#endif
	...
	if ((!tmax && TERRAIN_MAX != 8) || (tmax && tmax != TERRAIN_MAX))
		ErrH.Abort("Incorrect Terrain Max");
...
#ifdef _SURMAP_
	scenarioName =
		strdup(iniparser_getstring_surmap(dict_name, secCreation, "Build Scenario File"));
	...
	DEFAULT_TERRAIN =
		atoi(iniparser_getstring_surmap(dict_name, secCreation, "Default Terrain Type"))
		<< TERRAIN_OFFSET;
#endif
	iniparser_freedict(dict_name);
}
...
void vrtMap::reload(int nWorld) {
	on = true;
	if (nWorld >= maxWorld || nWorld < 0)
		ErrH.Abort("World Index out of range");
#ifdef _SURMAP_
...
#	endif
	...
	if (!fmap.open(fname, XS_IN))
		...
		fmap.close();
	if (!exist)
		ErrH.Abort("Can't load world,XERR_USER,-1,fname");
	...
	if (KeepON) {
		...
		if (need) {
			...
			if(_chsize(h,V_SIZE*H2_SIZE)){
				...
				ErrH.Abort("Недостаточно места на жестком диске для работы в режиме сохранения всех
изменеий!"); #else ErrH.Abort("Can't create KEEP File. Please, free more disk space for this feature
working."); #endif
				}
			_close(h);*/
			kmap.open(kname, XS_OUT | XS_NOSHARING /*|XS_NOBUFFERING*/);
	...
	if (isCompressed) {
		...
		InitSplay(fmap);
	} else if (fmap.size() != (int)map_size_x * (int)map_size_y * 2)
		ErrH.Abort("Incorrect vmp-file size");
...
void vrtMap::dump_terrain() {
	std::cout << "Start dump terrain H2_SIZE:" << H2_SIZE << " H_SIZE:" << H_SIZE
			  << " V_SIZE:" << V_SIZE << std::endl;
...
void vrtMap::change(int up, int down) {
	...
	while (unlinkIndex < 4 || linkIndex < 4) {
		...
		BorderType borderType = linkOpened == unlinkOpened ? SKIP : (linkOpened ? LINK : UNLINK);
...
#ifdef DEBUG_MAP_REQUESTS
	std::cout << "map request [" << up << " .. " << down << "] current [" << upLine << " .. "
			  << downLine << "]" << std::endl;
#endif
		...
		for (int i = 2; i < borders.size(); ++i) {
			...
			case UNLINK: {
				...
				if (up <= down) {
					...
					std::cout << "unlink [" << up << " .. " << down
							  << "] free nodes before: " << freeMax;
#endif
					delink(up, down);
#ifdef DEBUG_MAP_REQUESTS
	...
	for (int i = 0; i < map_size_y; ++i) {
		...
		if (shouldExists != (lineTcolor[i] != nullptr)) {
			std::cout << "map error line [" << i << "] "
					  << (shouldExists ? " should be loaded " : " should be unloaded")
					  << " but NOT!" << std::endl;
			assert(false);
		}
	}
	...
	do {
		if (!lineTcolor[i]) {
			if (freeMax <= 1) {
				...
				return;
				//				XBuffer buf;
				//				buf < "up: " <= upLine < " down: " <= downLine < " size: " <=
				// getDelta(downLine,upLine); 				ErrH.Abort("Line Buffer
				// overflow",XERR_USER,-1,buf.GetBuf());
			}
			freeMax--;
			lineTcolor[i] = (p = use());
#ifdef SESSION
...
void LoadVPR(int ind) {
	...
	if (ff.size() != (int)(2 * 4 + (1 + 4 + 4) * 4 + 2 * net_size + 2 * POWER * 4 +
						   2 * PART_MAX * POWER * 4 + PART_MAX * 4))
		ErrH.Abort("Incorrect VPR size");
...
void vrtMap::scaling(int XSrcSize, int cx, int cy, int xc, int yc, int xside, int yside) {
	...
	request(
		MIN(y0, y1) - MAX_RADIUS / 2, MAX(y0, y1) + MAX_RADIUS / 2, MIN(x0, x1) - 4, MAX(x0, x1) + 4
	);
	...
	if (DepthShow)
		...
		// 			std::cout<<"DOUBLE_LEVEL:"<<(unsigned int)DOUBLE_LEVEL<<" OBJSHADOW:"<<(unsigned
		// int)OBJSHADOW<<std::endl;
		unsigned char *p;
		for (i = 0; i < ysize; i++) {
			fx = tfx;
			fy = tfy;
			...
			for (j = 0; j < xsize; j++, vp++) {
				p = data + XCYCL(fx >> 16);
				// 					std::cout<<"D:"<<(unsigned int)*vp<<" H_SIZE:"<<H_SIZE<<" *(p +
				// H_SIZE):"<<(unsigned int)*(p + H_SIZE)<<std::endl;
				switch (debug_view) {
				case 10:
					*vp = (*p >> 2);
					break;
				...
				case -1: // SHADOW Bits
					*vp =
						*(p + H_SIZE) & OBJSHADOW ? 255 : *(ltc[YCYCL(fy >> 16)] + XCYCL(fx >> 16));
					break;
				case -2: // DOUBLE LEVEL Bits
					*vp = *(p + H_SIZE) & DOUBLE_LEVEL ? 255
													   : *(ltc[YCYCL(fy >> 16)] + XCYCL(fx >> 16));
					break;
				case -3: //! DOUBLE LEVEL Bits
					*vp = !(*(p + H_SIZE) & DOUBLE_LEVEL)
							  ? 255
							  : *(ltc[YCYCL(fy >> 16)] + XCYCL(fx >> 16));
					break;
				case 0:
...
void vrtMap::turning(
	...
	// std::cout<<"vrtMap::turning XSrcSize:"<<XSrcSize<<" Turn:"<<Turn<<" cx:"<<cx<<" cy:"<<cy<<"
	// xc:"<<xc<<" yc:"<<yc<<" XDstSize:"<<XDstSize<<" YDstSize:"<<YDstSize<<std::endl;
...
#define SLOW_INT_TURNING
	...
	request(
		MIN(MIN(MIN(edge_y1, edge_y2), edge_y3), edge_y4) - MAX_RADIUS / 2,
		MAX(MAX(MAX(edge_y1, edge_y2), edge_y3), edge_y4) + MAX_RADIUS / 2,
		0,
		0
	);
	...
	request(
		MIN(MIN(MIN(y0, y1), y2), y3) - MAX_RADIUS / 2,
		MAX(MAX(MAX(y0, y1), y2), y3) + MAX_RADIUS / 2,
		0,
		0
	);
...
#	endif
	...
	//	  if(TestMode)
	//		  k_x =
	// int((double)XSrcSize/(double)cosTurn/(double)XDstSize*(double)0x10000*(double)0x10000); else
