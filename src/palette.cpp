...
void PalEvidence(char *tpal, char *pal) {
	...
	while (log) {
		...
		for (i = 0; i < 256; i++) {
			...
			tpal[3 * i + 1] += SPEED;
			tpal[3 * i + 2] += SPEED;
			if (tpal[3 * i + 0] >= pal[3 * i + 0])
				tpal[3 * i + 0] = pal[3 * i + 0];
			else
				log = 1;
			if (tpal[3 * i + 1] >= pal[3 * i + 1])
				tpal[3 * i + 1] = pal[3 * i + 1];
			else
				log = 1;
			if (tpal[3 * i + 2] >= pal[3 * i + 2])
				tpal[3 * i + 2] = pal[3 * i + 2];
			else
				log = 1;
		}
	...
	while (log) {
		...
		for (i = 0; i < 256; i++) {
			if (pal[3 * i + 0] <= SPEED)
				...
				pal[3 * i + 0] -= SPEED, log = 1;
			if (pal[3 * i + 1] <= SPEED)
				pal[3 * i + 1] = 0;
			else
				pal[3 * i + 1] -= SPEED, log = 1;
			if (pal[3 * i + 2] <= SPEED)
				pal[3 * i + 2] = 0;
			else
				pal[3 * i + 2] -= SPEED, log = 1;
		}
...
void PaletteTransform::set(uchar *_from, uchar *_to, int _n0, int _sz, int *_event) {
	...
	for (i = 0, j = 3 * n0; i < 3 * sz; i++, j++) {
		if (to[j] > from[j])
			delta[j] = 2 + ((int)to[j] - (int)from[j]) / 32;
		else if (to[j] < from[j])
			delta[j] = -2 - ((int)from[j] - (int)to[j]) / 32;
	}
	inProgress = 1;
}
...
void pal_iter0(int legacy_step) {
	if (PAL_WAVE_TERRAIN == -1 || PAL_WAVE_TERRAIN >= TERRAIN_MAX)
		return;
	const int DSIZE = 8;
	const int PRD = 64;
