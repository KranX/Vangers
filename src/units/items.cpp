...
static inline void accumulate_runtime_vector_step(
	const Vector &legacy_step,
	double &accum_x,
	double &accum_y,
	double &accum_z,
	Vector &out_step
...
void ItemsDispatcher::Init(Parser &in) {
	...
	for (j = 0; j < MAX_ITEM_TYPE; j++)
		StuffItemLink[j] = (StuffObject *)0x83838383;
	...
	if (!NetworkON) {
		for (i = 0; i < ProtoCryptTableSize[CurrentWorld]; i++) {
			if (ProtoCryptTable[CurrentWorld][i].Enable) {
				if (ProtoCryptTable[CurrentWorld][i].SensorType == 2 ||
					ProtoCryptTable[CurrentWorld][i].SensorType == 6) {
					for (j = 0; j < ITEM_LUCK_MAX; j++) {
						if ((int)(RND(100)) <= aiCutLuck) {
							if (!RND(8))
								uvsCreateNewItem(
									ProtoCryptTable[CurrentWorld][i].R_curr.x +
										(ITEM_LUCK_RADIUS * CO[RND(PI * 2)] >> 16),
									ProtoCryptTable[CurrentWorld][i].R_curr.y +
										(ITEM_LUCK_RADIUS * SI[RND(PI * 2)] >> 16),
									ProtoCryptTable[CurrentWorld][i].R_curr.z,
									uvsGenerateItemForCrypt(6),
									CurrentWorld
								);
							else
								uvsCreateNewItem(
									...
									ProtoCryptTable[CurrentWorld][i].R_curr.y +
										(ITEM_LUCK_RADIUS * SI[RND(PI * 2)] >> 16),
									ProtoCryptTable[CurrentWorld][i].R_curr.z,
									uvsGenerateItemForCrypt(2),
									CurrentWorld
								);
						};
					};
				} else {
					if (ProtoCryptTable[CurrentWorld][i].SensorType == 1 ||
						ProtoCryptTable[CurrentWorld][i].SensorType == 7)
						uvsCreateNewItem(
							ProtoCryptTable[CurrentWorld][i].R_curr.x,
							ProtoCryptTable[CurrentWorld][i].R_curr.y,
							ProtoCryptTable[CurrentWorld][i].R_curr.z,
							uvsGenerateItemForCrypt(ProtoCryptTable[CurrentWorld][i].SensorType),
							CurrentWorld
						);
					else
						uvsCreateNewItem(
							...
							ProtoCryptTable[CurrentWorld][i].R_curr.y,
							ProtoCryptTable[CurrentWorld][i].R_curr.z,
							ProtoCryptTable[CurrentWorld][i].z0,
							CurrentWorld
						);
				};
			};
...
void ItemsDispatcher::Close(void) {
	...
	if (NetworkON) {
		...
		for (i = 0; i < ProtoCryptTableSize[CurrentWorld]; i++) {
			if (ProtoCryptTable[CurrentWorld][i].SensorType == 3 ||
				ProtoCryptTable[CurrentWorld][i].SensorType == 5 ||
				(ProtoCryptTable[CurrentWorld][i].SensorType == 4 &&
					ProtoCryptTable[CurrentWorld][i].z0 == UVS_ITEM_TYPE::SPUMMY)) {
		...
		while (p) {
			...
			if (p->ActIntBuffer.type == ACI_CONLARVER) {
				...
				if (p->CreateMode != STUFF_CREATE_RELAX) {
					if (((int)(RND(100)) < aiCutLuck || p->Hour == ConTimer.hour ||
							p->ActIntBuffer.type == ACI_TABUTASK_SUCCESSFUL ||
							p->ActIntBuffer.type == ACI_TABUTASK ||
							CurrentWorld >= MAIN_WORLD_MAX) &&
						(p->ActIntBuffer.type != ACI_ZEEX &&
							p->ActIntBuffer.type != ACI_BEEBOORAT &&
							p->ActIntBuffer.type != ACI_ELEEPOD &&
							p->ActIntBuffer.type != ACI_TABUTASK_FAILED &&
							p->ActIntBuffer.type != ACI_BOORAWCHIK)) {
						g = new uvsItem(p->uvsDeviceType);
						g->param1 = p->ActIntBuffer.data0;
						g->param2 = p->ActIntBuffer.data1;
						g->pos_x = p->R_curr.x;
					...
					for (i = 0; i < ProtoCryptTableSize[CurrentWorld]; i++) {
						if ((ProtoCryptTable[CurrentWorld][i].SensorType == 3 ||
								...
								(ProtoCryptTable[CurrentWorld][i].SensorType == 4 &&
									ProtoCryptTable[CurrentWorld][i].z0 ==
										UVS_ITEM_TYPE::SPUMMY)) &&
							ProtoCryptTable[CurrentWorld][i].z0 == p->uvsDeviceType) {
							if (getDistY(ProtoCryptTable[CurrentWorld][i].R_curr.y, p->R_curr.y) <
									p->radius &&
								getDistX(ProtoCryptTable[CurrentWorld][i].R_curr.x, p->R_curr.x) <
									p->radius)
		...
		if (ConTimer.day >= 16) {
			if (!RND(3) && !aciWorldLinkExist(WORLD_FOSTRAL, WORLD_WEEXOW, 0) &&
				!aciWorldLinkExist(WORLD_GLORX, WORLD_KHOX, 0) &&
				!aciWorldLinkExist(WORLD_NECROSS, WORLD_BOOZEENA, 0) &&
				!aciWorldLinkExist(WORLD_NECROSS, WORLD_ARKONOY, 0))
				Item2ShopAction(5);
...
void StuffObject::Quant(void) {
	...
	if (NetworkON && (Status & SOBJ_WAIT_CONFIRMATION))
		NetOwnerQuant();
	lv = Visibility;
	GetVisible();
	...
	if (ActIntBuffer.type == ACI_CONLARVER &&
		Hour != ConTimer.hour /* && Minutes == ConTimer.min*/) {
	...
	if (Visibility == VISIBLE) {
		...
		case ACI_CONLARVER:
			if (LightData)
				LightData->set_position(R_curr.x, R_curr.y, 512);
			else
				LightData =
					MapD.CreateLight(R_curr.x, R_curr.y, 512, radius * 2, 63, LIGHT_TYPE::STATIC);
			break;
		case ACI_EMPTY_CONLARVER:
			if (LightData)
				...
				LightData =
					MapD.CreateLight(R_curr.x, R_curr.y, 512, radius, 63, LIGHT_TYPE::DYNAMIC);
			break;
		case ACI_MECHANIC_MESSIAH:
			if (CurrentWorld == WORLD_XPLO && ActD.FunctionSpobsDestroyActive) {
				if (!ActD.SpobsDestroy && (dynamic_state & GROUND_COLLISION) &&
					GetTouchSensor("Spot1")) {
					ActD.SpobsDestroy = 1;
			...
			if (CurrentWorld == WORLD_THREALL) {
				if (ActD.ThreallDestroy) {
					if (LightData)
						...
						LightData =
							MapD.CreateLight(R_curr.x, R_curr.y, 512, 80, 63, LIGHT_TYPE::DYNAMIC);
					if (R_curr.z < 255)
						continuous_impulse(Vector(0, 0, 64), RND(15), RND(8));
				} else {
					if ((dynamic_state & GROUND_COLLISION) && GetTouchSensor("SIGN") &&
						ActD.FunctionThreallDestroyActive) {
		...
		case ACI_BOORAWCHIK:
			...
			break;
		default:
			if ((ActIntBuffer.type == ACI_ROTTEN_KERNOBOO ||
					ActIntBuffer.type == ACI_ROTTEN_PIPETKA ||
					ActIntBuffer.type == ACI_ROTTEN_WEEZYK ||
					ActIntBuffer.type == ACI_DEAD_ELEECH) &&
		...
		if (lv == UNVISIBLE) {
			if (CreateMode == STUFF_CREATE_RELAX)
				...
				if (uvsDeviceType == UVS_ITEM_TYPE::KERNOBOO ||
					uvsDeviceType == UVS_ITEM_TYPE::PIPETKA)
...
void StuffObject::DeviceIn(void) {
	...
	if (NetworkON && (Owner->Status & SOBJ_ACTIVE)) {
		NETWORK_OUT_STREAM.begin_item_transfer(
			ITEM_TRANSFER_PICKUP, NetID, NetDeviceID, xImpulse, yImpulse, radius
		);
		NETWORK_OUT_STREAM < (uchar)(DataID);
		NETWORK_OUT_STREAM < (int)(NetID);
		NETWORK_OUT_STREAM < (short)(R_curr.z);
...
void GunDevice::DeviceIn(void) {
	...
	if (pData->BulletID == BULLET_TYPE_ID::HYPNOTISE) {
		...
		case ACI_MECHOSCOPE:
			if (NetworkON) {
				if (CheckLiveVangerPointer(Owner) && CheckLiveVangerPointer(PrevOwner) &&
					PrevOwner != Owner && (Owner->Status & SOBJ_ACTIVE) &&
					PrevOwner->Visibility == VISIBLE &&
					!(PrevOwner->Status & SOBJ_WAIT_CONFIRMATION)) {
				...
				if (CheckLiveVangerPointer(Owner) && CheckLiveVangerPointer(PrevOwner) &&
					PrevOwner != Owner && PrevOwner->Visibility == VISIBLE &&
					Owner->Visibility == VISIBLE &&
					((PrevOwner->Status & SOBJ_ACTIVE) || (Owner->Status & SOBJ_ACTIVE))) {
					if (ActD.Active)
			...
			ActIntBuffer.type =
				uvsSetItemType(uvsDeviceType, ActIntBuffer.data0, ActIntBuffer.data1);
...
void GunDevice::Quant(void) {
	if (Time > 0) {
		Time--;
		if (Time == 0 &&
			(ActIntBuffer.type == ACI_AMPUTATOR || ActIntBuffer.type == ACI_DEGRADATOR ||
				ActIntBuffer.type == ACI_MECHOSCOPE)) {
			ActIntBuffer.data1 = 0;
			ActIntBuffer.type =
				uvsSetItemType(uvsDeviceType, ActIntBuffer.data0, ActIntBuffer.data1);
			set_body_color(COLORS_IDS::BODY_GRAY);
		} else {
			if (ActIntBuffer.type == ACI_EMPTY_AMPUTATOR ||
				ActIntBuffer.type == ACI_EMPTY_DEGRADATOR ||
				ActIntBuffer.type == ACI_EMPTY_MECHOSCOPE) {
...
void StuffObject::ImpulseAction(Vector v1, int flag, Vector v2) {
	...
	if (CreateMode == STUFF_CREATE_RELAX)
		CreateMode = STUFF_CREATE_EXRELAX;
	else
		CreateMode = STUFF_CREATE_NONE;
...
void StuffObject::DeviceOut(Vector v1, int flag, Vector v2) {
	...
	if (!NetworkON || (Owner->Status & SOBJ_ACTIVE)) {
		if (flag) {
			if (ActIntBuffer.type == ACI_EMPTY_CONLARVER) {
				FindDolly = 1;
				R_curr = Vector(
					XCYCL(R_curr.x + GeneralMousePoint.x), YCYCL(R_curr.y + GeneralMousePoint.y), 0
				);
	...
	if (NetworkON && (Owner->Status & SOBJ_ACTIVE)) {
		NETWORK_OUT_STREAM.begin_item_transfer(
			ITEM_TRANSFER_DROP, NetDeviceID, NetID, xImpulse, yImpulse, radius
		);
		NETWORK_OUT_STREAM < (uchar)(DataID);
		NETWORK_OUT_STREAM < (int)(NetDeviceID);
		NETWORK_OUT_STREAM < (short)(R_curr.z);
...
void BulletObject::CreateBullet(
	Vector fv,
	Vector tv,
	GeneralObject *target,
	WorldBulletTemplate *p,
	GeneralObject *_Owner,
	int _speed
) {
	int d;
	R_prev = R_curr = fv;
...
void BulletObject::CreateBullet(GunSlot *p, WorldBulletTemplate *n) {
	...
	Speed = n->Speed;
	if (BulletMode & BULLET_CONTROL_MODE::SPEED)
		Speed += p->RealSpeed;

	if (BulletID == BULLET_TYPE_ID::LASER) {
		LegacyLaserDraw = 1;
		vDelta = Vector(Speed, machotine_laser_random_spread(), 0) *
				 p->mFire; // machotine bullet dispersion
	} else
		vDelta = Vector(Speed, 0, 0) * p->mFire;

	if (TargetObject && (BulletMode & BULLET_CONTROL_MODE::AIM)) {
		vDelta = Vector(
			getDistX(TargetObject->R_curr.x, R_curr.x),
			getDistY(TargetObject->R_curr.y, R_curr.y),
			TargetObject->R_curr.z - R_curr.z
		);
		d = vDelta.vabs();
		vDelta = vDelta * Speed / vDelta.vabs();
	};
...
void BulletObject::Event(int type) {
	...
	case BULLET_EVENT_ID::MAP_DESTROY:
		...
		if (!TntEnable) {
			...
			case BULLET_SHOW_TYPE_ID::PARTICLE:
				...
				if (tp) {
					tp += R_curr.x + H_SIZE;
					if (GET_TERRAIN_TYPE(*tp) == WATER_TERRAIN &&
						FloodLvl[R_curr.y >> WPART_POWER] >= R_curr.z) {
						if (!RND(2) || CurrentWorld != WORLD_GLORX)
							EffD.CreateDeform(R_curr, DEFORM_WATER_ONLY, DT_DEFORM02);
					...
					if (CraterType >= 0)
						MapD.CreateCrater(R_curr, CraterType);
				};
				break;
			case BULLET_SHOW_TYPE_ID::FIREBALL:
				CreateDestroyEffect(
					R_curr, CraterType, DT_DEFORM02, DEFORM_WATER_ONLY, -1, DT_FIRE_BALL02
				);
				break;
	...
	case BULLET_EVENT_ID::TOUCH:
		...
		case BULLET_SHOW_TYPE_ID::CRATER:
			//					EffD.CreateDeform(R_curr + Vector(16 - RND(32),16 -
			// RND(32),0),DEFORM_ALL,PASSING_WAVE_PROCESS);
			// EffD.CreateDeform(R_curr + Vector(16 - RND(32),16 -
			// RND(32),0),DEFORM_ALL,PASSING_WAVE_PROCESS); EffD.CreateDeform(R_curr + Vector(16 -
			// RND(32),16 - RND(32),0),DEFORM_ALL,PASSING_WAVE_PROCESS);
			// EffD.CreateDeform(R_curr +
			// Vector(16 - RND(32),16 - RND(32),0),DEFORM_ALL,PASSING_WAVE_PROCESS);
			if (CraterType >= 0)
				MapD.CreateCrater(R_curr, CraterType);
			break;
		};
...
void BulletObject::TimeOutQuant(void) {
	...
	if (TargetObject) {
		...
		if ((TargetObject->Status & SOBJ_DISCONNECT) ||
			(TargetObject->ID == ID_VANGER &&
				(!((VangerUnit *)(TargetObject))->ExternalDraw ||
					((VangerUnit *)(TargetObject))->draw_mode != NORMAL_DRAW_MODE ||
					((VangerUnit *)(TargetObject))->BeebonationFlag)))
			TargetObject = NULL;
	};
	...
	if (BulletMode & BULLET_CONTROL_MODE::TARGET) {
		...
		case BULLET_TARGET_MODE::RANDOM:
			if (bullet_random_target_legacy_step() && RND(100) < 10)
				vTarget = Vector(
					BMAX_TARGET_VECTOR - RND(BMAX_TARGET_VECTOR2),
					BMAX_TARGET_VECTOR - RND(BMAX_TARGET_VECTOR2),
					BMAX_TARGET_VECTOR - RND(BMAX_TARGET_VECTOR2)
				);
			break;
	...
	if (Visibility == VISIBLE) {
		...
		while (g) {
			if (g->ID == PROTRACTOR_PALLADIUM) {
				...
				if (d < PALLADIUM_RADIUS) {
					...
					accumulate_runtime_vector_step(
						vDelta, MoveAccumX, MoveAccumY, MoveAccumZ, runtime_step
					);
					R_curr += runtime_step;
					cycleTor(R_curr.x, R_curr.y);
				};
		...
		if (BulletMode & BULLET_CONTROL_MODE::FLY) {
			...
			if (ShowID == BULLET_SHOW_TYPE_ID::CRATER && ExtShowType) {
				if (BigGetAlt(R_curr, alt, ExtShowType))
					MapLevel = 1;
				else {
					MapLevel = 0;
					alt = 0;
...
void BulletObject::Touch(GeneralObject *p) {
	if (p == Owner)
		return;
	if (p->ID == ID_VANGER) {
		if (((VangerUnit *)(p))->BeebonationFlag)
			return;
		if (Owner)
			PUT_GLOBAL_EVENT(AI_EVENT_COLLISION, ID_BULLET, Owner, (VangerUnit *)(p));

		if (BulletMode & BULLET_CONTROL_MODE::IMPULSE) {
			if (/*(BulletMode & BULLET_CONTROL_MODE::FLY) && */ (
				ShowID != BULLET_SHOW_TYPE_ID::CRATER
			))
				((VangerUnit *)(p))
					->instant_impulse(
						vDelta, Power / BULLET_IMPULSE_POWER, Power / BULLET_IMPULSE_ARM
					);
			else {
				((VangerUnit *)(p))
					->instant_impulse(
						Vector(0, 0, 64),
						4 * Power / BULLET_IMPULSE_POWER,
						4 * Power / BULLET_IMPULSE_ARM
					);
			};
		};
...
void BulletObject::DrawQuant(void) {
	...
	case BULLET_SHOW_TYPE_ID::DEFORM:
		if (AdvancedView)
			...
			s = G2LS(R_curr, tx, ty);
		if (EffD.DeformData[ShowType].CheckOffset(FrameCount))
			FrameCount = 0;
			...
			if (ExtShowType) {
				if (BulletScale) {
					if (LightData)
						...
						LightData = MapD.CreateLight(
							R_curr.x, R_curr.y, R_curr.z, 20, 32, LIGHT_TYPE::STATIC
						);

					if (create_crater_spot)
						MapD.CreateLavaSpot(
							R_curr, 5, 5, 20, 10, 0, 0, 1, 8, 83, 0, 4, 1, ExtShowType
						);
	...
	case BULLET_SHOW_TYPE_ID::LASER: {
		Vector motionDraw =
			Vector(getDistX(R_prev.x, R_curr.x), getDistY(R_prev.y, R_curr.y), R_prev.z - R_curr.z);
		Vector rawDraw;
...
StuffObject *addDevice(
	...
	if (p) {
		...
		if (Owner)
			p->CreateDevice(Vector(x, y, z), Owner, n);
		else {
			if (cMode == STUFF_CREATE_NONE) {
				if (z == ITEM_DEFAULT_Z)
					p->CreateStuff(Vector(x, y, z), n, STUFF_CREATE_NONE);
				else
					p->CreateStuff(Vector(x, y, z), n, STUFF_CREATE_RELAX);
			} else
				p->CreateStuff(Vector(x, y, z), n, cMode);
		};
...
void JumpBallObject::Quant(void) {
	...
	if (Visibility == VISIBLE) {
		...
		while (g) {
			if (g->ID == PROTRACTOR_PALLADIUM) {
				vCheck = Vector(
					getDistX(R_curr.x, g->vR.x), getDistY(R_curr.y, g->vR.y), R_curr.z - g->vR.z
				);
				l = vCheck.vabs();
		...
		if (dynamic_state & GROUND_COLLISION) {
			...
			if (Mode == BULLET_TARGET_MODE::CONTROL)
				CreateDestroyEffect(
					R_curr, MAP_POINT_CRATER03, DT_DEFORM02, DEFORM_WATER_ONLY, -1, DT_FIRE_BALL02
				);
			else
				CreateDestroyEffect(
					R_curr, MAP_POINT_CRATER10, DT_DEFORM02, DEFORM_WATER_ONLY, EFF_EXPLOSION03, -1
				);
			if (ActD.Active)
...
void JumpBallObject::Touch(GeneralObject *p) {
	if (p->ID == ID_VANGER) {
		...
		if (Mode == BULLET_TARGET_MODE::CONTROL) {
			...
			((VangerUnit *)(p))
				->instant_impulse(Vector(32 - RND(64), 32 - RND(64), RND(64)), 8 + RND(15), 20);
			if (ActD.Active)
				SOUND_BARREL_DESTROY(getDistX(ActD.Active->R_curr.x, R_curr.x))
		} else {
...
void CreateDestroyEffect(
	...
	if (GET_TERRAIN_TYPE(*tp) == WATER_TERRAIN && FloodLvl[v.y >> WPART_POWER] >= v.z) {
		if (def_id >= 0)
			EffD.CreateDeform(v, def_flag, def_id);
		wp = (WaterParticleObject *)(EffD.GetObject(EFF_PARTICLE04));
		if (wp) {
		...
		if (expl_id >= 0)
			EffD.CreateExplosion(v, expl_id);
		else if (fire_id >= 0)
			EffD.CreateFireBall(v, fire_id);
	};
	if (crat_id >= 0)
		MapD.CreateCrater(v, crat_id);
};
...
static void aciUpdateWorldMousePoint(int tx, int ty) {
	if (ActD.Active)
		GeneralMousePoint =
			Vector(getDistX(tx, ActD.Active->R_curr.x), getDistY(ty, ActD.Active->R_curr.y), 0);
}
...
int aciGetScreenItem(int x, int y) {
	...
	if (mp)
		return ((StuffObject *)(mp))->ActIntBuffer.type;
	return -1;
};
...
void SkyFarmerObject::CreateSkyFarmer(
	int x_pos,
	int y_pos,
	int x_speed,
	int y_speed,
	...
	/*	nTarget = 0;
		vCheck =
	   Vector(getDistX(SkyFarmerWayTable[0].R_curr.x,R_curr.x),getDistY(SkyFarmerWayTable[0].R_curr.y,R_curr.y),0);
		mDist = vCheck.abs2();
		for(i = 1;i < SkyFarmerWayTableSize;i++){
			vCheck =
	   Vector(getDistX(SkyFarmerWayTable[i].R_curr.x,R_curr.x),getDistY(SkyFarmerWayTable[i].R_curr.y,R_curr.y),0);
			d = vCheck.abs2();
			if(d < mDist){
				mDist = d;
				nTarget = i;
...
void SkyFarmerObject::Quant(void) {
	...
	if (d < radius * 4) {
		...
		if (d < radius * 7)
			...
			if (skyfarmer_random_legacy_step() && !RND(3))
				skyfarmer_set_direction(vTrack);
		};
	};
	...
	if (skyfarmer_fly_direction == 0) {
		...
		if (SeedCount <= 0)
			...
			scale_size =
				2 * original_scale_size / 5 +
				2 * DropCount * original_scale_size / (5 * MAX_DROP_POINT) +
				DropCount * SeedCount * original_scale_size / (MAX_DROP_SEED * 5 * MAX_DROP_POINT);
		if (DropCount == 0)
			skyfarmer_end();
	};
	analysis();
...
void addFarmer(int x_pos, int y_pos, int x_speed, int y_speed, int corn_type, int corn, int time) {
	(FarmerD.CreateSkyFarmer())
		->CreateSkyFarmer(x_pos, y_pos, x_speed, y_speed, corn_type, corn, time);
};
			...
			if (keep_log < SAFE_STUFF_MAX) {
				...
				while (g) {
					...
					if (g->uvsDeviceType == ind) {
						...
						g->ActIntBuffer.type = uvsSetItemType(
							g->uvsDeviceType, g->ActIntBuffer.data0, g->ActIntBuffer.data1
						);
						if (n->Status & SOBJ_ACTIVE) {
							switch (ind) {
...
void WorldBulletTemplate::Init(Parser &in) {
	...
	if (!strcmp(name, "{")) {
		...
		while (1) {
			...
			if (!strcmp(name, "}"))
				break;
			BulletMode |= BULLET_CONTROL_MODE_VALUE[Name2Int(
				name, BULLET_CONTROL_MODE_NAME, MAX_BULLET_CONTROL_MODE_NAME
			)];
		};
	} else
		BulletMode = BULLET_CONTROL_MODE_VALUE[Name2Int(
...
void FishWarrior::Quant(void) {
	...
	if (Visibility == VISIBLE) {
		...
		case FISH_WARRIOR_PATROL:
			if (TargetObject && TargetObject->ExternalDraw &&
				TargetObject->draw_mode == NORMAL_DRAW_MODE && !(TargetObject->BeebonationFlag)) {
				if (AttackTime > 0) {
					vTarget = Vector(
						getDistX(TargetObject->R_curr.x, R_curr.x),
						getDistY(TargetObject->R_curr.y, R_curr.y),
	...
	if (NetworkON) {
		if (Time-- <= 0) {
			...
			case 0:
				CreateDestroyEffect(
					R_curr, MAP_POINT_CRATER09, DT_DEFORM02, DEFORM_WATER_ONLY, EFF_EXPLOSION01, -1
				);
...
void HordeSource::Touch(GeneralObject *p) {
	...
	case ID_BULLET:
	case ID_JUMPBALL:
		//			EffD.CreateExplosion(R_curr + Vector(0,0,radius),EFF_EXPLOSION01,this,1 <<
		// 15,0);
			...
			if (TargetObject) {
				if (TargetObject->Status & SOBJ_DISCONNECT) {
					...
					while (g) {
						if (g->ID == PROTRACTOR_PALLADIUM) {
							v = Vector(
								getDistX(R_curr.x, g->vR.x),
								getDistY(R_curr.y, g->vR.y),
								R_curr.z - g->vR.z
							);
							d = v.vabs();
							if (d < PALLADIUM_RADIUS) {
					...
					if (!g) {
						if (!(TargetObject->dynamic_state &
								(GROUND_COLLISION | WHEELS_TOUCH | TOUCH_OF_WATER)) &&
							TargetObject && TargetObject->ExternalDraw &&
							TargetObject->draw_mode == NORMAL_DRAW_MODE &&
							!(TargetObject->BeebonationFlag)) {
							vTarget = Vector(
								getDistX(vZone.x, R_curr.x),
								getDistY(vZone.y, R_curr.y),
								vZone.z - R_curr.z
							);
							d = vTarget.vabs();
							if (d > AttackRadius) {
						...
						if (d) {
							...
							accumulate_runtime_vector_step(
								legacy_step, SteerAccumX, SteerAccumY, SteerAccumZ, steer_step
							);
							vDelta += steer_step;
							d = vDelta.vabs();
							if (d > Speed)
					...
					accumulate_runtime_vector_step(
						vDelta, MoveAccumX, MoveAccumY, MoveAccumZ, vMove
					);
					R_curr += vMove;
					cycleTor(R_curr.x, R_curr.y);
				};
				...
				if (!g) {
					vTarget = Vector(
						getDistX(vZone.x, R_curr.x), getDistY(vZone.y, R_curr.y), vZone.z - R_curr.z
					);
					...
					while (n) {
						if (((VangerUnit *)(n))->Visibility == VISIBLE) {
							...
							if (d < AttackRadius) {
								if (!(n->dynamic_state &
										(GROUND_COLLISION | WHEELS_TOUCH | TOUCH_OF_WATER)) &&
									n->ExternalDraw && n->draw_mode == NORMAL_DRAW_MODE &&
									!(n->BeebonationFlag)) {
...
void SkyFarmerList::Init(void) {
	...
	if (memcmp(sign, "VLS1", strlen("VLS1")))
		ErrH.Abort("Bad SkyFarmer Way Point Format");
	...
	for (i = 0; i < SkyFarmerWayTableSize; i++) {
		...
		if (SkyFarmerWayTable[i].SensorType < 0 &&
			SkyFarmerWayTable[i].SensorType >= SkyFarmerWayTableSize)
	...
	while (l) {
		addFarmer(
			((uvsFlyFarmer *)(l))->pos_x,
			((uvsFlyFarmer *)(l))->pos_y,
			((uvsFlyFarmer *)(l))->x_speed,
			((uvsFlyFarmer *)(l))->y_speed,
			((uvsFlyFarmer *)(l))->corn_type,
			((uvsFlyFarmer *)(l))->corn,
			((uvsFlyFarmer *)(l))->timer
		);
		l = l->enext;
	};
...
void HordeList::Init(void) {
	...
	if (NetworkON) {
		...
		for (i = 0; i < max; i++)
			(CreateHorde())
				->CreateHorde(
					Vector(NetRnd.Get(map_size_x), NetRnd.Get(map_size_y), 270), 20, 240, 400, NULL
				);
	} else {
		switch (CurrentWorld) {
		case WORLD_WEEXOW:
		case WORLD_ARKONOY:
			for (i = 0; i < MAX_HORDE_OBJECT; i++)
				(CreateHorde())
					->CreateHorde(
						Vector(RND(map_size_x), RND(map_size_y), 270), 20, 240, 400, NULL
					);
			break;
		};
	};
...
void HordeSourceList::Init(void) {
	...
	if (NetworkON) {
		...
		for (i = 0; i < max; i++)
			(CreateSource())
				->CreateSource(
					Vector(NetRnd.Get(map_size_x), NetRnd.Get(map_size_y), 270),
					ModelD.FindModel("Horde"),
					20,
					60
				);
	} else {
		...
		case WORLD_XPLO:
			for (i = 0; i < MAX_HORDE_SOURCE_OBJECT; i++)
				(CreateSource())
					->CreateSource(
						Vector(RND(map_size_x), RND(map_size_y), 270),
						ModelD.FindModel("Horde"),
						20,
						60
					);
	...
	while (p) {
		...
		if (p->Status & SOBJ_DISCONNECT) {
			(HordeD.CreateHorde())
				->CreateHorde(
					p->R_curr,
					((HordeSource *)(p))->HordeRadius,
					((HordeSource *)(p))->zHorde,
					400,
					NULL
				);
			FreeUnit(p);
	...
	//zmod 1.17 as it was before
		...
		if(CurrentWorld == WORLD_WEEXOW && !NetworkON){
			...
			for(i = 0;i < MAX_FISH_WARRIOR;i++){
				Data[i].Init();
				Data[i].CreateFish(Vector(RND(map_size_x),RND(map_size_y),10),20,RND(2*PI),5,2000,FISH_WARRIOR_PATROL,ModelD.FindModel("FishWarrior"),NULL,100
	<< 16); ConnectTypeList(&Data[i]);
			};
		}else Data = NULL;
	*/
	int i;
	if (!NetworkON) {
		if (CurrentWorld == WORLD_WEEXOW) {
			...
			for (i = 0; i < MAX_FISH_WARRIOR; i++) {
				Data[i].Init();
				Data[i].CreateFish(
					Vector(RND(map_size_x), RND(map_size_y), 10),
					20,
					RND(2 * PI),
					5,
					2000,
...
void StuffObject::NetOwnerQuant(int impulse) {
	...
	if (NetOwner) {
		...
		if (Owner) {
			...
			if (impulse)
				ImpulseAction(R_curr, OutFlag, Vector(xImpulse, yImpulse, 0));
			Status &= ~SOBJ_WAIT_CONFIRMATION;
		};
	};
...
void StuffObject::NetEvent(int type, int id, int creator, int x, int y) {
	...
	snprintf(
		net_event_extra,
		sizeof(net_event_extra),
		"phase=begin event_type=%d id=0x%08X creator=%d x=%d y=%d body_size=%d",
		type,
		(unsigned int)id,
		creator,
		x,
	...
	case UPDATE_OBJECT:
		...
		snprintf(
			net_event_extra,
			sizeof(net_event_extra),
			"phase=after_update event_type=%d id=0x%08X creator=%d x=%d y=%d",
			type,
			(unsigned int)id,
			creator,
			x,
	...
	case CREATE_OBJECT:
		...
		if (NetOwner) {
			...
			case STUFF_CREATE_TRACK:
				...
				precise_impulse(
					R_curr,
					XCYCL(R_curr.x + MAX_STUFF_SCATTER - RND(2 * MAX_STUFF_SCATTER)),
					YCYCL(R_curr.y + MAX_STUFF_SCATTER - RND(2 * MAX_STUFF_SCATTER))
				);
				break;
			case STUFF_CREATE_RELAX:
				switch_analysis(1);
		...
		snprintf(
			net_event_extra,
			sizeof(net_event_extra),
			"phase=after_create event_type=%d id=0x%08X creator=%d x=%d y=%d",
			type,
			(unsigned int)id,
			creator,
			x,
	...
	case DELETE_OBJECT:
		...
		if (!ch) {
			if (Owner) {
				NetworkLogStuffObject(
					"StuffObject::NetEvent",
					this,
					"event_type=DELETE_OBJECT delete_reason=remove_from_owner_inventory"
				);
				Owner->CheckOutDevice(this);
				if (Owner->Status & SOBJ_ACTIVE) {
					ActD.CheckDevice(this);
				...
				NetworkLogStuffObject(
					"StuffObject::NetEvent",
					this,
					"event_type=DELETE_OBJECT delete_reason=disconnect_world_object"
				);
				Status |= SOBJ_DISCONNECT;
			}
		} else {
			...
			NetworkLogStuffObject(
				"StuffObject::NetEvent",
				this,
				"event_type=DELETE_OBJECT delete_reason=wait_confirmation"
			);
		}
		break;
	};
...
void ItemsDispatcher::NetItemState(int item_state, int previous_id, int item_id) {
	...
	network_log_printf(
		"ItemsDispatcher::NetItemState",
		"phase=begin item_state=%d previous_id=0x%08X item_id=0x%08X creator=%d body_size=%d",
		item_state,
		...
		if (p) {
			if (p->Status & SOBJ_DISCONNECT) {
				NetworkLogStuffObject(
					"ItemsDispatcher::NetItemState",
					p,
					"decision=previous_object_already_disconnected"
				);
			} else {
				...
				NetworkLogStuffObject(
					"ItemsDispatcher::NetItemState",
					p,
					"decision=previous_object_marked_wait_confirmation"
				);
			}
		} else {
			network_log_printf(
				"ItemsDispatcher::NetItemState",
				"item_state=%d previous_id=0x%08X item_id=0x%08X decision=previous_object_missing",
				item_state,
				(unsigned int)previous_id,
				(unsigned int)item_id
			);
	...
	default:
		network_log_printf(
			"ItemsDispatcher::NetItemState",
			"item_state=%d previous_id=0x%08X item_id=0x%08X decision=ignored_bad_item_type",
			item_state,
			(unsigned int)previous_id,
			(unsigned int)item_id
		);
	...
	network_log_printf(
		"ItemsDispatcher::NetItemState",
		"phase=end item_state=%d previous_id=0x%08X item_id=0x%08X",
		item_state,
		(unsigned int)previous_id,
		(unsigned int)item_id
	);
...
void ItemsDispatcher::NetItemRemoved(int item_id, int paired_id, int reason) {
	...
	network_log_printf(
		"ItemsDispatcher::NetItemRemoved",
		"phase=begin item_id=0x%08X paired_id=0x%08X reason=%d found=%d",
		(unsigned int)item_id,
		(unsigned int)paired_id,
		reason,
		p ? 1 : 0
	);
	...
	if (!p) {
		network_log_printf(
			"ItemsDispatcher::NetItemRemoved",
			"item_id=0x%08X paired_id=0x%08X reason=%d delete_reason=ignored_missing_object "
			"decision=ignored_missing_object",
			(unsigned int)item_id,
			(unsigned int)paired_id,
			reason
		);
	...
	if (p->Owner) {
		NetworkLogStuffObject(
			"ItemsDispatcher::NetItemRemoved", p, "delete_reason=remove_from_owner_inventory"
		);
		...
		NetworkLogStuffObject(
			"ItemsDispatcher::NetItemRemoved", p, "delete_reason=disconnect_world_object"
		);
	...
	network_log_printf(
		"ItemsDispatcher::NetItemRemoved",
		"phase=end item_id=0x%08X paired_id=0x%08X reason=%d",
		(unsigned int)item_id,
		(unsigned int)paired_id,
		reason
	);
...
void ItemsDispatcher::NetDevice(int type, int id) {
	...
	network_log_printf(
		"ItemsDispatcher::NetDevice",
		"phase=begin event_type=%d id=0x%08X creator=%d body_size=%d",
		type,
		(unsigned int)id,
		NETWORK_IN_STREAM.current_creator(),
		NETWORK_IN_STREAM.current_body_size()
	...
	if (p) {
		...
		if (p) {
			if (p->Status & SOBJ_DISCONNECT) {
				NetworkLogStuffObject(
					"ItemsDispatcher::NetDevice", p, "decision=ignored_disconnected"
				);
				NETWORK_IN_STREAM.ignore_event();
			} else
				p->NetEvent(
			...
			if (type == UPDATE_OBJECT) {
				NETWORK_IN_STREAM > t;
				p = (StuffObject *)(ItemD.GetObject(
					DevicetStorageID[ItemD.DeviceTypeData[t]->StuffType]
				));
				if (p) {
					p->DataID = t;
					NetworkLogStuffObject(
						"ItemsDispatcher::NetDevice", p, "decision=create_missing_from_update"
					);
					p->NetEvent(
						CREATE_OBJECT,
						id,
					...
					network_log_printf(
						"ItemsDispatcher::NetDevice",
						"event_type=%d id=0x%08X delete_reason=ignored_missing_object "
						"decision=ignored_missing_object",
						type,
						(unsigned int)id
					);
...
void ItemsDispatcher::NetEvent(int type, int id) {
	...
	if (p) {
		...
		if (p) {
			if (p->Status & SOBJ_DISCONNECT) {
				NetworkLogStuffObject(
					"ItemsDispatcher::NetEvent", p, "decision=ignored_disconnected"
				);
				NETWORK_IN_STREAM.ignore_event();
			} else
				p->NetEvent(
			...
			if (type == UPDATE_OBJECT) {
				...
				if (p) {
					p->DataID = t;
					NetworkLogStuffObject(
						"ItemsDispatcher::NetEvent", p, "decision=create_missing_from_update"
					);
					p->NetEvent(
						CREATE_OBJECT,
						id,
...
void GloryPlace::Init(int ind) {
	...
	if (z_my_server_data.mod_id == Z_MODS_KHOXRUN_ID) {
		...
		case 0:
			...
			R_curr.y = 7036;
			World = WORLD_GLORX;
			break; // НЕ МЕНЯТЬ !!! Связано с багом вылета клиента при смерти в пассе. если чек не
				   // на трех мирах.
		case 1:
			R_curr.x = 390;
			R_curr.y = 1266;
	...
	// classic
	if (ind == 0) {
		World = GloryRnd.aiRnd(3); // НЕ МЕНЯТЬ !!! Связано с багом вылета клиента при смерти в
								   // пассе. если чек не на трех мирах.
	} else {
		...
		if (z_my_server_data.mod_id == Z_MODS_RAFARUN_ID) // tarakan'i bega/ excludes hmok &&
														  // threall
		...
		if (ActD.Active) {
			if (!LightData)
				LightData = MapD.CreateLight(
					...
					GLORY_PLACE_RADIUS,
					32,
					LIGHT_TYPE::DYNAMIC | LIGHT_TYPE::TOR
				);
			...
			if ((dx * dx + dy * dy) < GLORY_PLACE_RADIUS * GLORY_PLACE_RADIUS &&
				((ActD.Active->dynamic_state & GROUND_COLLISION) ||
					(ActD.Active->dynamic_state & TRACTION_WHEEL_TOUCH) ||
					(ActD.Active->dynamic_state & STEER_WHEEL_TOUCH))) {
				if (LightData) {
					LightData->Destroy();
					LightData = NULL;
				};
				MapD.CreateLight(
					R_curr.x, R_curr.y, R_curr.z, GLORY_PLACE_RADIUS, 32, LIGHT_TYPE::STATIONARY
				);
				for (i = 0; i < 5; i++)
					EffD.CreateDeform(
						Vector(
							XCYCL(R_curr.x + GLORY_PLACE_RADIUS - RND(2 * GLORY_PLACE_RADIUS)),
							YCYCL(R_curr.y + GLORY_PLACE_RADIUS - RND(2 * GLORY_PLACE_RADIUS)),
							255
						),
						DEFORM_ALL,
...
void aiPromptTaskMessage(int l) {
	...
	if (l >= 0) {
		if (lang() == RUSSIAN)
			aiMessageBuffer.add_str(0, (unsigned char *)rSuccessTaskMessageData);
		else
			aiMessageBuffer.add_str(0, (unsigned char *)SuccessTaskMessageData);
	} else {
		if (lang() == RUSSIAN)
			aiMessageBuffer.add_str(0, (unsigned char *)rFailedTaskMessageData);
		else
			aiMessageBuffer.add_str(0, (unsigned char *)FailedTaskMessageData);
	};
	...
	RaceTxtBuff.init();
	if (lang() == RUSSIAN)
		RaceTxtBuff < rTaskMessageLuck;
	else
		RaceTxtBuff < TaskMessageLuck;
	if (l > 0)
		RaceTxtBuff < " +";
	else
		RaceTxtBuff < " -";
...
void aiPromptDominanceMessage(int d) {
	...
	RaceTxtBuff.init();
	if (lang() == RUSSIAN)
		RaceTxtBuff < rDominanceMessageData;
	else
		RaceTxtBuff < DominanceMessageData;
	if (d > 0)
		RaceTxtBuff < " +";
	else
		RaceTxtBuff < " -";
...
void ChangeItemData(int d) {
	...
	while (p) {
		...
		if (uvsChangeGoodsParam(
				p->uvsDeviceType, p->ActIntBuffer.data0, p->ActIntBuffer.data1, d
			)) {
			p->ActIntBuffer.type =
				uvsSetItemType(p->uvsDeviceType, p->ActIntBuffer.data0, p->ActIntBuffer.data1);
			...
			case ACI_TABUTASK_FAILED:
				SOUND_FAILED();
				//					ShowTaskMessage(-TabuTable[p->ActIntBuffer.data1 &
				// 0xffff]->luck);
				p->SetStuffColor();
				break;
...
void ChangeTabutaskItem(int type, int status) {
	...
	p = (StuffObject *)(ItemD.Tail);
	while (p) {
		if (p->uvsDeviceType == UVS_ITEM_TYPE::TABUTASK &&
			uvsChangeTabuTask(p->ActIntBuffer.data0, p->ActIntBuffer.data1, type, status)) {
			p->ActIntBuffer.type =
				uvsSetItemType(p->uvsDeviceType, p->ActIntBuffer.data0, p->ActIntBuffer.data1);
			switch (p->ActIntBuffer.type) {
			case ACI_TABUTASK_FAILED:
...
void GunDevice::CreateGun(void) {
	PrevOwner = NULL;
	if (uvsDeviceType == UVS_ITEM_TYPE::AMPUTATOR || uvsDeviceType == UVS_ITEM_TYPE::DEGRADATOR ||
		uvsDeviceType == UVS_ITEM_TYPE::MECHOSCOPE)
		set_body_color(COLORS_IDS::BODY_GRAY);
	//	ActIntBuffer.data1 = 0;
	//	ActIntBuffer.type = uvsSetItemType(uvsDeviceType,ActIntBuffer.data0,ActIntBuffer.data1);
};
...
int ItemsDispatcher::CreateEnableCrypt(void) {
	...
	for (i = 0; i < ProtoCryptTableSize[CurrentWorld]; i++) {
		if ((ProtoCryptTable[CurrentWorld][i].SensorType == 2 ||
				ProtoCryptTable[CurrentWorld][i].SensorType == 6 ||
				ProtoCryptTable[CurrentWorld][i].SensorType == 1 ||
				ProtoCryptTable[CurrentWorld][i].SensorType == 7) &&
...
void ItemsDispatcher::CryptQuant(void) {
	...
	if (crypt_quant_legacy_step() && ActD.Active && Num < Total / 4 &&
		!RND(200 * ActD.NumVisibleVanger)) {
					...
					if (ProtoCryptTable[CurrentWorld][i].SensorType == 2 ||
						...
						if (ProtoCryptTable[CurrentWorld][i].SensorType == 1 ||
							...
							uvsCreateNewItem(
								...
								ProtoCryptTable[CurrentWorld][i].R_curr.y,
								ProtoCryptTable[CurrentWorld][i].R_curr.z,
								uvsGenerateItemForCrypt(
									ProtoCryptTable[CurrentWorld][i].SensorType
								),
								CurrentWorld
							);
					};
