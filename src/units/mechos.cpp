...
static int CheckLiveVangerPointer(VangerUnit *p) {
	...
	while (n) {
		if (n == (GeneralObject *)p) {
			...
			v = (VangerUnit *)n;
			if (!v->uvsPoint || !v->uvsPoint->Pmechos)
				return 0;
...
char CheckAddLink(LinkType *l, BranchType *b, int s) {
	...
	/*	l += s;
		...
		return 1;*/
	if (((uintptr_t)(l) + s * sizeof(LinkType)) >= (uintptr_t)(b->Link + b->NumLink))
		return 0;
	return 1;
};
...
char CheckDecLink(LinkType *l, BranchType *b, int s) {
	...
	/*	l -= s;
		...
		return 1;*/
	if (((uintptr_t)(l)-s * sizeof(LinkType)) < (uintptr_t)(b->Link))
		return 0;
	return 1;
};
...
char CheckInNode(Vector &v, NodeType *n) {
	...
	pbx = *dx;
	pby = *dy;
	if (GetAngle(v.x, v.y, dx[nn - 1], dy[nn - 1], pbx, pby) < 0)
		return 0;
	for (i = 1; i < nn; i++) {
		dx++;
		dy++;
		bx = *dx;
		by = *dy;
		if (GetAngle(v.x, v.y, pbx, pby, bx, by) < 0)
			return 0;
		pbx = bx;
		pby = by;
	};
...
char CheckInLink(Vector &v, LinkType *pl, LinkType *nl) {
	...
	if (GetAngle(v.x, v.y, pl->xr, pl->yr, pl->xl, pl->yl) < 0)
		return 0; // PREV
	if (GetAngle(v.x, v.y, pl->xl, pl->yl, nl->xl, nl->yl) < 0)
		return 0; // LEFT
	if (GetAngle(v.x, v.y, nl->xr, nl->yr, pl->xr, pl->yr) < 0)
		return 0; // RIGHT
	return 1;
};
...
/*void PointDist(int x,int y,int z,int x1,int y1,int z1,int r,int radius,char dir,Vector& vd,Vector&
v)
{
	int l,d,m;
	int r;
...
char GetNodeDist(TrackLinkType *p1, TrackLinkType *p2) {
	...
	if (p1->PointStatus & TRK_BRANCH_MASK) {
		if (p2->PointStatus & TRK_BRANCH_MASK) {
			if (p1->pBranch == p2->pBranch)
				return 0;
			else if (p1->pBranch->pBeg == p2->pBranch->pBeg ||
					 p1->pBranch->pBeg == p2->pBranch->pEnd ||
					 p1->pBranch->pEnd == p2->pBranch->pBeg ||
					 p1->pBranch->pEnd == p2->pBranch->pEnd)
				return 2;
		} else if (p1->pBranch->pBeg == p2->pNode || p1->pBranch->pEnd == p2->pNode)
			return 1;
	} else {
		if (p2->PointStatus & TRK_BRANCH_MASK) {
			if (p2->pBranch->pBeg == p1->pNode || p2->pBranch->pEnd == p1->pNode)
				return 1;
		} else {
			n = p1->pNode;
			if (n == p2->pNode)
			...
			for (i = 0; i < n->NumBranch; i++) {
				if (n->Status[i]) {
					if (n->pBranches[i]->pEnd == p2->pNode)
						return 2;
				} else {
					if (n->pBranches[i]->pBeg == p2->pNode)
						return 2;
				};
			};
		};
...
void ActionUnit::CreateActionUnit(
...
void ActionUnit::MixVector(void) {
	...
	if (SpeedDir > 0) {
		...
		DeltaSpeed =
			MaxVelocity -
			(2 * MaxVelocity * SpeedAngleTable[EnvirAngle] * SpeedLenTable[EnvirLen] >> 20);
	} else {
		MoveAngle = (RudderAngleTable[rPI(a + PI)] * RudderLenTable[EnvirLen]) >> 20;
		DeltaSpeed =
			(2 * MaxVelocity * SpeedAngleTable[PI - EnvirAngle] * SpeedLenTable[EnvirLen] >> 20) -
			MaxVelocity;
	};
};
...
void ActionUnit::CalcTrackVector(Vector &v, int &len, int &angle1, int &angle2) {
	len = v.vabs();
	if (len >= MAX_TOUCH_VECTOR)
		len = MAX_TOUCH_VECTOR - 1;
...
void ActionUnit::Action(void) {
	...
	if (MoveAngle > 0) {
		if (MoveAngle > MECHOS_ROT_DELTA)
			MoveAngle = MECHOS_ROT_DELTA;
	} else {
		if (MoveAngle < -MECHOS_ROT_DELTA)
			MoveAngle = -MECHOS_ROT_DELTA;
	};
	...
	if (abs(raw_move_angle) < PI / 10)
		controls(CONTROLS::TURBO_QUANT, 83);
	...
	if (d > 0) {
		if (d > CurrSpeed)
			controls(CONTROLS::BRAKE_QUANT, 83);
		else
			controls(CONTROLS::TRACTION_INCREASE, 83);
	} else {
		if (d < CurrSpeed)
			controls(CONTROLS::BRAKE_QUANT, 83);
	...
	if (Speed >= 0)
		controls(CONTROLS::STEER_BY_ANGLE, -steer_angle);
	else
		controls(CONTROLS::STEER_BY_ANGLE, steer_angle);
};
...
void ActionUnit::HideAction(void) {
	...
	int dx = ActionHideMoveAccumX > 0.0 ? (int)floor(ActionHideMoveAccumX)
										: (int)ceil(ActionHideMoveAccumX);
	int dy = ActionHideMoveAccumY > 0.0 ? (int)floor(ActionHideMoveAccumY)
										: (int)ceil(ActionHideMoveAccumY);
...
void ActionUnit::DrawQuant(void) {
	...
	if (dynamic_state & TOUCH_OF_WATER) {
		if (wProcess->CheckOffset(WavePhase))
			WavePhase = 0;
...
void VangerUnit::BulletCollision(int pow, GeneralObject *p) {
	if (ActD.Active && Visibility == VISIBLE) {
		if (p)
			SOUND_HIT(getDistX(ActD.Active->R_curr.x, R_curr.x))
		else {
			if (Status & SOBJ_ACTIVE)
				SOUND_DAMAGE()
		};
	...
	if (pa > 0 && Armor <= 0 && p && NetworkON && p->ID == ID_VANGER) {
		...
		case VAN_WAR:
			if (!(my_server_data.Van_War.TeamMode) ||
				((VangerUnit *)(p))->uvsPoint->Pmechos->color != uvsPoint->Pmechos->color)
	...
	if (Status & SOBJ_ACTIVE) {
		...
		if (p && p->ID == ID_VANGER && (p->Status & SOBJ_ACTIVE)) {
			...
			if (s)
				uvsCheckKronIventTabuTask(UVS_KRON_EVENT::SHOT, s);
...
void TrackUnit::GetBranch(void) {
	...
	case TRK_IN_NODE:
		...
		if (!CheckInNode(vPoint, pNode)) {
			if (CheckInBranch(vPoint, pPrevLink, pNextLink, pBranch))
...
void TrackUnit::WayInit(void) {
	...
	for (i = 0; i < HideTrack.NumNode; i++)
		CalcWayDist[i] = 0;
	if (OtherFlag & MECHOS_TARGET_MOVE) {
		if (TargetPoint.PointStatus & TRK_BRANCH_MASK) {
			if (TargetPoint.pBranch->pBeg)
				CalcWayDist[TargetPoint.pBranch->pBeg->index] = 1;
			if (TargetPoint.pBranch->pEnd)
				CalcWayDist[TargetPoint.pBranch->pEnd->index] = 1;
		} else
			CalcWayDist[TargetPoint.pNode->index] = 1;
				...
				if (DestroyPhase & 3) {
					...
					if (wp) {
						wp->CreateParticle(
							...
							5,
							Vector(
								R_curr.x + radius - RND(2 * radius),
								R_curr.y + radius - RND(2 * radius),
								R_curr.z
							)
						);
	...
	if (DestroyPhase++ == 27) {
		if (Status & SOBJ_ACTIVE) {
			if (NetworkON) {
				...
				case PASSEMBLOSS:
					if (UsedCheckNum == 0)
						ChangeWorldConstraction = GloryPlaceData[0].World;
					else {
...
void TrackUnit::GetNode(TrackLinkType *tp, char &dir) {
	...
	if (TargetPoint.PointStatus & TRK_BRANCH_MASK) {
		...
		if (nn != -1) {
			if (nn <= -1 || nn >= n->NumBranch)
				ErrH.Abort("Bad Short Way");
	...
	if (tp->pBranch->pBeg == n) {
		...
		if (tp->pBranch->pEnd == n) {
			...
			ErrH.Abort("Error make WayTable");
	};
};
...
void TrackLinkType::ChangeLink(void) {
	...
	do {
		if (GetAngle(
				vPoint.x, vPoint.y, pNextLink->xl, pNextLink->yl, pNextLink->xr, pNextLink->yr
			) < 0 &&
			vLog != 2) {
			pNextLink++;
			...
			if (GetAngle(
					vPoint.x, vPoint.y, pPrevLink->xr, pPrevLink->yr, pPrevLink->xl, pPrevLink->yl
				) < 0 &&
				vLog != 1) {
				pNextLink--;
...
char TrackUnit::TestAnalysis(TrackLinkType &tp) {
	...
	case TRK_IN_NODE:
		if (CheckInNode(tp.vPoint, tp.pNode))
			return 1;
		else {
		...
		if (MoveDir) {
			if (tp->pBranch->pEnd) {
				...
				if (tp->pBranch->pBeg) {
					...
					ErrH.Abort("Bad Node's Link to Branch");
			};
			return 1;
		} else {
			if (tp->pBranch->pBeg) {
	...
	if (SpeedDir != 0) {
		...
		case TRK_IN_BRANCH:
			...
			if (front_step) {
				if (SpeedDir > 0) {
					if (MoveDir)
						FrontPoint.AddLink(front_step);
					else
						FrontPoint.DecLink(front_step);
				} else {
					if (MoveDir)
						FrontPoint.DecLink(front_step);
					else
			...
			if ((TargetPoint.PointStatus & TRK_BRANCH_MASK) && pBranch == TargetPoint.pBranch) {
				...
				if (FrontPoint.PointStatus == TRK_WAIT_NODE) {
					...
					if (SpeedDir == 0)
						...
						if (FrontDir == -1)
							FrontPoint.PointStatus = TRK_TARGET_NODE;
						else if (FrontPoint.pBranch == pBranch) {
							MoveDir ^= 1;
							FrontPoint.PointStatus = TRK_IN_BRANCH;
						};
					};
		...
		case TRK_WAIT_NODE:
			if (PointStatus & TRK_NODE_MASK) {
				...
				if (front_step) {
					if (SpeedDir > 0) {
						if (MoveDir) {
							if (CheckAddLink(pNextLink, pBranch, front_step))
								FrontPoint.PointStatus = TRK_IN_BRANCH;
						} else {
							if (CheckDecLink(pPrevLink, pBranch, front_step))
								FrontPoint.PointStatus = TRK_IN_BRANCH;
						};
					} else {
						if (MoveDir) {
		...
		case TRK_WAIT_BRANCH:
			if (PointStatus & TRK_BRANCH_MASK) {
				FrontPoint.PointStatus = TRK_IN_BRANCH;
				//					if(pBranch != FrontPoint.pBranch) ErrH.Abort("Error in Wait
				// Branch");
			};
			break;
			//-----------------------------------------------------------------------------------------------
		...
		case TRK_IN_BRANCH:
			if (SpeedDir > 0) {
				if (MoveDir)
					vDirect = Vector(
						getDistX(FrontPoint.pNextLink->x, XCYCL(R_curr.x + vUp.x)),
						getDistY(FrontPoint.pNextLink->y, R_curr.y + vUp.y),
						0
					);
				else
					vDirect = Vector(
						getDistX(FrontPoint.pPrevLink->x, XCYCL(R_curr.x + vUp.x)),
						getDistY(FrontPoint.pPrevLink->y, R_curr.y + vUp.y),
						0
					);
			} else {
				if (MoveDir)
					vDirect = Vector(
						getDistX(FrontPoint.pNextLink->x, XCYCL(R_curr.x + vDown.x)),
						getDistY(FrontPoint.pNextLink->y, R_curr.y + vDown.y),
						0
					);
				else
					vDirect = Vector(
						getDistX(FrontPoint.pPrevLink->x, XCYCL(R_curr.x + vDown.x)),
						getDistY(FrontPoint.pPrevLink->y, R_curr.y + vDown.y),
						0
					);
			};
			break;
		...
		case TRK_TARGET_NODE:
			if (SpeedDir > 0)
				vDirect = Vector(
					getDistX(FrontPoint.pNode->x, XCYCL(R_curr.x + vUp.x)),
					getDistY(FrontPoint.pNode->y, R_curr.y + vUp.y),
					0
				);
			else
				vDirect = Vector(
					getDistX(FrontPoint.pNode->x, XCYCL(R_curr.x + vDown.x)),
					getDistY(FrontPoint.pNode->y, R_curr.y + vDown.y),
					0
				);
			break;
		};
...
void TrackUnit::GetTrackVector(void) {
	...
	if (PointStatus & TRK_BRANCH_MASK) {
		...
		LinkDist(
			tx, ty, pPrevLink->xl, pPrevLink->yl, pNextLink->xl, pNextLink->yl, radius, vCheck
		);
		LinkDist(
			tx, ty, pNextLink->xr, pNextLink->yr, pPrevLink->xr, pPrevLink->yr, radius, vCheck
		);
		LinkDist(
			vPoint.x,
			vPoint.y,
...
void ActionDispatcher::Init(Parser &in) {
	...
	for (i = 0; i < PI; i++)
		SpeedAngleTable[i] = (i * 1024) / PI;
	for (i = 0; i < PI * 2; i++)
		RudderAngleTable[i] = (int)(sin((double)(i)*M_PI / (double)(PI)) * 1024.) * PI / 2;
	...
	if (Active && Active->ID == ID_VANGER) {
		for (i = 0; i < MAX_ACTIVE_SLOT; i++) {
			if (Slot[i] && Slot[i]->ActIntBuffer.type == ACI_RADAR_DEVICE) {
				...
				lp_log = 1;
				while (p) {
					if (p != Active && p->Visibility == VISIBLE &&
						p->ExternalMode == EXTERNAL_MODE_NORMAL && p->ExternalDraw &&
		...
		if (LocatorPoint) {
			LightData.set_position(
				LocatorPoint->R_curr.x, LocatorPoint->R_curr.y, LocatorPoint->R_curr.z
			);
			LightData.Quant();
		};
		...
		if (Active && Active->uvsPoint->Pworld &&
			Active->uvsPoint->Pworld->gIndex == CurrentWorld) {
			if (NetworkON) {
				drop_log = 0;
				RaceTxtBuff.init();
				aciRacingFlag = 1;
				...
				case VAN_WAR:
					...
					if (my_server_data.Van_War.TeamMode) {
						...
						n_total = 10;
						for (i = 0; i < 10; i++) {
							if (n_score[i] >= my_server_data.Van_War.MaxKills)
								drop_log = 1;
							if (i != my_player_body.color) {
								if (n_score[i] > n_score[my_player_body.color]) {
									...
									if (n_score[i] == n_score[my_player_body.color] &&
										c_score[i] > c_score[my_player_body.color])
										n_position++;
								};
							};
						};
						...
						while (pd) {
							if (pd->status == GAMING_STATUS) {
								if (pd->body.kills >= my_server_data.Van_War.MaxKills)
									drop_log = 1;
								if (pd->client_ID != GlobalStationID) {
									if ((pd->body.kills - pd->body.deaths) > n_current)
										...
										if ((pd->body.kills - pd->body.deaths) == n_current &&
											pd->body.beebos > my_player_body.beebos)
											n_position++;
									};
								};
								n_total++;
						...
						if (pd->status == GAMING_STATUS) {
							if (pd->client_ID != GlobalStationID) {
								if (pd->body.PassemblossStat.CheckpointLighting > n_current)
									...
									if (pd->body.PassemblossStat.CheckpointLighting == n_current) {
										if (pd->body.PassemblossStat.MaxTime <
											my_player_body.PassemblossStat.MaxTime)
											n_position++;
										else {
											if (pd->body.PassemblossStat.MaxTime ==
													my_player_body.PassemblossStat.MaxTime &&
												UsedCheckNum < GloryPlaceNum) {
												if (pd->body.world == CurrentWorld) {
													if (GloryPlaceData[UsedCheckNum].World ==
														CurrentWorld) {
														if (CurrentWorld < MAIN_WORLD_MAX - 1) {
															if (abs(pd->y -
																	GloryPlaceData[UsedCheckNum]
																		.R_curr.y) <
																abs(ActD.Active->R_curr.y -
																	GloryPlaceData[UsedCheckNum]
																		.R_curr.y))
																n_position++;
														} else {
														...
														pass = GetPassage(
															CurrentWorld,
															GloryPlaceData[UsedCheckNum].World
														);
														if (pass) {
															if (CurrentWorld < MAIN_WORLD_MAX - 1) {
																if (abs(pd->y - pass->pos_y) <
																	abs(ActD.Active->R_curr.y -
																		pass->pos_y))
																	n_position++;
															} else {
																if (abs(
																		getDistY(pd->y, pass->pos_y)
																	) <
																	abs(getDistY(
																		ActD.Active->R_curr.y,
																		pass->pos_y
				...
				case MECHOSOMA:
					...
					if (my_server_data.Mechosoma.TeamMode) {
						...
						for (i = 0; i < 10; i++) {
							if (i != my_player_body.color) {
								if ((n_score[i] + n2_score[i]) >
									(n_score[my_player_body.color] +
										n2_score[my_player_body.color])) {
									n_position++;
								} else {
									if ((n_score[i] + n2_score[i]) ==
											(n_score[my_player_body.color] +
						...
						RaceTxtBuff <= n_position < " TEAM";
						if (n_score[my_player_body.color] >=
								my_server_data.Mechosoma.ProductQuantity1 &&
							n2_score[my_player_body.color] >=
								my_server_data.Mechosoma.ProductQuantity2) {
							GameOverID = GAME_OVER_NETWORK;
							ActD.Active->Status |= SOBJ_DISCONNECT;
							GameQuantReturnValue = RTO_LOADING3_ID;
						};
						...
						n_position = 1;
						n_total = 0;
						n_current = my_player_body.MechosomaStat.ItemCount1 +
									my_player_body.MechosomaStat.ItemCount2;
						while (pd) {
							if (pd->status == GAMING_STATUS) {
								if (pd->client_ID != GlobalStationID) {
									if ((pd->body.MechosomaStat.ItemCount1 +
											pd->body.MechosomaStat.ItemCount2) > n_current)
										n_position++;
									else {
										if ((pd->body.MechosomaStat.ItemCount1 +
												pd->body.MechosomaStat.ItemCount2) == n_current) {
											if (pd->body.MechosomaStat.MinTransitTime <
												my_player_body.MechosomaStat.MinTransitTime)
												n_position++;
											else {
												if (pd->body.MechosomaStat.MinTransitTime ==
														my_player_body.MechosomaStat
															.MinTransitTime &&
													pd->body.beebos > my_player_body.beebos)
													n_position++;
											};
					...
					if (uvsCurrentCycle == 1 && Active->VangerRaceStatus == VANGER_RACE_ELR) {
						...
						while (p) {
							if (p->VangerRaceStatus == VANGER_RACE_ELR) {
								if (p != Active && p->R_curr.y > Active->R_curr.y)
									my++;
								i++;
							};
							p = (VangerUnit *)(p->NextTypeList);
						...
						RaceTxtBuff <= (my + uvsGameRaceFinish()) < "|" <=
							/*uvsGameRaceNumber()*/ i;
						strcpy(aciCurRaceInfo, RaceTxtBuff.GetBuf());
						aciRacingFlag = 1;
					};
				...
				case WORLD_NECROSS:
					if (uvsCurrentCycle == 1) {
						...
						p = (VangerUnit *)(ActD.Tail);
						while (p) {
							//						if(p->uvsPoint->status ==
							// UVS_VANGER_STATUS::RACE_HUNTER){
						...
						if (!BigZek) {
							...
							while (BigZek) {
								if (BigZek->ActIntBuffer.type == ACI_WEEZYK)
									break;
							...
							if (BigZek->Owner) {
								...
								while (p) {
									if (p->VangerRaceStatus == VANGER_RACE_ZLP) {
										if (p != BigZek->Owner &&
											p->R_curr.y > BigZek->Owner->R_curr.y)
											my++;
										i++;
									};
									p = (VangerUnit *)(p->NextTypeList);
		...
		if (aiCutLuck > 100)
			...
			aiCutLuck = 0;
		if (aiCutDominance > 100)
			aiCutDominance = 100;
		else if (aiCutDominance < -100)
			aiCutDominance = -100;
...
void ActionDispatcher::ChangeLocator(void) {
	if (LocatorNum) {
		if (LocatorOffset == -1)
			...
			LocatorOffset = (LocatorOffset + 1) % LocatorNum;
...
void TrackUnit::Hide2Show(void) {
	/*	if(pBranch){
			if(pNextLink){
				if(pNextLink->Level == 1) set_3D(SET_3D_TO_THE_UPPER_LEVEL
	   ,R_curr.x,R_curr.y,pNextLink->z,radius / 2,-Angle,Speed); else{ if(pNextLink->Level == 0)
	   set_3D(SET_3D_TO_THE_LOWER_LEVEL,R_curr.x,R_curr.y,pNextLink->z,radius / 2,-Angle,Speed);
					else ErrH.Abort("Bad Level");
				};
			}else ErrH.Abort("Bad Link in Hide2Show");
		}else{
			if(pNode){
				if(pNode->Level == 1) set_3D(SET_3D_TO_THE_UPPER_LEVEL
	   ,R_curr.x,R_curr.y,pNode->z,radius / 2,-Angle,Speed); else{ if(pNode->Level == 0)
	   set_3D(SET_3D_TO_THE_LOWER_LEVEL,R_curr.x,R_curr.y,pNode->z,radius / 2,-Angle,Speed); else
	   ErrH.Abort("Bad Level");
				};
	...
	if (pBranch) {
		if (pNextLink)
			set_3D(SET_3D_DIRECT_PLACE, R_curr.x, R_curr.y, pNextLink->z + zmax, 0, -Angle, Speed);
		//		else ErrH.Abort("Bad Link in Hide2Show");
	} else {
		if (pNode)
			set_3D(SET_3D_DIRECT_PLACE, R_curr.x, R_curr.y, pNode->z + zmax, 0, -Angle, Speed);
		//		else ErrH.Abort("Bad TrackInfo in Hide2Show");
	};
};
...
void VangerUnit::Hide2Show(void) {
	if (aiMoveMode == AI_MOVE_TRACK) {
		if (pBranch) {
			if (pNextLink)
				set_3D(
					SET_3D_DIRECT_PLACE, R_curr.x, R_curr.y, pNextLink->z + zmax, 0, -Angle, Speed
				);
			//			else ErrH.Abort("Bad Link in Hide2Show");
		} else {
			if (pNode)
...
void ActionDispatcher::keyhandler(int key) {
	if (Active) {
		if (Active->Status & SOBJ_DISCONNECT)
			Active = NULL;
		else
			Active->keyhandler(key);
	};
};
...
void camera_quant(int X, int Y, int Turn, double V_abs) {
	...
	if (time_vibration) {
		...
		camera_vibration_x += (k * sin(1.276 * vibration_t) + .8 * sin(2.878 * vibration_t) +
								  0.3 * sin(23.876 * vibration_t)) *
							  XTCORE_FRAME_NORMAL;
		ViewX += (t = round(camera_vibration_x));
		camera_vibration_x -= t;
		camera_vibration_y += (k * sin(0.981 * vibration_t) + .8 * sin(2.98 * vibration_t) +
								  0.3 * sin(20.82 * vibration_t)) *
							  XTCORE_FRAME_NORMAL;
	...
	int s = camera_slope_enable
				? (V_abs < camera_vmax
						  ? camera_slope_min + (-SLOPE_MAX - camera_slope_min) * V_abs / camera_vmax
						  : -SLOPE_MAX)
				: camera_slope_min;
	...
	camera_vt += (double)DistPi(camera_rotate_enable ? Turn : 0, TurnAngle) * camera_mit *
				 XTCORE_FRAME_NORMAL;
...
void InsectUnit::CreateInsect(void) {
	...
	HideMoveAccumY = 0.0;
	Target = R_curr +
			 Vector(INSECT_RADIUS - RND(INSECT_RADIUS2), INSECT_RADIUS - RND(INSECT_RADIUS2), 0);
	if (!RND(InsectD.NumInsect[2] * INSECT_PRICE_DATA[2]))
		BeebType = 2;
	else {
		if (!RND(InsectD.NumInsect[1] * INSECT_PRICE_DATA[1]))
			BeebType = 1;
		else
			BeebType = 0;
	};
...
void InsectUnit::Quant(void) {
	...
	if (Status & SOBJ_AUTOMAT) {
		...
		if (vInsectTarget.y == -1) {
			...
			Target = vInsectTarget;

		dx = getDistX(Target.x, R_curr.x);
		dy = getDistY(Target.y, R_curr.y);

		/*		if(dx == 0 && dy == 0){
					if(ActD.Active) Target = Vector(ActD.Active->R_curr.x + INSECT_RADIUS -
		   RND(INSECT_RADIUS2),ActD.Active->R_curr.y + INSECT_RADIUS - RND(INSECT_RADIUS2),0); else
		   Target = R_curr + Vector(INSECT_RADIUS - RND(INSECT_RADIUS2),INSECT_RADIUS -
		   RND(INSECT_RADIUS2),0); cycleTor(Target.x,Target.y);
				};*/
		...
		if (NumCalcUnit)
			legacy_direct /= NumCalcUnit;

		runtime_direct = DBV(legacy_direct) * XTCORE_FRAME_NORMAL;

		MoveAngle = runtime_direct() ? rPI((int)RTOG(runtime_direct.psi())) : 0;
		DeltaSpeed = runtime_accumulated_scalar_step(
			(double)legacy_direct.vabs() * XTCORE_FRAME_NORMAL, VisibleDirectSpeedAccum
		);
...
void InsectUnit::Touch(GeneralObject *p) {
	...
	case ID_VANGER:
		if (p->Status & SOBJ_ACTIVE) {
			...
			if (BeebType == 2)
				uvsCheckKronIventTabuTask(UVS_KRON_EVENT::GOLD_BEEB, 1);
			else
				uvsCheckKronIventTabuTask(UVS_KRON_EVENT::BEEB, 1);
			if (NetworkON) {
...
void VangerUnit::DrawQuant(void) {
	...
	if (!ExternalDraw || (Status & SOBJ_WAIT_CONFIRMATION))
		return;
	TrackUnit::DrawQuant();
	Vector vCheck;
				...
				if (CurrentWorld == WORLD_THREALL) {
					if (threall_lava_damage_tick()) {
						if (NetworkON)
							...
							BulletCollision((MaxEnergy + MaxArmor) / 20, NULL);
					}
				} else {
					if (!(dynamic_state & TOUCH_OF_AIR)) {
...
#	ifdef TEST_TRACK
	...
	if (Status & SOBJ_ACTIVE) {
		...
		if (TestLevelFlag) {
			if (pNode) {
				if (pNode->TrackCount) {
					/*line_trace(Vector(pNode->BorderX[0],pNode->BorderY[0],159),Vector(pNode->BorderX[pNode->NumBranch
					- 1],pNode->BorderY[pNode->NumBranch - 1],159)); for(i = 1;i <
					pNode->NumBranch;i++){
						line_trace(Vector(pNode->BorderX[i],pNode->BorderY[i],159),Vector(pNode->BorderX[i
					- 1],pNode->BorderY[i - 1],159));
						line_trace(Vector(pNode->BorderX[i],pNode->BorderY[i],159),Vector(pNode->x,pNode->y,159));
					};*/
				} else {
					/*line_trace(Vector(pNode->BorderX[0],pNode->BorderY[0],cc2),Vector(pNode->BorderX[pNode->NumBranch
					- 1],pNode->BorderY[pNode->NumBranch - 1],cc2)); for(i = 1;i <
					pNode->NumBranch;i++){
						line_trace(Vector(pNode->BorderX[i],pNode->BorderY[i],cc2),Vector(pNode->BorderX[i
					- 1],pNode->BorderY[i - 1],cc2));
			...
			if (pNode) {
				if (pNode->Level) {
					/*line_trace(Vector(pNode->BorderX[0],pNode->BorderY[0],175),Vector(pNode->BorderX[pNode->NumBranch
					- 1],pNode->BorderY[pNode->NumBranch - 1],175)); for(i = 1;i <
					pNode->NumBranch;i++){
						line_trace(Vector(pNode->BorderX[i],pNode->BorderY[i],175),Vector(pNode->BorderX[i
					- 1],pNode->BorderY[i - 1],175));
...
#	else
			...
			while(van){
				if(van->Visibility == VISIBLE && !(van->Status & SOBJ_ACTIVE)){
					if(TouchSphere(R_curr,vCheck,van->R_curr,van->radius,i))
						line_trace(Vector(XCYCL(van->R_curr.x -
	   radius),van->R_curr.y,159),Vector(XCYCL(van->R_curr.x + radius),van->R_curr.y,175));
				};
	...
	if (PointStatus == TRK_IN_NODE) {
		if (pNode->Level) {
			line_trace(
				Vector(pNode->BorderX[0], pNode->BorderY[0], 175),
				Vector(
					pNode->BorderX[pNode->NumBranch - 1], pNode->BorderY[pNode->NumBranch - 1], 175
				)
			);
			for (i = 1; i < pNode->NumBranch; i++) {
				line_trace(
					Vector(pNode->BorderX[i], pNode->BorderY[i], 175),
					Vector(pNode->BorderX[i - 1], pNode->BorderY[i - 1], 175)
				);
				line_trace(
					Vector(pNode->BorderX[i], pNode->BorderY[i], 175),
					Vector(pNode->x, pNode->y, 175)
				);
			};
		} else {
			line_trace(
				Vector(pNode->BorderX[0], pNode->BorderY[0], 159),
				Vector(
					pNode->BorderX[pNode->NumBranch - 1], pNode->BorderY[pNode->NumBranch - 1], 159
				)
			);
			for (i = 1; i < pNode->NumBranch; i++) {
				line_trace(
					Vector(pNode->BorderX[i], pNode->BorderY[i], 159),
					Vector(pNode->BorderX[i - 1], pNode->BorderY[i - 1], 159)
				);
				line_trace(
					Vector(pNode->BorderX[i], pNode->BorderY[i], 159),
					Vector(pNode->x, pNode->y, 159)
				);
			};
		};
	};
	...
	line_trace(
		Vector(R_curr.x, R_curr.y, 170),
		Vector(R_curr.x + vTarget.x / 2, R_curr.y + vTarget.y / 2, 170)
	);

	if (SpeedDir == 0 || (OtherFlag & MECHOS_CALC_WAY)) {
		line_trace(
			Vector(FrontPoint.pNextLink->xl, FrontPoint.pNextLink->yl, 183),
			Vector(FrontPoint.pNextLink->xr, FrontPoint.pNextLink->yr, 159)
		);
		line_trace(
			Vector(FrontPoint.pPrevLink->xl, FrontPoint.pPrevLink->yl, 183),
			Vector(FrontPoint.pPrevLink->xr, FrontPoint.pPrevLink->yr, 159)
		);
	} else {
		line_trace(
			Vector(FrontPoint.pNextLink->xl, FrontPoint.pNextLink->yl, 183),
			Vector(FrontPoint.pNextLink->xr, FrontPoint.pNextLink->yr, 183)
		);
		line_trace(
			Vector(FrontPoint.pPrevLink->xl, FrontPoint.pPrevLink->yl, 183),
			Vector(FrontPoint.pPrevLink->xr, FrontPoint.pPrevLink->yr, 183)
		);
	};

	line_trace(
		Vector(pNextLink->xl, pNextLink->yl, 195), Vector(pNextLink->xr, pNextLink->yr, 210)
	);
	line_trace(
		Vector(pPrevLink->xl, pPrevLink->yl, 195), Vector(pPrevLink->xr, pPrevLink->yr, 210)
	);
#	endif
...
void VangerUnit::Quant(void) {
	...
	if (Status & SOBJ_WAIT_CONFIRMATION)
		return;
	RuffaGunTime++;
	...
	if (VangerRaceStatus != VANGER_RACE_ZLP)
	...
	while (p) {
		...
		case ACI_ELEECH:
			if (CurrentWorld == WORLD_FOSTRAL)
				VangerRaceStatus = VANGER_RACE_ELR;
			break;
		case ACI_KERNOBOO:
			if (CurrentWorld == WORLD_GLORX)
				VangerRaceStatus = VANGER_RACE_KRW;
			break;
		case ACI_PIPETKA:
			if (CurrentWorld == WORLD_GLORX)
				VangerRaceStatus = VANGER_RACE_PPS;
			break;
		case ACI_WEEZYK:
	...
	if ((Status & SOBJ_ACTIVE) && CurrentWorld == WORLD_NECROSS && uvsCurrentCycle == 1 &&
		uvsGamerWaitGame)
		VangerRaceStatus = VANGER_RACE_ZLP;
	...
	if (NetworkON && !(Status & SOBJ_ACTIVE)) {
		/*		if(aciWorldIndex != -1){
					...
					aciWorldIndex = -1;
					EffD.CreateRingOfLord(EFF_PARTICLE06,R_curr +
		   Vector(0,0,80),radius*2,200,111,111,radius << 7); ExternalLock = 1; ExternalObject =
		   NULL;
		...
		if (Status & SOBJ_AUTOMAT) {
			...
			if (aciWorldIndex != -1) {
				if (!(Status & SOBJ_AUTOMAT)) {
					...
					uvsPoint->Pworld = WorldTable[aciWorldIndex];
					EffD.CreateRingOfLord(
						EFF_PARTICLE06,
						R_curr + Vector(0, 0, 80),
						radius * 2,
						200,
						111,
						111,
			...
			if (aciTeleportEvent != 0) {
				if (!(Status & SOBJ_AUTOMAT)) {
					...
					switch_analysis(1);
					for (i = 0; i < 5; i++)
						EffD.CreateParticleGenerator(
							R_curr, R_curr, Vector(5, 0, 0) * DBM((int)(RND(PI * 2)), Z_AXIS)
						);
					SOUND_BOOT_START();
				};
				aciTeleportEvent = 0;
	...
	if (!NetworkON && CurrentWorld == WORLD_KHOX && ExternalMode == EXTERNAL_MODE_NORMAL &&
		!PalIterLock && PalCD.PalEnable) {
		if (KhoxPoison < 0) {
			UseOxigenResource();
		} else {
			for (i = 0; i < 128 * 3; i++)
				palbufOrg[i] = palbuf[i] =
					(uchar)(KhoxPoison * (int)(palbufSrc[i]) / MaxKhoxPoison);
	...
	ItemQuant();
	if (uvsPoint)
		uvsPoint->update(R_curr.x, R_curr.y);
	...
	if (NetworkON && (Status & SOBJ_ACTIVE)) {
		...
		//		static prevViewY = 0;
		//		if(abs(getDistY(prevViewY,ViewY)) > 10 && IS_PAST(last_send_set_position +
		// average_lag)){ NETWORK_OUT_STREAM.set_position(ViewX,ViewY,round(2.*(fabs(curGMap ->
		// xsize*sinTurnInvFlt) + fabs(curGMap -> ysize*cosTurnInvFlt))*0.5)); 			prevViewY =
		// ViewY; 			last_send_set_position = SDL_GetTicks();
		//			}

		static int need_to_send_vanger = 0;
		if (prev_controls != current_controls || dynamic_state & ITS_MOVING ||
			IS_PAST(last_send_time + 2000))
		...
		if (need_to_send_vanger && IS_PAST(last_send_time + vanger_send_interval)) {
			...
			int scr_size = round(
				(fabs(curGMap->xsize * sinTurnInvFlt) + fabs(curGMap->ysize * cosTurnInvFlt)) * .5
			);
		...
		if (VangerChanger) {
			if (!CheckLiveVangerPointer(VangerChanger)) {
				...
				if (VangerChanger->VangerChanger != this) {
					...
					if (NetChanger == GET_STATION(VangerChanger->NetID) &&
						VangerChanger->NetChanger == GlobalStationID && VangerChangerCount == 2)
...
void VangerUnit::InitEnvironment(void) {
	...
	if (NetworkON) {
		...
		if (pNetPlayer && pNetPlayer->body.CarIndex != uvsPoint->Pmechos->type &&
			!(Status & SOBJ_ACTIVE))
			ChangeVangerProcess();
	} else {
				...
				if (aiActionID == AI_ACTION_BRAKE) {
					...
					if (i > -1) {
						do {
							...
							l = radius + st->radius;
							if (abs(vCheck.x) < l) {
								if (R_curr.z > ((SensorDataType *)(st))->z0 - radius &&
									R_curr.z < ((SensorDataType *)(st))->z1 + radius) {
									vCheck.y = getDistY(st->R_curr.y, R_curr.y);
					...
					if (!mole_on && !SensorEnable) {
						...
						y1 = R_curr.y + radius + MAX_SENSOR_RADIUS;
						i = FindFirstStatic(
							y0, y1, (StaticObject **)SensorSortedData, SnsTableSize
						);
					...
					if (aiStatus & AI_STATUS_TNT) {
						if (PowerFlag & VANGER_POWER_RUFFA_GUN) {
							if (RuffaGunTime >= ruffa_gun_wait_ticks()) {
								...
								g->CreateBullet(
									Vector(R_curr.x, R_curr.y, R_curr.z),
									Vector(
										XCYCL(vCheck.x + R_curr.x),
										YCYCL(vCheck.y + R_curr.y),
										R_curr.z + vCheck.z
									),
									NULL,
									&GameBulletData[WD_BULLET_RUFFA],
									this,
									Speed
								);
								if (ActD.Active)
							...
							for (i = 0; i < ItemMatrix->NumSlot; i++) {
								if (GunSlotData[ItemMatrix->nSlot[i]].pData &&
									(GunSlotData[ItemMatrix->nSlot[i]].pData->BulletMode &
										BULLET_CONTROL_MODE::FLY) &&
									(GunSlotData[ItemMatrix->nSlot[i]].pData->Power < l ||
										l == -1)) {
								...
								if (abs(vCheck.x) < l) {
									if (R_curr.z > ((SensorDataType *)(st))->z0 - radius &&
										...
										if (!strcmp(((SensorDataType *)(st))->Name, "UniDoor")) {
											if (CheckPointCount >= MAX_KHOX_CHECKPOINT) {
												vCheck.y = getDistY(st->R_curr.y, R_curr.y);
												if ((vCheck.x * vCheck.x + vCheck.y * vCheck.y) <
													l * l) {
													st->Touch(this);
													TouchSensor((SensorDataType *)st);
												};
						...
						do {
							...
							if (st->Enable) {
								...
								if (abs(vCheck.x) < l) {
									if (R_curr.z > ((SensorDataType *)(st))->z0 - radius &&
										R_curr.z < ((SensorDataType *)(st))->z1 + radius) {
										if (!strcmp(((SensorDataType *)(st))->Name, "SpotDoor1") ||
											!strcmp(((SensorDataType *)(st))->Name, "SpotDoor2")) {
											...
											aiMessageQueue.Send(
												AI_MESSAGE_SPOBS, Speed
											); // aiMessageData[AI_MESSAGE_SPOBS].Send(Speed);
										} else {
											if (strcmp(((SensorDataType *)(st))->Name, "Spot1") ||
												(!(ActD.FunctionSpobsDestroyActive) &&
													!(ActD.SpobsDestroy) && ActD.XploKeyEnable)) {
												vCheck.y = getDistY(st->R_curr.y, R_curr.y);
												if ((vCheck.x * vCheck.x + vCheck.y * vCheck.y) <
													l * l) {
													if (((SensorDataType *)(st))->data6)
														aiMessageTouch(
															Speed,
															((SensorDataType *)(st))->data6 - 1
														);
													st->Touch(this);
													TouchSensor((SensorDataType *)st);
												};
								...
								if (abs(vCheck.x) < l) {
									if (R_curr.z > ((SensorDataType *)(st))->z0 - radius &&
										...
										if (!strcmp(((SensorDataType *)(st))->Name, "SIGN")) {
											if (!NetworkON && ActD.Active &&
												ActD.Active->ExternalMode == EXTERNAL_MODE_NORMAL &&
												ActD.Active->ExternalDraw && !ActD.ThreallDestroy) {
												vCheck.y = getDistY(st->R_curr.y, R_curr.y);
												if ((vCheck.x * vCheck.x + vCheck.y * vCheck.y) <
													l * l) {
													st->Touch(this);
											...
											if ((vCheck.x * vCheck.x + vCheck.y * vCheck.y) <
												...
												if (((SensorDataType *)(st))->data6)
													aiMessageTouch(
														Speed, ((SensorDataType *)(st))->data6 - 1
													);
												st->Touch(this);
												TouchSensor((SensorDataType *)st);
											};
					...
					if (st->Enable) {
						...
						if (abs(vCheck.x) < l) {
							...
							if ((vCheck.x * vCheck.x + vCheck.y * vCheck.y) < l * l) {
								st->Touch(this);
								if (ExternalSensor) {
									if (ExternalSensor == ExternalObject && ExternalObject != st)
										ExternalSensor = (SensorDataType *)(st);
								} else
									ExternalSensor = (SensorDataType *)(st);
							};
						};
							...
							if(abs(dx) < ViewRadius){
								switch(p->ID){
									case ID_VANGER:
										...
										if(r > 0){
											PointDist(-dx,-dy,R_curr.z - p->R_curr.z,r,2*radius +
	   p->radius,1,vUp,vTarget); NumCalcUnit++;
										};
										break;
								};
							};
...
void VangerUnit::AutomaticTouchSensor(SensorDataType *p) // znfo !!!
{
	...
	if (p->Enable) {
		...
		case SensorTypeList::TRAP:
			if (!(Status & SOBJ_ACTIVE)) {
				if (ExternalSensor) {
					if (ExternalSensor == ExternalObject && ExternalObject != p)
						ExternalSensor = p;
				} else
					ExternalSensor = p;
			};
			...
			if (!(Status & SOBJ_ACTIVE)) {
				if (p->Owner) {
					if (p->Owner->Type == EngineTypeList::DOOR) {
						...
						if (d) {
							if (aiLocalTarget && aiLocalTarget->Type == UNIT_ORDER_ENTER &&
								FindDoor(d, etype) == (aiLocalTarget->Obj).EnterT) {
								switch (etype) {
								...
								case EngineTypeList::ESCAVE:
									if (TouchKeyObject &&
										TouchKeyObject->SensorType == SensorTypeList::ESCAVE &&
										d->Mode == EngineModeList::OPEN && !d->ProcessFlag)
										d->CloseDoor(10);
									break;
								};
							};
		...
		if (p->SensorType == SensorTypeList::SPOT || p->SensorType == SensorTypeList::ESCAVE ||
...
void VangerUnit::TouchSensor(SensorDataType *p) {
	...
	case SensorTypeList::ARMOR_UPDATE:
		if (Armor < MaxArmor) {
			...
			aiMessageQueue.Send(
				AI_MESSAGE_ARMOR, Speed, 2, 0
			); // aiMessageData[AI_MESSAGE_ARMOR].Send(Speed,2,0);
			SOUND_CHARGE_REPARE();
		};
		aiMessageQueue.Send(
			AI_MESSAGE_ARMOR, Speed, 1
	...
	case SensorTypeList::RANDOMIZE_UPDATE:
		aiMessageQueue.Send(
			AI_MESSAGE_RANDOM_UPDATE, Speed, 1
		); // aiMessageData[AI_MESSAGE_RANDOM_UPDATE].Send(Speed,1);
			...
			while (!r_log) {
				...
				case 2:
					if (Energy < MaxEnergy - 1) {
						aiMessageQueue.Send(
							AI_MESSAGE_ENERGY, Speed, 1, 0
						); // aiMessageData[AI_MESSAGE_ENERGY].Send(Speed,1,0);
						Energy = MaxEnergy - 1;
						r_log = 1;
				...
				case 3:
					if (Energy > 0) {
						aiMessageQueue.Send(
							AI_MESSAGE_DENERGY, Speed, 1, 0
						); // aiMessageData[AI_MESSAGE_DENERGY].Send(Speed,1,0);
						Energy = 0;
						r_log = 1;
						SOUND_DISCHARGE();
					};
	...
	case SensorTypeList::FLY_UPDATE:
		aiMessageQueue.Send(
			AI_MESSAGE_COPTER, Speed, 1
		); // aiMessageData[AI_MESSAGE_COPTER].Send(Speed,1);
		UpdateStationTouched = 1;
		if (!UpdateStationActive) {
			UpdateStationActive = 1;
	...
	case SensorTypeList::FIRE_UPDATE:
		aiMessageQueue.Send(
			AI_MESSAGE_GHORB, Speed, 1
		); // aiMessageData[AI_MESSAGE_GHORB].Send(Speed,1);
		ChargeWeapon(this, ACI_GHORB_GEAR_LIGHT, 1);
		ChargeWeapon(this, ACI_GHORB_GEAR_HEAVY, 1);
	...
	case SensorTypeList::KEY_UPDATE:
		aiMessageQueue.Send(
			AI_MESSAGE_SPIRAL, Speed, 1
		); // aiMessageData[AI_MESSAGE_SPIRAL].Send(Speed,1);
		if (PassageCount < MaxPassageCount) {
			aiMessageQueue.Send(
				AI_MESSAGE_SPIRAL, Speed, 2, 0
			); // aiMessageData[AI_MESSAGE_SPIRAL].Send(Speed,2,0);
			aciWorldLinksON();
...
void VangerUnit::SensorQuant(void) {
	...
	if (!ExternalLock && ExternalLastSensor != ExternalSensor) {
		if (ExternalSensor) {
			...
			case SensorTypeList::SPOT:
				if (Status & SOBJ_AUTOMAT) {
					if (aiLocalTarget && aiLocalTarget->Type == UNIT_ORDER_ENTER &&
						(aiLocalTarget->Obj).EnterT ==
							((EnterEngine *)(ExternalSensor->Owner))->Owner) {
			...
			case SensorTypeList::PASSAGE:
				if (Status & SOBJ_AUTOMAT) {
					if (aiLocalTarget && aiLocalTarget->Type == UNIT_ORDER_PASSAGE &&
						((aiLocalTarget->Obj).PassageT)->ActionLink == ExternalSensor) {
						...
						if (Visibility == VISIBLE) {
							...
							if (Status & SOBJ_ACTIVE) {
								camera_direct(
									ExternalSensor->R_curr.x,
									ExternalSensor->R_curr.y,
									1 << 7,
									0,
									0,
									ExternalTime + 1
					...
					if (aciWorldLinkExist(
							((PassageEngine *)(ExternalSensor->Owner))->uvsPort->Pworld->gIndex,
							((PassageEngine *)(ExternalSensor->Owner))->uvsPort->Poutput->gIndex
						)) {
						if (!ActD.PassageTouchEnable) {
							PassageCount--;
							aciSendEvent2actint(ACI_LOCK_INTERFACE, NULL);
							...
							if (Visibility == VISIBLE) {
								ExternalTime =
									ExternalSensor->Owner->DeactiveTime * GAME_TIME_COEFF;
								CreateParticleRotor(ExternalSensor->R_curr, ExternalSensor->radius);
								if (Status & SOBJ_ACTIVE) {
									camera_direct(
										ExternalSensor->R_curr.x,
			...
			case SensorTypeList::TRAP:
				if (ExternalSensor->Index == 1) {
					if (((TrainEngine *)(ExternalSensor->Owner))->LockFlag & DOOR_CLOSE_LOCK)
						break;
					ExternalObject = ((TrainEngine *)(ExternalSensor->Owner))->TrainLink[0];
				} else {
					if (((TrainEngine *)(ExternalSensor->Owner))->LockFlag & DOOR_OPEN_LOCK)
						break;
			...
			case SensorTypeList::EARTH_PASSAGE:
				if (GamerResult.earth_unable && aciCurJumpCount > 0) {
					...
					instant_impulse(Vector(32 - RND(64), 32 - RND(64), RND(64)), RND(30), RND(10));
				break;
			};
		};
	};
	...
	case EXTERNAL_MODE_TEST_MOVE:
		if (ExternalTime <= 0) {
			...
			vCheck = Vector(getDistX(pNextLink->x, pPrevLink->x), pNextLink->y - pPrevLink->y, 0);
			set_3D(
				SET_3D_CHOOSE_LEVEL, vGoTest.x, vGoTest.y, vGoTest.z, radius / 2, -vCheck.psi(), 0
			);
		} else {
			vCheck =
				Vector(getDistX(vGoTest.x, R_curr.x), vGoTest.y - R_curr.y, vGoTest.z - R_curr.z);
			d = vCheck.vabs();
			l = d / ExternalTime;
			vCheck *= l;
			vCheck /= d;
	...
	case EXTERNAL_MODE_MOVE:
		if (ExternalTime <= 0) {
			...
			vCheck = Vector(
				getDistX(ExternalObject->R_curr.x, R_curr.x),
				ExternalObject->R_curr.y - R_curr.y,
				ExternalObject->R_curr.z - R_curr.z
			);
			d = vCheck.vabs();
			a = vCheck.psi();
			l = d / ExternalTime;
			...
			cycleTor(R_curr.x, R_curr.y);
			if (Visibility == VISIBLE)
				set_3D(SET_3D_DIRECT_PLACE, R_curr.x, R_curr.y, R_curr.z, 0, -Angle, Speed);
			ExternalTime--;
		};
		break;
	...
	case EXTERNAL_MODE_ESCAVE_IN:
		...
		if (ExternalTime <= 0) {
			...
			ExternalLock = 0;
			uvsPoint->Pescave =
				(uvsEscave *)(((EnterEngine *)(ExternalObject->Owner))->Owner->Owner);
			uvsPoint->Pspot = NULL;
			Go2Universe();
			...
			if (ActD.LuckyFunction)
				...
				if (ActD.ThreallDestroy) {
					if (ActD.SpobsDestroy)
						GameOverID = GAME_OVER_ALL_LOCKED;
					else
						GameOverID = GAME_OVER_INFERNAL_LOCKED;
				} else {
					if (ActD.SpobsDestroy)
						GameOverID = GAME_OVER_SPOBS_LOCKED;
					else
						GameOverID = GAME_OVER_LUCKY;
				};
			};
		...
		case AI_MOVE_FUNCTION_WHEEL:
			if (aiRelaxTime > aiRelaxTimeMax && !(aiStatus & AI_STATUS_TARGET)) {
				...
				if ((dynamic_state & GROUND_COLLISION) && !(dynamic_state & TRACTION_WHEEL_TOUCH)) {
					if (a > 0)
						controls(CONTROLS::LEFT_SIDE_IMPULSE, 20);
					else
						controls(CONTROLS::RIGHT_SIDE_IMPULSE, 20);
				};
		...
		case AI_MOVE_FUNCTION_WHEEL:
			if (aiStatus & AI_STATUS_WHEEL) {
				...
				if (!(aiStatus & AI_STATUS_TARGET) || WallCollisionTime) {
					if (aiStatus & AI_STATUS_FLY)
						aiMoveFunction = AI_MOVE_FUNCTION_FLY;
					else
						aiMoveFunction = AI_MOVE_FUNCTION_IMPULSE;
				};
			} else {
				if (aiStatus & AI_STATUS_FLY)
					aiMoveFunction = AI_MOVE_FUNCTION_FLY;
		...
		case AI_MOVE_FUNCTION_FLY:
			if ((aiStatus & AI_STATUS_TARGET) || !(aiStatus & AI_STATUS_FLY)) {
				...
				if (aiRelaxTime > aiRelaxTimeMax * 5) {
					if (aiStatus & AI_MOVE_FUNCTION_IMPULSE)
						aiMoveFunction = AI_MOVE_FUNCTION_IMPULSE;
					else
						aiMoveFunction = AI_MOVE_FUNCTION_WHEEL;
				} else {
		...
		case AI_MOVE_FUNCTION_IMPULSE:
			if (!(aiStatus & AI_STATUS_IMPULSE) || (aiStatus & AI_STATUS_TARGET)) {
				...
				case 0:
					if (dynamic_state & TOUCH_OF_GROUND) {
						vCheck = Vector(
							getDistX(TargetPoint.vPoint.x, R_curr.x),
							getDistY(TargetPoint.vPoint.y, R_curr.y),
							0
						);
						d = vCheck.vabs();
						if (d < aiMaxJumpRadius)
							jump_power = d * (max_jump_power - 1) / aiMaxJumpRadius;
						else
							jump_power = max_jump_power - 1;
						controls(CONTROLS::JUMP_USING_ACCUMULATED_POWER, 20);
						aiReactionMode = 1;
						aiRelaxTime = 0;
						aiReactionCheckTime = 0;
				...
				case 1:
					if (aiRelaxTime > aiRelaxTimeMax || WallCollisionTime) {
						...
						if (aiReactionCheckTime >=
							reaction_check_timeout_ticks(aiReactionCheckTimeMax)) {
							aiMoveFunction = AI_MOVE_FUNCTION_WHEEL;
							aiReactionMode = 1;
							aiRelaxTime = 0;
							aiReactionCheckTime = 0;
				...
				case 2:
					...
					if (abs(a) < PI / 8 || aiReactionCheckTime > 2 * aiRelaxTimeMax)
						aiReactionMode = 0;
					else {
						aiReactionCheckTime++;
...
void VangerUnit::CreateVangerUnit(void) {
	...
	NetRuffaGunTime = 0;
	PrevImpuseFrame = 0;
	PrevNetFunction83Time = NetFunction83Time = NetProtractorFunctionTime = NetMessiahFunctionTime =
		0;
};
...
void VangerUnit::AddFree(void) {
	...
	if (Status & SOBJ_ACTIVE) {
		if (vSetVangerFlag != -1) {
			...
			for (i = 0; i < 5; i++) {
				...
				EffD.CreateParticleGenerator(
					vCheck,
					R_curr,
					Vector(5, 0, 0) * DBM((int)(RND(PI * 2)), Z_AXIS),
					PG_STYLE_REINCAR
				);
			};
		...
		if (NetworkON && Visibility == VISIBLE && pNetPlayer &&
			GetDistTime(NetGlobalTime, pNetPlayer->body.BirthTime) <
				256 * 5 + 256 * (ROTOR_PROCESS_LIFE_TIME * GAME_TIME_COEFF) / 20) {
			ExternalMode = EXTERNAL_MODE_PASS_OUT;
			ExternalObject = NULL;
			ExternalTime = ROTOR_PROCESS_LIFE_TIME * GAME_TIME_COEFF;
			ExternalLock = 1;
...
void VangerUnit::AddPassage(SensorDataType *p) {
	if (Status & SOBJ_ACTIVE) {
		...
		if (NetworkON) {
			if (Visibility == VISIBLE && pNetPlayer &&
				GetDistTime(NetGlobalTime, pNetPlayer->body.BirthTime) <
					256 * 5 + 256 * (ROTOR_PROCESS_LIFE_TIME * GAME_TIME_COEFF) / 20) {
				ExternalMode = EXTERNAL_MODE_PASS_OUT;
				ExternalObject = p;
...
VangerUnit *addVanger(uvsVanger *p, int x, int y, int Human) {
	...
	if (n) {
		...
		if (Human) {
			...
			n->CreateActionUnit(
				NumHumanModel, SOBJ_ACTIVE, Vector(x, y, 128), 0, SET_3D_DIRECT_PLACE
			);
		} else
			n->CreateActionUnit(
				ModelD.FindModel(uvsMechosTable[p->Pmechos->type]->name),
				SOBJ_AUTOMAT,
				Vector(x, y, 128),
				0,
				SET_3D_DIRECT_PLACE
...
void ActionDispatcher::PromptChangeCycle(void) {
	if (CurrentWorld == WORLD_FOSTRAL && Active && PromptChangeCycleCount < 7 &&
		!(WorldTable[WORLD_GLORX]->GamerVisit)) {
		if (iGetOptionValue(6) || NetworkON)
			return;
...
void ActionDispatcher::PromptInit(int ind) {
	...
	if (ind)
		PromptIncubatorFreeVisit++;
	else
		PromptPodishFreeVisit++;
...
void ActionDispatcher::PromptQuant(void) {
	...
	if (!NetworkON && Active) {
		...
		switch (CurrentWorld) {
		case WORLD_GLORX:
			if (Active->VangerRaceStatus == VANGER_RACE_NONE && !(ConTimer.min % 30) &&
				ConTimer.sec == 0 && !aciWorldLinkExist(WORLD_GLORX, WORLD_NECROSS, 0)) {
				switch (dgD->bStatus) {
				case 0:
			...
			break;
		case WORLD_NECROSS:
			if (Active->VangerRaceStatus == VANGER_RACE_NONE && ConTimer.min == 0 &&
				ConTimer.sec == 0 && !aciWorldLinkExist(WORLD_GLORX, WORLD_XPLO, 0))
				aiMessageQueue.Send(AI_MESSAGE_NECROSS_MOVE, 0, 0xff, 0);
			break;
		case WORLD_FOSTRAL:
			if (Active->VangerRaceStatus == VANGER_RACE_NONE &&
				!WorldTable[WORLD_GLORX]->GamerVisit) {
				if (PromptCurrentWay) {
					if (PromptIncubatorFreeVisit < 3 && PromptIncubatorCount < 3) {
						if (!(ConTimer.min % 3) && ConTimer.sec == 0 &&
							!GetStuffObject(Active, ACI_NYMBOS)) {
							PromptIncubatorCount++;
							aiMessageQueue.Send(
								AI_MESSAGE_NYMBOS_LOST, 0, 0xff, 0
							); // aiMessageData[AI_MESSAGE_NYMBOS_LOST].Send(0,0xff,0);
							return;
						};
					};
					...
					if (PromptIncubatorFreeVisit < 2) {
						if (getDistY(PromptPrevY, Active->R_curr.y) > 1000) {
							...
							if (PromptPrevTime > prompt_prev_time_threshold_ticks()) {
								...
								PromptPrevTime = 0;
								aiMessageQueue.Send(
									AI_MESSAGE_INCUBATOR_WAY, 0, 0xff, 0
								); // aiMessageData[AI_MESSAGE_INCUBATOR_WAY].Send(0,0xff,0);
								if (!GetCompasTarget()) {
									if (lang() == RUSSIAN)
										SelectCompasTarget(rCmpIncubator);
									else
										SelectCompasTarget(eCmpIncubator);
					...
					if (PromptIncubatorFreeVisit < 2 &&
						...
						&& ConTimer.sec == 0 &&
						ConTimer.counter < 36000) { // do not show message if an 10 game hour(30 min
													// real) has passed
					...
					if (PromptPodishFreeVisit < 3 && PromptPodishCount < 3) {
						if (!(ConTimer.min % 3) && ConTimer.sec == 0 &&
							!GetStuffObject(Active, ACI_PHLEGMA)) {
							PromptPodishCount++;
							aiMessageQueue.Send(
								AI_MESSAGE_PHLEGMA_LOST, 0, 0xff, 0
							); // aiMessageData[AI_MESSAGE_PHLEGMA_LOST].Send(0,0xff,0);
							return;
						};
					};
					...
					if (PromptPodishFreeVisit < 2) {
						if (getDistY(Active->R_curr.y, PromptPrevY) > 1000) {
							...
							if (PromptPrevTime > prompt_prev_time_threshold_ticks()) {
								PromptPrevTime = 0;
								aiMessageQueue.Send(
									AI_MESSAGE_PODISH_WAY, 0, 0xff, 0
								); // aiMessageData[AI_MESSAGE_PODISH_WAY].Send(0,0xff,0);
								if (!GetCompasTarget()) {
									if (lang() == RUSSIAN)
										SelectCompasTarget(rCmpPodish);
									else
										SelectCompasTarget(eCmpPodish);
									aciRefreshTargetsMenu();
								};
								return;
							} else
				...
				if (ConTimer.min == 0 && ConTimer.sec == 0) {
					if (aciWorldLinkExist(WORLD_FOSTRAL, WORLD_GLORX)) {
						aiMessageQueue.Send(
							AI_MESSAGE_GLORX_WAY, 0, 0xff, 0
						); // aiMessageData[AI_MESSAGE_GLORX_WAY].Send(0,0xff,0);
						if (!GetCompasTarget()) {
							if (lang() == RUSSIAN)
								SelectCompasTarget(rCmpPassGlorx);
							else
								SelectCompasTarget(eCmpPassGlorx);
							aciRefreshTargetsMenu();
						};
					} else {
						if (uvsCurrentCycle == 0) {
							if (GamerResult.game_elr_result < 150) {
								aiMessageQueue.Send(
									AI_MESSAGE_ELR_LOW, 0, 0xff, 0
								); // aiMessageData[AI_MESSAGE_ELR_LOW].Send(0,0xff,0);
								if (!GetCompasTarget()) {
									if (lang() == RUSSIAN)
										SelectCompasTarget(rCmpIncubator);
									else
								...
								if (GamerResult.game_elr_result < 250) {
									aiMessageQueue.Send(
										AI_MESSAGE_ELR_HI, 0, 0xff, 0
									); // aiMessageData[AI_MESSAGE_ELR_HI].Send(0,0xff,0);
									if (!GetCompasTarget()) {
										if (lang() == RUSSIAN)
											SelectCompasTarget(rCmpIncubator);
										else
...
VangerUnit *addVanger(uvsVanger *p, uvsEscave *origin, int Human) {
	...
	if (n) {
		...
		n->CreateUnitType(p);
		if (s->Owner->Type == EngineTypeList::ESCAVE)
			ang = ((EscaveEngine *)(s->Owner))->StartAngle;
		else
			ang = RND(PI * 2);
		...
		if (NetworkON) {
			if (Human)
				n->NetCreateVanger(NULL, origin, NULL);
			else
				n->NetCreateSlave();
		};
...
VangerUnit *addVanger(uvsVanger *p, uvsSpot *origin, int Human) {
	...
	if (n) {
		...
		if (NetworkON) {
			if (Human)
				n->NetCreateVanger(NULL, NULL, origin);
			else
				n->NetCreateSlave();
		};
		if (Human)
		...
		if (PowerFlag & VANGER_POWER_RUFFA_GUN) {
			if (RuffaGunTime >= ruffa_gun_wait_ticks()) {
				...
				g->CreateBullet(
					Vector(R_curr.x, R_curr.y, R_curr.z),
					Vector(
						XCYCL(vCheck.x + R_curr.x), YCYCL(vCheck.y + R_curr.y), R_curr.z + vCheck.z
					),
					NULL,
					&GameBulletData[WD_BULLET_RUFFA],
					this,
...
void VangerUnit::CreateParticleMechos(const Vector &v, int r, int _type) {
	...
	particleAmount = 10; // defines how much particles will be created when mechos enters the world,
						 // better but that value to config
		...
		if (p) {
			...
			for (i = 0; i < model->num_poly; i++) {
				...
				for (vi = 0; vi < particleAmount; vi++) {
					p->AddVertex2(
						vPos + Vector(RND(dr), RND(dr), 0),
						Vector(1 - RND(particleAmount), 1 - RND(particleAmount), 0) + v +
							(A * Vector(
									 model->polygons[i].middle_x,
									 model->polygons[i].middle_y,
									 model->polygons[i].middle_z
								 )),
					...
					// 	Vector(1 - RND(8),1- RND(8),0) + v +
					// (A*Vector(model->polygons[i].middle_x,model->polygons[i].middle_y,model->polygons[i].middle_z)),
	...
	if (Visibility == VISIBLE) {
		...
		if (p) {
			...
			for (i = 0; i < (p->NumParticle - model->num_poly * 2); i++) {
				vertexTarget =
					v + (Vector(r - realRND(r << 1), r - realRND(r << 1), r - realRND(r << 1)));
				// vertexTarget *= GAME_TIME_COEFF;
				spd1 = (15 << 8) + realRND(20 << 8);
				p->AddVertex(
					vertexTarget, FIRE_COLOR_FIRST + realRND(FIRE_PROCESS_COLOR_MAX), spd1, spd2
				);
			}
	...
	if(Visibility == VISIBLE){
		...
		if(p){
			...
			for(i = 0;i < model->num_poly;i++){
				...
				p->AddVertex(R_curr +
(Vector(model->polygons[i].middle_x,model->polygons[i].middle_y,model->polygons[i].middle_z)*(A_scl)),
						color_offset + (1 << (7 - color_shift)) - 1,
...
(Vector(model->polygons[i].middle_x,model->polygons[i].middle_y,model->polygons[i].middle_z)*(A_scl)),
			...
			for(i = 0;i < (p->NumParticle - model->num_poly*2);i++)
				p->AddVertex(v + Vector(r - realRND(r << 1),r - realRND(r << 1),r - realRND(r <<
1)), FIRE_COLOR_FIRST + realRND(FIRE_PROCESS_COLOR_MAX), (15 << 8) + realRND(20 << 8),1 << 8);
...
void uvsUnitType::DelDevice(StuffObject *p) {
	...
	if ((!p->PrevDeviceList) && (!p->NextDeviceList))
		DeviceData = NULL;
	else {
		if (p->PrevDeviceList)
			p->PrevDeviceList->NextDeviceList = p->NextDeviceList;
		else
			DeviceData = (StuffObject *)(p->NextDeviceList);
		if (p->NextDeviceList)
			p->NextDeviceList->PrevDeviceList = p->PrevDeviceList;
	};
	NumDevice--;
};
	...
	if (Status & SOBJ_ACTIVE) {
		...
		if (vSetVangerFlag != -1) {
			...
			if (vSetVangerFlag == 0) {
				...
				if (!l)
					ErrH.Abort("Gamer Slave Not Found");
...
void VangerUnit::Go2World(void) {
	...
	for (i = 0; i < ItemMatrix->FullNum; i++)
		ItemMatrixData[i] = 0;
	...
	for (i = 0; i < ItemMatrix->NumSlot; i++)
		GunSlotData[ItemMatrix->nSlot[i]].OpenSlot(
			ItemMatrix->nSlot[i], this, ItemMatrix->nSlot[i]
		);
			...
			while (p) {
				...
				if (!aciReInitItemXY(&p->ActIntBuffer, p->R_curr.x, p->R_curr.y)) {
					if (!aciPutItem(p->ActIntBuffer.type)) {
						uvsKronDeleteItem(
							p->uvsDeviceType, p->ActIntBuffer.data0, p->ActIntBuffer.data1
						);
						ObjectDestroy(p);
						CheckOutDevice(p);
						ActD.CheckDevice(p);
...
char VangerUnit::CheckInDevice(StuffObject *p) {
	...
	if (!(Status & SOBJ_ACTIVE)) {
		...
		if (l) {
			...
			if (p->ActIntBuffer.type == ACI_DEGRADATOR || p->ActIntBuffer.type == ACI_AMPUTATOR ||
				p->ActIntBuffer.type == ACI_MECHOSCOPE)
				return CHECK_DEVICE_IN;
			return CHECK_DEVICE_OUT;
		};
	};
...
void VangerUnit::CheckOutDevice(StuffObject *p) {
	...
	if (!(Status & SOBJ_ACTIVE)) {
		for (i = 0; i < MAX_ACTIVE_SLOT; i++)
			if (GunSlotData[i].pData && GunSlotData[i].ItemData == p)
				GunSlotData[i].CloseGun();
	};
	...
	case ACI_PHLEGMA:
		...
		if (i < SAFE_STUFF_MAX) {
			n = DeviceData;
			while (n) {
				if (n->ActIntBuffer.type == ACI_WEEZYK &&
					!uvsCheckItemOnLive(UVS_ITEM_TYPE::WEEZYK)) {
					n->uvsDeviceType = UVS_ITEM_TYPE::DEAD_WEEZYK;
					n->ActIntBuffer.type = uvsSetItemType(
						n->uvsDeviceType, n->ActIntBuffer.data0, n->ActIntBuffer.data1
					);
					if (Status & SOBJ_ACTIVE) {
	...
	case ACI_WEEZYK:
		if (!uvsCheckItemOnLive(UVS_ITEM_TYPE::WEEZYK)) {
			...
			p->ActIntBuffer.type =
				uvsSetItemType(p->uvsDeviceType, p->ActIntBuffer.data0, p->ActIntBuffer.data1);
			if (Status & SOBJ_ACTIVE)
				uvsOnKillItem();
		};
		break;
	case ACI_TOXICK:
		...
		if (i < SAFE_STUFF_MAX) {
			n = DeviceData;
			while (n) {
				if (n->ActIntBuffer.type == ACI_ELEECH &&
					!uvsCheckItemOnLive(UVS_ITEM_TYPE::ELEECH)) {
...
void BunchEvent(int type) {
	...
	case BUNCH_CHANGE_CYCLE:
		...
		if (WorldPalCurrent >= WorldPalNum)
			WorldPalCurrent = 0;
...
void ActionDispatcher::CreateActive(VangerUnit *p) {
	...
	if (Active)
		DrawResourceMaxValue = MaxOxigenBar;
	else
		DrawResourceMaxValue = 0;
	DrawResourceValue = 0;
	DrawResourceTime = 0;
	SpummyRunner = 0;
};
	...
	if (ActD.Active) {
		...
		case ACI_ACTIVATE_ITEM:
			...
			case ACI_EMPTY_CIRTAINER:
				...
				if (active->dynamic_state & (GROUND_COLLISION | WHEELS_TOUCH)) {
					active->uvsPoint->gatherCirt(
						active->R_curr.x,
						active->R_curr.y,
						dev->ActIntBuffer.data0,
						dev->ActIntBuffer.data1
					);
					p->type = uvsSetItemType(
						dev->uvsDeviceType, dev->ActIntBuffer.data0, dev->ActIntBuffer.data1
					);
					aciChangeItem(p);
				};
				break;
		...
		case ACI_SHOW_ITEM_TEXT:
			if (p->type == ACI_TABUTASK_SUCCESSFUL) {
				...
				dev = resolve_stuff_owner(active, p);
				if (dev && uvsPrepareItemToDiagen(
							   dev->uvsDeviceType, p->data0, p->data1, what, id, name_from, name_to
						   ))
...
void ActionDispatcher::SlotIn(int n, StuffObject *p) {
	...
	case DEVICE_ID_GUN:
		...
		if (uvsKronActive && !GameD.FirstQuant)
...
void ActionDispatcher::ActiveSlot(int n) {
	if (Slot[n] && Active && Slot[n]->StuffType == DEVICE_ID_GUN &&
		!isTerminator(Slot[n]->ActIntBuffer.type))
...
void uvsUnitType::AddOxigenResource(void) {
	...
	if (OxigenResource < MaxOxigenResource - 10) {
		OxigenResource += 10;
		if (ActD.Active->Status & SOBJ_ACTIVE)
			ActD.DrawResourceValue = OxigenResource;
	};
};
...
void uvsUnitType::UseOxigenResource(void) {
	...
	if (OxigenResource > 0) {
		OxigenResource--;
		if (ActD.Active && ActD.Active->Status & SOBJ_ACTIVE)
			ActD.DrawResourceValue = OxigenResource;
	} else {
		Armor = 0;
		Energy = 0;
...
void CompasObject::Open(void) {
	...
	while (p) {
		...
		case CMP_TARGET_ESCAVE:
			...
			if (pe == NULL)
				ErrH.Abort("Bad Compas Target Open");
...
void CreateArtefactTarget(StuffObject *n) {
	...
	if (!p) {
		if (lang() == RUSSIAN) {
			...
			case ACI_PROTRACTOR:
				CompasObj.AddTarget(
					CMP_OBJECT_ITEM, UnitOrderType(n), NULL, rProtractorCompasTarget
				);
				break;
			case ACI_MECHANIC_MESSIAH:
			...
			case ACI_FUNCTION83:
				CompasObj.AddTarget(
					CMP_OBJECT_ITEM, UnitOrderType(n), NULL, rFunction83CompasTarget
				);
				break;
			case ACI_SPUMMY:
			...
			case ACI_PROTRACTOR:
				CompasObj.AddTarget(
					CMP_OBJECT_ITEM, UnitOrderType(n), NULL, ProtractorCompasTarget
				);
				break;
			case ACI_MECHANIC_MESSIAH:
			...
			case ACI_FUNCTION83:
				CompasObj.AddTarget(
					CMP_OBJECT_ITEM, UnitOrderType(n), NULL, Function83CompasTarget
				);
				break;
			case ACI_SPUMMY:
		...
		if (TabuTable[i]->status == UVS_TABUTASK_STATUS::ACTIVE) {
			...
			case UVS_TARGET::VANGER:
				...
				if (!n) {
					RaceTxtBuff.init();
					if (lang() == RUSSIAN)
						RaceTxtBuff < rTaskCompasTarget <= i;
					else
						RaceTxtBuff < TaskCompasTarget <= i;

					CompasObj.AddTarget(
						CMP_OBJECT_VANGER, UnitOrderType(c), NULL, RaceTxtBuff.GetBuf()
					);
	...
	if (my_server_data.GameType == PASSEMBLOSS && UsedCheckNum < GloryPlaceNum) {
		...
		if (!n) {
			if (lang() == RUSSIAN) {
				CompasObj.AddTarget(
					CMP_OBJECT_VECTOR,
					UnitOrderType(target.x, target.y, target.z),
					NULL,
					rCheckPointCompasTarget
				);
...
void CompasObject::Quant(void) {
	...
	if (!CurrentTarget || !ActD.Active || ActD.Active->ExternalMode != EXTERNAL_MODE_NORMAL ||
		!(ActD.Active->ExternalDraw))
		return;
	...
	case CMP_OBJECT_SPOT:
		R_curr = (((CurrentTarget->Data).EnterT)->GetEnterCenter(ActD.Active->R_curr)->ActionLink)
					 ->R_curr;
		break;
	...
	if (d) {
		...
		d = vMove.vabs();
		if (d)
			vMove = vMove * (MAX_COMPAS_SPEED * XTCORE_FRAME_NORMAL / d);
	};
	...
	/*	if(ty < ScreenCY){
			if(tx < ScreenCX)
				aOutText32clip(tx + 32,ty + 32,165 | (2 << 16),RaceTxtBuff.address(),0,0,0);
				  ...
				  aOutText32clip(tx - strlen(RaceTxtBuff.address())*10,ty + 32,165 | (2 <<
	   16),RaceTxtBuff.address(),0,0,0); }else{ if(tx < ScreenCX) aOutText32clip(tx + 32,ty - 18,165
	   | (2 << 16),RaceTxtBuff.address(),0,0,0); else aOutText32clip(tx -
	   strlen(RaceTxtBuff.address())*10,ty - 18,165 | (2 << 16),RaceTxtBuff.address(),0,0,0);
		};*/
	XGR_SetClip(cclx, ccly, ccrx, ccry);
	XGR_SetClipMode(ccm);
};
...
void SelectCompasTarget(const char *n) {
	CompasTargetType *p;
	if (ActD.Active)
		ActD.Active->aiResolveFind.ClearResolve();
	if (!n) {
...
void CharacterDataType::GetInternalStuffOrder(
	int d,
	int a,
	int item,
	StuffObject *s,
...
int ActionDispatcher::NewFunction(int id, int tp) {
	...
	if (id == PROTRACTOR_PREPASSAGE) {
		...
		while (p) {
			if (p->ID > PROTRACTOR_PREPASSAGE && p->ID <= id)
				p->Time = 0;
			if (p->ID == tp) {
				if (ActD.mfActive == ActD.Active) {
	...
	if (Time <= 0) {
		...
		case PROTRACTOR_JESTEROID:
			if (ActD.pfActive && Time < LifeTime - (SIGNATOR_DELAY * GAME_TIME_COEFF)) {
				if (ActD.pfActive->R_curr.z < 400)
					ActD.pfActive->continuous_impulse(Vector(0, 0, 64), RND(25), 0);
				else
					ActD.pfActive->continuous_impulse(Vector(0, 0, 0), 1, RND(50));
				...
				if (Time < LifeTime - (SIGNATOR_DELAY * GAME_TIME_COEFF)) {
					if (!(ActD.pfActive->mole_on)) {
						...
						if (Time < LifeTime - (2 * GAME_TIME_COEFF)) {
							ActD.pfActive->set_3D(
								SET_3D_DIRECT_PLACE,
								vR.x,
								vR.y,
								-32,
								0,
								-((VangerUnit *)(ActD.pfActive))->Angle,
								0
							);
					...
					if (Time == LifeTime - (SIGNATOR_DELAY * GAME_TIME_COEFF)) {
						...
						for (i = 0; i < MOLERIZATOR_NUM; i++)
							MapD.CreateDust(
								Vector(
									XCYCL(vR.x + MOLERIZATOR_RADIUS - RND(2 * MOLERIZATOR_RADIUS)),
									YCYCL(vR.y + MOLERIZATOR_RADIUS - RND(2 * MOLERIZATOR_RADIUS)),
									0
								),
								MAP_DUST_PROCESS
							);
		...
		case PROTRACTOR_PREPASSAGE:
			if (ActD.pfActive && Time == LifeTime - (SIGNATOR_DELAY * GAME_TIME_COEFF)) {
				...
				ActD.pfActive->CreateParticleMechos(
					ActD.pfActive->R_curr,
					300,
					(LifeTime - (SIGNATOR_DELAY * GAME_TIME_COEFF)) / GAME_TIME_COEFF
				);
			};
			break;
		case MECHANIC_ITEM_FALL:
			...
			if (ActD.mfActive) {
				if (Time == LifeTime - (SKY_QUAKE_TIME * GAME_TIME_COEFF)) {
					...
					if (Time == LifeTime - (SKY_QUAKE_DELAY * GAME_TIME_COEFF)) {
						...
						((VangerUnit *)(ActD.mfActive))
							->convert_to_beeb(&(ModelD.ActiveModel(ModelD.FindModel("Bug"))));
			...
			if (ActD.mfActive) {
				if (Time > LifeTime - (SKY_QUAKE_DELAY * GAME_TIME_COEFF)) {
					if (Time == LifeTime - (SKY_QUAKE_TIME * GAME_TIME_COEFF)) {
						if (ActD.mfActive == ActD.Active) {
							SkyQuake2.set(
								ScreenCX, ScreenCY, 20, SKY_QUAKE_RADIUS, SKY_QUAKE_DELTA
							);
							SkyQuakeEnable2 = 1;
						};
					};
					...
					if (fire_garden_spawn_legacy_step(Time)) {
						...
						n->CreateBullet(
							vTrack, vCheck, NULL, &GameBulletData[WD_BULLET_FIRE_GARDEN], NULL
						);
					};
				};
			};
...
void ActionDispatcher::FunctionQuant(void) {
		...
		if (pfActive) {
			...
			if (NetFunctionProtractor != ch) {
				if (ch & 64) {
					...
					if (GetDistTime(NetGlobalTime, pfActive->NetProtractorFunctionTime) >
						256 * SIGNATOR_DELAY / 20)
						p_flag = 1;
				} else
		...
		if (mfActive) {
			...
			if (NetFunctionMessiah != ch) {
				if (ch & 128) {
					...
					if (GetDistTime(NetGlobalTime, mfActive->NetMessiahFunctionTime) >
						256 * (SKY_QUAKE_DELAY * GAME_TIME_COEFF) / 20)
						m_flag = 1;
				} else
	...
	if (FunctionSpobsDestroyActive > 0)
		FunctionSpobsDestroyActive--;
	...
	if (FunctionThreallDestroyActive > 0) {
		if (ActD.Active) {
			if (!ProtractorLight)
				ProtractorLight = MapD.CreateLight(
					ActD.Active->R_curr.x,
					ActD.Active->R_curr.y,
					512,
					ActD.Active->radius * 2,
					63,
					LIGHT_TYPE::DYNAMIC
				);
			else
				ProtractorLight->set_position(ActD.Active->R_curr.x, ActD.Active->R_curr.y, 512);
		};
	} else {
		if (ProtractorLight) {
			ProtractorLight->Destroy();
		...
		if (pfActive) {
			...
			case ACI_PROTRACTOR_EVENT1:
				if (!NetworkON && NewFunction(PROTRACTOR_PREPASSAGE, PROTRACTOR_OPEN_SPODS)) {
					AddCoolFunction(
						PROTRACTOR_OPEN_SPODS, pfActive->R_curr, SIGNATOR_DELAY, p_flag
					);
					p_new = 1;
				};
				break;
			case ACI_PROTRACTOR_EVENT2:
				if (NewFunction(PROTRACTOR_PREPASSAGE, PROTRACTOR_SCALE_DOWN)) {
					AddCoolFunction(
						PROTRACTOR_SCALE_DOWN, pfActive->R_curr, 0xffffff + SIGNATOR_DELAY, p_flag
					);
					p_new = 1;
				};
				break;
			case ACI_PROTRACTOR_EVENT3:
				if (NewFunction(PROTRACTOR_PREPASSAGE, PROTRACTOR_PALLADIUM)) {
					AddCoolFunction(
						PROTRACTOR_PALLADIUM, pfActive->R_curr, 0xffffff + SIGNATOR_DELAY, p_flag
					);
					p_new = 1;
				};
				break;
			case ACI_PROTRACTOR_EVENT4:
				if (NewFunction(PROTRACTOR_PREPASSAGE, PROTRACTOR_BEEBOS_DANCE)) {
					AddCoolFunction(
						PROTRACTOR_BEEBOS_DANCE, pfActive->R_curr, 0xffffff + SIGNATOR_DELAY, p_flag
					);
					p_new = 1;
				};
				break;
			case ACI_PROTRACTOR_EVENT5:
				if (NewFunction(PROTRACTOR_PREPASSAGE, PROTRACTOR_JESTEROID)) {
					AddCoolFunction(
						PROTRACTOR_JESTEROID, pfActive->R_curr, 0xffffff + SIGNATOR_DELAY, p_flag
					);
					p_new = 1;
				};
				break;
			case ACI_PROTRACTOR_EVENT6:
				if (NewFunction(PROTRACTOR_PREPASSAGE, PROTRACTOR_MOLERIZATOR)) {
					AddCoolFunction(
						PROTRACTOR_MOLERIZATOR, pfActive->R_curr, 500 + SIGNATOR_DELAY, p_flag
					);
					p_new = 1;
				};
				break;
			case ACI_PROTRACTOR_EVENT7:
				if (NewFunction(PROTRACTOR_PREPASSAGE, PROTRACTOR_PREPASSAGE)) {
					if (!p_flag) {
						AddCoolFunction(
							PROTRACTOR_PREPASSAGE, pfActive->R_curr, 200 + SIGNATOR_DELAY, p_flag
						);
						p_new = 1;
					};
				};
		...
		if (mfActive) {
			...
			case ACI_MECH_MESSIAH_EVENT1:
				if (NewFunction(MECHANIC_BEEB_NATION, MECHANIC_UNVISIBLE)) {
					AddCoolFunction(
						MECHANIC_UNVISIBLE, mfActive->R_curr, SKY_QUAKE_DELAY + 0xffffff, m_flag
					);
					m_new = 1;
				};
				break;
			...
			case ACI_MECH_MESSIAH_EVENT3:
				if (NewFunction(MECHANIC_BEEB_NATION, MECHANIC_BEEB_NATION)) {
					AddCoolFunction(
						MECHANIC_BEEB_NATION, mfActive->R_curr, SKY_QUAKE_DELAY + 0xffffff, m_flag
					);
					m_new = 1;
				};
				break;
			case ACI_MECH_MESSIAH_EVENT4:
				if (NewFunction(MECHANIC_BEEB_NATION, MECHANIC_FIRE_GARDEN)) {
					if (!m_flag) {
						AddCoolFunction(
							MECHANIC_FIRE_GARDEN, mfActive->R_curr, SKY_QUAKE_DELAY + 15, m_flag
						);
						m_new = 1;
					};
				};
			...
			case ACI_MECH_MESSIAH_EVENT5:
				if (NewFunction(MECHANIC_BEEB_NATION, MECHANIC_ITEM_FALL)) {
					if (!m_flag) {
						AddCoolFunction(
							MECHANIC_ITEM_FALL, mfActive->R_curr, SKY_QUAKE_DELAY, m_flag
						);
						m_new = 1;
					};
				};
...
void uvsDolly::ActiveQuant(void) {
	...
	p = (BaseObject *)(ItemD.Tail);
	while (p) {
		if (p->ID == ID_STUFF && ((StuffObject *)(p))->ActIntBuffer.type == ACI_EMPTY_CONLARVER &&
			((StuffObject *)(p))->FindDolly) {
	...
	if (!tail_counter) {
		...
		tail_counter++;
	} else if (++tail_counter == DOLLY_TAIL_PERIOD)
		tail_counter = 0;
};
...
void InsectList::Init(void) {
	...
	if (CurrentWorld != WORLD_KHOX && CurrentWorld != WORLD_HMOK && CurrentWorld != WORLD_WEEXOW &&
		!uvsCurrentWorldUnable) {
		...
		for (i = 0; i < MAX_INSECT_UNIT; i++) {
			...
			Data[i].CreateActionUnit(
				...
				SOBJ_AUTOMAT,
				Vector(RND(clip_mask_x), RND(clip_mask_y), -1),
				0,
				SET_3D_CHOOSE_LEVEL
			);
			Data[i].CreateInsect();
		...
		if (sign) {
			if (c_log) {
				...
				case ACI_MACHOTINE_GUN_HEAVY:
					aiMessageQueue.Send(
						AI_MESSAGE_MACHOTINE, p->Speed, 1, 0
					); // aiMessageData[AI_MESSAGE_MACHOTINE].Send(p->Speed,1,0);
					SOUND_CHARGE_MACHOTIN();
					break;
				case ACI_GHORB_GEAR_LIGHT:
				case ACI_GHORB_GEAR_HEAVY:
					aiMessageQueue.Send(
						AI_MESSAGE_GHORB, p->Speed, 2, 0
					); // aiMessageData[AI_MESSAGE_GHORB].Send(p->Speed,2,0);
					SOUND_CHARGE_GHORB();
					break;
				};
			};
			...
			if (c_log) {
				...
				case ACI_GHORB_GEAR_HEAVY:
					aiMessageQueue.Send(
						AI_MESSAGE_DGHORB, p->Speed, 1, 0
					); // aiMessageData[AI_MESSAGE_DGHORB].Send(p->Speed,1,0);
					SOUND_DISCHARGE();
					break;
				};
			};
...
int ChargeDevice(VangerUnit *p, int ind, int sign) {
	...
	if (p->Status & SOBJ_ACTIVE) {
		...
		if (sign) {
			if (c_log) {
				aiMessageQueue.Send(
					AI_MESSAGE_COPTER, p->Speed, 2, 0
				); // aiMessageData[AI_MESSAGE_COPTER].Send(p->Speed,2,0);
				SOUND_CHARGE_RIGS();
			};
		} else {
			if (c_log) {
				aiMessageQueue.Send(
					AI_MESSAGE_DCOPTER, p->Speed, 1, 0
				); // aiMessageData[AI_MESSAGE_DCOPTER].Send(p->Speed,1,0);
				SOUND_DISCHARGE();
			};
		};
	} else {
...
void GunSlot::NetEvent(unsigned int time) {
	...
	if (Owner && Owner->Visibility == VISIBLE && !(Owner->Status & SOBJ_WAIT_CONFIRMATION) &&
		GetDistTime(NetGlobalTime, time) < 5 * 256) {
		if (t)
			aiTargetObject = ActD.GetNetObject(t);
		else
			aiTargetObject = NULL;
	} else {
		FireCount = NetFireCount;
...
void GunSlot::NetUpdate(void) {
	...
	if (ActD.LocatorPoint)
		NETWORK_OUT_STREAM < (int)(ActD.LocatorPoint->NetID);
	else {
		if (TargetObject)
			NETWORK_OUT_STREAM < (int)(TargetObject->NetID);
		else
			NETWORK_OUT_STREAM < (int)(0);
	};
	NETWORK_OUT_STREAM.end_body();
...
void GunSlot::Fire(void) {
	...
	if (GunStatus != GUN_READY || Owner->BeebonationFlag || !(Owner->ExternalDraw))
		return;
	...
	case BULLET_TYPE_ID::TERMINATOR:
		...
		case GUN_FIRE:
			...
			if ((1 << ItemData->ActIntBuffer.slot) & Owner->slots_existence)
				vFire += Owner->A_l2g *
						 (Owner->R_slots[ItemData->ActIntBuffer.slot] * Owner->scale_real);
...
void UnitItemMatrix::Open(Parser &in) {
	...
	if (NumID > 0) {
		...
		if (NumSlot > 0) {
			...
			for (i = 0; i < NumSlot; i++)
				SlotSize[i] = in.get_int();
			in.search_name("nSlot");
			for (i = 0; i < NumSlot; i++)
				nSlot[i] = in.get_int() - 1;
...
void VangerUnit::TargetAnalysis(void) {
	...
	vCheck = Vector(
		getDistX(TargetPoint.vPoint.x, R_curr.x), getDistY(TargetPoint.vPoint.y, R_curr.y), 0
	);
	d = vCheck.vabs();
	if (aiMoveMode == AI_MOVE_TRACK) {
		TrackQuant();
		...
		if (SpeedDir > 0)
			vDirect = Vector(
				getDistX(TargetPoint.vPoint.x, XCYCL(R_curr.x + vUp.x)),
				getDistY(TargetPoint.vPoint.y, R_curr.y + vUp.y),
				0
			);
		else
			vDirect = Vector(
				getDistX(TargetPoint.vPoint.x, XCYCL(R_curr.x + vDown.x)),
				getDistY(TargetPoint.vPoint.y, R_curr.y + vDown.y),
				0
			);
		TrackQuant();
	};
...
void VangerUnit::ResolveGenerator(void) {
	...
	if (dynamic_state & VERTICAL_WALL_COLLISION)
		WallCollisionTime = wall_collision_ticks();
	else if (WallCollisionTime > 0)
		WallCollisionTime--;

	if (abs(TractionValue) < MAX_TRACTION_CHECK_DELTA)
		DeltaTractionTime = traction_low_state_ticks();
	else if (DeltaTractionTime > 0)
		DeltaTractionTime--;

	if (aiAlarmTime > 0)
		aiAlarmTime--;
	if (SpeedFactorTime > 0)
		SpeedFactorTime--;
	...
	if (!LowArmor && Armor < ArmorAlarm) {
		...
		if (sns && !aiResolveFind.FindResolve(UNIT_ORDER_SENSOR, UnitOrderType(sns))) {
			AddFindResolve(UNIT_ORDER_SENSOR, UnitOrderType(sns), 0, AI_RESOLVE_STATUS_TARGET)
				->Time = way_recalc_arm_next_quant(MaxWayCount);
			LowArmor = 1;
		};
	};
	...
	if (Visibility == VISIBLE && CoptePoint && !aiAlarmTime)
		aiStatus |= AI_STATUS_FLY;

	if ((Visibility == VISIBLE && !RND(visible_check_factor)) ||
		(Visibility == UNVISIBLE && !RND(unvisible_check_factor))) {
	...
	for (i = 0; i < aiNumGlobalEvent[aiReadEvent][AI_EVENT_CAPTURE]; i++) {
		if (n->Refresh) {
			if (n->Subj == this) {
				if (aiResolveFind.FindResolve(
						UNIT_ORDER_STUFF, UnitOrderType((StuffObject *)(n->Obj))
					))
					aiResolveFind.ClearResolve(
						UNIT_ORDER_STUFF, UnitOrderType((StuffObject *)(n->Obj))
					);
		...
		if (n->Refresh) {
			...
			case ID_VANGER:
				if (n->Subj == this && aiRelaxTime > aiRelaxTimeMax) {
					vCheck = Vector(
						getDistX(n->Obj->R_curr.x, R_curr.x),
						getDistY(n->Obj->R_curr.y, R_curr.y),
						0
					);
					...
					if (abs(a) < PI / 4)
						AddEventTime(
							(VangerUnit *)(n->Obj), aiAddFrontCollision, AI_ADD_FRONT_COLLISION
						);
					else {
						if (abs(a) > 3 * PI / 4)
							AddEventTime(
								(VangerUnit *)(n->Obj), aiAddBackCollision, AI_ADD_BACK_COLLISION
							);
						else
							AddEventTime(
								(VangerUnit *)(n->Obj), aiAddSideCollision, AI_ADD_SIDE_COLLISION
							);
					};
				};
				break;
...
void VangerUnit::GamerOrderInit(void) {
	...
	case CMP_OBJECT_ESCAVE:
	case CMP_OBJECT_SPOT:
		AddFindResolve(UNIT_ORDER_ENTER, CompasObj.CurrentTarget->Data)->Time =
			way_recalc_arm_next_quant(MaxWayCount);
		break;
	case CMP_OBJECT_PASSAGE:
		AddFindResolve(UNIT_ORDER_PASSAGE, CompasObj.CurrentTarget->Data)->Time =
			way_recalc_arm_next_quant(MaxWayCount);
		break;
	case CMP_OBJECT_SENSOR:
		AddFindResolve(UNIT_ORDER_SENSOR, CompasObj.CurrentTarget->Data)->Time =
			way_recalc_arm_next_quant(MaxWayCount);
		break;
	};
};
...
void VangerUnit::MainOrderInit(void) {
	...
	if (aiActionID == AI_ACTION_FARMER && SeedNum < MaxSeed) {
		(p = AddFindResolve(
			 UNIT_ORDER_VECTOR,
			 UnitOrderType(RND(clip_mask_x), 300 + RND(clip_mask_y - 600), 0),
			 0,
			 AI_RESOLVE_STATUS_VIEW
		 ))
			->Time = way_recalc_arm_next_quant(MaxWayCount);
	} else {
		p = NULL;
		...
		case UVS_TARGET::ESCAVE:
			(p = AddFindResolve(UNIT_ORDER_ENTER, g->unitPtr))->Time =
				way_recalc_arm_next_quant(MaxWayCount);
			break;
		...
		case UVS_TARGET::PASSAGE:
			(p = AddFindResolve(UNIT_ORDER_PASSAGE, g->unitPtr))->Time =
				way_recalc_arm_next_quant(MaxWayCount);
			break;
		case UVS_TARGET::DOLLY:
			(p = AddFindResolve(UNIT_ORDER_DOLL, UnitOrderType((uvsDolly *)(g))))->Time =
				way_recalc_arm_next_quant(MaxWayCount);
			break;
		case UVS_TARGET::VANGER:
			if (ActD.Active) {
				(p = AddFindResolve(UNIT_ORDER_VANGER, ActD.Active))->Time =
					way_recalc_arm_next_quant(MaxWayCount);
						...
						if (aiRunBranch != pBranch || aiRunNode != pNode) {
							...
							if (TargetPoint.PointStatus & TRK_NODE_MASK) {
								...
								if (pBranch == TargetPoint.pBranch &&
									(pNextLink == TargetPoint.pPrevLink ||
										pNextLink == TargetPoint.pNextLink ||
										pPrevLink == TargetPoint.pPrevLink ||
										pPrevLink == TargetPoint.pNextLink)) {
									NoWayEnable = AI_NO_WAY_NONE;
									aiMoveMode = AI_MOVE_TRACK;
									OtherFlag |= MECHOS_RECALC_FRONT;
								};
...
aiUnitResolve *VangerUnit::AddFindResolve(int type, UnitOrderType obj, int level, int status) {
	...
	case UNIT_ORDER_STUFF:
		if ((p->Obj.StuffT)->Owner)
			TargetPoint.vPoint = ((p->Obj.StuffT)->Owner)->R_curr;
		else
			TargetPoint.vPoint = (p->Obj.StuffT)->R_curr;
		break;
	...
	if (aiResolveWayEnable == 0) {
		...
		case UNIT_ORDER_STUFF:
			...
			if (p->rStatus & AI_RESOLVE_STATUS_VIEW) {
				...
				if (!(p->Obj.StuffT)->Owner && (p->Obj.StuffT)->Visibility == UNVISIBLE &&
					Visibility == UNVISIBLE) {
		...
		case UNIT_ORDER_DOLL:
			TargetPoint.vPoint = Vector((p->Obj.DollT)->pos_x, (p->Obj.DollT)->pos_y, 0);
			vCheck = Vector(
				getDistX((p->Obj.DollT)->pos_x, R_curr.x),
				getDistX((p->Obj.DollT)->pos_y, R_curr.y),
				0
			);
			if (vCheck.vabs() < (int)(RND(radius * 5))) {
				c = GetStuffObject(this, ACI_EMPTY_CIRTAINER);
				if (c) {
					c->ActIntBuffer.data0 =
						p->Obj.DollT->getCirt((p->Obj.DollT)->pos_x, (p->Obj.DollT)->pos_y);
					c->ActIntBuffer.data1 = 1 << (p->Obj.DollT)->gIndex;
					c->ActIntBuffer.type = uvsSetItemType(
						c->uvsDeviceType, c->ActIntBuffer.data0, c->ActIntBuffer.data1
					);
				};
				uvsPoint->goHome();
				ResolveFindDestroy(p);
		...
		case AI_MOVE_TRACK:
			if (CheckReturn2Track()) {
				if (PointStatus & TRK_NODE_MASK) {
					...
					if ((TargetPoint.PointStatus & TRK_BRANCH_MASK) &&
						pBranch == TargetPoint.pBranch &&
						(pNextLink == TargetPoint.pNextLink || pPrevLink == TargetPoint.pNextLink ||
							pNextLink == TargetPoint.pPrevLink ||
							pPrevLink == TargetPoint.pPrevLink))
						aiMoveMode = AI_MOVE_POINT;
				};
...
int VangerUnit::CheckSpeetle(void) {
	...
	for (i = 0; i < ItemMatrix->NumSlot; i++) {
		if (GunSlotData[ItemMatrix->nSlot[i]].pData &&
			(GunSlotData[ItemMatrix->nSlot[i]].ItemData->ActIntBuffer.type ==
...
void VangerUnit::WeaponGenerator(void) {
	...
	for (i = 0; i < ItemMatrix->NumSlot; i++) {
		if (!GunSlotData[ItemMatrix->nSlot[i]].pData) {
			...
			p = DeviceData;
			while (p) {
				if (p->StuffType == DEVICE_ID_GUN && p->ActIntBuffer.slot == -1 &&
					p->ActIntBuffer.data1 > 0 && p->SizeID >= ms &&
					((GunDevice *)(p))->pData->Power >= mp) {
					if (p->SizeID > 0) {
						if (p->SizeID <= s) {
							ms = p->SizeID;
							pp = (GunDevice *)(p);
			...
			if (pp) {
				...
				if (pp->pData->WaitTime)
					WeaponDelta += pp->pData->Power / pp->pData->WaitTime;
				else
					WeaponDelta += pp->pData->Power;
...
int VangerUnit::CheckNearTrack(void) {
	...
	cl = NULL;
	for (i = 0; i < HideTrack.NumBranch; i++, mb++) {
		if ((mb->pBeg && WayDist[mb->pBeg->index] < cDist) ||
			(mb->pEnd && WayDist[mb->pEnd->index] < cDist)) {
			ml = mb->Link;
...
int GunSlot::CheckTarget(ActionUnit *p) {
	...
	if (Owner->aiMoveFunction == AI_MOVE_FUNCTION_WHEEL && (Owner->PowerFlag & VANGER_POWER_FLY) &&
		GetStuffObject(Owner, ACI_RADAR_DEVICE)) {
		if (pData->BulletMode & BULLET_CONTROL_MODE::AIM) {
			vEnd = p->R_curr;
			vCheck =
				Vector(getDistX(vEnd.x, vStart.x), getDistY(vEnd.y, vStart.y), vEnd.z - vStart.z);
			if (vCheck.abs2() > d * d)
				return 0;
			if (!(pData->BulletMode & BULLET_CONTROL_MODE::FLY) && MapLineTrace(vStart, vEnd))
				return 0;
			aiTargetObject = p;
		} else {
...
aiUnitEvent *VangerUnit::AddEvent(int id, GeneralObject *obj, VangerUnit *subj) {
	...
	switch (id) {
	case AI_EVENT_ITEM:
		aiResolveFind.AddResolve(UNIT_ORDER_STUFF, UnitOrderType((StuffObject *)(obj)))->Time =
			way_recalc_arm_next_quant(MaxWayCount);
		break;
	};
	return l;
...
int VangerUnit::GetAllert(VangerUnit *p) {
	...
	for (i = 0; i < p->ItemMatrix->NumSlot; i++) {
		...
		if (gs->pData) {
			if (gs->pData->WaitTime)
				s += gs->pData->Power / gs->pData->WaitTime;
			else
				s += gs->pData->Power;
		};
	};
	...
	if (WeaponDelta) {
		if (s == 0)
			...
			s = ((Armor + Energy) / s) - ((p->Armor + p->Energy) / WeaponDelta);
	} else {
		if (s)
			s = -(Armor + Energy) / s;
		else
	...
	default:
		if (PowerFlag & VANGER_POWER_RUFFA_GUN) {
			...
			case UVS_VANGER_STATUS::RACE_HUNTER:
				...
				break;
			default:
				if (CurrentWorld == WORLD_GLORX && (uvsCurrentCycle == 1 || uvsCurrentCycle == 2) &&
					!RND(3)) {
					aiVangerRadius = 400;
					aiItemRadius = 400;

					if (uvsCurrentCycle == 1)
						aiFirstItem = ACI_PIPETKA;
					else
						aiFirstItem = ACI_KERNOBOO;
					aiSecondItem = -1;
...
int GetEscaveDist(void) {
	...
	while (p1) {
		if (((uvsEscave *)p1)->Pworld->gIndex == CurrentWorld) {
			vCheck =
				Vector(getDistX(p1->pos_x, Gamer->pos_x), getDistY(p1->pos_y, Gamer->pos_y), 0);
			d = vCheck.vabs();
			if (d < md || md == -1)
				md = d;
		}
	...
	while (p2) {
		if (((uvsSpot *)p2)->Pworld->gIndex == CurrentWorld) {
			vCheck =
				Vector(getDistX(p2->pos_x, Gamer->pos_x), getDistY(p2->pos_y, Gamer->pos_y), 0);
			d = vCheck.vabs();
			if (d < md || md == -1)
				md = d;
		}
	...
	if (md == -1) {
		...
		while (p3) {
			if (((uvsPassage *)p3)->Pworld->gIndex == CurrentWorld) {
				vCheck =
					Vector(getDistX(p3->pos_x, Gamer->pos_x), getDistY(p3->pos_y, Gamer->pos_y), 0);
				d = vCheck.vabs();
				if (d < md || md == -1)
					md = d;
			}
...
void NetStatisticUpdate(int id) {
	...
	case VAN_WAR:
		...
		case NET_STATISTICS_KILL:
			...
			if (MinKillTime == 0 || MinKillTime > d)
				MinKillTime = d;
			if (MaxKillTime == 0 || MaxKillTime < d)
				MaxKillTime = d;
			PrevKillTime = t;
			if (MinKillTime == MaxKillTime)
				my_player_body.VanVarStat.KillFreq = MaxKillTime;
			else
				my_player_body.VanVarStat.KillFreq = (MaxKillTime - MinKillTime) / 2;
			break;
		case NET_STATISTICS_DEATH:
			my_player_body.deaths++;
			d = labs((long)(t - PrevLifeTime));
			if (MinLifeTime == 0 || MinLifeTime > d)
				MinLifeTime = d;
			if (MaxLifeTime == 0 || MaxLifeTime < d)
				MaxLifeTime = d;
			PrevLifeTime = t;
			if (MinLifeTime == MaxLifeTime)
				my_player_body.VanVarStat.DeathFreq = MaxLifeTime;
			else
				my_player_body.VanVarStat.DeathFreq = (MaxLifeTime - MinLifeTime) / 2;
			break;
		};
		break;
	case PASSEMBLOSS:
		...
		case NET_STATISTICS_END_RACE:
			my_player_body.PassemblossStat.CheckpointLighting = UsedCheckNum;
			my_player_body.PassemblossStat.TotalTime = my_player_body.PassemblossStat.MaxTime =
				labs((long)(t - StartGameTime));
			break;
		};
		break;
	case MECHOSOMA:
...
void VangerUnit::DestroyEnvironment(void) {
	...
	while (p) {
		...
		if (p->Visibility == VISIBLE && p != this) {
			vCheck = Vector(
				getDistX(p->R_curr.x, R_curr.x),
				getDistY(p->R_curr.y, R_curr.y),
				p->R_curr.z - R_curr.z
			);
			d = vCheck.vabs();
			if (d < ml) {
...
int VangerUnit::UsingCopterig(int decr_8) {
	...
	if (uvsPoint->Pmechos->type == FLY_DEVICE_DEFAULT)
		return 1;
	...
	if (UseDeviceMask & DEVICE_MASK_COPTE) {
		if (Status & SOBJ_ACTIVE) {
			...
			for (i = 0; i < MAX_ACTIVE_SLOT; i++) {
				if (ActD.Slot[i] && ActD.Slot[i]->ActIntBuffer.type == ACI_COPTE_RIG) {
					...
					if (p->ActIntBuffer.data1 <= 0) {
						...
						p->ActIntBuffer.type = uvsSetItemType(
							p->uvsDeviceType, p->ActIntBuffer.data0, p->ActIntBuffer.data1
						);
			...
			if (aiMoveFunction != AI_MOVE_FUNCTION_FLY)
				return 0;
			if (!decr_8)
				return 1;
			if (CoptePoint->ActIntBuffer.data1 <= 0) {
				CoptePoint->ActIntBuffer.data1 = 0;
				CoptePoint->ActIntBuffer.type = uvsSetItemType(
					CoptePoint->uvsDeviceType,
					CoptePoint->ActIntBuffer.data0,
...
int VangerUnit::UsingCrotrig(int decr_8) {
	...
	if (CurrentWorld == WORLD_HMOK)
		return 0;
	if (uvsPoint->Pmechos->type == MOLE_DEVICE_DEFAULT)
		return 1;
		...
		if (Status & SOBJ_ACTIVE) {
			...
			/*			if(!decr_8) return 1;
						if(MolePoint->ActIntBuffer.data1 <= 0){
							...
							MolePoint->ActIntBuffer.type =
			   uvsSetItemType(MolePoint->uvsDeviceType,MolePoint->ActIntBuffer.data0,MolePoint->ActIntBuffer.data1);
...
int VangerUnit::UsingCutterig(int decr_8) {
	...
	if (uvsPoint->Pmechos->type == SWIM_DEVICE_DEFAULT || PowerFlag & VANGER_POWER_SWIM_ALWAYS)
		return 1;
		...
		if (Status & SOBJ_ACTIVE) {
			...
			/*			if(!decr_8) return 1;
						if(SwimPoint->ActIntBuffer.data1 <= 0){
							...
							SwimPoint->ActIntBuffer.type =
			   uvsSetItemType(SwimPoint->uvsDeviceType,SwimPoint->ActIntBuffer.data0,SwimPoint->ActIntBuffer.data1);
...
void VangerUnit::NetCreateVanger(uvsPassage *pp, uvsEscave *pe, uvsSpot *ps) {
	...
	// NETWORK_OUT_STREAM.set_position(ViewX,ViewY,round(2.*(fabs(curGMap -> xsize*sinTurnInvFlt) +
	// fabs(curGMap -> ysize*cosTurnInvFlt))*0.5));
	NETWORK_OUT_STREAM.create_permanent_object(NetID, R_curr.x, R_curr.y, radius);
	int scr_size =
		round((fabs(curGMap->xsize * sinTurnInvFlt) + fabs(curGMap->ysize * cosTurnInvFlt)) * .5);
	NETWORK_OUT_STREAM < (unsigned short)(scr_size < 0xffff ? scr_size : 0xffff);
	Send();
	NETWORK_OUT_STREAM.end_body();
	...
	if (!pp && !pe && !ps) {
		...
		if (pp)
			my_player_body.Data0 = pp->Poutput->gIndex;
		else
			my_player_body.Data0 = -1;

		if (pe)
			my_player_body.Data1 = pe->TabuTaskID;
		else {
			if (ps)
				my_player_body.Data1 = ps->TabuTaskID;
			else
				my_player_body.Data1 = -1;
		};
	};
	send_player_body(my_player_body);
};
...
void VangerUnit::InitPlayerPoint(PlayerData *p) {
	...
	for (i = 0; i < MAX_ACTIVE_SLOT; i++) {
		if (p->SlotNetID[i]) {
			...
			if (Visibility == UNVISIBLE)
				GunSlotData[i].FireCount = GunSlotData[i].NetFireCount;
		};
	};
			...
			if (p->status == GAMING_STATUS) {
				if (p->CreatePlayerFlag == 3) {
					if (p->body.NetID == 0) {
						...
						if (p->body.world == CurrentWorld && p->body.CarIndex != 255 &&
							p->body.NetID != p->LastDestroyedNetID) {
							p->CreatePlayerFlag = 0;
							p->LastDestroyedNetID = 0;
						};
					};
				};
				if (p->body.world == CurrentWorld && p->body.NetID != 0 &&
					p->body.CarIndex != 255 && p->CreatePlayerFlag == 0)
					NetEvent4Uvs(p);
				if (p->CreatePlayerFlag == 1) {
					if (!((uvsVanger *)(p->uvsPoint))->Pescave &&
						!((uvsVanger *)(p->uvsPoint))->Pspot &&
						!((uvsVanger *)(p->uvsPoint))->Ppassage)
						v = addVanger((uvsVanger *)(p->uvsPoint), p->body.Data0, p->body.Data1);
					else
						v = addVanger(
							(uvsVanger *)(p->uvsPoint), ((uvsVanger *)(p->uvsPoint))->Ppassage
						);
					if (v)
						v->InitPlayerPoint(p);
				};
				...
				if (p->status == INITIAL_STATUS &&
					(p->world || p->body.world || p->body.NetID || p->body.CarIndex != 255))
					request_total_players_data_list_query_async();
				if (ActD.Active)
					ActD.Active->pNetPlayer = p;
			};
		} else {
			if (ActD.Active)
				ActD.Active->pNetPlayer = p;
...
void NetEvent4Uvs(PlayerData *p) {
	p->uvsPoint =
		(void *)(uvsCreateNetVanger(p->body.CarIndex, p->body.color, p->body.Data0, p->body.Data1));
...
void ObjectDestroy(GeneralObject *n, int mode) {
	...
	if (n->ID == ID_VANGER) {
		if (mode) {
			if (NetworkON && !(n->Status & SOBJ_ACTIVE) && ((VangerUnit *)(n))->pNetPlayer)
				ClearPhantomTarget(((VangerUnit *)(n))->pNetPlayer->client_ID);
			...
			if (((VangerUnit *)(n))->aiActionID == AI_ACTION_BRAKE && ActD.Active &&
				!(ActD.Active->Status & SOBJ_DISCONNECT)) {
				...
				while (p) {
					if (p->uvsPoint->shape == UVS_VANGER_SHAPE::GAMER_SLAVE) {
						aciAddTeleportMenuItem(
							uvsMechosType_to_AciInt(p->uvsPoint->Pmechos->type), i
						);
						p->VangerCloneID = i;
						i++;
					};
				...
				if (CurrentWorld < MAIN_WORLD_MAX - 1 && (i - TELEPORT_ESCAVE_ID - 1) <= 5)
...
int FuckOFFinWater(void) {
	if (ActD.Active && (ActD.Active->dynamic_state & TOUCH_OF_WATER))
		return 1;
	return 0;
};
...
void CreateBoozeeniadaString(void) {
	...
	CHECK_BSIGN_DATA = new char[NUM_CHECK_BSIGN + 1];
	memset(CHECK_BSIGN_DATA, '\0', NUM_CHECK_BSIGN + 1);
...
void VangerUnit::CalcAiFactor(VangerUnit *p, int time) {
...
#ifdef SAVE_AI_STATUS
	if (p->Status & SOBJ_ACTIVE)
		SaveAiStaus("InitFactor", this, time);
#endif
...
#ifdef SAVE_AI_STATUS
	if (p->Status & SOBJ_ACTIVE)
		SaveAiStaus("AlarmFactor", this, time);
#endif

	if ((PointStatus & TRK_BRANCH_MASK) && (p->PointStatus & TRK_BRANCH_MASK) &&
		pBranch == p->pBranch) {
		if (MoveDir) {
	...
	if (NoWayDirect != -1)
		aiResolveFactor[AI_FACTOR_NOWAY] = -100.0;

#ifdef SAVE_AI_STATUS
	if (p->Status & SOBJ_ACTIVE)
		SaveAiStaus("NoWayFactor", this, time);
#endif
...
#ifdef SAVE_AI_STATUS
	if (p->Status & SOBJ_ACTIVE)
		SaveAiStaus("LowFactor", this, time);
#endif

	if (aiEvent.Num > 0)
		aiResolveFactor[AI_FACTOR_ATTACK] += aiMultyAttack * (aiEvent.Num - 1);
	if (aiResolveAttack.Num > 0)
		aiResolveFactor[AI_FACTOR_ATTACK] += aiTargetAttack * (aiResolveAttack.Num - 1);
	if (aiResolveFind.Num > 0)
		aiResolveFactor[AI_FACTOR_ATTACK] += aiTargetFind * (aiResolveFind.Num - 1);

#ifdef SAVE_AI_STATUS
	if (p->Status & SOBJ_ACTIVE)
		SaveAiStaus("MultyFactor", this, time);
#endif
	...
	while (dl) {
		if (dl->biosNindex == aiModifier && dl->Pworld->gIndex == CurrentWorld) {
			...
			if (dd < AI_MAX_DOLLY_FACTOR) {
				if (uvsPoint->CurrentDolly() == dl->gIndex)
					aiResolveFactor[AI_FACTOR_RUN] += dd * aiDollyRun / AI_MAX_DOLLY_FACTOR;
				else
					aiResolveFactor[AI_FACTOR_ATTACK] += dd * aiDollyAttack / AI_MAX_DOLLY_FACTOR;
			};
		};
		dl = (uvsDolly *)(dl->next);
	};
	...
	if ((p->Status & SOBJ_AUTOMAT)) {
		if (RND(300) < 250) {
			if (p->Visibility == VISIBLE) {
				if (aiResolveFactor[AI_FACTOR_ATTACK] > 0)
					aiResolveFactor[AI_FACTOR_ATTACK] *= .5;
			} else {
				if (aiResolveFactor[AI_FACTOR_RUN] > 0)
					aiResolveFactor[AI_FACTOR_RUN] *= .5;
				if (aiResolveFactor[AI_FACTOR_ATTACK] > 0)
					aiResolveFactor[AI_FACTOR_ATTACK] *= .25;
			};
			if (aiResolveFactor[AI_FACTOR_NOWAY] > 0)
				aiResolveFactor[AI_FACTOR_NOWAY] *= .5;
		};
	} else {
		aiResolveFactor[AI_FACTOR_ATTACK] +=
			aiDominanceAttack * (float)(aiCutDominance) / (float)(100.0);
		aiResolveFactor[AI_FACTOR_RUN] += aiDominanceRun * (float)(aiCutDominance) / (float)(100.0);
	};

#ifdef SAVE_AI_STATUS
	if (p->Status & SOBJ_ACTIVE)
		SaveAiStaus("DominanceFactor", this, time);
#endif
...
#ifdef SAVE_AI_STATUS
	if (p->Status & SOBJ_ACTIVE)
		SaveAiStaus("FuryFactor", this, time);
#endif
		...
		if (!AddAttackTime) {
			if (time >= aiFindLevel)
				AddFindResolve(
					UNIT_ORDER_VANGER,
					UnitOrderType(p),
					round((float)(time)*aiAttackTime) + 10,
					AI_RESOLVE_STATUS_VAR
				)
					->Time = way_recalc_arm_next_quant(MaxWayCount);
			aiResolveAttack
				.AddResolve(
...
int getSpobsState(void) {
	if (ActD.SpobsDestroy == 0)
		return 1;
	return 0;
};

int getThreallState(void) {
	if (ActD.ThreallDestroy == 0)
		return 1;
	return 0;
};
...
void VangerUnit::ShellNetEvent(int type, int id, int creator, int time, int x, int y, int radius_) {
	...
	case CREATE_OBJECT:
		...
		if (!(Status & SOBJ_WAIT_CONFIRMATION) && Visibility == VISIBLE &&
			...
			RuffaGunTime >= ruffa_gun_wait_ticks()) {
			RuffaGunTime = 0;
			g = BulletD.CreateBullet();
			vCheck = Vector(64, 0, 0) * RotMat;
			g->CreateBullet(
		...
		if (NetFunction83Time != PrevNetFunction83Time &&
			GetDistTime(NetGlobalTime, NetFunction83Time) < 256 * ROTOR_PROCESS_LIFE_TIME / 20) {
			CreateParticleRotor(R_curr, 83);
			ExternalDraw = 0;
			switch_analysis(1);
		};
	...
	while (p) {
		if (p->ShellNetID == id) {
			if (p->Status & (SOBJ_DISCONNECT | SOBJ_ACTIVE)) {
				network_log_object_event(
					"IN",
					type,
					id,
					NETWORK_IN_STREAM.current_creator(),
					NETWORK_IN_STREAM.current_time(),
					NETWORK_IN_STREAM.current_x(),
					NETWORK_IN_STREAM.current_y(),
					NETWORK_IN_STREAM.current_radius(),
					NETWORK_IN_STREAM.current_body_size(),
					"ignored_shell_owner_inactive_or_local"
				);
				NETWORK_IN_STREAM.ignore_event();
			} else {
				p->ShellNetEvent(
	...
	if (p) {
		if (p->Status & (SOBJ_DISCONNECT | SOBJ_ACTIVE)) {
			network_log_object_event(
				...
				NETWORK_IN_STREAM.current_radius(),
				NETWORK_IN_STREAM.current_body_size(),
				"ignored_vanger_inactive_or_local"
			);
			NETWORK_IN_STREAM.ignore_event();
		} else {
			p->NetEvent(
...
void VangerUnit::DischargeItem(StuffObject *p) {
	...
	case ACI_GHORB_GEAR_HEAVY:
		if (NetworkON)
			p->ActIntBuffer.data1 = RND(p->ActIntBuffer.data1);
		else
			p->ActIntBuffer.data1 = RND(aiCutLuck) * p->ActIntBuffer.data1 / 100;
		CheckOutDevice(p);
		if (Status & SOBJ_ACTIVE) {
			ActD.CheckDevice(p);
			aciRemoveItem(&(p->ActIntBuffer));
	...
	case ACI_CRUSTEST_CANNON_AMMO:
		if (p->ActIntBuffer.data1 > 1) {
			if (NetworkON)
				p->ActIntBuffer.data1 = 1 + RND(p->ActIntBuffer.data1 - 1);
			else
				p->ActIntBuffer.data1 = 1 + RND(aiCutLuck) * (p->ActIntBuffer.data1 - 1) / 100;
			CheckOutDevice(p);
			if (Status & SOBJ_ACTIVE) {
				ActD.CheckDevice(p);
				aciRemoveItem(&(p->ActIntBuffer));
...
void XpeditionOFF(int type) // Finish the game!
{
	if (uvsKronActive && ActD.Active) {
		...
		camera_direct(
			...
			0,
			0,
			(ROTOR_PROCESS_LIFE_TIME * GAME_TIME_COEFF) + 1
		);
...
const char *NetworkEscaveName[NETWORK_NUM_ESCAVE] =
	{"Podish", "Incubator", "VigBoo", "Lampasso", "Ogorod", "ZeePa", "B-Zone"};
	...
	for (i = 0; i < NETWORK_NUM_ESCAVE; i++) {
		if (!strcmp(name, NetworkEscaveName[i])) {
			...
			for (j = ActD.NetEnterOffset + 1; j < SnsTableSize; j++) {
				if (SensorObjectData[j]->SensorType == SensorTypeList::PLAYER_BIRTH &&
					SensorObjectData[j]->data5 == i) {
					t = j;
					break;
				};
			};
	...
	if (uvsKronActive && ActD.Active && p->name) {
		...
		case MECHOSOMA:
			if (p->body.MechosomaStat.ItemCount1 >= my_server_data.Mechosoma.ProductQuantity1 &&
				...
				RaceTxtBuff < p->name;
				if (lang() == RUSSIAN)
					RaceTxtBuff < rPlayerWinnerSecondMessage;
				else
					RaceTxtBuff < PlayerWinnerSecondMessage;
				...
				RaceTxtBuff < p->name;
				if (lang() == RUSSIAN)
					RaceTxtBuff < rPlayerFirstDropedMessage;
				else
					RaceTxtBuff < PlayerFirstDropedMessage;
				aiMessageBuffer.align_type = 0;
				aiMessageBuffer.TimeBuf[0] = 200;
				aiMessageBuffer.ColBuf[0] = 143;
				aiMessageBuffer.TimeBuf[1] = 200;
				...
				if (lang() == RUSSIAN)
					aiMessageBuffer.add_str(1, (uchar *)(rPlayerSecondDropedMessage));
				else
					aiMessageBuffer.add_str(1, (uchar *)(PlayerSecondDropedMessage));
				aciSendPrompt(&aiMessageBuffer);
			};
			break;
		case PASSEMBLOSS:
			if (p->body.PassemblossStat.CheckpointLighting >=
				my_server_data.Passembloss.CheckpointsNumber) {
				RaceTxtBuff < p->name;
				if (lang() == RUSSIAN)
					RaceTxtBuff < rPlayerWinnerSecondMessage;
				else
