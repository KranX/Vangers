...
struct StuffObject: Object {
	...
	int CheckItemOver(void) {
		if (ActIntBuffer.type == ACI_PROTRACTOR || ActIntBuffer.type == ACI_MECHANIC_MESSIAH ||
			ActIntBuffer.type == ACI_FUNCTION83 || ActIntBuffer.type == ACI_SPUMMY ||
			ActIntBuffer.type == ACI_BOOT_SECTOR || ActIntBuffer.type == ACI_PEELOT ||
			ActIntBuffer.type == ACI_LEEPURINGA || ActIntBuffer.type == ACI_SANDOLL_PART1 ||
			ActIntBuffer.type == ACI_SANDOLL_PART2 || ActIntBuffer.type == ACI_LAST_MOGGY_PART1 ||
			...
			ActIntBuffer.type == ACI_LAWN_MOWER_PART2 || ActIntBuffer.type == ACI_WORMASTER_PART1 ||
			ActIntBuffer.type == ACI_WORMASTER_PART2 ||
			ActIntBuffer.type == ACI_TABUTASK_SUCCESSFUL || ActIntBuffer.type == ACI_TABUTASK)
			return 0;
		else
			return 1;
	};
...
struct BULLET_TARGET_MODE {
	enum { NONE = 0, CONTROL, LEFT_ROUND, RIGHT_ROUND, RANDOM, WALL };
};
...
struct BULLET_EVENT_ID {
	enum { MAP_DESTROY = 0, LIFE_TIME_DESTROY, TOUCH, HIDE_LIFE_TIME_DESTROY, HIDE_MAP_DESTROY };
};

struct BULLET_SHOW_TYPE_ID {
	enum { PARTICLE = 0, FIREBALL, DEFORM, DUST, CRATER, JUMPBALL, LASER };
};

struct BULLET_TYPE_ID {
	enum { CHAIN_GUN = 0, ROCKET, FIREBALL, TERMINATOR, JUMPBALL, LASER, HORDE, HYPNOTISE };
};
