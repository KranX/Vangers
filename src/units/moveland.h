...
struct MapPointType: GeneralObject {
	int Angle;
	int Type;
	void Create(const Vector &v, int type = 0, int angle = 0) {
		R_curr = v;
		Type = type;
		Angle = angle;
	};
...
struct MapTraceType: GeneralObject {
	...
	void Create(int _x0, int _y0, int _x1, int _y1, int nx, int ny, char f) {
		x0 = _x0;
		y0 = _y0;
		x1 = _x1;
		y1 = _y1;
...
struct MLFrame {
	...
	void clear(void) {
		period = 1;
		delta = terrain = c_delta = c_terrain = NULL;
		signBits = NULL;
		surfType = 99;
		csd = cst = 0;
		sz = ss = sx = sy = 0;
	...
	int
	quantAbs(int &step, int dx, int dy, MLAtype *alt, int dry, int render = 1, int fastMode = 0);
	...
	void Render(int dx, int dy) {
		regRender(x0 + dx, y0 + dy, x0 + dx + sx, y0 + dy + sy, 0);
	}
#ifdef _SURMAP_
	void accept(
		uchar *_delta,
...
struct MobileLocation {
	...
	int quant(
		...
		int skipVZ = 0,
		int skipCheck = 0
	); // skipVZ - пропускать проверку на видимость в Surmap (1 в setPhase)
	...
	void setDry(int dry) {
		if (dry < 0)
			dry = 0;
		else if (dry >= TERRAIN_MAX)
			dry = TERRAIN_MAX - 1;
		DryTerrain = dry;
	}
	void setKeyPhase(int n, int val) {
		KeyPhase[n] = val;
	}
	...
	// сразу устанавливает указанную фазу (прокручивает без рендера нужные периоды и рендерит фазу
	// nPhase) если nPhase не существует ничего не происходит
	void setPhase(int nPhase, int noChange = 0);
	// выставляет конечную фазу, на которой ML останавливается и ждет следующих указаний
	// если nPhase == -1, то ML входит в безостановочный режим, если фазы nPhase не существует
	// (слишком большая), то она становится последней возможной
	void goPhase(int nPhase);

	void setKeyPhase(int n) {
		if (n < 0)
			setPhase(-1);
		else
			setPhase(KeyPhase[n]);
	}
	void goKeyPhase(int n) {
		if (n < 0)
			goPhase(-1);
		else
			goPhase(KeyPhase[n]);
	...
	int isGoFinish(void) {
		return getCurPhase() == goPh;
	};

#ifdef _ROAD_
	int getCurPhase(void) {
		if (cStage == maxStage - 1)
			return 0;
		else
			return cStage + 1;
	}
...
struct BaseValoc {
	...
	virtual BaseValoc **getTail(void) {
		return NULL;
	}
