...
const int UNIQUE_ITEM_ID[NUM_UNIQUE_ITEM] = {UVS_ITEM_TYPE::SANDOLL_PART1,
	UVS_ITEM_TYPE::SANDOLL_PART2,
	UVS_ITEM_TYPE::LAST_MOGGY_PART1,
	UVS_ITEM_TYPE::LAST_MOGGY_PART2,
	UVS_ITEM_TYPE::QUEEN_FROG_PART2,
	UVS_ITEM_TYPE::LAWN_MOWER_PART1,
	UVS_ITEM_TYPE::LAWN_MOWER_PART2,
	UVS_ITEM_TYPE::WORMASTER_PART2};

const int NUM_UNIQUE_ARTEFACT = 4;
const int UNIQUE_ARTEFACT_ID[NUM_UNIQUE_ARTEFACT] = {UVS_ITEM_TYPE::PROTRACTOR,
...
void InitProtoCrypt(void) {
	...
	for (i = 0; i < WORLD_MAX; i++) {
		...
		if (memcmp(sign, "VLS1", strlen("VLS1")))
			ErrH.Abort("Bad CryptList Format");
		...
		for (j = 0; j < ProtoCryptTableSize[i]; j++) {
			...
			case 6:
			case 7:
				ProtoCryptTable[i][j].z0 =
					uvsGenerateItemForCrypt(ProtoCryptTable[i][j].SensorType);
				break;
			case 3:
				ProtoCryptTable[i][j].z0 = UNIQUE_ITEM_ID[n];
				n = (n + 1) % NUM_UNIQUE_ITEM;
				//					fout < "World: " <= i < "\tX: " <=
				// ProtoCryptTable[i][j].R_curr.x < "\tY:" <= ProtoCryptTable[i][j].R_curr.y < "\n";
				break;
			case 5:
		...
		if (lang() == RUSSIAN) {
			...
			/*			switch(CurrentWorld){
							case WORLD_FOSTRAL:
								...
								AddTarget2Compas(CMP_TARGET_SENSOR,(void*)("ArmorUpdate1"),"Repair
			   Station I"); AddTarget2Compas(CMP_TARGET_SENSOR,(void*)("ArmorUpdate2"),"Repair
			   Station II"); AddTarget2Compas(CMP_TARGET_SENSOR,(void*)("FlyUpdate1"),"Rigs
			   Station"); AddTarget2Compas(CMP_TARGET_SENSOR,(void*)("FireUpdate1"),"ghOrb
			   Station"); AddTarget2Compas(CMP_TARGET_SENSOR,(void*)("RandomUpdate1"),"Broken
			   Station I"); AddTarget2Compas(CMP_TARGET_SENSOR,(void*)("RandomUpdate2"),"Broken
			   Station II"); AddTarget2Compas(CMP_TARGET_SENSOR,(void*)("KeyUpdate1"),"Spiral
			   Station"); AddTarget2Compas(CMP_TARGET_PASSAGE,(void*)("F2G"),"Passage to Glorx");
								AddTarget2Compas(CMP_TARGET_PASSAGE,(void*)("F2W"),"Passage to
			   Weexow"); break; case WORLD_GLORX:
								...
								AddTarget2Compas(CMP_TARGET_PASSAGE,(void*)("G2F"),"Passage to
			   Fostral"); AddTarget2Compas(CMP_TARGET_PASSAGE,(void*)("G2N"),"Passage to  Necross");
								AddTarget2Compas(CMP_TARGET_PASSAGE,(void*)("G2X"),"Passage to
			   Xplo"); AddTarget2Compas(CMP_TARGET_PASSAGE,(void*)("G2K"),"Passage to  Khox");
								AddTarget2Compas(CMP_TARGET_SENSOR,(void*)("FlyUpdate01"),"Rigs
			   Station I"); AddTarget2Compas(CMP_TARGET_SENSOR,(void*)("FlyUpdate02"),"Rigs Station
			   II"); AddTarget2Compas(CMP_TARGET_SENSOR,(void*)("ArmorUpdate01"),"Repair Station
			   I"); AddTarget2Compas(CMP_TARGET_SENSOR,(void*)("ArmorUpdate02"),"Repair Station
			   II"); AddTarget2Compas(CMP_TARGET_SENSOR,(void*)("FireUpdate01"),"ghOrb Station");
								AddTarget2Compas(CMP_TARGET_SENSOR,(void*)("KeyUpdate01"),"Spiral
			   Station"); break; case WORLD_NECROSS:
								AddTarget2Compas(CMP_TARGET_SPOT,(void*)("B-Zone"),"B-Zone");
								AddTarget2Compas(CMP_TARGET_ESCAVE,(void*)("ZeePa"),"ZeePa");
								AddTarget2Compas(CMP_TARGET_PASSAGE,(void*)("N2G"),"Passage to
			   Glorx"); AddTarget2Compas(CMP_TARGET_PASSAGE,(void*)("N2B"),"Passage to  Boozeena");
								AddTarget2Compas(CMP_TARGET_PASSAGE,(void*)("N2A"),"Passage to
			   Ark'a'Znoy"); AddTarget2Compas(CMP_TARGET_SENSOR,(void*)("FlyUpdate01"),"Rigs
			   Station"); AddTarget2Compas(CMP_TARGET_SENSOR,(void*)("ArmorUpdate01"),"Repair
			   Station"); AddTarget2Compas(CMP_TARGET_SENSOR,(void*)("KeyUpdate01"),"Spiral
			   Station"); AddTarget2Compas(CMP_TARGET_SENSOR,(void*)("RandomUpdate01"),"Broken
			   Station"); break; case WORLD_XPLO:
								AddTarget2Compas(CMP_TARGET_SPOT,(void*)("Spobs"),"Spobs");
								AddTarget2Compas(CMP_TARGET_PASSAGE,(void*)("X2G"),"Passage to
			   Glorx"); AddTarget2Compas(CMP_TARGET_PASSAGE,(void*)("X2T"),"Passage to Threall");
								break;
							case WORLD_KHOX:
								AddTarget2Compas(CMP_TARGET_PASSAGE,(void*)("K2G"),"Passage to
			   Glorx"); break; case WORLD_BOOZEENA:
								AddTarget2Compas(CMP_TARGET_PASSAGE,(void*)("B2N"),"Passage to
			   Necross"); break; case WORLD_WEEXOW:
								AddTarget2Compas(CMP_TARGET_PASSAGE,(void*)("W2F"),"Passage to
			   Fostral"); break; case WORLD_THREALL:
								AddTarget2Compas(CMP_TARGET_PASSAGE,(void*)("T2X"),"Passage to
			   Xplo"); break; case WORLD_ARKONOY:
								AddTarget2Compas(CMP_TARGET_PASSAGE,(void*)("A2N"),"Passage to
			   Necross"); break;
						};*/
		};
...
void StorageType::Deactive(GeneralObject *p) {
	...
	if (ObjectPointer < Max)
		Data[ObjectPointer++] = p;
	else
		ErrH.Abort("Object stack overflow");
};
...
void UnitList::DisconnectTypeList(GeneralObject *p) {
	if ((!p->PrevTypeList) && (!p->NextTypeList))
		Tail = NULL;
	else {
		if (p->PrevTypeList)
			p->PrevTypeList->NextTypeList = p->NextTypeList;
		else
			Tail = p->NextTypeList;
		if (p->NextTypeList)
			p->NextTypeList->PrevTypeList = p->PrevTypeList;
	};
	Num--;
};
...
void setMapPixel(int px, int py, int col) {
	int xx, yy;
	G2L(px, py, xx, yy);
	if (xx < UcutLeft + 1 || xx > UcutRight - 1 || yy < VcutUp + 1 || yy > VcutDown - 1)
		return;
	...
	if (p) {
		...
		if ((*t) & DOUBLE_LEVEL) {
			if (x & 1) {
				...
				d = *(p - 1);
				if (d < z &&
					(d + (((GET_DELTA(*(t - 1)) << 2) + GET_DELTA(*t) + 1) << DELTA_SHIFT)) > z)
					return 0;
			} else {
				if (*(p + 1) < z)
					return 0;
				d = *p;
				if (d < z &&
					(d + (((GET_DELTA(*t) << 2) + GET_DELTA(*(t + 1)) + 1) << DELTA_SHIFT)) > z)
					return 0;
			};
		} else if ((*p) < z)
			return 0;
	...
	if (p) {
		...
		if ((*t) & DOUBLE_LEVEL) {
			if (v.x & 1) {
				...
				d = *(p - 1);
				if (d < v.z) {
					if ((d + (((GET_DELTA(*(t - 1)) << 2) + GET_DELTA(*t) + 1) << DELTA_SHIFT)) >
						v.z) {
						alt = d;
						return 0;
					} else {
				...
				d = *p;
				if (d < v.z) {
					if ((d + (((GET_DELTA(*t) << 2) + GET_DELTA(*(t + 1)) + 1) << DELTA_SHIFT)) >
						v.z) {
						alt = d;
						return 0;
					} else {
...
void VangerUnit::DrawMechosParticle(int x, int y, int speed, int level, int n) {
	...
	if (Armor < MaxArmor && frame % (int)GAME_TIME_COEFF == 0 && (int)(RND(MaxArmor)) > Armor &&
	...
	if (speed != 0) {
			...
			if (level) {
				...
				if (trn == 1) {
					...
					if (frame % (int)GAME_TIME_COEFF == 0) {
						if (PrevWheelY[n] != 0)
							MapD.CreateTrace(
								x, y, PrevWheelX[n], PrevWheelY[n], track_nx, track_ny, 1
							);
						PrevWheelX[n] = x;
						PrevWheelY[n] = y;
					}
				...
				if (trn == 1) {
					...
					if (frame % (int)GAME_TIME_COEFF == 0) {
						if (PrevWheelY[n] != 0)
							MapD.CreateTrace(
								x, y, PrevWheelX[n], PrevWheelY[n], track_nx, track_ny, 0
							);
						PrevWheelX[n] = x;
						PrevWheelY[n] = y;
					}
		...
		if (Status & SOBJ_ACTIVE) {
			...
			if (!TypeMap)
				return;
			TypeMap += H_SIZE + x;
			if (level && ((*TypeMap) & DOUBLE_LEVEL) && GET_REAL_TERRAIN(TypeMap, x) == 1)
				NullTime++;
			else
				NullTime = 0;
		};
	...
	while (p) {
		if (p->Visibility == VISIBLE) {
			...
			if (!ViewTail) {
				...
				if (n)
					ConnectNextView(p, pp);
				else
					ConnectPrevView(p, pp);
			};
		};
		p = p->NextBaseList;
	};
...
void GameObjectDispatcher::ConnectNextView(BaseObject *p, BaseObject *link) {
	...
	if (link->PrevViewList)
		link->PrevViewList->NextViewList = p;
	else
		ViewTail = p;
	link->PrevViewList = p;
};
...
void GameObjectDispatcher::ConnectPrevView(BaseObject *p, BaseObject *link) {
	...
	if (link->NextViewList)
		link->NextViewList->PrevViewList = p;
	link->NextViewList = p;
};
...
int FirstColorPlace[WORLD_MAX][NUM_COLOR_PLACE] = {
	{83, 83, 83, 83, 83, 83, 83, 83, 128, 144, 160, 176, 184, 192, 224, 240},
...
int LastColorPlace[WORLD_MAX][NUM_COLOR_PLACE] = {
	{83, 83, 83, 83, 83, 83, 83, 83, 143, 159, 175, 183, 191, 223, 239, 254},
...
const int DecColorPlace[WORLD_MAX][NUM_COLOR_PLACE] = {
	{4, 15, 5, 5, 7, 3, 7, 7, 17, 15, 20, 4, 32, 32, 32, 32},
...
const int GrayColorCycle[WORLD_MAX][NUM_COLOR_PLACE] = {
	{63, 63, 63, 0, 0, 0, 0, 63, 0, 0, 0, 0, 0, 0, 0, 0},
	...
	if (uvsCurrentWorldUnable && CurrentWorld < MAIN_WORLD_MAX - 1) {
		...
		for (k = 0; k < WorldPalNum; k++) {
			for (i = 0; i < NUM_COLOR_PLACE; i++) {
				for (j = FirstColorPlace[CurrentWorld][i]; j <= LastColorPlace[CurrentWorld][i];
					j++) {
					vCheck = Vector(
						WorldPalData[k][j * 3],
						WorldPalData[k][j * 3 + 1],
						WorldPalData[k][j * 3 + 2]
					);
					dc = 63 * vCheck.vabs() / MaxVector;
					WorldPalData[k][j * 3] =
						WorldPalData[k][j * 3] +
						GrayColorCycle[CurrentWorld][i] * (dc - WorldPalData[k][j * 3]) / 63;
					WorldPalData[k][j * 3 + 1] =
						WorldPalData[k][j * 3 + 1] +
						GrayColorCycle[CurrentWorld][i] * (dc - WorldPalData[k][j * 3 + 1]) / 63;
					WorldPalData[k][j * 3 + 2] =
						WorldPalData[k][j * 3 + 2] +
						GrayColorCycle[CurrentWorld][i] * (dc - WorldPalData[k][j * 3 + 2]) / 63;
				};
			};
		};
	};
	...
	if (GeneralSystemSkip) {
				...
				for (i = 0; i < NUM_COLOR_PLACE; i++) {
					for (j = FirstColorPlace[CurrentWorld][i]; j <= LastColorPlace[CurrentWorld][i];
						...
						WorldPalData[k][j * 3] =
							...
							GrayColorCycle[CurrentWorld][i] * (dc - WorldPalData[k][j * 3]) / 63;
						WorldPalData[k][j * 3 + 1] =
							WorldPalData[k][j * 3 + 1] + GrayColorCycle[CurrentWorld][i] *
															 (dc - WorldPalData[k][j * 3 + 1]) / 63;
						WorldPalData[k][j * 3 + 2] =
							WorldPalData[k][j * 3 + 2] + GrayColorCycle[CurrentWorld][i] *
															 (dc - WorldPalData[k][j * 3 + 2]) / 63;
					};
				};
			};
		};
		...
		for (k = 0; k < 256; k++) {
			for (i = 0; i < 256; i++) {
				...
				ti = FIRE_COLOR_FIRST;
				vColor = Vector(
					palbuf[FIRE_COLOR_FIRST * 3],
					palbuf[FIRE_COLOR_FIRST * 3 + 1],
					palbuf[FIRE_COLOR_FIRST * 3 + 2]
				);
		...
		for (k = 0; k < NUM_COLOR_PLACE; k++) {
			...
			for (i = FirstColorPlace[CurrentWorld][k]; i <= LastColorPlace[CurrentWorld][k]; i++) {
				...
				vColor = vColor * d1 / dc;
				vCheck = Vector(
							 palbufOrg[FirstColorPlace[CurrentWorld][k]],
							 palbufOrg[FirstColorPlace[CurrentWorld][k] + 1],
							 palbufOrg[FirstColorPlace[CurrentWorld][k] + 2]
						 ) -
						 vColor;
				nmin = FirstColorPlace[CurrentWorld][k];
				cmin = vCheck.vabs();

				for (j = FirstColorPlace[CurrentWorld][k] + 1; j <= LastColorPlace[CurrentWorld][k];
					j++) {
					vCheck = Vector(palbufOrg[j * 3], palbufOrg[j * 3 + 1], palbufOrg[j * 3 + 2]) -
							 vColor;
					d2 = vCheck.vabs();
					if (d2 <= cmin) {
						nmin = j;
				...
				if (nmin > i)
					ShadowColorTable[i] = i;
				else
					ShadowColorTable[i] = nmin;
			};
		};
		...
		for (n = 0; n < TERRAIN_MAX; n++) {
			if (n == 1 || n == 7) {
				...
				for (k = 0; k < 256; k++) {
					for (i = 0; i < 256; i++) {
						vColor =
							Vector(palbufOrg[i * 3], palbufOrg[i * 3 + 1], palbufOrg[i * 3 + 2]);
						d1 = vColor.vabs() + (MaxVector - vColor.vabs()) * k / 256;
						d1 = lmin + d1 * cdelta / MaxVector;

						ti = BEGCOLOR[n];
						vColor = Vector(
							palbufOrg[BEGCOLOR[n] * 3],
							palbufOrg[BEGCOLOR[n] * 3 + 1],
							palbufOrg[BEGCOLOR[n] * 3 + 2]
						);
						dec = abs(vColor.vabs() - d1);

						for (j = BEGCOLOR[n] + 1; j < ENDCOLOR[n]; j++) {
							vColor = Vector(
								palbufOrg[j * 3], palbufOrg[j * 3 + 1], palbufOrg[j * 3 + 2]
							);
							d2 = vColor.vabs();
							dc = abs(d2 - d1);
							if (dc < dec) {
		...
		for (i = 0; i < 8; i++)
			register_color(
				COLORS_IDS::MATERIAL_0 + i,
				FirstColorPlace[CurrentWorld][i],
				LastColorPlace[CurrentWorld][i]
			);
	};
};

int WorldLightParam[WORLD_MAX][3] = {{205, 256, 160},
	{205, 256, 160},
	{205, 256, 160},
	{160, 0, 0},
	{160, 0, 0},
...
void BaseObject::GetVisible(void) {
	/*	if(DepthShow){
			...
			if(zz < -radius || abs(xx) > zz*TurnSideX/ViewZ + radius*2 ||  abs(yy) >
	   zz*TurnSideY/ViewZ + radius*2) Visibility = UNVISIBLE; else Visibility = VISIBLE; }else{
...
// #endif //ZMOD_BETA
			...
			if (!actCurrentViewDolly) {
				...
				if (!actCurrentViewStuff) {
					...
					if (actCurrentViewObject &&
						(actCurrentViewObject->Status & SOBJ_WAIT_CONFIRMATION)) {
						do {
							actCurrentViewObject =
								(VangerUnit *)(actCurrentViewObject)->NextTypeList;
						} while (actCurrentViewObject &&
								 (actCurrentViewObject->Status & SOBJ_WAIT_CONFIRMATION));
					};

					actCurrentViewDolly = (uvsDolly *)(DollyTail);
					if (actCurrentViewDolly &&
						(actCurrentViewDolly->Pworld->gIndex != CurrentWorld ||
							actCurrentViewDolly->status)) {
						do {
							actCurrentViewDolly = (uvsDolly *)(actCurrentViewDolly->next);
						} while (actCurrentViewDolly &&
								 (actCurrentViewDolly->Pworld->gIndex != CurrentWorld ||
									 actCurrentViewDolly->status));
					};
					return -1;
				...
				actCurrentViewDolly = (uvsDolly *)(DollyTail);
				if (actCurrentViewDolly && (actCurrentViewDolly->Pworld->gIndex != CurrentWorld ||
											   actCurrentViewDolly->status)) {
					do {
						actCurrentViewDolly = (uvsDolly *)(actCurrentViewDolly->next);
					} while (actCurrentViewDolly &&
			...
			do {
				actCurrentViewDolly = (uvsDolly *)(actCurrentViewDolly->next);
			} while (actCurrentViewDolly && (actCurrentViewDolly->Pworld->gIndex != CurrentWorld ||
												actCurrentViewDolly->status));
	...
	case ID_VANGER:
		if (actCurrentViewObject->VangerRaceStatus != VANGER_RACE_NONE /* && aciRacingFlag*/) {
			...
			do {
				...
				actCurrentViewObject && (actCurrentViewObject->Status & SOBJ_WAIT_CONFIRMATION));
			return 3;
		};
...
static void
ScreenLineTraceImpl(Vector &v0, Vector &v1, uchar *ColorTable, uchar flag, int laserGlow) {
	int x0, y0, x1, y1;
	int dx, dy, k, cx, cy;
	int tx, ty;
	Vector vDelta, vC;
		...
		if (abs(dx) > abs(dy)) {
			...
			while (dx) {
				...
				if (cx > UcutLeft && cx < UcutRight && ty > VcutUp && ty < VcutDown) {
					if (GetAltLevel(
							Vector(vC.x >> FIXED_SHIFT, vC.y >> FIXED_SHIFT, vC.z >> FIXED_SHIFT)
						)) {
...
GeneralObject *MemoryStorageType::GetPoint(void) {
	...
	if (n)
		...
		ErrH.Abort("Bad Tail in Units Heap");
	return (g->GetPoint());
};
...
void StorageClusterType::PutPoint(GeneralObject *p) {
	if (CurrPoint >= MaxPoint)
		ErrH.Abort("Memory stack overflow");
	if ((long long)(p) < (long long)(Data) ||
		(long long)(p) >= (long long)(Data + MaxPoint * PointSize))
		ErrH.Abort("Bad Units Memory Area");
	...
	while ((type = NETWORK_IN_STREAM.current_event())) {
		if (!(type & AUXILIARY_EVENT)) {
			...
			default:
				network_log_printf(
					"IN",
					"%s decision=ignored_aux_in_dispatcher body_size=%d",
					type == 0 ? "UNKNOWN_EVENT" : "AUX_EVENT",
					NETWORK_IN_STREAM.current_body_size()
				);
				NETWORK_IN_STREAM.ignore_event();
				break;
...
int NetInit(ServerFindChain *p) {
	...
	NETWORK_OUT_STREAM.register_name(
		(char *)(aciGetPlayerName() ? aciGetPlayerName() : "Finger"),
		(char *)(aciGetPlayerPassword() ? aciGetPlayerPassword() : "Password")
	);
	std::cout << "NetInit - [2]" << std::endl;
	// NETWORK_OUT_STREAM.register_name(aciGetPlayerName() ? aciGetPlayerName() : "Finger",
	// "Password");
...
void NetworkWorldOpen(void) {
	...
	//	NETWORK_OUT_STREAM.set_position(ViewX,ViewY,round((fabs(curGMap -> xsize*sinTurnInvFlt) +
	// fabs(curGMap -> ysize*cosTurnInvFlt))*0.5)); 	NETWORK_OUT_STREAM.send(1);
	...
	if (NetworkLoadEnable) {
		for (i = 0; i < TntTableSize; i++) {
			NETWORK_OUT_STREAM.create_permanent_object(
				TntObjectData[i]->NetID,
				TntObjectData[i]->R_curr.x,
				TntObjectData[i]->R_curr.y,
				TntObjectData[i]->radius
			);
		...
		for (i = 0; i < NumLocation; i++) {
			NETWORK_OUT_STREAM.create_permanent_object(
				LocationData[i]->NetID,
				LocationData[i]->R_curr.x,
				LocationData[i]->R_curr.y,
				LocationData[i]->radius
			);
		...
		/*		for(i = 0;i < ProtoCryptTableSize[CurrentWorld];i++){
		   ...
		   ProtoCryptTable[CurrentWorld][i].SensorType == 2 ||
		...
		if (art_log) {
			for (i = 0; i < ProtoCryptTableSize[CurrentWorld]; i++) {
				if ((ProtoCryptTable[CurrentWorld][i].SensorType == 3 ||
					...
					ProtoCryptTable[CurrentWorld][i].z0 != UVS_ITEM_TYPE::SECTOR &&
					ProtoCryptTable[CurrentWorld][i].z0 != UVS_ITEM_TYPE::SPUMMY &&
					ProtoCryptTable[CurrentWorld][i].z0 != UVS_ITEM_TYPE::PEELOT)
					uvsCreateNewItem(
...
void aiMessageType::Load(Parser &in) {
	...
	Type = in.get_int();
	if (lang() == RUSSIAN)
		in.search_name("rNumString");
	else
		in.search_name("NumString");
	Num = in.get_int();
...
void NetworkSetTnt(int id) {
	if (!NetworkLoadEnable)
		TntObjectData[(id & 0xffff) - 1]->NetID = id;
...
void PalPoint::Set(int mode, int time, uchar *p1, uchar *p2) {
	...
	case CPAL_CHANGE_CYCLE:
		...
		DeltaWorldLightParam =
			((WorldLightParam[CurrentWorld][WorldPalCurrent] << 8) - CurrentWorldLightParam) / time;
		break;
	...
	case CPAL_HIDE_PASSAGE:
		...
		for (i = 0; i < 768; i++) {
			...
			if (i / 3 >= BEGCOLOR[5] && i / 3 <= ENDCOLOR[5])
				...
				DeltaColor[i] = -FirstColor[i] / Time;
		};
		break;
	...
	case CPAL_SPOBS_TO:
		for (i = BEGCOLOR[SPOBS_PAL_TERRAIN] * 3; i <= ENDCOLOR[SPOBS_PAL_TERRAIN] * 3; i++) {
			palbufSrc[i] = palbufOrg[i] =
				5 * (i - BEGCOLOR[SPOBS_PAL_TERRAIN] * 3) /
				(3 * (ENDCOLOR[SPOBS_PAL_TERRAIN] - BEGCOLOR[SPOBS_PAL_TERRAIN]));
		};
	...
	case CPAL_THREALL_TO:
		for (j = 0; j < TERRAIN_MAX; j++) {
			for (i = BEGCOLOR[j]; i <= ENDCOLOR[j]; i++) {
				if (j == 3 || j == 5 || j == 6 || j == 4)
					break;
				palbufSrc[i * 3] = palbufOrg[i * 3] =
					63 * (i - BEGCOLOR[j]) / (ENDCOLOR[j] - BEGCOLOR[j]);
				palbufSrc[i * 3 + 1] = palbufOrg[i * 3 + 1] =
					63 * (i - BEGCOLOR[j]) / (ENDCOLOR[j] - BEGCOLOR[j]);
				palbufSrc[i * 3 + 2] = palbufOrg[i * 3 + 2] =
					63 * (i - BEGCOLOR[j]) / (ENDCOLOR[j] - BEGCOLOR[j]);
			};
		};
		for (i = 0; i < 768; i++) {
...
void aiMessageList::Send(int ind, int speed, int n, int sf) {
	aiMessageListElem *p;
	static int PrevSpeed = 0;
	if (!ActD.Active || !(ActD.Active->ExternalDraw))
		return;
	if (abs(speed) != 0)
		PrevSpeed = ai_message_speed_tabu_ticks();
	else
		PrevSpeed--;
	if (sf && PrevSpeed > MAX_AI_MESSAGE_SPEED)
		return;
...
int CheckThreallMessiah(void) {
	if (ActD.Active && GetStuffObject(ActD.Active, ACI_MECHANIC_MESSIAH))
		return 1;
	return 0;
};
