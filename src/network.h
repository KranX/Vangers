...
*******************************************************************************/
// send message modes
enum { MESSAGE_FOR_ALL, MESSAGE_FOR_TEAM, MESSAGE_FOR_PLAYER };
...
struct MessageDispatcher: XList {
	...
	MessageDispatcher(int max_num_messages = 100): XList() {
		max_number_of_messages = max_num_messages;
	}
	MessageElement *first() {
		return (MessageElement *)fPtr;
	}
	MessageElement *last() {
		return (MessageElement *)lPtr;
	}
...
class OutputEventBuffer: public XBuffer {
  ...
  public:
	...
	int send_simple_query(
		int event,
		unsigned char data,
		XSocket &sock = main_socket
	); // Sends two-byte events
	int send_simple_query(
		int event,
		unsigned char data1,
		unsigned char data2,
		XSocket &sock = main_socket
	); // Sends three-byte events
};
...
class InputEventBuffer: public XBuffer {
  ...
  public:
	...
	int receive(
		XSocket &sock = main_socket,
		int dont_free = 0
	); // 0 if there isn't any enents,  first EventID othewise
	int receive_waiting_for_event(
		int event,
		XSocket &sock = main_socket,
		int skip_if_aint = 0
	); // Waits for event for a waiting_delay and moves it ot the begining of buffer. All others
	   // events will be saved

	int current_event() {
		return event_ID;
	} // 0 if there isn't any enents,  EventID othewise
	int current_ID() {
		return object_ID;
	}
	int current_creator() {
	...
	int current_item_state() {
		return item_state;
	}
	int current_item_previous_ID() {
		return item_previous_ID;
	}
	int current_item_removed_paired_ID() {
		return item_removed_paired_ID;
	}
	int current_item_removed_reason() {
		return item_removed_reason;
	}
	// 1. If there isn't such a datum in the event (x,y for delete_object), no error will be
	// generated
...
int send_server_data(
	...
	ServerData *data,
	zServerData *zdata
); // returns 1 if _You_'ve configured server, otherwise data and name will be changed
int is_current_server_configured(
	char *name,
	ServerData *data,
	zServerData *zdata
);						   // changes name and data if server is configured
...
#define CREATE_NET_ID(d) \
	((++LocalStationCounter[(d & (~(1 << 31))) >> 16] & 0xffff) | d | LocalNetEnvironment)
#define CREATE_STUFF_NET_ID(d)                                                             \
	(((++LocalStationCounter[(d & (~(1 << 31))) >> 16] + stuff_ID_offsets) & 0xffff) | d | \
		LocalNetEnvironment)
