...
struct UVS_TABUTASK_STATUS {
	enum { ACTIVE = 1, GOOD = 2, BAD = 4, SLEEP = 8, OK = 16 };
};

struct UVS_GAME_ID {
	enum { ELEECH = 1, PIPETKA, KERNOBOO, WEEZYK };
};

struct UVS_CAR_TYPE {
	enum { RAFFA = 0, LIGHT, MICROBUS, ATW, TRACK, SPECIAL };
};

struct UVS_BUNCH_STATUS {
	enum { KILL_GAMER = 0x01, UNABLE = 0x02 };
};

struct UVS_DOLLY_STATUS {
	enum { CATCH = 0x01, UNABLE = 0x02 };
};

struct UVS_DOLLY_TYPE {
	enum { OWN = 0x01, ALIEN = 0x02 };
};

struct UVS_MECHOS_USED {
	enum { RACE = 0x01, HARVEST = 0x02, CARAVAN = 0x04, KILLER = 0x08, USES = 0x10 };
};

typedef int UvsObjectType;
struct UVS_OBJECT {
	enum { DOLLY = 1, VANGER, FLY_FARMER };
};

struct UVS_TOWN {
	enum { SPOT = 0, ESCAVE = 1 };
};
...
struct UVS_ORDER {
	enum { NONE = 0, MOVING, HUNTING, CIRT_GATHERING };
};
...
struct PrmFile {
	...
	char *getAtom(
		void
	); // получить следующий атом - строку, разделенную с другими пустотой (или в кавычках)
	void finit(void) {
		delete[] buf;
	}
};
...
struct listElem {
	...
	void link(
		listElem *&tail
	); // добавляет объект (this) в список с хвостом на tail (обычно глобальный указатель) в конец
	listElem *seekName(const char *name); // ищет по списку (this как хвост) name, сравнивая его со
										  // значением виртуального метода GetName()
...
struct uvsActInt: listElem {
	...
	uvsActInt(void): listElem() {
		type = price = sell_price = param1 = param2 = 0;
		pos_x = pos_y = 0;
	}
...
struct uvsOrder {
	...
	uvsOrder(void) {
		type = UVS_ORDER::NONE;
		target = UVS_TARGET::NONE;
		Ptarget = NULL;
		event = UVS_EVENT::DEATH;
	}
};
...
struct uvsCultGame {
	...
	uvsCultGame(void) {
		GameType = typeTownBeg = typeTownEnd = GoodsTypeBeg = GoodsTypeEnd = GoodsTypeLast =
			GoodsTypeBegCount = GoodsTypeEndCount = GoodsBegCount = GoodsEndCount = result = total =
				flag = first = all = sucks = 0;
		start = 0;
		townBeg = townEnd = NULL;
		score = NULL;
	}
...
struct uvsBunch: listElem {
	...
	uvsBunch(void): listElem() {
		Pescave = NULL;
		biosNindex = cycleN = currentStage = vngResource = 0;
		cycleTable = NULL;
		status = 0;
		id = 0;
	}
	...
	char *GetCurrentStageName(void) {
		return cycleTable[currentStage].name;
	}
	// сколько на складе нюхи?
	int GetCirtQ(void) {
		return cycleTable[currentStage].cirtQ;
	}
	// привезли еще нюхи
	int addCirt(uvsItem *);
	void randVRes(void) {
		vngResource = RND(V_PRICE) + V_PRICE / 2;
	}
	void Check(char *);
};
...
struct uvsWorld: listElem {
	...
	uvsWorld(void): listElem() {
		name = NULL;
		gIndex = x_size = y_size = GamerVisit = escTmax = sptTmax = pssTmax = locked = 0;
		escT = NULL;
		sptT = NULL;
		pssT = NULL;
		Pitem = NULL;
	...
	// возвращает указатель на Куклу, производящую нюху для биоса biosInd, если она есть на этом
	// мире или создает таковую при ее отсутствии
	uvsDolly *generateDolly(int biosInd);
	// возвращает случ. spot мира
	uvsSpot *getRandSpot(void) {
		return sptT[RND(sptTmax)];
	}
...
struct uvsEscave: uvsTarget, listElem {
	...
	uvsEscave(void): uvsTarget(), listElem() {
		...
		name = NULL;
		Ptrade = Pitem = NULL;
		TradeCount = locked = GamerVisit = 0;
		Pworld = NULL;
		Pbunch = NULL;
		Pshop = NULL;
		Pgood = NULL;
		TabuTaskID = TabuTaskCount = TabuTaskGood = 0;
		PassagerFrom = 0;
		PassagerWait = 0;
		lastTabuTaskGood = 0;
	}
	...
	int GetCurrentStage(void) {
		return Pbunch->currentStage;
	}
	char *GetCurrentStageName(void) {
		return Pbunch->GetCurrentStageName();
	}
...
struct uvsSpot: uvsTarget, listElem {
	...
	uvsSpot(void): uvsTarget(), listElem() {
		name = NULL;
		Ptrade = NULL;
		TradeCount = locked = GamerVisit = 0;
		Pworld = NULL;
...
struct uvsPassage: uvsTarget, listElem {
	...
	uvsPassage(void): uvsTarget(), listElem() {
		name = NULL;
		Pworld = NULL;
		Poutput = NULL;
		locked = 0;
...
struct uvsCrypt: listElem, uvsTarget {
	...
	uvsCrypt(void): listElem(), uvsTarget() {
		type = 0;
		pos_z = 0;
	}
	uvsCrypt(int _x, int _y, int _z, int _type) {
		type = _type;
		pos_z = _z;
		pos_y = _y;
		pos_x = _x;
...
struct uvsItemType {
	...
	uvsItemType(void) {
		name = NULL;
		gamer_use = param1 = param2 = type = SteelerTypeFull = SteelerTypeEmpty = size = count = 0;
	}
...
struct uvsItem: uvsTarget, listElem {
	...
	uvsItem(void): uvsTarget(), listElem() {
		type = 0;
		param1 = 0;
		param2 = 0;
		pos_z = 0;
	...
	uvsItem(int itemType, int x = 0, int y = 0) {
		type = itemType;
		pos_x = x;
		pos_y = y;
		param1 = uvsItemTable[itemType]->param1;
		param2 = uvsItemTable[itemType]->param2;
		pos_z = 0;
	};
	uvsItem(int x, int y, int z, int itemType) {
		type = itemType;
		pos_x = x;
		pos_y = y;
		param1 = uvsItemTable[itemType]->param1;
...
struct uvsMechosType {
	...
	uvsMechosType(void) {
		name = NULL;
		constractor = price = sell_price = MaxSpeed = MaxArmor = MaxEnergy = DeltaEnergy =
			DropEnergy = MaxFire = MaxWater = MaxOxigen = MaxFly = MaxTeleport = box[0] = box[1] =
				box[2] = box[3] = 0;
...
struct uvsDolly: uvsElement, uvsTarget, listElem {
	...
	void setSpeed(
		void
	); // устанавливает новый (случайный) вектор и модуль скорости, возвращает количество нюхи
...
struct uvsVanger: uvsElement, uvsTarget {
	...
	int isBunchReady(void) {
		return !owner->vngResource;
	}
	...
	int makeJump(
		void
	); // выйти из коридора на др. его конце, возвращает 1 если удачно, 0 - если выход занят
	...
	int isActive(void) {
		if (status != UVS_VANGER_STATUS::RACE_HUNTER && status != UVS_VANGER_STATUS::RACE &&
			status != UVS_VANGER_STATUS::MOVEMENT && status != UVS_VANGER_STATUS::FREE_MOVEMENT &&
			status != UVS_VANGER_STATUS::GATHERING && status != UVS_VANGER_STATUS::WAIT_GAMER)
			return 0;
