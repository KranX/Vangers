...
const int ICON_SX = 7;
const int ICON_SY = 7;
static uchar ICON_TABU[ICON_SX * ICON_SY] = {SYSCOLOR,
	SYSCOLOR,
	SYSCOLOR,
	SYSCOLOR,
	SYSCOLOR,
	...
	0,
	0};
static uchar ICON_SLAVE[ICON_SX * ICON_SY] = {0,
	0,
	0,
	0,
	0,
	...
	0,
	0};
static uchar ICON_RANGER[ICON_SX * ICON_SY] = {0,
	0,
	0,
	0,
	0,
	...
	0,
	0};
static uchar ICON_GAMER[ICON_SX * ICON_SY] = {0,
	0,
	0,
	0,
	0,
	...
	0,
	0};
static uchar ICON_THIEF[ICON_SX * ICON_SY] = {0,
	0,
	0,
	0,
	0,
	...
	0,
	0};
static uchar ICON_CARAVAN[ICON_SX * ICON_SY] = {0,
	0,
	0,
	0,
	0,
	...
	0,
	0};
static uchar ICON_KILLER[ICON_SX * ICON_SY] = {SYSCOLOR,
	0,
	0,
	0,
	SYSCOLOR,
	...
	0,
	0};
static uchar ICON_GSLAVE[ICON_SX * ICON_SY] = {SYSCOLOR,
	0,
	0,
	0,
	0,
...
static uchar ICON_FLY_FARMER[ICON_SX * ICON_SY] = {0,
	0,
	0,
	SYSCOLOR,
	0,
...
void StandScreenPrepare(void) {
	...
	const int wsy = 145;
#endif
	for (int y = 80 + 16 + 2, i = 0; i < MAIN_WORLD_MAX; i++, y += wsy + 3)
		scrTable[i] = new WorldScreen(y, wsy, getWorld(i));
}
...
void WorldScreen::Quant(void) {
	...
	info < "                  " < Pworld->name;
	if (Pworld->escTmax) {
		info < " [Stage " <= Pworld->escT[0]->GetCurrentStage() < ": " <
			Pworld->escT[0]->GetCurrentStageName() < "]";
#ifndef TINY_MODE
	...
	for (i = 0; i < Pworld->escTmax; i++) {
		...
		yy = y + GetY(pe->pos_x);
		XGR_Rectangle(
			xx - ESCAVE_SIDE, yy - ESCAVE_SIDE, ESCAVE_SIZE, ESCAVE_SIZE, COL1, COL1, XGR_FILLED
		);
		s = pe->name;
		yy += ESCAVE_SIDE + 1;
		len = strlen(s) * 8;
	...
	while (ppe) {
		...
		XGR_PutSpr(
			xx - ICON_SX / 2, yy - ICON_SY / 2, ICON_SX, ICON_SY, ICON_FLY_FARMER, XGR_HIDDEN_FON
		);
...
void WorldScreen::VangerQuant(uvsVanger *pv) {
	...
	case UVS_VANGER_SHAPE::BUNCH_SLAVE:
		XGR_PutSpr(
			xx - ICON_SX / 2, yy - ICON_SY / 2, ICON_SX, ICON_SY, ICON_SLAVE, XGR_HIDDEN_FON
		);
		break;
	case UVS_VANGER_SHAPE::RANGER:
		XGR_PutSpr(
			xx - ICON_SX / 2, yy - ICON_SY / 2, ICON_SX, ICON_SY, ICON_RANGER, XGR_HIDDEN_FON
		);
		break;
	case UVS_VANGER_SHAPE::GAMER:
		XGR_PutSpr(
			xx - ICON_SX / 2, yy - ICON_SY / 2, ICON_SX, ICON_SY, ICON_GAMER, XGR_HIDDEN_FON
		);
		break;
	case UVS_VANGER_SHAPE::THIEF:
		XGR_PutSpr(
			xx - ICON_SX / 2, yy - ICON_SY / 2, ICON_SX, ICON_SY, ICON_THIEF, XGR_HIDDEN_FON
		);
		break;
	case UVS_VANGER_SHAPE::CARAVAN:
		XGR_PutSpr(
			xx - ICON_SX / 2, yy - ICON_SY / 2, ICON_SX, ICON_SY, ICON_CARAVAN, XGR_HIDDEN_FON
		);
		break;
	case UVS_VANGER_SHAPE::KILLER:
		XGR_PutSpr(
			xx - ICON_SX / 2, yy - ICON_SY / 2, ICON_SX, ICON_SY, ICON_KILLER, XGR_HIDDEN_FON
		);
		break;
	case UVS_VANGER_SHAPE::GAMER_SLAVE:
		XGR_PutSpr(
			xx - ICON_SX / 2, yy - ICON_SY / 2, ICON_SX, ICON_SY, ICON_GSLAVE, XGR_HIDDEN_FON
		);
		break;
	}
}
...
void ScreenQuant(void) {
...
#ifndef TINY_MODE
	buf.init();
	buf < "Total: " <= c_All_Number < " S: " <= c_S_Number < " R: " <= c_R_Number < " A: " <=
		c_A_Number;
	...
	while (p) {
		...
		case UVS_OBJECT::DOLLY:
			...
			if (ind != -1)
				scrTable[ind]->DollyQuant((uvsDolly *)p);
			break;
		case UVS_OBJECT::VANGER:
			ind = ((uvsVanger *)p)->Pworld->gIndex;
			if (ind != -1)
				scrTable[ind]->VangerQuant((uvsVanger *)p);
			break;
		}
		p = p->enext;
	}
