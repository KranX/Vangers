...
struct DG_EXTERNS {
	enum {
		ELR_RESULT, // если получил элика то возвращается номер которым ты приехал среди остальных
					// участников элирекции (наличие элика здесь не учитывать!!!), если не
					// участвовал то 0
		...
		INV_BAD_CIRT, // коды начинающиеся с INV_ возвращают количество предметов в инвентори на
					  // момент попадания в эскейв, то есть отнимание придметов неучитывается!
		INV_GOOD_CIRT,
		INV_NYMBOS,
		INV_PHLEGMA,
		INV_HEROIN,
...
struct dgFile {
	...
	dgFile(char *fname, int _len = 0, bool verbose = false) {
		load(fname, _len, verbose);
	}
	dgFile(void) {
		buf = NULL;
		len = index = -1;
...
struct dgQuery {
	...
	char *findQLfirst(int ind) {
		return answers[qlInd = ind]->findQLfirst();
	}
	char *findQLnext(void) {
		return answers[qlInd]->findQLnext();
	}
	int getIndex(void);
};
...
struct dgMolecule {
	...
	dgAtom *getVariantsAtom(void) {
		if (curA)
			return curA->variants ? curA : NULL;
		return NULL;
	}
	void reset(void) {
		curA = tail;
...
struct dgRoom {
	...
	inline dgCell *getGRID(int x, int y) {
		if (x < 0 || x >= gridSX || y < 0 || y >= gridSY)
			return NULL;
		return *(grid + y * gridSX + x);
	}
	inline int getSTATUS(int x, int y) {
		if (x < 0 || x >= gridSX || y < 0 || y >= gridSY)
			return DG_CELLSTATUS::EMPTY;
		return *(status + y * gridSX + x);
	}
	inline void setGRID(int x, int y, dgCell *pc) {
		if (x < 0 || x >= gridSX || y < 0 || y >= gridSY)
			return;
		*(grid + y * gridSX + x) = pc;
	}
	inline void setSTATUS(int x, int y, uchar val) {
		if (x < 0 || x >= gridSX || y < 0 || y >= gridSY)
			return;
		*(status + y * gridSX + x) = val;
	}

	void nextState(int x, int y);
	void explodeState(int x, int y);
	void backup(void) {
		memcpy(backup_status, status, gridSX * gridSY);
	}
	void restore(void) {
		memcpy(status, backup_status, gridSX * gridSY);
	}

	dgRoom(DiagenDispatcher *_owner) {
		roomName = NULL;
		...
		gridSX = gridSY = 0;
		startX = startY = 0;
		status = backup_status = NULL;
		cIndex = -1;
		currentM = NULL;
		visitCounter = comingCounter = 0;
