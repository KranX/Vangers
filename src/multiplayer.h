...
const int ITEM_TRANSFER =
	0x96; // TransferType(1), OldID(4), OldTime(4), DeleteBody(1), NewCreateObjectBody
...
const int TOTAL_LIST_OF_PLAYERS_DATA =
	0xCC; // Number(1),{ client_ID(1), BodySize(2) , Body, Name(str) }

const int ATTACH_TO_GAME_RESPONSE = 0xC3;	  // Game(4), Configured(1) = 1/0, GameBirthTime(4),
											  // Client_ID(1), object_ID_offsets[16](short)
...
// кароче эти айди используются (или по крайней мере должны) только в трех... уже двух,
// вышеперечисленных евентах а поэтому все на йух
...
// Unique Mechouses
enum { LastMoggy, QueenFrog, Sandoll, LawnMower, Wormaster };
...
struct zServerData {
	...
	void clear() {
		memset(this, 0, sizeof(zServerData));
	}
	zServerData() {
		clear();
	}
...
struct PlayerBody {
	...
	void clear() {
		memset(this, 0, sizeof(PlayerBody));
		CarIndex = 255;
	}
	PlayerBody() {
		clear();
