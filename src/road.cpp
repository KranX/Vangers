...
#ifdef _DEMO_
	std::cout << "\"" << XGraphWndID
			  << ": One For The Road\" Cover Demo by K-D Lab (SDL Version)\n";
#else
	if (lang() == GERMAN) {
...
void LoadingRTO1::Init(int id) {
	...
	if (!MuteLog && EffectInUsePriory) {
		if (!iGetOptionValue(iSOUND_ON)) {
			SetSoundVolume(
				iGetOptionValue(iSOUND_VOLUME_CUR) * 256 / iGetOptionValue(iSOUND_VOLUME_MAX)
			);
		} else {
			MuteLog = 1;
...
void PalettePrepare(void) {
	...
	ffp.read(
		palbufC + 3 * (ENDCOLOR[TERRAIN_MAX - 1] + 1), 768 - 3 - 3 * (ENDCOLOR[TERRAIN_MAX - 1])
	);
	ffp.close();
	...
	for (i = 0; i < 16; i++)
		palbuf[3 * (224 + i) + 0] = palbuf[3 * (224 + i) + 1] = palbuf[3 * (224 + i) + 2] =
			i * 4; //(uchar)(10 + (pow((double)i/16.,.70)*UNIT)*54/UNIT);
...
#ifdef _DEMO_
					if (argv[i][j + 2] == 'e' && argv[i][j + 3] == 'e' && argv[i][j + 4] == 'b' &&
						argv[i][j + 5] == 'o' && argv[i][j + 6] == 's' && argv[i][j + 7] == '$')
						beebos = 100000;
#else
					if (argv[i][j + 2] == 'E' && argv[i][j + 3] == 'E' && argv[i][j + 4] == 'b' &&
						argv[i][j + 5] == 'o' && argv[i][j + 6] == 's' && argv[i][j + 7] == '!')
...
// void creat_poster_pixels_copy(SDL_Surface *src, SDL_Rect *srcrect, SDL_Surface *dst, SDL_Rect
// *dstrect) { 	int x,y, src_x, src_y, dst_x, dst_y; 	for (y = 0;y < srcrect->h; y++) { 		for
// (x = 0;x <
// srcrect->w; x++) { 			dst_y = dstrect->y+y; 			dst_x = dstrect->x+x; src_y =
// srcrect->y+y; 			src_x = srcrect->x+x; 			if
// (dst_y<dst->h&&dst_x<dst->w&&src_y<src->h&&src_x<src->w) {
...
//			BackD.restore();
//			MLquant();*/
//			std::cout<<"TurnSecX:"<<TurnSecX<<" ViewX:"<<ViewX<<" ViewY:"<<ViewY<<"
// curGMap->xc:"<<curGMap->xc<<" curGMap->yc:"<<curGMap->yc
//			<<" curGMap->xside:"<<curGMap->xside<<" curGMap->yside:"<<curGMap->yside<<std::endl;
//			vMap ->
// scaling(TurnSecX,ViewX,ViewY,curGMap->xc,curGMap->yc,curGMap->xside,curGMap->yside);
//
...
void calc_view_factors() {
	...
	Light =
		Vector(DBV(ModulatedNORMAL, 0, ModulatedNORMAL * SHADOWDEEP / 256.).norm(ModulatedNORMAL));
	...
	if (GeneralSystemSkip && !ChangeWorldSkipQuant) {
		...
		if (DepthShow) {
			if (SkipShow) {
				// Наклон изображения
				vMap->SlopTurnSkip(
					TurnAngle, SlopeAngle, ViewZ, focus, ViewX, ViewY, xc, yc, xsize / 2, ysize / 2
				);
			} else {
				vMap->scaling_3D(
					A_g2s, ViewZ, focus, ViewX, ViewY, xc, yc, xside, yside, TurnAngle
				);
			}
		} else {
			if (TurnAngle) {
		...
		/*
			if(recorder_mode){
				if(blink){
					c_rectangle(xc + xside - 80 - 2,yc - yside + 60 - 2,8 + 6,16 + 4,224 + 15,224 +
		   15,OUTLINED); sysfont.draw(xc + xside - 80,yc - yside + 60,(unsigned char*)(recorder_mode
		   == RECORDER_RECORD ? "W" : "R"),224 + 15,-1);
					}
				if(CLOCK() - clcnt > 10){
					blink = 1 - blink;
					clcnt = CLOCK();
		...
		*/
		...
		if (!FirstDraw) {
			...
			if (message_dispatcher.ListSize > 0 && message_dispatcher.new_messages) {
				...
				while (msg && zCount < zCHAT_ROWLIMIT &&
					   msg->time + zCHAT_TIMELIMIT > SDL_GetTicks()) {
					zCount++;
					switch (msg->color) {
					case 0:
						zColor = zCOLOR_GREEN;
					...
					case 7:
						...
						break;
					case 8:
						zColor = zCOLOR_CAMOUFLAGE;
						break;
					case 9:
						zColor = zCOLOR_PATROL;
						break;
					default:
						zColor = zCOLOR_WHITE;
					}
					...
					if (!iChatMUTE) {
						zchatfont.draw(
							xc - xside + 80,
							yc - yside + 20 + (zCHAT_ROWLIMIT * zCHAT_ROWHEIGHT) -
								(zCount * zCHAT_ROWHEIGHT),
					...
					if (msg == message_dispatcher.first())
						break;
			...
			if (prmFlag & PRM_FPS) {
				sysfont.draw(
					xc + xside - 150, yc - yside + 80, (unsigned char *)fps_string, 224 + 15, -1
				);
				status.init();
				...
				sysfont.draw(
					xc + xside - 150,
					yc - yside + 96,
					(unsigned char *)status.GetBuf(),
					224 + 15,
					-1
				);
				if (NetworkON)
					sysfont.drawtext(
						xc - xside + 3, yc - yside + 32, network_analysis_buffer.address(), 255, -1
					);
#ifdef SICHER_DEBUG
				status.init();
			...
			case 1:
				status.init();
				if (ActD.Active) {
					status <= ActD.Active->R_curr.x < "," <= ActD.Active->R_curr.y < "," <=
						ActD.Active->R_curr.z < ";" <= ActD.Active->radius;
					sysfont.draw(
						xc - xside + 3,
						yc - yside + 3,
						(unsigned char *)status.GetBuf(),
					...
					sysfont.draw(
						xc + xside - 80, yc - yside + 3, (unsigned char *)fps_string, 224 + 15, -1
					);
					status.init();
					status <= (ActD.UnitStorage[ACTION_VANGER].Max -
								  ActD.UnitStorage[ACTION_VANGER].ObjectPointer);
					sysfont.draw(
						UcutRight - 200,
						VcutDown - 70,
						(unsigned char *)status.GetBuf(),
...
void set_map_to_fullscreen() {
	curGMap->change(XGR_MAXX / 2, XGR_MAXY / 2, 0, XGR_MAXX / 2, XGR_MAXY / 2);
	Redraw = 1;
...
void set_map_to_ibs(ibsObject *ibs) {
	if (XGR_Obj.get_screen_scale_x() == 1) {
		curGMap->change(ibs->SideX, ibs->SideY, 0, ibs->CenterX, ibs->CenterY);
		Redraw = 1;
...
void *operator new(unsigned int sz) {
	...
	if (memStart) {
		...
		if (++memIndex == MEMPTR_MAX)
			ErrH.Abort("memStat: overflow");
		memQ += sz;
...
void operator delete(void *p) {
	...
	if (memStart) {
		...
		if (i < memIndex) {
			//			fmem < "delete:\t\t" <= memSize[i] < "\t\ttotal:\t\t" <= memQ < "\t\t" <=
			// memDcnt < "\r\n";
			memDcnt++;
			memPtr[i] = NULL;
			memSize[i] = 0;
...
void LoadingMessage(int flush) {
	...
	if (lang() == RUSSIAN) {
		...
		if (XGR_MAXX == 800)
			buf < "(C)1997-2014. Alle Rechte vorbehalten. Release Version 1.3. Gleich geht's "
				  "weiter...";
		else
...
#	ifdef BETA_TESTING
	...
	if (!str)
		ErrH.Abort("Software is NOT properly registered. Please, address to developers!");
	return str;
#	else
