...
struct iMultiGameParameter {
	...
	void add_obj(iListElement *p, int id = 0) {
		objData[id] = p;
	}
...
iScreen *create_screen(void) {
	...
	iBitmapElement *bt = (iBitmapElement *)iCreateScreenElement(
		I_BITMAP_ELEM
	);								  // ’ЁЇл н«Ґ¬Ґ­в®ў - enum iElementTypes ў ISCREEN.H
	...
	iScrollerElement *scl = (iScrollerElement *)iCreateScreenElement(
		I_SCROLLER_ELEM, "Test scroller"
	); // ‡¤Ґбм Ё¬  н«Ґ¬Ґ­в  § ¤ Ґвбп, в.Є. ­  ­ҐЈ®
...
void iPrepareOptions(void) {
	...
	iScrOpt[iSOUND_VOLUME_CUR] =
		new iScreenOption(iSCROLLER, 0, "Sound screen", "SndVolumeScroller");
	iScrOpt[iSOUND_VOLUME_MAX] =
		new iScreenOption(iSCROLLER, 1, "Sound screen", "SndVolumeScroller");
	...
	iScrOpt[iMUSIC_VOLUME_CUR] =
		new iScreenOption(iSCROLLER, 0, "Sound screen", "MusVolumeScroller");
	iScrOpt[iMUSIC_VOLUME_MAX] =
		new iScreenOption(iSCROLLER, 1, "Sound screen", "MusVolumeScroller");
	...
	iScrOpt[iPLAYER_NAME_CR] =
		new iScreenOption(iSTRING, 0, "Identification screen2", "PlayerName");
	...
	iScrOpt[iPLAYER_PASSWORD] =
		new iScreenOption(iSTRING, 0, "Identification screen", "PasswordString");
	iScrOpt[iPLAYER_PASSWORD_CR] =
		new iScreenOption(iSTRING, 0, "Identification screen2", "PasswordString");
	...
	iScrOpt[iAUTO_ACCELERATION] =
		new iScreenOption(iTRIGGER, 0, "Controls screen", "AutoAccelerationTrig");
		...
		for (i = 1; i < iKEY_MAX_ID; i++) {
			for (j = 0; j < iKEY_OBJECT_SIZE; j++) {
				if (!iControlsStr[i * iKEY_OBJECT_SIZE + j]) {
					...
					if (el)
						iControlsStr[i * iKEY_OBJECT_SIZE + j] = el;
				}
			}
		}
	}
...
void iInitMultiGames(void) {
	...
	for (i = 0; i < iMP_MAX_PLACE; i++) {
		...
		iPlaces[i]->init(
			"MultiGame Results screen", XPlace.address(), XStr.address(), XNum.address()
		);
	}
	for (i = 0; i < iMP_MAX_RESULT; i++) {
		...
		iResults[i]->init(
			"MultiGame Results screen 2", XPlace.address(), XStr.address(), XNum.address()
		);
	}
		...
		while (flag) {
			...
			for (i = 0; i < iNumPlayers - 1; i++) {
				if (iCurMultiGame == iMP_VAN_WAR || iCurMultiGame == iMP_HUNTAGE) {
					if (iPlayers[i]->body.kills - iPlayers[i]->body.deaths <
						iPlayers[i + 1]->body.kills - iPlayers[i + 1]->body.deaths) {
						p = iPlayers[i];
				...
				if (iCurMultiGame == iMP_MECHOSOMA) {
					if (iPlayers[i]->body.MechosomaStat.ItemCount1 +
							iPlayers[i]->body.MechosomaStat.ItemCount2 <
						iPlayers[i + 1]->body.MechosomaStat.ItemCount1 +
							iPlayers[i + 1]->body.MechosomaStat.ItemCount2) {
						p = iPlayers[i];
						iPlayers[i] = iPlayers[i + 1];
						iPlayers[i + 1] = p;
						flag = 1;
				...
				if (iCurMultiGame == iMP_PASSEMBLOSS) {
					if (iPlayers[i]->body.PassemblossStat.CheckpointLighting <
						iPlayers[i + 1]->body.PassemblossStat.CheckpointLighting) {
						p = iPlayers[i];
						iPlayers[i] = iPlayers[i + 1];
						iPlayers[i + 1] = p;
						flag = 1;
				...
				if (iCurMultiGame == iMP_MUSTODONT) {
					if (!iPlayers[i]->body.MustodontStat.MakeTime &&
						iPlayers[i + 1]->body.MustodontStat.MakeTime) {
						p = iPlayers[i];
						iPlayers[i] = iPlayers[i + 1];
						iPlayers[i + 1] = p;
						flag = 1;
					...
					if (iPlayers[i]->body.MustodontStat.MakeTime &&
						iPlayers[i + 1]->body.MustodontStat.MakeTime &&
						iPlayers[i]->body.MustodontStat.MakeTime >
							iPlayers[i + 1]->body.MustodontStat.MakeTime) {
						p = iPlayers[i];
						iPlayers[i] = iPlayers[i + 1];
						iPlayers[i + 1] = p;
...
void iSortPlayers(int mode)
{
	...
	while(flag){
		...
		for(i = 0; i < iNumPlayers - 1; i ++){
			if(iCurMultiGame == iMP_VAN_WAR || iCurMultiGame == iMP_HUNTAGE){
				if(iPlayers[i] -> body.kills - iPlayers[i] -> body.deaths < iPlayers[i + 1] ->
body.kills - iPlayers[i + 1] -> body.deaths){ p = iPlayers[i]; iPlayers[i] = iPlayers[i + 1];
					iPlayers[i + 1] = p;
					flag = 1;
				}
			}
			if(iCurMultiGame == iMP_MECHOSOMA){
				if(iPlayers[i] -> body.MechosomaStat.ItemCount1 + iPlayers[i] ->
body.MechosomaStat.ItemCount2 < iPlayers[i + 1] -> body.MechosomaStat.ItemCount1 + iPlayers[i + 1]
-> body.MechosomaStat.ItemCount2){ p = iPlayers[i]; iPlayers[i] = iPlayers[i + 1]; iPlayers[i + 1] =
p; flag = 1;
				}
			}
			if(iCurMultiGame == iMP_PASSEMBLOSS){
				if(!mode){
					if(!iPlayers[i] -> body.PassemblossStat.TotalTime && iPlayers[i + 1] ->
body.PassemblossStat.TotalTime){ p = iPlayers[i]; iPlayers[i] = iPlayers[i + 1]; iPlayers[i + 1] =
p; flag = 1;
					}
					if(iPlayers[i] -> body.PassemblossStat.TotalTime && iPlayers[i + 1] ->
body.PassemblossStat.TotalTime && iPlayers[i] -> body.PassemblossStat.TotalTime > iPlayers[i + 1] ->
body.PassemblossStat.TotalTime){ p = iPlayers[i]; iPlayers[i] = iPlayers[i + 1]; iPlayers[i + 1] =
p; flag = 1;
					}
				}
				else {
					if(iPlayers[i] -> body.PassemblossStat.CheckpointLighting < iPlayers[i + 1] ->
body.PassemblossStat.CheckpointLighting){ p = iPlayers[i]; iPlayers[i] = iPlayers[i + 1]; iPlayers[i
+ 1] = p; flag = 1;
					}
				}
			}
			if(iCurMultiGame == iMP_MUSTODONT){
				if(!iPlayers[i] -> body.MustodontStat.MakeTime && iPlayers[i + 1] ->
body.MustodontStat.MakeTime){ p = iPlayers[i]; iPlayers[i] = iPlayers[i + 1]; iPlayers[i + 1] = p;
					flag = 1;
				}
				if(iPlayers[i] -> body.MustodontStat.MakeTime && iPlayers[i + 1] ->
body.MustodontStat.MakeTime && iPlayers[i] -> body.MustodontStat.MakeTime > iPlayers[i + 1] ->
body.MustodontStat.MakeTime){ p = iPlayers[i]; iPlayers[i] = iPlayers[i + 1]; iPlayers[i + 1] = p;
					flag = 1;
				}
			}
...
void iPrepareHallOfFame(void) {
	...
	num = top_list.size();
	if (num > iMP_MAX_HALL_RESULT)
		num = iMP_MAX_HALL_RESULT;
	p = top_list.first();
