...
void iScreenObject::copy(iScreenObject *dest) {
	...
	if (flags & OBJ_TRIGGER) {
		...
		for (i = 0; i < p->num_state; i++) {
			dest_el = iCreateScreenElement(
				((iScreenElement *)p->StateElem[i])->type,
				((iScreenElement *)p->StateElem[i])->ID_ptr.c_str(),
				(iScreenElement *)p->StateElem[i]
			);
...
void iScreenElement::copy(iScreenElement *dest) {
	...
	case I_AVI_ELEM:
		...
		if (((iAVIElement *)dest)->border_type == AVI_SHAPED_BORDER) {
			...
			strcpy(
				((iAVIElement *)dest)->border_shape_file, ((iAVIElement *)this)->border_shape_file
			);
		}
	...
	case I_AVI_BORDER_ELEM:
		...
		((iAVIBorderElement *)dest)->border_null_level =
			((iAVIBorderElement *)this)->border_null_level;
		...
		if (((iAVIBorderElement *)dest)->border_type == AVI_SHAPED_BORDER) {
			...
			strcpy(
				((iAVIBorderElement *)dest)->border_shape_file,
				((iAVIBorderElement *)this)->border_shape_file
			);
		}
		break;
	}
...
void iAVIElement::load(void) {
	if (!(flags & EL_DATA_LOADED)) {
		...
		*iResBuf < iVideoPathDefault < avi_name;
		...
		if (!AVIopen(
				iResBuf->address(), AVI_NOTIMER | AVI_NODRAW | AVI_LOOPING | AVI_NOPALETTE, 0, &data
			)) {
			iResBuf->init();
			*iResBuf < iVideoPath < avi_name;
			if (!AVIopen(
					iResBuf->address(),
					AVI_NOTIMER | AVI_NODRAW | AVI_LOOPING | AVI_NOPALETTE,
					0,
					&data
				)) {
				iResBuf->init();
...
void iScreenElement::redraw(int x, int y, int sc, int sm, int hide_mode) {
	int i, v = 0, maxv = 0, bsx = 0, bsy = 0, bs = 0, sp = 0, pv = 0, offs, lev = 0,
		   col = iS_STR_COL_START, col_sz = iS_STR_COL_SIZE, dx1, dx2, dy1, dy2;
	...
	case I_S_STRING_ELEM:
		...
		if (((iScreenObject *)owner)->flags & OBJ_SELECTABLE &&
			!(((iScreenObject *)owner)->flags & OBJ_SELECTED)) {
	...
	if (!sm) {
		...
		case I_STRING_ELEM:
			if (terrainNum == -1)
				iPutStr(
					x + lX,
					y + lY,
					((iStringElement *)this)->font,
					data,
					0,
					sc,
					((iStringElement *)this)->space,
					null_lev,
					hide_mode
				);
			else
		...
		case I_S_STRING_ELEM:
			...
			dx2 = (x + lX + SizeX < I_MAP_RES_X - 1 - S_STRING_DELTA)
					  ? S_STRING_DELTA
					  : ((I_MAP_RES_X - 1) - (x + lX + SizeX));
			dy1 = (y + lY < S_STRING_DELTA) ? (y + lY) : S_STRING_DELTA;
			dy2 = (y + lY + SizeY < I_RES_Y - 1 - S_STRING_DELTA)
					  ? S_STRING_DELTA
					  : ((I_RES_Y - 1) - (y + lY + SizeY));
			bsx = SizeX + dx1 + dx2;
			bsy = SizeY + dy1 + dy2;
			...
			//				  iPutS_Str(dx1,dy1,((iS_StringElement*)this) ->
			// font,col,data,bsx,bsy,p,((iStringElement*)this) -> space);
			//				  s_smooth_buf(bsx,bsy,p,0,iS_STR_SEG_START_COL);

			aPutStr32(
				dx1,
				dy1,
				((iS_StringElement *)this)->font,
				col,
				col_sz,
				data,
				bsx,
		...
		case I_BITMAP_ELEM:
			...
			if (!(flags & EL_PICTURE)) {
				...
				if (terrainNum == -1)
					...
					put_tbuf(
						x + lX, y + lY, SizeX, SizeY, p, data, null_lev, hide_mode, terrainNum
					);
			} else {
				lev = (null_lev * abs(sc)) >> 8;
				if (lev) {
		...
		case I_SCROLLER_ELEM:
			if (((iScrollerElement *)this)->dir == I_RIGHT) {
				...
				for (i = 0; i < pv; i++) {
					scale_buf(
						bsx, bsy, sc, (unsigned char *)(((iScrollerElement *)this)->fdata), p
					);
					if (terrainNum == -1)
				...
				for (i = pv; i < v; i++) {
					scale_buf(
						bsx,
						bsy,
						((iScrollerElement *)this)->scale,
						(unsigned char *)(((iScrollerElement *)this)->fdata),
						p
					);
					if (terrainNum == -1)
						put_buf(x + lX + offs, y + lY, bsx, bsy, p, data, null_lev, hide_mode);
			...
			if (sc) {
				...
				if (sc > 0)
					dscale_buf(
						...
						lX,
						0,
						((iTerrainElement *)this)->def_null_level
					);
				else
					dscale_buf(
						bsx,
...
void iScreenObject::init(void) {
	...
	while (p) {
		...
		if (!p->SizeX) {
			...
			if (p->type == I_STRING_ELEM)
				p->SizeX = iStrLen(
					(unsigned char *)((iStringElement *)p)->string,
					((iStringElement *)p)->font,
					((iStringElement *)p)->space
				);
			else {
				if (p->type == I_SCROLLER_ELEM) {
					...
					case I_RIGHT:
						p->SizeX =
							(((iScrollerElement *)p)->space + ((iScrollerElement *)p)->bSizeX) *
							(((iScrollerElement *)p)->maxValue + 2);
						break;
					case I_UP:
					case I_DOWN:
					...
					if (p->type == I_S_STRING_ELEM) {
						p->SizeX = aStrLen32(
							((iS_StringElement *)p)->string,
							((iS_StringElement *)p)->font,
							((iS_StringElement *)p)->space
						);
						//						  p -> SizeX = iS_StrLen((unsigned
						// char*)((iS_StringElement*)p) -> string,((iS_StringElement*)p) ->
						// font,((iS_StringElement*)p) -> space);
					}
				}
			}
		}
		if (!p->SizeY) {
			if (p->type == I_STRING_ELEM)
				...
				if (p->type == I_SCROLLER_ELEM) {
					...
					case I_DOWN:
						p->SizeY =
							(((iScrollerElement *)p)->space + ((iScrollerElement *)p)->bSizeY) *
							(((iScrollerElement *)p)->maxValue + 2);
						break;
					}
				} else {
					...
					//						  p -> SizeY = iScrFontTable[((iS_StringElement*)p) ->
					// font] -> SizeY;
				}
			}
		}
		if (p->type == I_AVI_ELEM) {
			...
			*iResBuf < iVideoPathDefault < ((iAVIElement *)p)->avi_name;
			...
			if (!p->SizeX)
				p->SizeX = AVIwidth(((iAVIElement *)p)->data);
			if (!p->SizeY)
				p->SizeY = AVIheight(((iAVIElement *)p)->data);
	...
	if (flags & OBJ_AUTO_SIZE && !(flags & OBJ_SET_COORDS) && !(flags & OBJ_AVI_PRESENT) &&
		!(flags & OBJ_PIC_PRESENT)) {
...
void iScreenObject::redraw(int mode) {
	...
	if (flags & OBJ_REINIT) {
		...
		if (FlushX > PosX)
			FlushX = PosX;
		if (FlushX + FlushSX < PosX + SizeX)
			FlushSX = PosX + SizeX - FlushX;
		if (FlushY > PosY)
			FlushY = PosY;
		if (FlushY + FlushSY < PosY + SizeY)
			FlushSY = PosY + SizeY - FlushY;
		getfon();
	}

	p = (iScreenElement *)ElementList->last;
	while (p) {
		if (!(p->flags & EL_NO_REDRAW) && p->type != I_S_STRING_ELEM && p->type != I_AVI_ELEM &&
			!(p->flags & EL_PICTURE))
	...
	p = (iScreenElement *)ElementList->last;
	while (p) {
		if (!(p->flags & EL_NO_REDRAW) &&
			(p->flags & EL_PICTURE || p->type == I_S_STRING_ELEM || p->type == I_AVI_ELEM))
			p->redraw(PosX, PosY, curHeightScale, SmoothLevel, hide_mode);
		p = (iScreenElement *)p->prev;
	}
#ifdef _DEBUG
					...
					if (bool(obj->flags & OBJ_LOCKED) == bool(p->flags & EV_IF_LOCKED)) {
						...
						while (cd) {
							if (((iScanCode *)cd)->code == sc &&
								(!(p->flags & EV_IF_SELECTED) || obj->flags & OBJ_SELECTED))
								HandleEvent(p);
							cd = cd->prev;
						}
					}
...
void iScreen::prepare(void) {
	...
	if (actIntLog)
		XGR_MouseObj.PromptColor =
			aciCurColorScheme[FM_SELECT_START] | (aciCurColorScheme[FM_SELECT_SIZE] << 16);
}
				...
				if (!mouse_sc || obj->CheckXY(x, y)) {
					...
					while (p) {
						...
						while (cd) {
							if (((iScanCode *)cd)->code == sc &&
								(!(p->flags & EV_IF_SELECTED) || obj->flags & OBJ_SELECTED) &&
								(!(p->flags & EV_IF_NOT_SELECTED) || !(obj->flags & OBJ_SELECTED)))
								HandleEvent(p);
							cd = cd->prev;
						}
						p = (iScreenEvent *)p->prev;
		...
		if (!(p->flags & EV_MULTI_LINE) || !cm->LineID || cm->LineID == iEvLineID) {
			...
			if (cm->start_time == p->cur_time) {
				...
				case EV_RESET_TEXTS:
					...
					break;
				case EV_NEXT_TEXT:
					if (copy_text_next((iScreen *)cm->obj, cm->value))
						return;
					break;
				case EV_PREV_TEXT:
					if (copy_text_prev((iScreen *)cm->obj, cm->value))
						return;
					break;
				case EV_STOP_AVI:
				...
				case EV_RISE_SCREEN:
					...
					while (obj) {
						...
						((iTerrainElement *)obj->ElementList->last)->level_delta =
							(sc - ((iTerrainElement *)obj->ElementList->last)->null_level) /
							cm->time;
				...
				case EV_HIDE_SCREEN:
					...
					while (obj) {
						...
						((iTerrainElement *)obj->ElementList->last)->level_delta =
							-((iTerrainElement *)obj->ElementList->last)->null_level / cm->time;
			...
			if (cm->start_time <= p->cur_time && cm->time + cm->start_time > p->cur_time) {
				...
				case EV_RISE_SCREEN:
					...
					while (obj) {
						obj->ScaleQuant();
						if (cm->time + cm->start_time == p->cur_time + 1) {
							((iTerrainElement *)obj->ElementList->last)->null_level =
								((iTerrainElement *)obj->ElementList->last)->def_null_level;
							obj->flags &= ~OBJ_HIDE;
						}
						obj = (iScreenObject *)obj->prev;
					}
...
void iBitmapElement::set_pal(void) {
	...
	if (!(flags & EL_PICTURE) || !strlen(palname))
		return;
		...
		if (!(ActiveEl->flags & EL_KEY_NAME)) {
			if (event->type == SDL_KEYDOWN) {
				...
				if (!(ActiveEl->flags & EL_NUMBER)) {
					...
					if ((unsigned char)event->text.text[0] >= '0' &&
						(unsigned char)event->text.text[0] <= '9') {
						code = event->text.text[0];
		...
		if (!curText) {
			...
			if (curText->curLine <= obj->ElementList->Size) {
				...
				if ((lang() != RUSSIAN && id > 0) || id > iTEXT_RUS1_ID - 1) {
					...
					curText->curLine = curText->numLines -
									   (curText->numLines % obj->ElementList->Size) +
									   obj->ElementList->Size * 2;
					key_trap(prev_code);
				}
				return 1;
			} else {
				if (curText->curLine >= obj->ElementList->Size * 2) {
					if (curText->curLine != curText->numLines)
						...
						curText->curLine -=
							obj->ElementList->Size + (curText->numLines % obj->ElementList->Size);
				} else {
					ActiveEv->flags &= ~EV_ACTIVE;
					ActiveEv = NULL;
					return 1;
