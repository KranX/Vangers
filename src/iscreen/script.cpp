...
void ScriptFile::load(const char *fname) {
	...
	while (process_includes()) {
		inc_count++;
		if (inc_count > iSCRIPT_INCLUDE_MAX)
			ErrH.Abort("process_includes() failed...");
	}
...
void ScriptFile::prepare(void) {
	...
	while (!script->EOF_Flag) {
		if (curBlock && curBlock->data && *curBlock->data == '$') {
			...
			flag = 0;
			while (def) {
				if (curLine->ID >= def->StartLine && curLine->ID < def->EndLine &&
					!strcmp(curBlock->data + 1, def->line->first->next->data)) {
					flag = 1;
					curBlock->clear();
					if (def->line->Size > 2)
			...
			if (!flag)
				handle_error("Bad define", curBlock->data + 1);
		}
		next_ptr();
	}
	reset();
...
int ScriptFile::check_math(void) {
	...
	for (i = 0; i < MT_MAX_CODE; i++) {
		if (*curBlock->data == *scMathOpNames[i] && !strcmp(curBlock->data, scMathOpNames[i]))
			return i + 1;
	}
#endif
	return 0;
...
#		ifdef _SAVE_PACKED_SCRIPT_
	...
	if (!out_sz)
		ErrH.Abort("Bad compressed script size...");
...
int scMathExpression::check_math(char *p) {
	int i;
	for (i = 0; i < MT_MAX_CODE; i++) {
		if (*p == *scMathOpNames[i] && !strcmp(p, scMathOpNames[i]))
			return (i + 1);
	}
	return 0;
}
