...
void ivrtMap::load(const char *name) {
	...
	if (isCompressed) {
		...
		st_table = NULL;
		sz_table = NULL;
		//		if(fmap.size() != (int)map_size_x*(int)imap_size_y*2) ErrH.Abort("Incorrect
		// ivmp-file size");
	}
...
void iLINE_render(int y) {
	...
	for (unsigned int x = 0; x < map_size_x; x++, pa++, pc++, pf++) {
		if (*pf & DOUBLE_LEVEL) {
			...
			if (*pf & SHADOW_MASK)
				*(pc - 1) = *pc = ipalCLR[type]
										 [256 + ((ilightCLR[type][255 - (lxVal - rxVal)] -
													 ((255 - *pa) >> H_CORRECTION)) >>
													SHADOW_CORRECTION)];
			else
				*(pc - 1) = *pc = ipalCLR[type]
	...
	for (i = 0; i < SizeX; i += 2) {
		if (*pf & DOUBLE_LEVEL) {
			...
			if (*(grid + level)) {
				...
				*(pf - 1) |= SHADOW_MASK;
				*(pc - 1) = *pc = palCLRlast
					[256 + ((lightCLRlast[255 - (lxVal - rxVal)] - ((255 - h) >> H_CORRECTION)) >>
							   SHADOW_CORRECTION)];
			} else {
				*pf &= ~SHADOW_MASK;
				*(pf - 1) &= ~SHADOW_MASK;
		...
		// пост проход
		{
			...
			while (bNeedScan && MaxPossibleAlt >= 0) {
				...
				if (*pf & DOUBLE_LEVEL) {
					...
					MaxAlt -= 3;
					MaxPossibleAlt -= 3;
					if (dh < MaxAlt || (*pf & SHADOW_MASK))
						bNeedScan = 1;
					if (*(grid + level)) {
						*pf |= SHADOW_MASK;
						*(pf - 1) |= SHADOW_MASK;
						*(pc - 1) = *pc = palCLRlast
							[256 + ((lightCLRlast[255 - (lxVal - rxVal)] -
										((255 - h) >> H_CORRECTION)) >>
									   SHADOW_CORRECTION)];
					} else {
						*pf &= ~SHADOW_MASK;
			...
			if (CurrScanLenght > BackScanLen)
				BackScanLen = CurrScanLenght;
		}
	}
}
...
void ipal_iter0(void) {
	if (iPAL_WAVE_TERRAIN == -1 || iPAL_WAVE_TERRAIN >= TERRAIN_MAX)
		return;
	const int DSIZE = 8;
	const int PRD = 64;
...
void ipal_iter2(void) {
	...
	for (int ind = 0; ind < iPAL_MAX; ind++) {
		...
		//		XGR_SetPal(ipalbuf,iBEGCOLOR[iPAL_TERRAIN[ind]],iENDCOLOR[iPAL_TERRAIN[ind]] -
		// iBEGCOLOR[iPAL_TERRAIN[ind]] + 1);
	}
}
