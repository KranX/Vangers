...
static void iChatNormalizeInput(void) {
	...
	iChatInput->rightDrawPosition =
		std::min(std::max(iChatInput->rightDrawPosition, iChatInput->leftDrawPosition), length);
	...
	if (iChatInput->cursorPosition < iChatInput->leftDrawPosition) {
		...
		iChatInput->rightDrawPosition =
			iChatInput->getRightDrawPositionByLeft(iChatInput->leftDrawPosition);
...
static int aciChatColors0[10] = {(214 | (2 << 16)),
	(230 | (2 << 16)),
	(244 | (2 << 16)),
	(150 | (2 << 16)),
	(177 | (2 << 16)),
...
static int aciChatColors1[10] = {(244 | (2 << 16)),
	(134 | (2 << 16)),
	(150 | (2 << 16)),
	(176 | (2 << 16)),
	(196 | (2 << 16)),
...
static char *iChatWorlds[] = {iSTR_WORLD_NONE_CHAR,
...
static char *iChatColors[] = {iSTR_COLOR_GREEN_CHAR,
...
void iChatInputField::redraw(void) {
	...
	XConv->init();
	*XConv < CurPlayerName <
		(char *)((">" + string.substr(leftDrawPosition, rightDrawPosition - leftDrawPosition))
				.c_str());
...
void iChatInputField::selectionRedraw(void) {
	...
	int leftSelectionPosition = std::min(
		std::max(std::min(cursorPosition, selectionPosition), leftDrawPosition), rightDrawPosition
	);
	int rightSelectionPosition = std::min(
		std::max(std::max(cursorPosition, selectionPosition), leftDrawPosition), rightDrawPosition
	);

	XConv->init();
	*XConv < CurPlayerName <
		(char *)((">" + string.substr(leftDrawPosition, leftSelectionPosition - leftDrawPosition))
				.c_str());
	int left_x = aTextWidth32(XConv->address(), font, 1);

	XConv->init();
	*XConv < CurPlayerName <
		(char *)((">" + string.substr(leftDrawPosition, rightSelectionPosition - leftDrawPosition))
				.c_str());
...
int iChatInputField::getLeftDrawPositionByRight(int rightPosition) {
	...
	while (leftPosition >= 1) {
		XConv->init();
		*XConv < CurPlayerName <
			(char *)((">" + string.substr(leftPosition, rightPosition - leftPosition)).c_str());
...
void iChatHistoryScreen::redraw(void) {
	...
	for (int i = position; i < std::min((int)(data.size()), position + ICS_HISTORY_MAX_MESSAGES);
		i++) {
...
void iChatHistoryScreen::redrawScroll(void) {
	...
	XGR_Rectangle(
		PosX + SizeX,
		PosY + round(percent * SizeY),
		scrollSizeX,
		round(percentSizeY * SizeY),
		color,
		color,
		XGR_FILLED
	);
...
void iChatInit(void) {
	...
	iChatHistory->init(
		x,
		y + (ICS_BUTTON_SIZE_Y + ICS_DELTA) * 1,
		ICS_HISTORY_SIZE_X,
		ICS_HISTORY_SIZE_Y,
		bcol,
		bcol
	...
	iChatInput->init(
		x,
		y + ICS_HISTORY_SIZE_Y + ICS_DELTA + (ICS_BUTTON_SIZE_Y + ICS_DELTA) * 1,
		ICS_HISTORY_SIZE_X,
		ICS_BUTTON_SIZE_Y,
		bcol,
		bcol
	...
	p = new iChatButton(2);
	p->init(
		x + ICS_HISTORY_SIZE_X + ICS_DELTA,
		y + (ICS_BUTTON_SIZE_Y + ICS_DELTA) * 1,
		ICS_BUTTON_SIZE_X,
		ICS_BUTTON_SIZE_Y,
		bcol,
	...
	p->init(
		x + ICS_HISTORY_SIZE_X + ICS_DELTA,
		y + ICS_DELTA + (ICS_BUTTON_SIZE_Y + ICS_DELTA / 5) * 2,
		ICS_BUTTON_SIZE_X / 2 - 1,
		ICS_BUTTON_SIZE_Y,
		bcol,
		bcol
	);
	...
	p->init(
		x + ICS_HISTORY_SIZE_X + ICS_DELTA,
		y + ICS_DELTA + (ICS_BUTTON_SIZE_Y + ICS_DELTA / 5) * 3,
		ICS_BUTTON_SIZE_X / 2 - 1,
		ICS_BUTTON_SIZE_Y,
		bcol,
		bcol
	...
	p->init(
		x + ICS_HISTORY_SIZE_X + ICS_DELTA,
		y + ICS_DELTA + (ICS_BUTTON_SIZE_Y + ICS_DELTA / 5) * 4,
		ICS_BUTTON_SIZE_X / 2 - 1,
		ICS_BUTTON_SIZE_Y,
		bcol,
		bcol
	...
	p->init(
		x + ICS_HISTORY_SIZE_X + ICS_DELTA,
		y + ICS_DELTA + (ICS_BUTTON_SIZE_Y + ICS_DELTA / 5) * 5,
		ICS_BUTTON_SIZE_X / 2 - 1,
		ICS_BUTTON_SIZE_Y,
		bcol,
		bcol
	...
	p->init(
		x + ICS_HISTORY_SIZE_X + ICS_DELTA,
		y + ICS_DELTA + (ICS_BUTTON_SIZE_Y + ICS_DELTA / 5) * 6,
		ICS_BUTTON_SIZE_X / 2 - 1,
		ICS_BUTTON_SIZE_Y,
		bcol,
		bcol
	...
	p = new iChatButton(2);
	p->init(
		x + ICS_HISTORY_SIZE_X + ICS_DELTA + ICS_BUTTON_SIZE_X / 2 + 1,
		y + ICS_DELTA + (ICS_BUTTON_SIZE_Y + ICS_DELTA / 5) * 2,
		ICS_BUTTON_SIZE_X / 2 - 1,
		ICS_BUTTON_SIZE_Y,
		bcol,
	...
	p = new iChatButton(2);
	p->init(
		x + ICS_HISTORY_SIZE_X + ICS_DELTA + ICS_BUTTON_SIZE_X / 4,
		y + (ICS_BUTTON_SIZE_Y + ICS_DELTA) * 6,
		ICS_BUTTON_SIZE_X / 2,
		ICS_BUTTON_SIZE_Y,
		bcol,
		bcol
	...
	p->init(
		x + ICS_HISTORY_SIZE_X + ICS_DELTA,
		y + (ICS_BUTTON_SIZE_Y + ICS_DELTA) * 7,
		ICS_BUTTON_SIZE_X,
		ICS_BUTTON_SIZE_Y,
		bcol,
		bcol
	...
	p->init(
		x + ICS_HISTORY_SIZE_X + ICS_DELTA,
		y + (ICS_BUTTON_SIZE_Y + ICS_DELTA) * 8,
		ICS_BUTTON_SIZE_X,
		ICS_BUTTON_SIZE_Y,
		bcol,
		bcol
	...
	p->init(
		x + ICS_HISTORY_SIZE_X + ICS_DELTA,
		y + (ICS_BUTTON_SIZE_Y + ICS_DELTA) * 9,
		ICS_BUTTON_SIZE_X,
		ICS_BUTTON_SIZE_Y,
		bcol,
		bcol
	...
	p->init(
		x + ICS_HISTORY_SIZE_X + ICS_DELTA,
		y + (ICS_BUTTON_SIZE_Y + ICS_DELTA) * 10,
		ICS_BUTTON_SIZE_X,
		ICS_BUTTON_SIZE_Y,
		bcol,
		bcol
	...
	p->init(
		x + ICS_HISTORY_SIZE_X + ICS_DELTA,
		y + (ICS_BUTTON_SIZE_Y + ICS_DELTA) * 11,
		ICS_BUTTON_SIZE_X,
		ICS_BUTTON_SIZE_Y,
		bcol,
		bcol
	...
	p->init(
		x + ICS_HISTORY_SIZE_X + ICS_DELTA + ICS_BUTTON_SIZE_X / 4,
		y + (ICS_BUTTON_SIZE_Y + ICS_DELTA) * 12,
		ICS_BUTTON_SIZE_X / 2,
		ICS_BUTTON_SIZE_Y,
		bcol,
		bcol
...
void iChatInputDrawCursor(void) {
	...
	int cursorPosition = std::min(
		iChatInput->rightDrawPosition,
		std::max(iChatInput->leftDrawPosition, iChatInput->cursorPosition)
	);

	iChatInput->XConv->init();
	*iChatInput->XConv < CurPlayerName <
		(char *)((
			">" +
			(iChatInput->string)
				.substr(iChatInput->leftDrawPosition, cursorPosition - iChatInput->leftDrawPosition)
		)
				.c_str());
	int x = aTextWidth32(iChatInput->XConv->address(), iChatInput->font, 1);

	XGR_Rectangle(
		iChatInput->PosX + x - cursorWidth / 2,
		iChatInput->PosY,
		cursorWidth,
		iChatInput->SizeY,
		color,
...
void iChatInputChar(const char *input_char) {
	...
	if (hfnt && chr < hfnt->Size) {
		...
		int rightSelectionPosition =
			std::max(iChatInput->cursorPosition, iChatInput->selectionPosition);
		...
		iChatInput->rightDrawPosition = std::max(
			std::min(iChatInput->rightDrawPosition, (int)((iChatInput->string).length())),
			iChatInput->cursorPosition + 1
		);
		iChatInput->leftDrawPosition =
			iChatInput->getLeftDrawPositionByRight(iChatInput->rightDrawPosition);
...
void iChatInputFlush(void) {
	...
	if ((iChatInput->string).length() > 1) {
		...
		for (int rightPosition = 2; rightPosition <= (int)(string.length()); rightPosition++) {
			iChatInput->XConv->init();
			*iChatInput->XConv < CurPlayerName <
				(char *)((": " + string.substr(leftPosition, rightPosition - leftPosition))
						.c_str());

			if (aTextWidth32(iChatInput->XConv->address(), iChatInput->font, 1) >
				iChatHistory->SizeX - 10) {
				message_dispatcher.send(
					(char *)(string.substr(leftPosition, rightPosition - leftPosition - 1).c_str()),
					iChatFilter,
					iChatFilterID
				);
		...
		message_dispatcher.send(
			(char *)(string.substr(leftPosition).c_str()), iChatFilter, iChatFilterID
		);
	}
...
void iChatInputBack(void) {
	...
	rightPosition =
		std::min(std::max(rightPosition, leftPosition), (int)((iChatInput->string).length()));
	if (leftPosition >= rightPosition)
		return;
	...
	std::string new_string =
		(iChatInput->string).substr(0, leftPosition) + (iChatInput->string).substr(rightPosition);
	...
	if (iChatInput->cursorPosition <= iChatInput->leftDrawPosition) {
		iChatInput->leftDrawPosition =
			iChatInput->getLeftDrawPositionByRight(iChatInput->cursorPosition + 1);
		iChatInput->rightDrawPosition =
			iChatInput->getRightDrawPositionByLeft(iChatInput->leftDrawPosition);
	} else {
		iChatInput->rightDrawPosition =
...
void iInitChatScreen(void) {
	...
	if (iChatHistory->position + ICS_HISTORY_MAX_MESSAGES == (int)((iChatHistory->data).size())) {
		iChatHistory->position =
			std::max(0, message_dispatcher.ListSize - ICS_HISTORY_MAX_MESSAGES);
	}
...
void iInitChatButtons(void) {
	...
	for (i = 0; i < 5; i++) {
		...
		bt = iGetChatButton(ICS_PLAYER1_BUTTON + i);
		iResBuf->init();
		*iResBuf < p->name < " " < iChatWorlds[p->body.world + 1] < "/" <
			iChatColors[p->body.color];
