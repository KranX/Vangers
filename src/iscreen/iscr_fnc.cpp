		...
		if (iMultiFlag) {
			...
			if (avaible_servers.size() > 0) {
				...
				if (autoconnectJoinGame) {
					...
					int serverId = 0;
					while (serverPtr) {
						if (serverPtr->game_ID == autoconnectGameID)
							break; // game match or new game
						if (serverPtr->game_ID && autoconnectGameID < 0)
							break; // first existent game
					...
					if (serverPtr) {
						...
						if (iEvLineID == 1) {
							...
							if (iEvLineID == 3) {
								iScrDisp->curScr =
									(iScreen *)iScrDisp->get_object("Server Info screen");
							} else if (iEvLineID == 4) {
								iScrDisp->curScr =
									(iScreen *)iScrDisp->get_object("Server Config screen");
							}
						}
					}
				}
	...
	if (!iPause) {
		...
		if (!(iScrDisp->flags & SD_EXIT)) {
			...
			if (iScrDisp->flags & MS_LEFT_PRESS) {
				...
				if (iChatON)
					iChatMouseQuant(iMouseX, iMouseY, 1);
			}
		...
		if (iScrDisp->ret_val == ACI_LOAD_CODE)
			aciLoadLog = 1;
		if (!actIntLog) {
			iSaveControls();
			iSaveData();
...
void aciSwapMatrices(void) {
	...
	if (b)
		m_owner = (iScreenObject *)b->owner;
	if (b1)
		m2_owner = (iScreenObject *)b1->owner;
...
void iHandleExtEvent(int code, int data) {
	...
	case iEXT_UPDATE_MUSIC_VOLUME:
		...
		xsSetVolumeMusic(
			iGetOptionValue(iMUSIC_VOLUME_CUR) * 256 / iGetOptionValue(iMUSIC_VOLUME_MAX)
		);
		//			SoundVolumeCD = xsGetVolumeCD();
		SoundVolumeCD = xsGetVolumeMusic();
#endif
	...
	case iEXT_CHECK_SERVER_CONFIG:
		// zmod fixed
		if (is_current_server_configured(
				iGetOptionValueCHR(iSERVER_NAME), &my_server_data, &z_my_server_data
			)) {
...
iScreenObject *iGetScreenObj(int x, int y) {
	...
	if (iMovedObjCount < iMOVED_OBJ_COUNT) {
		...
		if (iMovedObj && iMovedObj->CheckXY(x, y))
			return iMovedObj;
		while (p) {
			if (p->CheckXY(x, y))
				return p;
...
void iShowObjCoords(iScreenObject *p) {
	...
	if (iMovedObj) {
		...
		*iResBuf <= x < "\n" <= y;
		...
		XGR_OutText(
			iMOVED_OBJ_X + (iMOVED_OBJ_SX - sx) / 2,
			iMOVED_OBJ_Y + (iMOVED_OBJ_SY - sy) / 2,
			255,
			iResBuf->address()
		);
	}
...
void iMoveMouseObj(int dx, int dy) {
	...
	iregRender(
		iMovedObj->PosX,
		iMovedObj->PosY,
		iMovedObj->PosX + iMovedObj->SizeX,
		iMovedObj->PosY + iMovedObj->SizeY
	);
