...
#define aci_FREE_LIST(lst, p1, p2, tp) \
	p1 = (tp *)lst->fPtr;              \
	while (p1) {                       \
		p2 = (tp *)p1->next;           \
		delete p1;                     \
...
void aciScreenResourceBML::load(void) {
	...
	for (i = 0; i < NumFrames; i++)
		frameTable[i] = data + i * SizeX * SizeY;
...
void aciScreenEvent::Quant(void) {
	...
	if (nextQuant) {
		CurTimer++;
		if (CurTimer > acs_timer_limit_ticks(MaxTimer))
			Stop();
	}
}
...
void aciScreenDispatcher::FlushEvents(void) {
	...
	while (eventLine->size) {
		...
		// std::cout<<"aciScreenDispatcher::FlushEvents "<<ev->code<<"
		// QuantCode:"<<QuantCode<<std::endl;
	}
}
...
void aciScreenInputField::Quant(void) {
	...
	if (!(flags & ACS_ACTIVE_STRING)) {
		if (Color1 != -1 &&
			CheckXY(
				XGR_MouseObj.PosX + XGR_MouseObj.SpotX, XGR_MouseObj.PosY + XGR_MouseObj.SpotY
			)) {
			Color = Color1;
		}
	} else {
