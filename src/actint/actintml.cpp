...
void aciML_Dispatcher::key_trap(int k, int mode) {
	...
	if (!iScreenOffs && mode == AML_KEY &&
		(k == iMOUSE_LEFT_PRESS_CODE || k == iMOUSE_RIGHT_PRESS_CODE)) {
...
int aciML_Data::key_trap(int key, int mode) {
	...
	case AML_KEY:
		...
		while (ev) {
			ev_flag = 1;
			if (ev->flags & AML_IF_LOCKED && !locked)
				ev_flag = 0;
			if (ev->flags & AML_IF_NOT_LOCKED && locked)
				ev_flag = 0;
			if (ev_flag) {
				if (ev->startupType == mode && ev->check_key(key)) {
					if (ev->flags & AML_ACTIVE_EVENT && ev->flags & AML_NO_DEACTIVATE)
						return -1;
					if (ev->ChannelID) {
						ch_id = ev->ChannelID;
						if (!aciML_DropEvents(ch_id, ev->priority))
							return -1;
					}
					ev->start();
				}
...
void aciML_Event::start(void) {
	...
	if (flags & AML_ACTIVE_EVENT && flags & AML_NO_DEACTIVATE)
		return;
	cm = (aciML_EventCommand *)commands->last;
	while (cm) {
...
int aciML_Event::check_seq(void) {
	...
	while (cm) {
		if (cm->start_timer == cur_timer) {
			if (!(cm->flags & AML_COMMAND_STARTED))
				return 0;
			switch (cm->code) {
			case AML_WAIT_CHANNEL:
				if (isChannelActive(cm->data0))
					return 0;
				break;
			...
			case AML_PUT_IN_QUEUE:
				if (!isActiveIML(cm->data1))
					return 0;
				break;
			case AML_PUT_IN_CH_QUEUE:
				if (!isLockedIML(cm->data1) && (isInQueueIML(cm->data0, cm->data1)))
					return 0;
				break;
			case AML_ADD_IN_CH_QUEUE:
				if (!isLockedIML(cm->data1) &&
					(isInQueueIML(cm->data0, cm->data1) || !isActiveIML(cm->data1)))
					return 0;
				break;
			case AML_GO_PHASE:
			case AML_CLOSE_ML:
...
void aciML_ItemData::init(void) {
	...
	fh > sx > sy > sz > ox > oy;
	if (sz != 2)
		ErrH.Abort("Bad aciML_ItemData::frameData...");
...
void aciML_EventSeqLine::putSeq(aciML_EventSeq *p) {
	...
	if (!p || p->size >= AML_MAX_SEQ_EVENT - size)
		return;
