		...
		if (id == -1)
			...
			case NEW_GATE_SHUTTER:
				if (curMode == AS_INIT_LOC_DATA) {
					...
					t_id = script->read_idata();
					if (t_id < 0 || t_id >= locData->numGateShutters)
			...
			case NEW_MATRIX_SHUTTER:
				if (curMode == AS_INIT_LOC_DATA) {
					...
					if (t_id == 1)
						num = locData->numMatrixShutters1;
					else
						num = locData->numMatrixShutters2;
			...
			case SET_NUM_AVI_ID:
				if (curMode == AS_INIT_MATRIX) {
					...
					if (!invMat->numAviIDs)
						handle_error("Bad invMatrix::numAviIDs");
					...
					if (curMode == AS_INIT_ITEM) {
						...
						if (!invItm->numAviIDs)
							handle_error("Bad invItem::numAviIDs");
			...
			case INIT_AVI_ID:
				if (curMode == AS_INIT_ITEM) {
					if (!invItm->numAviIDs)
						handle_error("Null item numAviIDs...");
					t_id = script->read_idata();
					...
					if (curMode == AS_INIT_MATRIX) {
						if (!invMat->numAviIDs)
							handle_error("Null matrix numAviIDs...");
						t_id = script->read_idata();
