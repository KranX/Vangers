...
void aciLocationInfo::prepare(void) {
	...
	if (!scr)
		ErrH.Abort("Escave screen not found...");
	for (i = 0; i < ACI_NUM_S_OBJECTS; i++) {
		if (s_objIDs[i]) {
...
void CounterPanel::redraw(void) {
	int i, x, y, digits, ox, dx = aCellSize + 1, v = *value_ptr, col,
							 bcol = aciCurColorScheme[ACI_BACK_COL], col_sz = 0;
	char *ptr;
	unsigned char *buf = NULL, *p = NULL;
	iScreenObject *obj;

	if (ID == CREDITS_COUNTER)
		v = aciGetCurCredits();
...
void invItem::redraw(int x, int y, int drop_mode, int flush_mode) {
	...
	if (drop_mode) {
		...
		if (ShapeY[NumIndex] & 0x01)
			dx += (aCellSize >> 1);

		buf = new unsigned char[aCellSize * aCellSize];
		memset(buf, 0, aCellSize * aCellSize);
		aPutNum(
			aCellSize / 2 - aScrFonts[INV_FONT]->SizeX / 2,
			aCellSize / 2 - aScrFonts[INV_FONT]->SizeY / 2,
			INV_FONT,
			col,
			dropCount,
			aCellSize,
...
int invMatrix::check_fit(int x, int y, invItem *p) {
	...
	for (i = 0; i < dl; i++) {
		...
		if (matrix[offs]->type == AS_NO_CELL ||
			(tp != matrix[offs]->slotType && matrix[offs]->slotType != -1) ||
			(matrix[offs]->flags & AS_BUSY_CELL))
			return 0;
	}
	return 1;
}
...
void invMatrix::redraw_shadow_cells(void) {
	...
	for (i = 0; i < SizeY; i++) {
		...
		for (j = 0; j < SizeX; j++) {
			if (matrix[index]->type && !(matrix[index]->flags & AS_IN_SHADOW) &&
				(matrix[index]->flags & AS_REDRAW_CELL) && !(matrix[index]->flags & AS_BUSY_CELL)) {
				memcpy(p, aCellFrame, (dx + 1) * (dx + 1));
	...
	for (i = 0; i < SizeY; i++) {
		...
		for (j = 0; j < SizeX; j++) {
			if (matrix[index]->type && (matrix[index]->flags & AS_IN_SHADOW) &&
				!(matrix[index]->flags & AS_BUSY_CELL)) {
...
void invMatrix::flush(void) {
	...
	XGR_Rectangle(
		...
		ScreenSizeX,
		ScreenSizeY,
		aciCurColorScheme[INV_NUM_ITM_COL],
		aciCurColorScheme[INV_NUM_ITM_COL],
		XGR_OUTLINED
	);
#endif
...
void invMatrix::init(void) {
	int i, sz = SizeX * SizeY;
	if (!ScreenSizeX)
		ScreenSizeX = SizeX * aCellSize + (aCellSize >> 1) + 1;
	if (!ScreenSizeY)
		ScreenSizeY = SizeY * aCellSize - ((SizeY - 1) * (aCellSize >> 2)) + 1;
	maxLoad = 0;
	for (i = 0; i < sz; i++) {
		if (matrix[i]->type != AS_NO_CELL)
			maxLoad++;
	}
}
...
void actIntDispatcher::redraw(void) {
	...
	if (curMode == AS_INFO_MODE) {
		p = (fncMenu *)menuList->last;
		while (p) {
			if (!(p->flags & FM_ACTIVE) && p->curFunction && !(p->flags & FM_SUBMENU) &&
				!(p->flags & FM_HIDDEN))
				p->set_redraw();
...
void actIntDispatcher::i_redraw(void) {
	...
	if (flags & AS_ISCREEN_INV_MODE) {
		...
		if (qMenu && qMenu->flags & FMC_REDRAW)
			qMenu->redraw();
		p = (fncMenu *)i_menuList->last;
		while (p) {
			if (p->flags & FM_LOCATION_MENU && p->flags & FM_ACTIVE) {
...
void fncMenu::init_objects(void) {
	...
	for (i = 0; i < items->Size; i++) {
		...
		if (flags & FM_NO_ALIGN && sx < p->SizeX)
			sx = p->SizeX;
...
void fncMenuItem::redraw(int bsx, int bsy, unsigned char *buf, int x, int y) {
	...
	if (!(flags & FM_RANGE_FONT)) {
		if (flags & FM_SELECTED)
			...
			col = (aciCurColorScheme[FM_UNSELECT_BORDER_COL] << 8) |
				  aciCurColorScheme[FM_UNSELECT_COL];
	} else {
		if (flags & FM_SELECTED) {
			col = aciCurColorScheme[FM_SELECT_START];
			col_sz = aciCurColorScheme[FM_SELECT_SIZE];
			...
			col_sz =
				(aciCurColorScheme[FM_SELECT_SIZE] << 16) | aciCurColorScheme[FM_UNSELECT_SIZE];
		}
	}
#else
	col = (flags & FM_SELECTED) ? aciCurColorScheme[FM_SELECT_COL]
								: aciCurColorScheme[FM_UNSELECT_COL];
#endif
	if (flags & FM_RANGE_FONT)
		...
		if (items->Size) {
			if (prefix) {
				...
				if (postfix)
					curItem->redraw_str(
						...
						SizeY,
						buf,
						PrefixX + PrefixSX + curItem->SizeX + FM_PREFIX_DELTA * 2,
						PrefixY,
						postfix
					);
			}
			if (VItems < items->Size) {
				...
				if (!(flags & FM_NO_ALIGN)) {
					...
					down_obj->redraw(
						SizeX, SizeY, buf, down_obj->PosX + PrefixDelta, down_obj->PosY
					);
				} else {
					up_obj->redraw(
						sx, sy, buf, up_obj->PosX + PrefixDelta + PosX, up_obj->PosY + PosY
					);
					down_obj->redraw(
						sx, sy, buf, down_obj->PosX + PrefixDelta + PosX, down_obj->PosY + PosY
					);
				}
			}
			if (!(flags & FM_NO_ALIGN)) {
	...
	for (i = 0; i < items->Size; i++) {
		...
		if (p->color == -1) {
			if (!(flags & IP_RANGE_FONT)) {
				...
				col = (aciCurColorScheme[FM_SELECT_START] << 16) |
					  aciCurColorScheme[FM_UNSELECT_START];
				col_sz =
					(aciCurColorScheme[FM_SELECT_SIZE] << 16) | aciCurColorScheme[FM_UNSELECT_SIZE];
			}
		} else {
			if (!(flags & IP_RANGE_FONT)) {
				...
				if (p->color & (~0xFFFF)) {
					...
					col = (aciCurColorScheme[FM_SELECT_START] << 16) |
						  aciCurColorScheme[FM_SELECT_START];
					col_sz = (aciCurColorScheme[FM_SELECT_SIZE] << 16) |
							 aciCurColorScheme[FM_SELECT_SIZE];
				}
			}
		}
#else
		...
		if (flags & IP_NO_ALIGN) {
			...
			if (!(flags & IP_RANGE_FONT))
				x = OffsX +
					((SizeX - aStrLen((unsigned char *)p->ID_ptr.c_str(), font, hSpace)) >> 1);
			else
...
void actIntDispatcher::KeyQuant(void) {
	...
	if (aciKeyboardLocked && !(flags & AS_CHAT_MODE))
		return;
	...
	if (flags & AS_LOCKED) {
		if (flags & AS_CHAT_MODE) {
			...
			while (KeyBuf->size)
				iChatKeyQuant(KeyBuf->get());
			if (flags & aMS_LEFT_PRESS || flags & aMS_RIGHT_PRESS) {
				iChatMouseQuant(
					XGR_MouseObj.PosX + XGR_MouseObj.SpotX,
					XGR_MouseObj.PosY + XGR_MouseObj.SpotY,
					1
				);
			}
		}
	...
	if (flags & aMS_PRESS) {
		if (flags & aMS_LEFT_PRESS) {
			...
			case AS_INFO_MODE:
				...
				m = (fncMenu *)menuList->last;
				while (m) {
					if (!(m->flags & FM_HIDDEN) && !(m->flags & FM_LOCK) &&
						m->check_xy(iMouseX, iMouseY)) {
						if (m->flags & FM_ACTIVE) {
...
void actIntDispatcher::iKeyQuant(void) {
	...
	if (flags & AS_ISCREEN_INV_MODE) {
		...
		if (flags & aMS_MOVED) {
			...
			if (iMouseLPressFlag) {
				if (qMenu && qMenu->flags & FMC_DATA_INIT && qMenu->check_xy(iMouseX, iMouseY) &&
					qMenu->check_xy(iMousePrevX, iMousePrevY)) {
...
void actIntDispatcher::EventQuant(void) {
	...
	if (aciItmTextQueueSize && !(flags & AS_TEXT_MODE))
		aciReadItmText();
...
void fncMenu::step_up(void) {
	...
	while (!(curItem->flags & FM_VISIBLE)) {
		...
		if (!firstItem)
			firstItem = (fncMenuItem *)items->last;
		init_redraw();
	}
	if (trigger)
		trigger->curCount = 0;
...
void actIntDispatcher::init_submenu(fncMenu *m) {
	...
	if (m->curItem->flags & FM_SUBMENU_ITEM) {
		...
		if (!p)
			ErrH.Abort("init_submenu() failed...");
...
int actIntDispatcher::put_item(actintItemData *d) {
	...
	if (!p) {
		std::cout << "actIntDispatcher::put_item Item data not present... " << XERR_USER
				  << " id:" << id << std::endl;
	}
	...
	if (!curMatrix->auto_put_item(p1)) {
		...
		free_item(p1);
		std::cout << "actIntDispatcher::put_item !curMatrix -> auto_put_item(p1) id:" << id
				  << std::endl;
		return 0;
	}
	p1->item_ptr = d;
...
void actIntDispatcher::change_ibs(int id) {
	...
	curIbs = get_ibs(id);
	if (!curIbs)
		ErrH.Abort("IBS object not present", XERR_USER, id);
	load_ibs();
}
...
void actIntDispatcher::next_ibs(void) {
	...
	if (!curIbs)
		ErrH.Abort("IBS object not present");
	load_ibs();
}
...
void aciWorldMap::init(void) {
	...
	if (!redraw_buf)
		redraw_buf = new unsigned char[SizeX * SizeY];
	calc_shapes();
	remap_coords();
}
...
void aIndData::build_pict(void) {
	...
	if (flags & IND_DISABLED) {
		...
		aciPutBufSpr(
			...
			aIndBackBML->SizeY,
			SizeX,
			aIndBackBML->frames + aIndBackBML->SizeX * aIndBackBML->SizeY * (aIndBackBML->Size - 1),
			pict_buf,
			ACI_IND_ARROW_START,
			ACI_IND_ARROW_SIZE
		);
		...
		for (ind = NumVals - 1; ind >= 0; ind--) {
			...
			if (value[ind] >= *max_value[ind])
				value[ind] = (*max_value[ind]) - 1;
			...
			if (val >= aIndDataBML->Size)
				val = aIndDataBML->Size - 1;
			if (val < 0)
				val = 0;

			if (p2 < 0)
				p2 = 0;
			if (p2 >= aIndDataBML->Size)
				p2 = aIndDataBML->Size - 1;
			...
			aciPutBufSpr(
				...
				aIndDataBML->SizeY,
				SizeX,
				aIndDataBML->frames + aIndDataBML->SizeX * aIndDataBML->SizeY * p2,
				pict_buf,
				ACI_IND_FON_START,
				ACI_IND_FON_SIZE
			);
			aciPutBufSpr(
				...
				aIndDataBML->SizeY,
				SizeX,
				aIndDataBML->frames + aIndDataBML->SizeX * aIndDataBML->SizeY * val,
				pict_buf,
				col,
				col_sz
			);
			...
			aciPutBufSpr(
				...
				aIndArrowBML->SizeY,
				SizeX,
				aIndArrowBML->frames + aIndArrowBML->SizeX * aIndArrowBML->SizeY * p2,
				pict_buf,
				ACI_IND_ARROW_START,
				ACI_IND_ARROW_SIZE
			);
		...
		if (flags & IND_SHOW_VALUES) {
			for (ind = 0; ind < NumVals; ind++) {
				for (v = 0; v < *max_value[ind]; v++) {
					...
					if (val < 0)
						val = 0;
					if (val >= aIndArrowBML->Size)
						val = aIndArrowBML->Size - 1;
					aciPutBufSpr(
						0,
						0,
						aIndArrowBML->SizeX,
						aIndArrowBML->SizeY,
						SizeX,
						aIndArrowBML->frames + aIndArrowBML->SizeX * aIndArrowBML->SizeY * val,
						pict_buf,
						ACI_IND_ARROW_START,
						ACI_IND_ARROW_SIZE
					);
...
void actIntDispatcher::calc_load(void) {
	...
	aciCurLoad = 0;
	for (i = 0; i < sz; i++) {
		if (curMatrix->matrix[i]->flags & AS_BUSY_CELL)
			aciCurLoad++;
	}
...
void aciWorldMap::calc_shapes(void) {
	...
	XStream fh;
	if (!ShapeSizeX)
		ShapeSizeX = new short[3];
	if (!ShapeSizeY)
		ShapeSizeY = new short[3];
	if (!shapes) {
		shapes = new unsigned char *[3];
		for (i = 0; i < 3; i++) {
			if (!shape_files[i])
				ErrH.Abort("Map shape file not found...");
...
void fncMenu::set_prefix(unsigned char *p) {
	...
	if (postfix)
		psx = aStrLen(postfix, curItem->font, curItem->space) + FM_PREFIX_DELTA;
...
void actIntDispatcher::put_in_slot(actintItemData *d) {
	...
	if (flags & AS_INV_MOVE_ITEM && curItem == p) {
		...
		if (curMatrix->matrix[offs]->slotType == AS_DEVICE_SLOT)
			return;
	}
					...
					if (dvc) {
						...
						if (!ms_flag) {
							...
							if (iP) {
								iP->free_list();
								iP->add_item(
									curItem->ID_ptr.c_str(), -1, aciCurColorScheme[FM_SELECT_COL]
								);
								iP->add_items(curItem->numComments, curItem->comments);
								if (curItem->pTemplate)
									iP->add_item(aciGetItemLoad(curItem, 0));
...
void actIntDispatcher::load_data(XStream *fh) {
	...
	*fh > ver;
	...
	if (ver < 0 || ver > aciSaveVersion)
		ErrH.Abort("Bad save version...");
...
void fncMenu::init_curItem(void) {
	...
	if (!curItem)
		ErrH.Abort("init_curItem() failed...");
	if (VItems < items->Size)
		firstItem = curItem;
	else
...
void fncMenuSet::shift(int dx) {
	if (redraw_data->SizeX <= data->SizeX)
		return;
	PosX += dx;
	limitPosX();
}
...
void fncMenu::add_menu(fncMenu *p) {
	fncMenuItem *itm;
	if (!p->upMenuItem || check_menu(p->upMenuItem->fnc_code))
		return;
	itm = new fncMenuItem;
...
void actIntDispatcher::add_menu_item(fncMenu *p) {
	...
	while (m) {
		if (m->flags & FM_MAIN_MENU) {
			m->add_menu(p);
			if (curMode == AS_INFO_MODE && m->flags & FM_ACTIVE)
				m->set_redraw();
		}
		m = (fncMenu *)m->prev;
	}
...
void aciLocationShutterInfo::loadData(void) {
	...
	if (sx != Size.X || sy != Size.Y)
		ErrH.Abort("Bad shutter BMP size...");
...
int aciLocationShutterInfo::Quant(void) {
	...
	if (curPos.X == trgPos.X && curPos.Y == trgPos.Y)
		return 1;
	return 0;
}
...
void aciWorldMap::show_jump_count(void) {
	...
	for (i = 0; i < index0; i++) {
		mem_rectangle(
			x + JumpCountX, JumpCountY, sx0, JumpCountSizeY, SizeX, col_in1, col_out, 1, redraw_buf
		);
		x += sx0;
	}
	for (i = index0; i < index1; i++) {
		mem_rectangle(
			x + JumpCountX, JumpCountY, sx0, JumpCountSizeY, SizeX, col_in0, col_out, 1, redraw_buf
		);
		x += sx0;
	}
}
