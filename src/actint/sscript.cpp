		...
		if (id == -1)
			...
			case ACS_SET_COLOR:
				if (acsScriptMode == ACS_INIT_OBJECT && acsObj->type == ACS_INPUT_FIELD_OBJ) {
					num = script->read_idata();
					if (num == 0)
						((aciScreenInputField *)acsObj)->Color0 = script->read_idata();
					if (num == 1)
						((aciScreenInputField *)acsObj)->Color1 = script->read_idata();
					if (num == 2)
						((aciScreenInputField *)acsObj)->Color2 = script->read_idata();
				} else {
					handle_error("Misplaced option", acsOptIDs[id]);
				}
				break;
