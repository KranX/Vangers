...
#	endif
	...
	if (!aciItmTextQueue)
		aciItmTextQueue = new invItem *[ACI_ITEM_QUEUE_MAX];
...
int aStrLen(unsigned char *str, int font, int space) {
	...
	for (s = 0; s < sz; s++)
		len += aScrFonts[font]->SizeX -
			   (aScrFonts[font]->LeftOffs[str[s]] + aScrFonts[font]->RightOffs[str[s]]) + space;
	return len;
}
...
int aStrLen32(void *str, int font, int space) {
	...
	for (s = 0; s < sz; s++)
		len +=
			aScrFonts32[font]->SizeX -
			(aScrFonts32[font]->LeftOffs[str_buf[s]] + aScrFonts32[font]->RightOffs[str_buf[s]]) +
			space;
...
void aPutStr32(
	...
	int s, xs, ys, offs = 0, sz = strlen((char *)str), X = 0, col, col_sz, col1, col2, col_sz1,
				   col_sz2;
	int buf_offs = 0;
...
void show_map(int x, int y, int sx, int sy) {
	...
	/*
		if(scr_x > MAP_PTR_X && scr_x < sx - MAP_PTR_X && scr_y > MAP_PTR_X && scr_y < sy -
	   MAP_PTR_X) XGR_Rectangle(x + scr_x - MAP_PTR_X2,y + scr_y -
	   MAP_PTR_X2,MAP_PTR_X,MAP_PTR_X,aciMapVngColors[aciGamerColor],aciMapVngColors[aciGamerColor],XGR_FILLED);
	*/
	...
	for (_y = 0; _y < (1 << MAP_POWER_Y); _y += 256) {
		for (_x = 0; _x < (1 << MAP_POWER_X); _x += 256) {
			...
			if (scr_x > MAP_PTR_X && scr_x < sx - MAP_PTR_X && scr_y > MAP_PTR_X &&
				scr_y < sy - MAP_PTR_X) {
				switch (index) {
				case 1:
					XGR_Rectangle(
...
#	endif
				...
				case 7:
					...
					break;
				case 8:
					if (scr_x > MAP_PTR_XL && scr_x < sx - MAP_PTR_XL && scr_y > MAP_PTR_XL &&
						scr_y < sy - MAP_PTR_XL)
						XGR_Rectangle(
							x + scr_x - MAP_PTR_XL2,
							y + scr_y - MAP_PTR_XL2,
...
void load_map_bmp(int num) {
	...
	if (aScrDisp->wMap->world_ids[num] == -1 ||
		!(aScrDisp->map_names[aScrDisp->wMap->world_ids[num]]))
		ErrH.Abort("Map BMP not present...");
...
void aciSell_Item(invItem *p) {
	...
	if (!it->uvsDataPtr &&
		(!aScrDisp->curLocData->ExcludeItems[p->ID] || (p->flags & INV_ITEM_NO_PAY))) {
		it->uvsDataPtr = ptr;
	}
...
int aciCheckSellItem(void) {
	...
	if (p->owner)
		x += ((iScreen *)p->owner)->ScreenOffs;
	if (p->CheckXY(x, iMouseY))
		return 1;
	return 0;
}
...
void redraw_fnc(int x, int y, int sx, int sy, int fnc) {
	...
	case FMENU_WMAP:
		if (aScrDisp->wMap) {
			aScrDisp->wMap->redraw(
				x + ((sx - aScrDisp->wMap->SizeX) >> 1), y + ((sy - aScrDisp->wMap->SizeY) >> 1)
			);
		} else {
...
void aciSetTimePanel(void) {
	...
	iPl->add_item(
		p, -1, (aciCurColorScheme[FM_SELECT_BORDER_COL] << 8) | aciCurColorScheme[FM_SELECT_COL]
	);
...
void aciOpenWorldLink(int w1, int w2) {
	...
	if (id1 == -1 || id2 == -1)
		ErrH.Abort("Bad world ID...");

	p1 = aScrDisp->wMap->world_ptr[id1];
	p2 = aScrDisp->wMap->world_ptr[id2];

	if (!p1 || !p2)
		ErrH.Abort("Bad world ptr...");

	p1->links[id2] = 1;
	if (id1 != id2 && p2->links[id1])
		p2->links[id1] = 0;
	aScrDisp->wMap->flags |= WMAP_REBUILD_PIC;
	aciPrevJumpCount = -1;
}
...
void aciInitAviID(void) {
	...
	if (iEvLineID == MECHOS_MODE || iEvLineID == MECHOS_LIST_MODE ||
		...
		if (ip && m->mech_name) {
			...
			//			  ip ->
			// add_item(aciGetPrice((iListElement*)m,MECHOS_MODE,0),-1,aciCurColorScheme[FM_SELECT_COL]);
		}
	} else {
		p = (invItem *)iShopItem;
...
void aciChangeList(void) {
	...
	aciInitAviID();
	if (iEvLineID == MECHOS_LIST_MODE || iEvLineID == ITEMS_LIST_MODE ||
...
void aciInitShopItems(void) {
	...
	if (aScrDisp->curMatrix) {
		...
		while (u) {
			p = (invItem *)aScrDisp->curMatrix->items->last;
			while (p) {
				if (!p->uvsDataPtr && p->ID == u->type && p->MatrixX == u->pos_x &&
					p->MatrixY == u->pos_y) {
					p->uvsDataPtr = u;
					break;
				}
...
void aciSellMoveItem(void) {
	...
	if (!fl) {
		...
		if (p->flags & INV_ITEM_NO_PAY)
			exclude = 0;
		if (p->classID == AS_ITEM_CLASS && prev_mode != ITEMS_MODE &&
			prev_mode != ITEMS_LIST_MODE) {
			change_mode = CHANGE_2_ITEMS_MODE;
			if (iEvLineID != ITEMS_MODE && iEvLineID != ITEMS_LIST_MODE)
				iEvLineID = change_mode;
		}
		if (p->classID == AS_WEAPON_CLASS && prev_mode != WEAPONS_MODE &&
			prev_mode != WEAPONS_LIST_MODE) {
	...
	if (aScrDisp->flags & AS_INV_MOVE_ITEM) {
		if (aScrDisp->curItem->ID == id) {
			if (id1 == -1) {
				...
				u->type = it->ID;
				set_mouse_cursor(
					aScrDisp->curItem->frame,
					aScrDisp->curItem->ScreenSizeX,
					aScrDisp->curItem->ScreenSizeY
				);
			}
			ret = 1;
		}
...
void uvs_aciChangeOneItem(int id_from, int id_to, int x, int y) {
	...
	if (aScrDisp->flags & AS_INV_MOVE_ITEM) {
		if (aScrDisp->curItem->ID == id_from && aScrDisp->curItem->MatrixX == x &&
			aScrDisp->curItem->MatrixY == y) {
			it->clone(aScrDisp->curItem);
			set_mouse_cursor(
				aScrDisp->curItem->frame,
				aScrDisp->curItem->ScreenSizeX,
...
void aciStartSpeech(void) {
	aciML_EventSeq *seq;
	int i, unlock_id, prepare2_id, prepare_id, speech_id, end_id, end2_id, lock_id, speech_sz,
		dropLev = 0;
...
int aciCheckCredits(void) {
	...
	if (aScrDisp->flags & AS_INV_MOVE_ITEM) {
		if (iEvLineID == MECHOS_MODE || iEvLineID == MECHOS_LIST_MODE)
			return 0;
		if (aScrDisp->curItem->partData)
			return 1;
		if (!((uvsActInt *)aScrDisp->curItem->uvsDataPtr)->sell_price)
			return 0;
		return 1;
	}
	...
	if (iEvLineID == MECHOS_MODE || iEvLineID == MECHOS_LIST_MODE) {
		if (u->price > cr + aciGetCurMatrixPrice())
			return 0;
	} else {
		p = (invItem *)iShopItem;
		u = (uvsActInt *)p->uvsDataPtr;
...
int aciCheckItemCredits(void) {
	int cr = aciGetCurCredits();
	if (aScrDisp->flags & AS_INV_MOVE_ITEM) {
		if (((uvsActInt *)aScrDisp->curItem->uvsDataPtr)->price > cr)
			return 0;
		return 1;
	}
	return 0;
...
void aciBuildQuestList(void) {
	...
	if (aciEscaveDead || aciEscaveEmpty)
		return;
...
int aciCheckItemPickUp(actintItemData *p) {
	...
	if ((itm->classID == AS_ITEM_CLASS && aciPickupItems) ||
		(itm->classID == AS_WEAPON_CLASS && aciPickupDevices))
		return 1;
	return 0;
}
	...
	if (p->pTemplate) {
		...
		case 0:
			if (p->flags & INV_ITEM_SHOW_ESCAVE) {
				if (p->item_ptr)
					sprintf(
						aciXConv->address(), p->pTemplate, aciGetItemEscave(p->item_ptr->data1)
					);
			} else {
				if (p->item_ptr)
			...
			if (p->flags & INV_ITEM_SHOW_ESCAVE) {
				if (p->uvsDataPtr)
					sprintf(
						aciXConv->address(),
						p->pTemplate,
						aciGetItemEscave(((uvsActInt *)p->uvsDataPtr)->param2)
					);
			} else {
				if (p->uvsDataPtr)
					sprintf(
						aciXConv->address(), p->pTemplate, ((uvsActInt *)p->uvsDataPtr)->param2
					);
			}
			break;
		}
...
void aciAddTeleportMenuItem(int id, int fnc_id) {
	...
	if (id == -1) {
		if (lang() == RUSSIAN) {
			if (strcmp(aci_curLocationName, (char *)eCmpPodish) == 0)
				ptr = rCmpPodish;
			else if (strcmp(aci_curLocationName, (char *)eCmpVigBoo) == 0)
				ptr = rCmpVigBoo;
			else if (strcmp(aci_curLocationName, (char *)eCmpZeePa) == 0)
				ptr = rCmpZeePa;
		} else
...
void aciChangeMouseItem(void) {
	...
	set_mouse_cursor(
		aScrDisp->curItem->frame, aScrDisp->curItem->ScreenSizeX, aScrDisp->curItem->ScreenSizeY
	);
}
#endif
...
void aciPackFile(char *fname) {
	...
	*(unsigned int *)(p1 + 2) = (unsigned int)sz;
	...
	case Z_MEM_ERROR:
		...
		break;
	case Z_BUF_ERROR:
		std::cout << "not enough room in the output buffer." << std::endl;
		break;
	};
...
void aciUnPackFile(char *fname) {
	...
	if (*(short *)(p)) { // if label = 0 not compress
		...
		case Z_MEM_ERROR:
			std::cout << "DeCompress not enough memory." << std::endl;
			break;
		case Z_BUF_ERROR:
			std::cout << "DeCompress not enough room in the output buffer." << std::endl;
			break;
		case Z_DATA_ERROR:
			std::cout << "DeCompress error data." << std::endl;
			break;
		};
	} else {
...
void aciPrepareWorldsMenu(void) {
	...
	if (p) {
		...
		if (aScrDisp->flags & AS_WORLDS_INIT) {
			...
			while (p1) {
				if (p1->uvsID == CurrentWorld)
					p->deactivate_item(1000 + p1->ID);
...
int aciPutItem(int id, int x, int y) {
	...
	if (!p1)
		ErrH.Abort("Bad aciPutItem() argument...");
...
void acsHandleExtEvent(int code, int data0, int data1, int data2) {
	...
	case ACS_INIT_SOUND_VOLUME:
		acsScrD->SetScroller(
			ACS_SOUND_VOLUME, iGetOptionValue(iSOUND_VOLUME_CUR), iGetOptionValue(iSOUND_VOLUME_MAX)
		);
		break;
	...
	case ACS_INIT_MUSIC_VOLUME:
		acsScrD->SetScroller(
			ACS_MUSIC_VOLUME, iGetOptionValue(iMUSIC_VOLUME_CUR), iGetOptionValue(iMUSIC_VOLUME_MAX)
		);
		break;
		...
		if (aScrDisp->curMatrix) { // Существует ли матрица
			...
			while (iter_gamer) {
				...
				std::cout << "Can't find item GGamer type:" << iter_gamer->type
						  << " pos_x:" << iter_gamer->pos_x << " pos_y:" << iter_gamer->pos_y
			...
			end_find:
				...
				if (el != NULL) {
					...
					while (el2) {
						...
						if (item->pos_x == iter_gamer->pos_x && item->pos_y == iter_gamer->pos_y &&
							item->type == iter_gamer->type) {
							std::cout << "We find duplicate item GGamer type:" << iter_gamer->type
									  << " pos_x:" << iter_gamer->pos_x
									  << " pos_y:" << iter_gamer->pos_y << std::endl;
							goto next;
						}
						el2 = el2->next;
					}
...
void aciGetRubox(void) {
	...
	if (!aciPutItem(ACI_RUBOX, -1, -1))
		ErrH.Abort("aciPutItem() failed...");
...
void aciLoadUVSList(XStream &fh, uvsActInt **p, int list_type) {
	...
	for (i = 0; i < list_size; i++) {
		...
		// std::cout<<"aciLoadUVSList new "<<ptr<<" type:"<<ptr->type<<" price:"<<ptr->price<<"
		// param1:"<<ptr->param1<<" param2:"<<ptr->param2<<std::endl;
		ptr->link(el);
	}
	*p = (uvsActInt *)el;
}
...
void aciShowFrags(void) {
	...
	static int fragColors[10] = {ACI_FRAG_COL0,
		ACI_FRAG_COL1,
		ACI_FRAG_COL2,
		ACI_FRAG_COL3,
		ACI_FRAG_COL4,
	...
	if (!NetworkON || !players_list.size())
		return;
	...
	num = iNumPlayers;
	if (iCurMultiGame == 0)
		num += ACI_FRAG_ADDITIONAL_LINES; // Van-War
	...
	case 0: // VAN-WAR...
		...
		for (i = 0; i < ACI_FRAG_ADDITIONAL_LINES; i++) {
			...
			switch (i) {
			case 0:
				XBuf < aciSTR_RESTRICTIONS;
				break;
			case 1:
				XBuf < aciSTR_WINS < " " <= my_server_data.Van_War.MaxKills < ", " <
					aciSTR_MINUTES < " " <= my_server_data.Van_War.MaxTime;
				break;
			case 2:
				XBuf < "";
				break;
			case 3:
				XBuf < aciSTR_STATISTICS;
				break;
			}
		...
		for (i = 0; i < num - ACI_FRAG_ADDITIONAL_LINES; i++) {
			...
			XBuf < p->name < " (" < world_name < ") : " < aciSTR_WINS < " " <= p->body.kills <
				", " < aciSTR_LOSSES < " " <= p->body.deaths;
			aScrDisp->curPrompt->add_str(
				i + ACI_FRAG_ADDITIONAL_LINES, (unsigned char *)XBuf.address()
			);
	...
	case 1: // MECHOSOMA...
		for (i = 0; i < num; i++) {
			...
			XBuf.init();
			world_name = aScrDisp->wMap->world_ptr[aScrDisp->wMap->world_ids[p->body.world]]->name;
			XBuf < p->name < " (" < world_name < ") : " < aciSTR_Ware1 < " " <=
				p->body.MechosomaStat.ItemCount1 < "/" <= my_server_data.Mechosoma.ProductQuantity1;
			XBuf < " " < aciSTR_Ware2 < " " <= p->body.MechosomaStat.ItemCount2 < "/" <=
	...
	case 2: // PASSEMBLOSS...
		for (i = 0; i < num; i++) {
			...
			XBuf.init();
			world_name = aScrDisp->wMap->world_ptr[aScrDisp->wMap->world_ids[p->body.world]]->name;
			XBuf < p->name < " (" < world_name < ") : " < aciSTR_Checkpoints < " " <=
				p->body.PassemblossStat.CheckpointLighting < "/" <=
...
void aciInitPricePanel(InfoPanel *ip, iListElement *p, int mode, int sell_mode) {
	...
	if (mode == MECHOS_MODE) {
		...
		if (!sell_mode && m->pData) {
			aciXConv->init();
			*aciXConv < aciSTR_ENERGY_SHIELD < " " <
				(m->pData + ACI_MECHOS_ENERGY_SHIELD * ACI_MAX_PRM_LEN);
			ip->add_item(aciXConv->address(), -1, col);

			aciXConv->init();
			*aciXConv < aciSTR_RESTORING_SPEED < " " <
				(m->pData + ACI_MECHOS_RESTORING_SPEED * ACI_MAX_PRM_LEN);
			ip->add_item(aciXConv->address(), -1, col);

			aciXConv->init();
			*aciXConv < aciSTR_MECHANIC_ARMOR < " " <
				(m->pData + ACI_MECHOS_MECHANIC_ARMOR * ACI_MAX_PRM_LEN) < " " < aciSTR_VELOCITY <
				" " < (m->pData + ACI_MECHOS_VELOCITY * ACI_MAX_PRM_LEN);
			ip->add_item(aciXConv->address(), -1, col);

			aciXConv->init();
			*aciXConv < aciSTR_SPIRAL_CAPACITY < " " <
				(m->pData + ACI_MECHOS_SPIRAL_CAPACITY * ACI_MAX_PRM_LEN);
			ip->add_item(aciXConv->address(), -1, col);

			aciXConv->init();
			*aciXConv < aciSTR_AIR_RESERVE < " " <
				(m->pData + ACI_MECHOS_AIR_RESERVE * ACI_MAX_PRM_LEN);
			ip->add_item(aciXConv->address(), -1, col);
		}
	} else {
		it = (invItem *)p;
		if (it->pData && !sell_mode) {
			if (it->slotType == AS_WEAPON_SLOT || it->slotType == AS_TWEAPON_SLOT) {
				...
				*aciXConv < aciSTR_DAMAGE < " " < (it->pData + ACI_WEAPON_DAMAGE * ACI_MAX_PRM_LEN);
				...
				aciXConv->init();
				*aciXConv < aciSTR_SHOTS < " " <
					(it->pData + ACI_WEAPON_SHOTS_SEC * ACI_MAX_PRM_LEN);
				ip->add_item(aciXConv->address(), -1, col);
				...
				*aciXConv < aciSTR_BURST < " " < (it->pData + ACI_WEAPON_RANGE * ACI_MAX_PRM_LEN);
				...
				if (it->slotType == AS_DEVICE_SLOT) {
					if (!uvsCurrentWorldUnable) {
						aciXConv->init();
						*aciXConv < aciSTR_WORKING_TIME < " " <
							(it->pData + ACI_DEVICE_WORKING_TIME * ACI_MAX_PRM_LEN) <
							aciSTR_SECONDS;
						ip->add_item(aciXConv->address(), -1, col);
					}
				} else {
					aciXConv->init();
					*aciXConv < aciSTR_DAMAGE < " " <
						(it->pData + ACI_AMMO_DAMAGE * ACI_MAX_PRM_LEN);
					ip->add_item(aciXConv->address(), -1, col);

					aciXConv->init();
					*aciXConv < aciSTR_IN_PACK < " " <
						(it->pData + ACI_AMMO_IN_PACK * ACI_MAX_PRM_LEN);
					ip->add_item(aciXConv->address(), -1, col);
				}
			}
		}
...
void aciAutoSave(void) {
	if (NetworkON || aScrDisp->curLocData->WorldID == XPLO_ID)
		return;
...
int aTextWidth32(void *text, int font, int hspace) {
	...
	for (s = 0; s < sz; s++) {
		...
		if (chr != '\n') {
			if (chr != '\r') {
				len += xs + hspace - aScrFonts32[font]->RightOffs[chr] -
					   aScrFonts32[font]->LeftOffs[chr];
			}
		} else {
			if (len > max_len)
				max_len = len;
...
void acsCompressData(char *p) {
	...
	*(unsigned int *)(dest_buf + 2) = (unsigned int)src_sz;
	dest_sz -= 2 + 4;
	int stat =
		compress((Bytef *)(dest_buf + 2 + 4), (uLongf *)&(dest_sz), (Bytef *)src_buf, src_sz);
	dest_sz += 2 + 4;
	switch (stat) {
	// case Z_OK: std::cout<<"Compress ok."<<std::endl; break;
	case Z_MEM_ERROR:
...
void acsDecompressData(char *p) {
	...
	if (*(short *)(src_buf)) { // if label = 0 not compress
		...
		int stat = uncompress(
			(Bytef *)dest_buf, (uLongf *)&dest_sz, (Bytef *)(src_buf + 2 + 4), src_sz - 2 - 4
		);
		switch (stat) {
		// case Z_OK: std::cout<<"DeCompress ok."<<std::endl; break;
		case Z_MEM_ERROR:
...
void aciInitItmTextQueue(void) {
	...
	p = (invItem *)aScrDisp->curMatrix->items->last;
	while (p) {
		if (aciItmTextQueueSize < ACI_ITEM_QUEUE_MAX && p->ID == ACI_TABUTASK &&
	...
	p = (invItem *)aScrDisp->curMatrix->items->last;
	while (p) {
		if (aciItmTextQueueSize < ACI_ITEM_QUEUE_MAX &&
			(p->ID == ACI_ELEEPOD || p->ID == ACI_BEEBOORAT || p->ID == ACI_ZEEX) &&
			(p->flags & INV_ITEM_NEW)) {
			aciItmTextQueue[aciItmTextQueueSize] = p;
			aciItmTextQueueSize++;
		}
...
char *aciLoadPackedFile(XStream &fh, int &out_sz) // Text decompress
{
	...
	if (*(short *)(p)) { // if label = 0 not compress
		...
		int stat =
			uncompress((Bytef *)ptr, (uLongf *)&long_out_sz, (Bytef *)(p + 2 + 4), sz - 2 - 4);
		switch (stat) {
		// case Z_OK: std::cout<<"DeCompress ok."<<std::endl; break;
		case Z_MEM_ERROR:
			std::cout << "DeCompress not enough memory." << std::endl;
