...
Parser::Parser(const char *name, const char *del_chars): XBuffer(0, 0) {
	int i;
	char c;
	unsigned _time_;
	...
	if (c) {
		...
		long decompressed_size =
			*(unsigned int *)(compressed_buff + 2) + 12; // ZIP_GetExpandedSize(compressed_buff);
...
MemoryHeap::MemoryHeap(const char *message): overflow_message(message) {
	offset = size = 0;
}
...
void Normal::calc_intensity() // Metal
{
	...
	if (n_power) {
		...
		Intens = (power_table[(int(n_power) - 2 << 8) + (RV >> 7)] << 9) + (Ln >> 1);
	} else {
		int Ln = Vector(*this) * Refl;
		if (Ln < 0) {
			I = 0;
...
void clip_line(int x1, int y1, int x2, int y2, int color) {
	if (x1 >= UcutLeft && x1 <= UcutRight && y1 >= VcutUp && y1 <= VcutDown && x2 >= UcutLeft &&
		x2 <= UcutRight && y2 >= VcutUp && y2 <= VcutDown)
