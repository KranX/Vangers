...
inline int maxa(int x, int y, int z) {
...
inline int mina(int x, int y, int z) {
...
inline double fmaxa(double x, double y) {
	return fabs(x) > fabs(y) ? x : y;
}
inline double fmaxa(double x, double y, double z) {
	return fmaxa(fmaxa(x, y), z);
}
...
inline double fmina(double x, double y) {
	return fabs(x) < fabs(y) ? x : y;
}
inline double fmina(double x, double y, double z) {
	return fmina(fmina(x, y), z);
}
...
inline int round_sin(int x) {
	return x >= 0 ? (x + (1 << 14)) >> 15 : -((-x + (1 << 14)) >> 15);
}

/* Use instead of "x >> 15" for Round and correctly "-1 >> 15" */
inline int roundi(int x) {
	return x >= 0 ? (x + (1 << 14)) >> 15 : -((-x + (1 << 14)) >> 15);
}
...
inline float Sin(int angle) {
	return cos_table[(angle - Pi / 2) & ANGLE_MASK];
}
...
******************************************************************************/
#define DECOMPOSITION(v_tangent, v_normal, v, axis)    \
	{                                                  \
		v_tangent = (axis * (axis * v)) / axis.abs2(); \
...
inline Vector Vector::descart() const {
	double xy = _radius_ * Sin(_tetta_);
	return Vector(round(xy * Cos(_psi_)), round(xy * Sin(_psi_)), round(_radius_ * Cos(_tetta_)));
}
...
inline int DBV::operator==(const DBV &v) const {
	return fabs(x - v.x) < DBL_EPS && fabs(y - v.y) < DBL_EPS && fabs(z - v.z) < DBL_EPS;
}
inline int DBV::operator!=(const DBV &v) const {
	return fabs(x - v.x) > DBL_EPS || fabs(y - v.y) > DBL_EPS || fabs(z - v.z) > DBL_EPS;
}
...
inline DBV DBV::descart() const {
	double xx = _radius_ * sin(_tetta_);
	return DBV(xx * cos(_psi_), xx * sin(_psi_), _radius_ * cos(_tetta_));
}
...
inline DBM transpose(const DBM &m) {
	return DBM(m[0], m[3], m[6], m[1], m[4], m[7], m[2], m[5], m[8]);
}
