...
void Object::controls(int mode,int param)
{
	switch(mode){
		...
		case CONTROLS::JUMP_USING_ACCUMULATED_POWER:
			if (NetworkON && (strcmp(iScrOpt[iSERVER_NAME]->GetValueCHR(),"eleepod bath")==0 || strcmp(iScrOpt[iSERVER_NAME]->GetValueCHR(),"���� �������")==0)) break;
			if(jump_power){
				jump();
				if(active)
					SOUND_KIDPUSH();
...
void Object::direct_keyboard_control()
{
	...
	if(XKey.Pressed(VK_INSERT) | XKey.Pressed('A'))
		...
		if(jump_power && !(NetworkON && (strcmp(iScrOpt[iSERVER_NAME]->GetValueCHR(),"eleepod bath")==0 || strcmp(iScrOpt[iSERVER_NAME]->GetValueCHR(),"���� �������")==0)))
...
void Object::mechous_analysis(double dt)
{
	...
	if(Status & SOBJ_AUTOMAT){
		if(jump_power && ++jump_power > max_jump_power && !(NetworkON && (strcmp(iScrOpt[iSERVER_NAME]->GetValueCHR(),"eleepod bath")==0 || strcmp(iScrOpt[iSERVER_NAME]->GetValueCHR(),"���� �������")==0)))
			jump();
	} else {
		if(jump_power && !(NetworkON && (strcmp(iScrOpt[iSERVER_NAME]->GetValueCHR(),"eleepod bath")==0 || strcmp(iScrOpt[iSERVER_NAME]->GetValueCHR(),"���� �������")==0)) && CheckStartJump(this)){
			jump();
			if(active)
				SOUND_KIDPUSH();
		}
