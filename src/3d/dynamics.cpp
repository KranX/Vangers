...
#define SPHERICAL_TEST \
	(ID_INSECT | ID_JUMPBALL | ID_BULLET | ID_STATIC | ID_FIELD_SOURCE | ID_HORDE)
...
#define GET_THICKNESS(p)                                                                     \
	((GET_DELTA(*HIGH_LEVEL((p) + H_SIZE)) + (GET_DELTA(*LOW_LEVEL((p) + H_SIZE)) << 2) + 0) \
		<< DELTA_SHIFT)
...
#define WHEEL_TRACK(x_curr, y_curr, speed, level, n) \
	DrawMechosParticle(x_curr, y_curr, speed, level, n)
...
swamp_field_modulation(double rf_x, double rf_y, int field_radius, int fallback_radius) {
...
int check_dynamics_locals() {
	if (stop_flag | fish_test | skyfarmer_test)
		return 1;
	if (rudder_step != 40 || rudder_max != 512 || traction_increment != 44 ||
...
#	ifdef MSG_OUT
	...
	for (int j, i = 0; i < num_indices; i++) {
		...
		msg_buf < (i == curr_entry ? "> ENTRY: " : "  ENTRY: ") < name_entries[j] < " = " <= val <
			"\n";
	}
#	endif
	...
	case VK_ADD:
		...
		if (type_entries[indices_entry[curr_entry]] & INT_ENTRY)
			if (XKey.Pressed(VK_SHIFT))
				...
				(*(int *)pval_entries[indices_entry[curr_entry]]) +=
					abs((*(int *)pval_entries[indices_entry[curr_entry]])) / 16 + 1;
		else if (XKey.Pressed(VK_SHIFT))
...
double get_height(int xx, int yy, double z_dbl, int D, DBV &n, int &z_below) {
	...
	//	if(N1 > 0 && N0 > 0){
	//		n = DBV((double)Sx0/(double)N0 - (double)Sx1/(double)N1,(double)Sy0/(double)N0 -
	//(double)Sy1/(double)N1,0); 		n.norm(1); 		return k_elastic_xy*(double)N1;
	//		}
...
int VariablePolygon::lower_average(
	int xx,
	int yy,
	int zz,
	double &X0,
...
int VariablePolygon::upper_average(
	int xx,
	int yy,
	int zz,
	double &X0,
...
int VariablePolygon::lower_average_z(
	int x_offset,
	int y_offset,
	int z_offset,
	double &dZ,
...
void Object::update_coord(int camera_correction) {
	...
	if (!interpolation_on) {
		xx = R.x =
			wrap_world_coord(R.x, map_size_x, valid_world_coord_or_zero(R_curr.x, map_size_x));
		yy = R.y =
			wrap_world_coord(R.y, map_size_y, valid_world_coord_or_zero(R_curr.y, map_size_y));
		R_curr = R;
		A_scl = A_g2sZ * A_l2g;
	} else {
		DBV R_corr = R + dR_corr;
		xx = R_corr.x =
			wrap_world_coord(R_corr.x, map_size_x, valid_world_coord_or_zero(R_curr.x, map_size_x));
		yy = R_corr.y =
			wrap_world_coord(R_corr.y, map_size_y, valid_world_coord_or_zero(R_curr.y, map_size_y));
	...
	if (abs(R_curr.x - x_of_last_update) + abs(R_curr.y - y_of_last_update) >
		movement_detection_threshould) {
	...
	psi = (A_l2g[8] < 0 ? (-RTOG(atan2(A_l2g[0], A_l2g[1]))) : (RTOG(atan2(A_l2g[0], A_l2g[1])))) -
		  Pi / 2;
	psi &= ANGLE_MASK;
...
void Object::set_3D(int mode, int x, int y, int z, int dz, int angle, int speed) {
	...
	if (mode == SET_3D_CONST_Z)
		z = (int)R.z;
	else if (mode & (SET_3D_TO_THE_LOWER_LEVEL | SET_3D_TO_THE_UPPER_LEVEL | SET_3D_CHOOSE_LEVEL)) {
...
void Object::instant_impulse(int angle, int distance, int slope, int lever_arm) {
	instant_impulse(
		DBV(-Sin(slope) * Sin(angle), -Sin(slope) * Cos(angle), Cos(slope)), distance, lever_arm
	);
}
...
void Object::instant_impulse(const DBV &direct, int distance, int lever_arm) {
	...
	if (lever_arm) {
		DBV r =
			DBV(random(2 * lever_arm) - lever_arm,
...
int Object::get_max_jump_distance() {
	DBV V = DBV(Sin(Pi / 10), 0, Cos(Pi / 10)) *
			(5 * max_jump_power * k_distance_to_force * dt_impulse / pow(m, 0.3));
	DBV R(0, 0, 0);
	while (R.z >= 0) {
		V.z -= g * dt0;
		R += V * dt0;
...
void Object::controls(int mode, int param) {
	...
	case CONTROLS::JUMP_POWER_ACCUMULATION_ON:
		...
		if (!jump_power && dynamic_state & (WHEELS_TOUCH | GROUND_COLLISION | TOUCH_OF_WATER) &&
			!mole_on) {
			if (active)
				SOUND_KIDPULL();
			jump_power = 1;
	...
	case CONTROLS::HELICOPTER_UP:
		if (UsingCopterig(0) && !mole_on) {
			...
			if (device_transition_legacy_step() &&
				(helicopter += helicopter_height_incr) > max_helicopter_height)
	...
	case CONTROLS::HELICOPTER_DOWN:
		if (helicopter && device_transition_legacy_step() &&
			(helicopter -= helicopter_height_decr) < 0) {
			helicopter = 0;
	...
	case CONTROLS::FLOTATION_UP:
		if (dynamic_state & TOUCH_OF_WATER && archimedean < 256 &&
			device_transition_legacy_step() && (archimedean += 4) >= 256) {
			archimedean = 256;
			device_switch_latency = GET_DEVICE_LATENCY();
		}
		break;
	case CONTROLS::FLOTATION_DOWN:
		if (dynamic_state & TOUCH_OF_WATER && archimedean && device_transition_legacy_step() &&
			(archimedean -= 16) < 0)
			archimedean = 0;
		break;

	case CONTROLS::MOLE_DOWN:
		if (UsingCrotrig(0) && !(dynamic_state & TOUCH_OF_WATER) &&
			(dynamic_state & TOUCH_OF_GROUND) && !helicopter && check_mole_submerging()) {
			mole_on = 256;
			rudder = 0;
			StartMoleProcess();
			device_switch_latency = GET_DEVICE_LATENCY();
...
void Object::mechous_analysis(double dt) {
	...
	if (helicopter) {
		if (device_transition_legacy_step() &&
			!UsingCopterig((helicopter << 8) / max_helicopter_height))
			controls(CONTROLS::HELICOPTER_DOWN);
		...
		double fz = g * k_helicopter_thrust * pow(.5, sqr(dz / (double)helicopter)) *
					(1 + helicopter_ampl * Sin(helicopter_thrust_phase += helicopter_thrust_step));
		double fy = -heli_y_offset * air_speed_factor * global_air_speed_factor;
		F_global.x += A_l2g[1] * fy;
		F_global.y += A_l2g[4] * fy;
		F_global.z += fz;

		double dx = heli_x_offset + helicopter_circle_radius_x *
										Cos(helicopter_circle_phase += helicopter_circle_step);
	...
	if (in_water > 32) {
		int cutterig_active =
			UsingCutterig(device_transition_legacy_step() ? (dynamic_state & TOUCH_OF_AIR) : 0);
		if (cutterig_active)
			controls(CONTROLS::FLOTATION_UP);
		else
			controls(CONTROLS::FLOTATION_DOWN);
	...
	MoleInProcess =
		(mole_on && traction && dynamic_state & (GROUND_COLLISION | WHEELS_TOUCH)) ? 1 : 0;
	...
	if (active) {
		...
		//			if(traction > 0){
		//				left_border =
		// A_l2g*(DBV(-TerraMoverSx*xmax_real,TerraMoverSy*ymax_real,-TerraMoverSz*zmax_real)) + R;
		//				right_border =
		// A_l2g*(DBV(TerraMoverSx*xmax_real,TerraMoverSy*ymax_real,-TerraMoverSz*zmax_real)) + R;
		//				}
		//			else{
		...
		//				}
		//
		//			terra_moving_tool.set(left_border,
		// right_border,(A_l2g*V)*(dt*num_calls_analysis)); terra_moving_tool.make_dast();
		//			}

		const int lawn_mower = is_lawn_mower_prm(prm_name);
		if (lawn_mower)
			SetMotorFileIfChanged(
				(helicopter && !(dynamic_state & WHEELS_TOUCH)) ? LAWN_MOWER_FLIGHT_MOTOR_TYPE
																: LAWN_MOWER_GROUND_MOTOR_TYPE
			);
	...
	if (rudder && dynamic_state & WHEELS_TOUCH) {
		int rudder_decay =
			(int)(fabs(round(fabs(rudder * V.y * dt * num_calls_analysis * rudder_k_decr))) + 1);
...
void Object::basic_mechous_analysis(double dt, int last) {
	...
	if (dynamic_state & WHEELS_TOUCH) {
		V.y *=
			pow(1 + log(V_drag_wheel_speed) * mobility_factor * global_speed_factor / speed_factor *
						XTCORE_FRAME_NORMAL,
	...
	if (closest_field_object) {
		...
		case DangerTypeList::WHIRLPOOL:
			if (dynamic_state & TOUCH_OF_WATER) {
				device_modulation =
					water_immersion < 20
						? 1
						: (water_immersion < 200 ? (200 - water_immersion) / 200 : 0);
				double kr2 = 1 / (sqr(rf_x) + sqr(rf_y) + 1);
				double kr = sqrt(kr2);
				F_field +=
					DBV((field_rotator * rf_y - field_tractor_xy * rf_x) * kr2,
		...
		case DangerTypeList::TRAIN:
			if (dynamic_state & TOUCH_OF_WATER) {
				...
				double kr = sqrt(kr2);
				F_field +=
					DBV((train_field_rotator * rf_y - train_field_tractor_xy * rf_x) * kr2,
		...
		case DangerTypeList::FASTSAND:
			if (!mole_on) {
				modulation =
					swamp_field_modulation(rf_x, rf_y, closest_field_object->radius, radius);
				device_modulation =
					terrain_immersion < 20
						? 1
						: (terrain_immersion < 200 ? (200 - terrain_immersion) / 200 : 0);
	...
	terrain_analysis_flag =
		(Vz > 30 ? BREAKE_DBLEVEL_ENABLE : 0) | (Vz > 4 ? MIDDLE_LEVEL_PREFER : 0);
	// double scale_bound_real = scale_real;
	double scale_bound_real = scale_real * scale_bound;
	for (i = 0; i < num_bounds; i++) {
		r = DBV(bound->variable_polygons[i].middle_x,
				bound->variable_polygons[i].middle_y,
				bound->variable_polygons[i].middle_z) *
			scale_bound_real;
		rg0 = A_l2g * r;
		rg = rg0 + R;
		if (!mole_on) {
			...
			if (p) {
				if (GET_TERRAIN(*(p + (((int)rg.x & ~1) & clip_mask_x) + H_SIZE)) ==
					WATER_TERRAIN) {
		...
		if (nz < 0) { // lower average
			if ((code = bound->variable_polygons[i].lower_average(
					 x_offset, y_offset, round(rg.z), X0, Y0, dZ0, X1, Y1, dZ1
				 )) != 0) {
				...
				if ((code & 2) && xy_vs_z) { // horisontal mirror
					...
					if (u0 * normal > 0) {
						...
						u0 = normal * (u0 * normal);
						DBV P = (calc_collision_matrix(r1, J_inv).inverse() * u0) *
								(-horizontal_impulse_factor * modulation) * XTCORE_FRAME_NORMAL;
						V += P;
						W += J_inv * (r1 % P);
					...
					if (u0 * z_axis < 0) {
						...
						// else
						// u0 = u0*k_friction_impulse + z_axis*((u0*z_axis)*(1 -
						// k_friction_impulse));
						...
						if (stand_on_wheels)
							...
							u0 =
								u0 * k_friction_impulse + n * ((u0 * n) * (1 - k_friction_impulse));
						DBV P = (calc_collision_matrix(r, J_inv).inverse() * u0) *
								(-vertical_impulse_factor * modulation) * XTCORE_FRAME_NORMAL;
						V += P;
						W += J_inv * (r % P);
						if (P.vabs() > strong_ground_collision_threshould)
							dynamic_state |= STRONG_GROUND_COLLISION;
			...
			if ((code = bound->variable_polygons[i].upper_average(
					 x_offset, y_offset, round(rg.z), X0, Y0, dZ0, X1, Y1, dZ1
				 )) != 0) {
				terrain_immersion += fabs(dZ0) + fabs(dZ1);
				if (!(code & 1)) {
	...
	if (wheel_touch && stand_on_wheels && !mole_on) {
		for (i = 0; i < n_wheels; i++) {
			...
			if (1) {
				...
				// r.z = 0;
				vw = V + W % r;
				// F.y += df = f_traction - vw.y*(brake || (!wheels[i].steer && hand_brake) ?
				// f_brake_max : 0);
				F.y += df = f_traction;
			...
			wheel_continue:
				if (dZ > 0 && wheels[i].dZ > 0 && last &&
					get_upper_height(round(rg.x), round(rg.y)) < round(rg.z) + 15)
					WHEEL_TRACK(
						(int)round(rg.x),
						(int)round(rg.y),
						k_track,
						check_double_level((int)round(rg.x), (int)round(rg.y)),
						i
					);
				wheels[i].dZ = dZ;
			} else {
	...
	if ((wheel_touch || spring_touch) && in_water < 32 && V.vabs() < V_abs_min &&
		W.vabs() < W_abs_min) {
#ifdef MSG_OUT
		if (msg_log)
	...
	if (!(V.vabs() * V_drag < V_abs_stop && W.vabs() * W_drag < W_abs_stop)) {
		...
		if (r_diff > 0)
			...
			Vs += (z_axis * (radius * rolling_scale)) % W;
		// std::cout<<"dt:"<<dt<<" XT_FN:"<<XTCORE_FRAME_NORMAL<<" Vs.x:"<<Vs.x<<" Vs.y:"<<Vs.y<<"
		// Vs.z:"<<Vs.z<<std::endl;
		R += (A_l2g * Vs) * dt;
...
void Object::basic_debris_analysis(double dt) {
	...
	if (p)
		if (GET_TERRAIN(*(p + (((int)round(R.x) & ~1) & clip_mask_x) + H_SIZE)) == WATER_TERRAIN &&
			(dZ = (double)FloodLEVEL - R.z) > 0) {
			in_water = 256;
			if (archimedean)
	...
	if (ID != ID_JUMPBALL && spring_touch && in_water < 32 && V.vabs() < V_abs_min &&
		W.vabs() < W_abs_min) {
		V_drag *= pow(V_drag_coll, V_abs_min / (V.vabs() + 1e-10));
		W_drag *= pow(W_drag_coll, W_abs_min / (W.vabs() + 1e-10));
	}
	...
	if (!(ID != ID_JUMPBALL && V.vabs() * debris_v_drag < V_abs_stop &&
			W.vabs() * debris_w_drag < W_abs_stop)) {
		DBV Vs = V;
		if (spring_touch)
			Vs -= (z_axis * (radius * rolling_scale)) % W;
...
void Object::fish_analysis(double dt) {
	...
	if (log_f | log_b) {
		...
		if (log_f & log_b) {
			...
			DBV R1 = log_f ? A_g2l * DBV(h_factor_f.x, h_factor_f.y, 0) + r
						   : A_g2l * DBV(h_factor_b.x, h_factor_b.y, 0) - r;
			DBV normal = (R1 / DBV(xmax, ymax, zmax)).get_norm(1);
			DBV u0 = V + W % R1;
			if (u0 * normal > 0) {
				u0 = normal * (u0 * normal);
...
void Object::insect_analysis() {
	...
	if (p)
		if (GET_TERRAIN(*(p + (((int)R.x & ~1) & clip_mask_x) + H_SIZE)) == WATER_TERRAIN &&
			FloodLEVEL > round(R.z)) {
			in_water = 256;
...
int Object::check_terrain_type(int terrain_type) {
	...
	for (i = 0; i < num_bounds; i++) {
		r = (DBV(bound->variable_polygons[i].middle_x,
				...
				A_l2g +
			R;
		if (bound->variable_polygons[i].check_terrain_type(
				x_offset, y_offset, round(r.z), terrain_type
			))
			return 1;
	}
	return 0;
...
void Object::precise_impulse(Vector source_point, int x_dest, int y_dest) {
...

	{
		...
		N = -(t1 +
				Lambert_W(
					-exp(-(dz * t2 + t4 - 2.0 * dz * k + gdt2 + dz - v0dt) * t1 * t8 * t10) * t1 *
					(gdt2 * k + t4 - v0dt) * t8 / k * t10
				) +
				t23 * t24 * t2 + t23 * t27 * k - 2.0 * t23 * t24 * k + t1 * t10 + t23 * t24 -
				t23 * t27) /
			t1;
	}
...
int obb_disjoint(
	...
	r &= (t <= (ax + bx * Bf[0] + by * Bf[1] + bz * Bf[2]));
	if (!r)
		return 1;
	...
	r &= (t <= (bx + ax * Bf[0] + ay * Bf[3] + az * Bf[6]));
	if (!r)
		return 2;
	...
	r &= (t <= (ay + bx * Bf[3] + by * Bf[4] + bz * Bf[5]));
	if (!r)
		return 3;
	...
	r &= (t <= (az + bx * Bf[6] + by * Bf[7] + bz * Bf[8]));
	if (!r)
		return 4;
	...
	r &= (t <= (by + ax * Bf[1] + ay * Bf[4] + az * Bf[7]));
	if (!r)
		return 5;
	...
	r &= (t <= (bz + ax * Bf[2] + ay * Bf[5] + az * Bf[8]));
	if (!r)
		return 6;
	...
	r &= (t <= (ay * Bf[6] + az * Bf[3] + by * Bf[2] + bz * Bf[1]));
	if (!r)
		return 7;
	...
	r &= (t <= (ay * Bf[7] + az * Bf[4] + bx * Bf[2] + bz * Bf[0]));
	if (!r)
		return 8;
	...
	r &= (t <= (ay * Bf[8] + az * Bf[5] + bx * Bf[1] + by * Bf[0]));
	if (!r)
		return 9;
	...
	r &= (t <= (ax * Bf[6] + az * Bf[0] + by * Bf[5] + bz * Bf[4]));
	if (!r)
		return 10;
	...
	r &= (t <= (ax * Bf[7] + az * Bf[1] + bx * Bf[5] + bz * Bf[3]));
	if (!r)
		return 11;
	...
	r &= (t <= (ax * Bf[8] + az * Bf[2] + bx * Bf[4] + by * Bf[3]));
	if (!r)
		return 12;
	...
	r &= (t <= (ax * Bf[3] + ay * Bf[0] + by * Bf[8] + bz * Bf[7]));
	if (!r)
		return 13;
	...
	r &= (t <= (ax * Bf[4] + ay * Bf[1] + bx * Bf[8] + bz * Bf[6]));
	if (!r)
		return 14;
	...
	r &= (t <= (ax * Bf[5] + ay * Bf[2] + bx * Bf[7] + by * Bf[6]));
	if (!r)
		return 15;
...
int Object::test_object_to_baseobject(BaseObject *bobj) {
	if (sqr(getDistX(bobj->R_curr.x, R_curr.x)) + sqr(getDistY(bobj->R_curr.y, R_curr.y)) +
			sqr(bobj->R_curr.z - R_curr.z) <
		sqr(radius + bobj->radius)) {
		Object *obj = (Object *)bobj;
		if ((ID | obj->ID) & SPHERICAL_TEST) {
			if ((ID == ID_INSECT || (ID == ID_VANGER && old_appearance_storage)) &&
				obj->ID == ID_VANGER) {
			...
			if ((obj->ID == ID_INSECT || (obj->ID == ID_VANGER && obj->old_appearance_storage)) &&
				ID == ID_VANGER) {
		...
		// if(analysis_off || obj -> analysis_off || (after_db_coll > 0 && obj -> after_db_coll >
		// 0))
		...
		DBV R1 = box_to_box_intersection(
			RR, A_t2o * DBV(x2, 0, 0), A_t2o * DBV(0, y2, 0), A_t2o * DBV(0, 0, z2), x1, y1, z1
		);
		DBV R2 = R1 - RR;
		double m1 = m;
		double m2 = obj->m;
		...
		u0 = normal * (u0 * normal);
		DBM K = calc_collision_matrix(R1, J_inv) / m1 +
				calc_collision_matrix(R2, A_o2t * obj->J_inv * A_t2o) / m2;
		DBV P = (K.inverse() * u0) * (-2);

		// double df = fmina(fmina(R1.x - x1,R1.y - y1,R1.z - z1),fmina(R1.x + x1,R1.y + y1,R1.z +
		// z1))*k_elastic_db_coll; P -= RR.get_norm(df);
		...
		double k_size =
			sqr(obj->scale_size * original_scale_size / (obj->original_scale_size * scale_size));
...
void Object::Explosion(int m_TNT, int m_id) {
	...
	for (i = 0; i < n_debris; i++) {
		...
		*obj = *this;
		...
		obj->R = R + A_l2g * r;
		obj->V =
			DBV(random(2 * m_TNT) - m_TNT, random(2 * m_TNT) - m_TNT, random(2 * m_TNT) - m_TNT) *
			V_explosion;
		obj->W =
			DBV((double)random(256) * W_explosion,
				(double)random(256) * W_explosion,
				(double)random(256) * W_explosion);
		obj->update_coord();
...
void Object::Send(void) {
	...
	events_out < pack_short(V.x, V_pack_min, V_pack_max) < pack_short(V.y, V_pack_min, V_pack_max) <
		pack_short(V.z, V_pack_min, V_pack_max);
	events_out < pack_short(W.x, W_pack_min, W_pack_max) < pack_short(W.y, W_pack_min, W_pack_max) <
		pack_short(W.z, W_pack_min, W_pack_max);

	Quaternion q(DBM(1, -1, 1, DIAGONAL) * A_l2g);
	events_out < pack_short(q.x, Q_pack_min, Q_pack_max) < pack_short(q.y, Q_pack_min, Q_pack_max) <
		pack_short(q.z, Q_pack_min, Q_pack_max);

	/*
		fout < "V: " <= V.y < "\t" <=
	   unpack_short(pack_short(V.y,V_pack_min,V_pack_max),V_pack_min,V_pack_max) < "\n"; fout < "W:
	   " <= W.y < "\t" <= unpack_short(pack_short(W.y,W_pack_min,W_pack_max),W_pack_min,W_pack_max)
	   < "\n"; fout < "Q: " <= q.y < "\t" <=
	   unpack_short(pack_short(q.y,Q_pack_min,Q_pack_max),Q_pack_min,Q_pack_max) < "\n";
	*/
}
...
DBV interpolation(double t, double x[3], DBV y[3]) {
	...
	DBV A = (y[1] * x[2] - y[1] * x[0] - t3 + x[1] * y[0] - y[0] * x[2] + x[0] * y[2]) /
			(t8 * x[2] - t8 * x[0] - t12 + x[1] * t13 - t13 * x[2] + x[0] * t11);
...
DBV interpolation_factors(double t, double x[3]) {
	...
	return DBV(
		(-t21 + t19) * t7 * t13 + (-(t3 - t20) * t7 * t13 + (x[1] - x[2]) * t25 * t) * t,
