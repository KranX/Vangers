...
void Object::draw() {
	...
	Refl = A_g2l * Light;
	View = Vector(
		round(A_g2l[2] * ModulatedNORMAL),
		round(A_g2l[5] * ModulatedNORMAL),
		round(A_g2l[8] * ModulatedNORMAL)
	);
	...
	A_convert[4] = A_scl[4];
	A_convert[5] = A_scl[5];
	A_convert[6] = A_l2g[6] * scale_real;
	A_convert[7] = A_l2g[7] * scale_real;
	A_convert[8] = A_l2g[8] * scale_real;
	...
	if (ID & ID_VANGER) {
		...
		if (model == models)
			...
			for (i = 0; i < n_wheels; i++)
				if (wheels[i].steer)
					wheels[i].model.draw_child(
						Vector(wheels[i].model.x_off, wheels[i].model.y_off, wheels[i].model.z_off),
						A_c2p,
						transpose(A_c2p)
					);
	}
	...
	if (draw_mode == NORMAL_DRAW_MODE) {
		if (ID != ID_INSECT)
			...
			if (!DepthShow)
				DrawLinear(
					R_scr.x - draw_offset + vx,
					R_scr.y - draw_offset + vy,
					draw_size,
					draw_shift,
					draw_buffer,
					0
...
//		dynamic_state |= TOUCH_OF_AIR;
	...
	//		XGR_SetPixel(R_scr.x + (A_scl*R_slots[i]).x, R_scr.y +
	//(A_scl*R_slots[i]).y,slot_color[i]);
#endif
}
...
void Object::pixel_draw() {
	...
	if (DepthShow) {
		...
		Draw3DPlane(
			R_scr.x - draw_offset, R_scr.y - draw_offset, draw_size, draw_shift, draw_buffer, 1, 0
		);
	} else {
		double dz = (double)trace_shadow(R_curr.x, R_curr.y, R_curr.z) * ScaleMapInvFlt;
		int vx = -round(A_g2s[0] * dz);
		...
		DrawLinear(
			R_scr.x - draw_offset, R_scr.y - draw_offset, draw_size, draw_shift, draw_buffer, 1
		);
	}
...
void Polygon::draw_pixel() {
	...
	unsigned c =
		color_offset +
		(((unsigned)normals[0]->I_8 + normals[1]->I_8 + normals[2]->I_8) >> color_shift) / 3;
	unsigned char *draw_vp = draw_buffer + (((((y + CentrY) << draw_shift) + x + CentrX) << 1) &
											   (2 * DRAW_SX * DRAW_SY - 1));
	if (z > *draw_vp) {
		*draw_vp = z;
		*(draw_vp + 1) = c;
...
inline void image_line(int len, int fx, int fy) {
	...
	while (len-- > 0) {
		...
		if (linePtr != nullptr && (dz = *draw_dbuf) != 0) {
			...
			} else if ((type = *(mbuf + H_SIZE - 1)) & DOUBLE_LEVEL)
				draw_state |= (type & (7 << 3)) != 0 || z_low_level_water + dz > 0 ? TOUCH_OF_AIR
																				   : TOUCH_OF_WATER;
		}
		draw_vbuf++;
		draw_dbuf += 2;
...
#	endif
	...
	if (change_log) {
		...
		regRender(
			R_curr.x - draw_offset,
			R_curr.y - draw_offset,
			R_curr.x + draw_offset,
			R_curr.y + draw_offset
		);
	}
			...
			for (x = x0; x < x1; x += 2, dbuf += 4) {
				if ((z = *dbuf) != 0) {
					...
					if (!(type & DOUBLE_LEVEL)) { // type == (1<<3) == 8 это слой с тенью
						if (z < *p) {
							// std::cout<<"ground_pressing p1 z:"<<z<<" p:"<<(int)*p<<"
...
void cast_arbitrary_shadow_line(
	int x_bmp_16,
	int y_bmp_16,
	int xg0,
	int zg0,
...
void CastArbitraryShadow(int x, int y, int zg, int size, int shift, unsigned char *buffer) {
	...
	for (s = 1; s < 4; s++) {
		...
		for (; t < tmax; t++) {
			...
			if (draw_mbuf) {
				if (xy_swap) {
					cast_arbitrary_shadow_line(
						y0, x0, xg >> 16, zg, bmp_len >> 16, screen_len >> 16, screen_skip >> 16
					);
				} else {
					cast_arbitrary_shadow_line(
						x0, y0, xg >> 16, zg, bmp_len >> 16, screen_len >> 16, screen_skip >> 16
					);
				}
			}
			x0 += dx0;
		...
		if (t == tE) {
			screen_len =
				round((double)(x_bmp_corner_16 + x0 - (XS0 << 16)) * (double)d_1_x_s / 65536.);
			d_screen_len = 0;
			tE = -INT_INF;
			s--;
			if (t == tF) {
				screen_skip =
					round((double)(y_bmp_corner_16 + y0 - (YS1 << 16)) * (double)d_1_y_s / 65536.);
				d_screen_skip = d_1_y;
				tF = -INT_INF;
			}
			continue;
