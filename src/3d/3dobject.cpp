...
void Model::loadC3Dvariable(XBuffer &buf) {
...
#endif
			buf > vertices[i].x_8 > vertices[i].y_8 > vertices[i].z_8 > sort_info;
		}
	} else {
		int ti;
		for (i = 0; i < num_vert; i++) {
	...
	for (i = 0; i < num_norm; i++)
		buf > normals[i].x > normals[i].y > normals[i].z > normals[i].n_power > sort_info;

	for (i = 0; i < num_poly; i++) {
		buf > num > sort_info > color_id > color_shift > variable_polygons[i].flat_normal.x >
			variable_polygons[i].flat_normal.y > variable_polygons[i].flat_normal.z >
			variable_polygons[i].flat_normal.n_power > variable_polygons[i].middle_x >
			variable_polygons[i].middle_y > variable_polygons[i].middle_z;

		variable_polygons[i].color_id = static_cast<unsigned char>(
			color_id < static_cast<unsigned int>(COLORS_IDS::MAX_COLORS_IDS)
				? color_id
				: static_cast<unsigned int>(COLORS_IDS::BODY)
		);
...
void Model::loadC3D(XBuffer &buf) {
	...
	buf > rmax;
	buf > phi > psi > tetta;
	/*std::cout<<"num_vert:"<<num_vert<<" num_norm:"<<num_norm<<" num_poly:"<<num_poly<<"
	num_vert_total:"<<num_vert_total<<std::endl; std::cout<<"xmax:"<<xmax<<" ymax:"<<ymax<<"
	zmax:"<<zmax<<std::endl; std::cout<<"xmin:"<<xmin<<" ymin:"<<ymin<<" zmin:"<<zmin<<std::endl;
	...
	size = num_vert * static_cast<int>(sizeof(Vertex)) +
		   num_norm * static_cast<int>(sizeof(Normal)) +
		   num_poly * static_cast<int>(sizeof(Polygon));
	...
	for (i = 0; i < num_poly; i++) {
		buf > num > sort_info > color_id > color_shift > skip_char > skip_char > skip_char >
			skip_char > polygons[i].middle_x > polygons[i].middle_y > polygons[i].middle_z;
...
void Object::convert_to_beeb(Object *beeb) {
	if (beeb) {
		...
		*old_appearance_storage = *this;
		...
		memcpy(
			&n_models,
			&old_appearance_storage->n_models,
			(char *)&old_appearance_storage - (char *)&n_models
		);
...
void Object::loadM3D(char *name) {
	...
	for (i = 0; i < n_wheels; i++) {
		buf > wheels[i].steer > wheels[i].r > wheels[i].width > wheels[i].radius >
			wheels[i].bound_index;
		if (wheels[i].steer)
...
void Object::loadA3D(char *name) {
	...
	buf > body_color_offset > body_color_shift;
	// std::cout<<"n_models:"<<n_models<<" xmax:"<<xmax<<" ymax:"<<ymax<<" zmax:"<<zmax<<"
	// rmax:"<<rmax<<std::endl; std::cout<<"body_color_offset:"<<body_color_offset<<"
	// body_color_shift:"<<body_color_shift<<std::endl; ErrH.Abort(name, n_models);
