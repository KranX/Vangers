...
#define SOUND_START_MOTOR() \
	if (EngineNoise)        \
		StartEFFECT(EFF_DRIVING, 1);
#define SOUND_STOP_MOTOR() \
	if (EngineNoise)       \
		StopEFFECT(EFF_DRIVING);
#define SOUND_MOTOR_PARAMETER(speed) \
	if (EngineNoise)                 \
		SetMotorSound(speed);
...
#define SOUND_GLOBAL() \
	StartEFFECT((EFFECT_VALUE)((int)EFF_BACKGROUND_0 + realRND(sndBackgroundMax)));
...
#define SOUND_EXPLOSION(p)                       \
	{                                            \
		StartEFFECT(EFF_MECHOS_EXPLOSION, 0, p); \
	}
#define SOUND_BACK_EXPLOSION(p)                \
	{                                          \
		StartEFFECT(EFF_BACK_EXPLOSION, 0, p); \
	}
#define SOUND_EXPLOSION_CRUSTER(p)                \
	{                                             \
		StartEFFECT(EFF_CRUSTER_EXPLOSION, 0, p); \
		StartEFFECT(EFF_BACK_EXPLOSION, 0, p);    \
	}
...
#define SOUND_SWAMP(p)                \
	{                                 \
		StartEFFECT(EFF_SWAMP, 0, p); \
	}
#define SOUND_TEAR(p)                \
	{                                \
		StartEFFECT(EFF_TEAR, 0, p); \
	}
