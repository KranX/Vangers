...
static const char *SndMotorFileName[7 * 2] = {"raffa",
	"raffa2",
	"light",
	"light2",
	"microbus",
...
static SndParameters SndData[EFF_MAX] = {{0, 1, 255, 0, LOCAL_SOUND, "", NULL},
...
void StartEFFECT(EFFECT_VALUE EffectValue, int loop, int pan) {
	if (!EffectInUse || !SndData[EffectValue].lpDSB)
		return;
	...
	if (EngineNoise && EffectValue == EFF_DRIVING)
	...
	SoundPlay(
		SndData[EffectValue].lpDSB,
		SndData[EffectValue].channel,
		SndData[EffectValue].priority,
		SndData[EffectValue].cropos,
		flag
	);
}
...
void StopEFFECT(EFFECT_VALUE EffectValue) {
	if (!EffectInUse)
		return;
	SoundStop(SndData[EffectValue].channel);
	if (EngineNoise && EffectValue == EFF_DRIVING && SndData[EFF_STOP].lpDSB)
		StartEFFECT(EFF_STOP);
}
...
void SetSoundVolume(int value) {
	...
	if (EffectInUsePriory) {
		...
		// soundVolumecurrent = soundVolume = DB_MIN +
		// round(log10(9.0*log(double(value))/(log(2.0)*8) + 1.0)*DB_SIZE);
	}
}
