...
EXTERN double GAME_TIME_COEFF; // speed correction coefficient, use for static parameters only, like
							   // bullet speed, animation speed...
...
struct BaseRTO: XRuntimeObject {
	...
	void SetNext(int id) {
		NextID = id;
		IsMainMenu = id == RTO_MAIN_MENU_ID;
	}
	void SetFlag(int fl) {
		flags |= fl;
	}
...
struct GameQuantRTO: BaseRTO {
	...
	GameQuantRTO(int timer) {
		ID = RTO_GAME_QUANT_ID;
		Timer = timer;
	}
};
...
struct FirstEscaveOutRTO: BaseRTO {
	...
	FirstEscaveOutRTO(void) {
		ID = RTO_FIRST_ESCAVE_OUT_ID;
		Timer = RTO_ISCREEN_TIMER;
	}
};
...
struct ShowImageRTO: BaseRTO {
	...
	void SetName(const char *p, int id = 0) {
		strcpy(fileNames[id], p);
		count = 0;
		Flags[id] = 0;
	}
...
struct PaletteTransformRTO: BaseRTO {
	...
	void Init(int);
	void Finit();
	PaletteTransformRTO(void) {
		ID = RTO_PALETTE_TRANSFORM_ID;
		Timer = RTO_PALETTE_TIMER;
	}
};
