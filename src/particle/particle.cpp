...
void Particle::process(ParticleProcess *proc) {
	...
	if (LU)
		...
		proc->AddNear(X - 1, Y - 1, this, 0);
	if (U)
		NewValue += U->Value;
	else
		proc->AddNear(X, Y - 1, this, 1);
	if (RU)
		NewValue += RU->Value;
	else
		proc->AddNear(X + 1, Y - 1, this, 2);
	if (L)
		NewValue += L->Value;
	else
		proc->AddNear(X - 1, Y, this, 3);

	if (R)
		NewValue += R->Value;
	else
		proc->AddNear(X + 1, Y, this, 4);
	if (LD)
		NewValue += LD->Value;
	else
		proc->AddNear(X - 1, Y + 1, this, 5);
	if (D)
		NewValue += D->Value;
	else
		proc->AddNear(X, Y + 1, this, 6);
	if (RD)
		NewValue += RD->Value;
	else
		proc->AddNear(X + 1, Y + 1, this, 7);
}
		...
		if (*BackGround) {
			...
			for (int j = 0; j < PARTICLE_SIZE; j++)
				if (vMap->lineT[YCYCL(_Y + j)])
					memcpy(
						vMap->lineTcolor[YCYCL(_Y + j)] + _X, BackGround + (j << 2), PARTICLE_SIZE
					);
...
void Particle::valueDecrease() {
	...
	case 1:
		if (U)
			Value = ((3 * U->NewValue >> 3) + Value) >> 2;
		break;
	case 2:
		break;
	case 3:
		if (L)
			Value = ((3 * L->NewValue >> 3) + Value) >> 2;
		break;
	case 4:
		break;
	case 5:
		if (R)
			Value = ((3 * R->NewValue >> 3) + Value) >> 2;
		break;
	case 6:
		break;
	case 7:
		if (D)
			Value = ((3 * D->NewValue >> 3) + Value) >> 2;
		break;
	case 8:
		break;
	}
...
void ParticlePaletteTableInit(char unsigned *pal) {
	...
	for (i = 0; i < 256; i++) {
		...
		//		int I = ((19595*(int)pal[3*i] + 38470*(int)pal[3*i + 1] + 7471*(int)pal[3*i +
		// 2])>>16)/3;
		for (int j = 0; j < 256; j++) {
			int P = I >> 1;
	...
	for (i = BEGCOLOR[k]; i <= ENDCOLOR[k]; i++) {
		for (int j = 0; j < 8; j++) {
			ParticlePaletteTableDust[i + (j << 8)] =
				(i * (8 - j) + (j * ParticlePaletteTableDust[i + (j << 8)])) >> 3;
	...
	for (i = 0; i < TERRAIN_MAX; i++) {
		for (j = 0; j < 256; j++) {
			for (k = BEGCOLOR[i]; k <= ENDCOLOR[i]; k++) {
				...
				if (j < 8) {
					ParticlePaletteTableSmog[k + (j << 8)] =
						(k * (8 - j) + (j * ParticlePaletteTableSmog[k + (j << 8)])) >> 3;
				}
			} //  end for k
		} //  end for j
	} //  end fro i
	...
	for( i = 0; i < TERRAIN_MAX; i++){
		for( j = 0; j < 256; j++){
			...
			for( k = BEGCOLOR[i]; k <= ENDCOLOR[i]; k++){
				...
				if ( j < 8)
					//ParticlePaletteTable[k + (j << 8)] = ((((k - BEGCOLOR[i])>>1) +
BEGCOLOR[i])*(8-j) + (j*ParticlePaletteTable[k + (j << 8)]))>>3; ParticlePaletteTableSmog[k + (j <<
8)] = (k*(8-j) + (j*ParticlePaletteTableSmog[k + (j << 8)]))>>3;

				//if( ParticlePaletteTable[k + (j << 8)] > ENDCOLOR[i] ) ParticlePaletteTable[k + (j
<< 8)] = ENDCOLOR[i];
				//I++;
			}//  end for k
		}//  end for j
