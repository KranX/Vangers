...
int LightPoint::quant(void) {
	...
	x = xn;
	y = yn;
	int pulseChangeCount =
		(int)round(10 * GAME_TIME_COEFF); // get proper frame count for time pulsation
	...
	// std::cout<<"LightPoint::quant type:"<<type<<" R:"<<R<<" _R:"<<_R<<" E:"<<E<<"
				...
				while (dimR[j] - (R - i) > 0) {
					...
					if ((*llt) < _z) {
						...
						if (rr >= 0) {
							...
							if (light_table_pal_size <= first_shift || first_shift < 0) {
								std::cout << "LightPoint::quant wrong light_table_pal position"
										  << std::endl;
								break;
							}
							// std::cout<<"rr*rr:"<<rr*rr<<"_R:"<<_R<<" ee:"<<ee<<"A:"<<(ee << 8) +
							// llct[_x]<<" llct[_x]:"<<(long)llct[_x]<<" _x:"<<_x<<"
							// pal:"<<&light_table_pal<<std::endl;
					...
					if ((*llt1) < _z) {
						...
						if (rr >= 0) {
							ee = rr * rr;
							ee = (ee * _R) >> 16;
							// std::cout<<"B:"<<(ee << 8) + llct[_x]<<"
							// llct[_x]:"<<(long)llct[_x]<<" _x:"<<_x<<"
							// pal:"<<&light_table_pal<<std::endl;
	...
	for (i = 0; i < TERRAIN_MAX; i++) {
		for (j = 0; j < LIGHT_MAX_BRIGHTNESS; j++) {
			for (k = BEGCOLOR[i]; k <= ENDCOLOR[i]; k++) {
				...
				if (light_table_pal[k + (j << 8)] > ENDCOLOR[i])
					light_table_pal[k + (j << 8)] = ENDCOLOR[i];
			} //  end for k
		} //  end for j
	} //  end fro i
	...
	for (i = 0; i < TERRAIN_MAX; i++) {
		for (j = 0; j < 16; j++) {
			for (k = BEGCOLOR[i]; k <= ENDCOLOR[i]; k++) {
				...
				if (invisible_table[k + (j << 8)] > ENDCOLOR[i])
					invisible_table[k + (j << 8)] = ENDCOLOR[i];
				if (invisible_table[k + (j << 8)] < BEGCOLOR[i])
					invisible_table[k + (j << 8)] = BEGCOLOR[i];
			} //  end for k
		} //  end for j
	} //  end fro i
}
...
void LightPoint::CreateLight(int _r, int _e, int _t) {
	Status = 0;
	// std::cout<<"LightPoint::CreateLight _r:"<<_r<<" _e:"<<_e<<" _t:"<<_t<<"
	// light_table_pal:"<<(long long)light_table_pal<<std::endl; Activate Function
	...
	if (_e >= LIGHT_MAX_BRIGHTNESS)
		...
		energy = (_e) << 15;
	d_energy =
		(int)round(energy / 5 / GAME_TIME_COEFF); // recalculate dynamic energy for current FPS
	c_time = 0;
};
