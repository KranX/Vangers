...
void processParticleMapLineWithMask(
	unsigned char *&f1p,
	unsigned char *&f2p,
	int Xmax,
	TABLE_TYPE *&moff
...
void ParticleMapProcess::init_flame(unsigned char *f, int fade) {
	...
	for (i = 0; i < CurrNhotSpots; i++) {
		...
		case 2:
			tmp = random(
				4 * (HotSpotRadius / 2 - abs(r - HotSpotRadius / 2)) * WHITE_POINT / HotSpotRadius
			);
			break;
		case 3:
...
int ParticleMapProcess::process(
	char *vb,
	int Xc,
	int Yc,
	int XcS,
...
void ParticleMapProcess::activate(int type) {
	...
	activate(
		...
		Type->FadeTime,
		Type->FadePower,
		Type->DeviationRadius,
		Type->DeviationPeriod,
		Type->ProcedureType,
		Type->HotCentersRadius,
		type
	);
}
void ParticleMapProcess::activate(
...
void ParticleMapProcess::finitMask(void) {
	...
	if (!ParticleMapProcessMaskArray)
		return;
...
void processParticleMapLineWithMask(unsigned char*& f1p,unsigned char*& f2p,int Xmax,TABLE_TYPE*&
moff)
{
	int x;
	for(x = 0;x < Xmax;x++){
