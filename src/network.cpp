...
#	define OUT_EVENTS_LOG1(id, code)                                                         \
		{                                                                                     \
			fout < "< [" <= (GLOBAL_CLOCK() >> 8) < ":" <= frame < "]" < #id < ": " <= code < \
				"\n";                                                                         \
		}
#else
#	define IN_EVENTS_LOG(id)
...
static const char *network_event_name(int event_ID) {
	...
	case GAMES_LIST_QUERY:
		return "GAMES_LIST_QUERY";
	case TOP_LIST_QUERY:
		return "TOP_LIST_QUERY";
	case ATTACH_TO_GAME:
	...
	case WORLD_SNAPSHOT_BEGIN:
		return "WORLD_SNAPSHOT_BEGIN";
	case WORLD_SNAPSHOT_END:
		return "WORLD_SNAPSHOT_END";
	case ITEM_STATE:
		return "ITEM_STATE";
	case ITEM_REMOVED:
		return "ITEM_REMOVED";
	case zSERVER_VERSION_RESPONSE:
		return "zSERVER_VERSION_RESPONSE";
	case zGAME_DATA_RESPONSE:
		return "zGAME_DATA_RESPONSE";
	case zTIME_RESPONSE:
		return "zTIME_RESPONSE";
	case zCREATE_OBJECT_BY_CLIENT:
		return "zCREATE_OBJECT_BY_CLIENT";
	case zCREATE_OBJECT_BY_SERVER:
		return "zCREATE_OBJECT_BY_SERVER";
	default:
...
static void network_log_vprintf(const char *tag, const char *fmt, va_list args) {
	...
	if (network_log_last_world != CurrentWorld) {
		fprintf(
			network_log_file,
			"[%s tick=%d frame=%d world=%d station=%d] CONTEXT CurrentWorld changed old=%d "
			"new=%d\n",
			ts,
			(int)GLOBAL_CLOCK(),
			frame,
	...
	fprintf(
		network_log_file,
		"[%s tick=%d frame=%d world=%d station=%d] %s ",
		ts,
		(int)GLOBAL_CLOCK(),
		frame,
		CurrentWorld,
...
static void network_log_open(const ServerFindChain *p, const char *reason) {
	...
	network_log_printf(
		"client_start",
		"reason=%s build_hash=%s branch=%s protocol_version=%d/%d zserver_version=%d "
		"fps_mode=%.0ffps fps_coeff=%.3f resolution=%dx%d server_address=%s game_id=%d "
		"player_name=\"%s\" GlobalStationID=%d CurrentWorld=%d",
...
void network_log_object_event(
	...
	network_log_printf(
		direction ? direction : "OBJ",
		"%s id=0x%08X id_dec=%d station=%d world=%d type=%s type_id=%d counter=%d is_global=%d "
		"is_private=%d is_players_object=%d is_static=%d creator=%d time=%d x=%d y=%d radius=%d "
		"body_size=%d current_world=%d decision=%s",
		network_event_name(event),
...
void network_log_item_event(
	...
	network_log_printf(
		tag ? tag : "ITEM",
		"NetID=0x%08X NetID_dec=%d NetDeviceID=0x%08X NetDeviceID_dec=%d NetOwner=0x%08X "
		"NetOwner_dec=%d OwnerNetID=0x%08X OwnerNetID_dec=%d DataID=%d ActIntBuffer.type=%d "
		"ActIntBuffer.data0=%d ActIntBuffer.data1=%d CreateMode=%d OutFlag=%d Status=0x%X "
		"Visibility=%d current_world=%d %s",
		(unsigned int)NetID,
		NetID,
...
static void network_snapshot_reset(const char *reason) {
	if (network_snapshot_state != NETWORK_SNAPSHOT_IDLE || network_snapshot_world != -1) {
		network_log_printf(
			"SNAPSHOT",
			"state=%s world=%d decision=reset reason=%s",
...
static void network_snapshot_wait_for_world(int world, const char *reason) {
	...
	network_log_printf(
		"SNAPSHOT",
		"state=%s world=%d decision=wait_for_begin reason=%s",
		network_snapshot_state_name(network_snapshot_state),
		network_snapshot_world,
		reason ? reason : "unknown"
	);
...
static void network_snapshot_begin(int world) {
	if (world != CurrentWorld) {
		network_log_printf(
			"SNAPSHOT",
			"state=%s expected_world=%d marker_world=%d current_world=%d "
			"decision=ignored_wrong_world marker=begin",
			network_snapshot_state_name(network_snapshot_state),
			network_snapshot_world,
			world,
			CurrentWorld
	...
	if (network_snapshot_state == NETWORK_SNAPSHOT_LOADING) {
		network_log_printf(
			"SNAPSHOT",
			"state=%s world=%d marker_world=%d decision=duplicate_begin",
			network_snapshot_state_name(network_snapshot_state),
			network_snapshot_world,
			world
		);
	...
	if (network_snapshot_state == NETWORK_SNAPSHOT_IDLE) {
		network_log_printf(
			"SNAPSHOT",
			"state=%s world=%d marker_world=%d decision=unexpected_begin_but_accept",
			network_snapshot_state_name(network_snapshot_state),
			network_snapshot_world,
			world
		);
	...
	if (network_snapshot_state == NETWORK_SNAPSHOT_WAIT_BEGIN && network_snapshot_world != world) {
		network_log_printf(
			"SNAPSHOT",
			"state=%s expected_world=%d marker_world=%d current_world=%d "
			"decision=unexpected_begin_world_but_accept_current",
			network_snapshot_state_name(network_snapshot_state),
			network_snapshot_world,
			world,
			CurrentWorld
	...
	network_log_printf(
		"SNAPSHOT",
		"state=%s world=%d decision=begin",
		network_snapshot_state_name(network_snapshot_state),
		network_snapshot_world
	);
}
...
static void network_snapshot_end(int world) {
	if (network_snapshot_state == NETWORK_SNAPSHOT_IDLE) {
		network_log_printf(
			"SNAPSHOT",
			"state=%s world=%d marker_world=%d decision=unexpected_end_ignored",
			network_snapshot_state_name(network_snapshot_state),
			network_snapshot_world,
			world
		);
	...
	if (world != CurrentWorld ||
		(network_snapshot_world != -1 && network_snapshot_world != world)) {
		network_log_printf(
			"SNAPSHOT",
			"state=%s expected_world=%d marker_world=%d current_world=%d "
			"decision=unexpected_end_wrong_world",
			network_snapshot_state_name(network_snapshot_state),
			network_snapshot_world,
			world,
			CurrentWorld
	...
	network_log_printf(
		"SNAPSHOT",
		"state=%s world=%d marker_world=%d decision=end",
		network_snapshot_state_name(network_snapshot_state),
		network_snapshot_world,
		world
	);
...
static void network_snapshot_log_marker(int event_ID, int body_size, int marker_world) {
	network_log_printf(
		"IN",
		"%s body_size=%d marker_world=%d snapshot_state=%s snapshot_world=%d",
...
ServerFindChain::ServerFindChain(
	...
	if (!game_ID) {
		if (lang() == RUSSIAN) {
			// CP866 Новая игра на
			const unsigned char new_game_on[] = {0x8D,
				0xAE,
				0xA2,
				0xA0,
				0xEF,
	...
	if (!game_name)
		if (domain_name)
			...
			str_buf <= (IP & 0xff) < "." <= ((IP >> 8) & 0xff) < "." <= ((IP >> 16) & 0xff) < "." <=
				((IP >> 24) & 0xff);
	else
		str_buf < game_name;
...
int identification(XSocket &socket) {
	...
	if (!identificated) {
		...
		if (server_replied && !unexpected_response && server_protocol >= 0) {
			snprintf(
				...
				"Multiplayer protocol mismatch. The client expects protocol %d, but the server "
				"replied with protocol %d. Please update both the game client and the server.",
				SERVER_VERSION,
				server_protocol
			);
			...
			snprintf(
				error_message,
				sizeof(error_message),
				"The server rejected the multiplayer handshake. Please make sure both the game "
				"client and the server use protocol %d.",
				SERVER_VERSION
			);
		} else {
			snprintf(
				error_message,
				sizeof(error_message),
				"The server did not answer the multiplayer handshake. Please check the server "
				"address and make sure it uses protocol %d.",
				SERVER_VERSION
			);
		}
		std::cout << "Network:identificated is wrong! SV:" << SERVER_VERSION
				  << " SV2:" << server_protocol << " response:" << string << std::endl;
		socket.close();
	...
	// std::cout<<"Client auth: "<<(int)zevent_ID<<" "<<zSERVER_VERSION_RESPONSE<<"
	// "<<(int)zresponse<<" "<<zCLIENT_VERSION<<std::endl;
...
void OutputEventBuffer::begin_item_transfer(
	int transfer_type,
	int oldID,
	int newID,
	int x,
	...
	*this < newID < time < (unsigned short)x < (unsigned short)y < (unsigned short)radius;
	...
	network_log_printf(
		"OUT",
		"ITEM_TRANSFER transfer_type=%d old_id=0x%08X new_id=0x%08X x=%d y=%d radius=%d",
		transfer_type,
		(unsigned int)oldID,
		(unsigned int)newID,
		x,
...
void OutputEventBuffer::begin_create_z_object(int Type, int ID) {
	n_event++;
	pointer_to_size_of_event = tell();
	*this < short(0) < (unsigned char)zCREATE_OBJECT_BY_CLIENT < (unsigned int)GLOBAL_CLOCK() <
...
void OutputEventBuffer::set_world(int world, int y_size_of_world) {
	n_event++;
	*this < short(4) < (unsigned char)SET_WORLD < (unsigned char)world <
		(unsigned short)y_size_of_world;
	OUT_EVENTS_LOG1(SET_WORLD, world);
	network_log_printf(
		"OUT", "SET_WORLD requested_world=%d world_y_size=%d", world, y_size_of_world
	);
}
void OutputEventBuffer::set_position(int x, int y, int y_half_size_of_screen) {
	n_event++;
	*this < short(7) < (unsigned char)SET_POSITION < (unsigned short)x < (unsigned short)y <
		(unsigned short)y_half_size_of_screen;
	OUT_EVENTS_LOG1(SET_POSITION, y);
	network_log_printf(
		"OUT", "SET_POSITION x=%d y=%d y_half_size_of_screen=%d", x, y, y_half_size_of_screen
	);
	for (int i = pointer_to_the_first_event; i < pointer_to_the_last_event;
		i += full_event_size(i)) {
...
void OutputEventBuffer::register_name(char *name, char *password) {
	n_event++;
	*this < short(1 + strlen(name) + 1 + strlen(password) + 1) < (unsigned char)(REGISTER_NAME) <
		name < char(0) < password < char(0);
...
static void network_log_out_event_from_buffer(char *event_ptr) {
	...
	case ITEM_TRANSFER: {
		...
		network_log_printf(
			"OUT",
			"ITEM_TRANSFER transfer_type=%d old_id=0x%08X new_id=0x%08X old_time=%u new_time=%u "
			"x=%d y=%d radius=%d body_size=%d decision=queued",
			(int)transfer_type,
			(unsigned int)old_ID,
			(unsigned int)new_ID,
			old_time,
	...
	default:
		network_log_printf(
			"OUT",
			"%s body_size=%d raw_event=0x%02X",
			network_event_name(event),
			event_size - 1,
			event_ID
		);
...
void OutputEventBuffer::end_body() {
	...
	// std::cout<<"OutputEventBuffer::end_body size:"<<tell() - pointer_to_size_of_event -
	// sizeof(short int)<<std::endl;
	*(short *)(address() + pointer_to_size_of_event) =
		tell() - pointer_to_size_of_event - sizeof(short int);
	...
	if ((event_ID & (~ECHO_EVENT)) == UPDATE_OBJECT) {
		...
		for (i = pointer_to_the_first_event; i < pointer_to_the_last_event;
			i += full_event_size(i)) {
			...
			if ((ev_ID & (~ECHO_EVENT)) == UPDATE_OBJECT) {
				if (*((int *)(address() + i + 3)) == object_ID) {
					int event_size = full_event_size(i);
					memmove(address() + i, address() + i + event_size, tell() - (i + event_size));
					*(unsigned char *)(address() + pointer_to_size_of_event + 2) |=
						ev_ID & ECHO_EVENT;
					offset -= event_size;
					pointer_to_the_last_event -= event_size;
					n_event--;
...
#ifdef _FOUT_
	if (sent_size)
		fout < "Send: " <= SDL_GetTicks() < "\t" <= sent_size < "\t" <= unsent_size < "\t" <=
			n_event < "\n";
#endif
	if (!unsent_size) {
		...
		network_log_printf(
			"SOCKET_ERROR",
			"send_would_block sent_size=%d unsent_size=%d n_event=%d",
			sent_size,
			unsent_size,
			n_event
		);
...
int OutputEventBuffer::send_simple_query(
	int event,
	unsigned char data1,
	unsigned char data2,
	XSocket &sock
	...
	*this < short(3) < (unsigned char)event < data1 < data2;
	OUT_EVENTS_LOG1(send_simple_query, event);
	network_log_printf(
		"OUT", "%s data0=%d data1=%d body_size=2", network_event_name(event), (int)data1, (int)data2
	);
	return send(1, sock);
}
void OutputEventBuffer::set_player_body(PlayerBody &body) {
	...
	*this < short(1 + sizeof(PlayerBody)) < (unsigned char)SET_PLAYER_DATA;
	...
	network_log_printf(
		"OUT",
		"SET_PLAYER_DATA body_size=%d NetID=0x%08X world=%d color=%d",
		(int)sizeof(PlayerBody),
		(unsigned int)body.NetID,
		body.world,
		body.color
	);
}
...
int InputEventBuffer::receive(XSocket &sock, int dont_free) {
	...
	if (next_event_pointer != tell()) {
		XBuffer str;
		str < "Connection's problems: " <= event_ID < "  " <= object_ID < "  " <=
	...
	if (!add_size && !sock) {
		network_log_printf(
			"SOCKET_ERROR",
			"receive_failed filled_size=%u next_event_pointer=%u",
			filled_size,
			next_event_pointer
		);
		network_log_printf(
			"SOCKET_CLOSED",
			"reason=receive_failed filled_size=%u next_event_pointer=%u",
			filled_size,
			next_event_pointer
		);
	}
...
int InputEventBuffer::receive_waiting_for_event(int event, XSocket &sock, int skip_if_aint) {
	...
	if (!sock) {
		network_log_printf(
			"WAIT",
			"receive_waiting_for_event event=%s result=socket_closed",
			network_event_name(event)
		);
		if (!skip_if_aint) {
			if (lang() == RUSSIAN) {
	...
	for (;;) {
		while (current_event()) {
			// std::cout<<"current_event:"<<(int)current_event()<<" clock:"<<SDL_GetTicks()<<"
			// _end_time_:"<<_end_time_<<std::endl;
	...
	if (!skip_if_aint) {
		network_log_printf(
			"WAIT", "receive_waiting_for_event event=%s result=timeout", network_event_name(event)
		);
		if (lang() == RUSSIAN) {
			ErrH.Abort("Сервер не отвечает", XERR_USER, event);
		} else {
...
int InputEventBuffer::next_event() {
	...
	// std::cout<<"InputEventBuffer::next_event event_ID:"<<(int)event_ID<<"
	// client_ID:"<<(int)client_ID<<" next_event_pointer:"<<next_event_pointer
	...
	if (next_event_pointer != tell()) {
		XBuffer str;
		str < "Connection's problems: " <= prev_event_ID < "  " <= object_ID < "  " <=
			next_event_pointer - tell();
		ErrH.Abort(str.GetBuf(), XERR_USER);
	}
		...
		*this > object_ID;
		...
		if (network_snapshot_should_ignore_before_begin()) {
			network_log_object_event(
				...
				client_ID,
				time,
				event_ID == UPDATE_OBJECT ? x : -1,
				event_ID == UPDATE_OBJECT ? y : -1,
				-1,
				body_size,
				"ignored_before_snapshot_begin"
			);
			ignore_event();
		}
		...
		case SERVER_TIME:
			...
			*this > time;
			...
			body_size = 0;
			network_log_printf(
				"IN", "SERVER_TIME server_time=%d average_lag=%d", time, average_lag
			);
			break;
		case ATTACH_TO_GAME_RESPONSE:
		case GAME_DATA_RESPONSE:
		...
		case TOTAL_LIST_OF_PLAYERS_DATA:
			...
			if (network_snapshot_waiting_for_begin()) {
				network_log_printf(
					"SNAPSHOT",
					"event=%s body_size=%d state=%s snapshot_world=%d "
					"decision=ignored_before_snapshot_begin",
					network_event_name(event_ID),
					body_size,
					network_snapshot_state_name(network_snapshot_state),
					network_snapshot_world
		...
		case WORLD_SNAPSHOT_END: {
			...
			if (marker_world < 0) {
				network_log_printf(
					"SNAPSHOT",
					"state=%s expected_world=%d marker=%s decision=malformed_missing_world",
					network_snapshot_state_name(network_snapshot_state),
					network_snapshot_world,
					network_event_name(event_ID)
				);
		...
		case ITEM_STATE:
			...
			*this > y;
			...
			if (event_ID) {
				...
				network_log_printf(
					"IN",
					"ITEM_STATE state=%d previous_id=0x%08X item_id=0x%08X creator=%d time=%d x=%d "
					"y=%d body_size=%d decision=%s",
					(int)item_state,
					(unsigned int)item_previous_ID,
					(unsigned int)object_ID,
					(int)client_ID,
					time,
					(int)x,
		...
		case ITEM_REMOVED:
			...
			*this > item_removed_reason;
			...
			if (event_ID) {
				network_log_printf(
					"IN",
					"ITEM_REMOVED item_id=0x%08X paired_id=0x%08X reason=%d decision=%s",
					(unsigned int)object_ID,
		...
		case PLAYERS_RATING:
			network_log_printf(
				"IN", "%s body_size=%d", network_event_name(event_ID), event_size - 1
			);
			if (network_snapshot_waiting_for_begin()) {
				body_size = event_size - 1;
				network_log_printf(
		...
		case DIRECT_RECEIVING:
			*this > client_ID;
			network_log_printf(
				"IN", "DIRECT_RECEIVING creator=%d body_size=%d", client_ID, event_size - 2
			);
			if (get_byte()) {
				--*this;
...
void InputEventBuffer::ignore_event() {
	event_ID = 0;
	set(next_event_pointer);
	// std::cout<<"InputEventBuffer::ignore_event
	// next_event_pointer:"<<next_event_pointer<<std::endl;
}
/***********************************************************************
				Some utilites
***********************************************************************/
int connect_to_server(ServerFindChain *p) {
	...
	if (main_socket.is_open() && identification(main_socket)) {
		...
		network_log_printf(
			...
			"game_id=%d configured=%d game_birth_time=%u GlobalStationID=%d",
			current_server_addr.game_ID,
			current_server_addr.configured,
			game_birth_time_offset,
			GlobalStationID
		);
		...
		network_log_printf(
			"CONNECT_OK",
			"game_id=%d configured=%d GlobalStationID=%d CurrentWorld=%d",
			current_server_addr.game_ID,
			current_server_addr.configured,
			GlobalStationID,
			CurrentWorld
...
int restore_connection() {
	...
	if (!main_socket || !identification(main_socket)) {
		main_socket.close();
		network_log_printf(
			"SOCKET_CLOSED", "reason=reconnect_failed stage=connect_or_identification"
		);
		network_log_printf(
			"RECONNECT_FAIL",
			"stage=connect_or_identification attempts_left=%d",
			number_of_reconnection_attempt
		);
	...
	network_log_printf(
		"RECONNECT_OK",
		"game_id=%d GlobalStationID=%d CurrentWorld=%d",
		current_server_addr.game_ID,
		GlobalStationID,
		CurrentWorld
	);
...
void set_time_by_server(int n_measures) {
	...
	network_log_printf(
		"LAG",
		"set_time_by_server measures=%d average_lag=%d response_time=%.6f sigma=%.6f",
		n_measures,
		average_lag,
		response_time,
		time_synchronization_sigma
	);
}
int set_world(int world, int world_y_size) // znfo - send set_world event
{
	...
	if (got != SET_WORLD_RESPONSE) {
		network_log_printf(
			"SET_WORLD_RESPONSE",
			"requested_world=%d got=%s status=missing",
			world,
			network_event_name(got)
		);
		if (lang() == RUSSIAN) {
	...
	network_log_printf(
		"SET_WORLD_RESPONSE",
		"requested_world=%d response_world=%d status=%d",
		world,
		resp_world,
		resp_status
	);
	network_log_printf(
		"CONTEXT",
		"enable_transferring changed value=%d CurrentWorld=%d",
		enable_transferring,
		CurrentWorld
	);
...
void network_analysis(XBuffer &out, int integral) {
	...
	while (p) {
		out <= p->client_ID < "\ts:" <= p->status < "\tw:" <= p->world < "\tx:" <= p->x < "\ty:" <=
			p->y < "\tk:" <= p->body.kills < "\td:" <= p->body.deaths < "\t";
		if (p->name)
			out < p->name < "\t";
		out < "\n";
		p = p->next;
	...
	if (!integral) {
		...
		out < "Time synchronization, lag: " <= response_time < " Sec,  Sigma: " <=
			time_synchronization_sigma < " Sec\n";
		// out < "Average lag of events (C->S->C): " <= (double)delay_time/(delay_time_counter*256)
		// < " Sec\n";
		out < "Average lag of events (C->S->C): " <= average_lag < " mSec\n";
		out < "Send:  " <= round(n_sended_events / dt) < " EPS,  " <= round(n_sended_bytes / dt) <
			" BPS,  " <= round(n_sended_events / df) < " EPF,  " <= round(n_sended_bytes / df) <
			" BPF\n";
		out < "Receive: " <= round(n_received_events / dt) < " EPS,  " <=
			round(n_received_bytes / dt) < " BPS,  " <= round(n_received_events / df) < " EPF,  " <=
			round(n_received_bytes / df) < " BPF\n";
		...
		out < "Time synchronization, lag: " <= response_time < " Sec,  Sigma: " <=
			time_synchronization_sigma < " Sec\n";
		// out < "Average delay of events (C->S->C): " <=
		// (double)delay_time_global/(delay_time_counter_global*256) < " Sec\n";
		out < "Average lag of events (C->S->C): " <= average_lag < " mSec\n";
		out < "Send:   " <= n_sended_events_global / dt < " EPS,  " <= n_sended_bytes_global / dt <
			" BPS,  " <= n_sended_events_global / df < " EPF,  " <= n_sended_bytes_global / df <
			" BPF\n";
		out < "Receive: " <= n_received_events_global / dt < " EPS,  " <=
			n_received_bytes_global / dt < " BPS,  " <= n_received_events_global / df < " EPF,  " <=
			n_received_bytes_global / df < " BPF\n";
	...
	if (!network_clock_global) {
		...
		n_sended_events_global = n_sended_bytes_global = n_received_events_global =
			n_received_bytes_global = delay_time_global = 0;
	}
}
...
struct MemTestBlock {
	...
	MemTestBlock(unsigned char *p, int _size, int _CRC) {
		ptr = p;
		size = _size;
		CRC = _CRC;
		next = prev = 0;
