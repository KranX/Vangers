...
void S3Danalyze(void) {
	...
	uchar *surface;
	if (!S3Dside)
		surface = upper_buffer, empty = 0;
	else
		surface = lower_buffer, empty = 255;
	uchar *pa;
		...
		for (i = 0; i < xysize; i++, p++)
			if (*p != empty) {
				...
				if (S3DnoiseLevel && S3DnoiseAmp)
					if ((int)realRND(100) < S3DnoiseLevel)
						v += S3DnoiseAmp - realRND((S3DnoiseAmp << 1) + 1);
				pa = pa0 + (xx = XCYCL(x + i));
				switch (S3Dmode) {
				case 0:
					vv = v;
...
void MLedit(int n) {
	//	curGMap -> gotoxy(mlobj -> table[0].x0 + mlobj -> table[0].sx/2,mlobj -> table[0].y0 + mlobj
	//-> table[0].sy/2);
	vMap->squeeze();
	MLreset();
	vMap->flush();
...
void MLFrame::cut(int cx0, int cy0, int csx, int csy) {
	...
	for (j = 0; j < nsy; j++, y++) {
		x = dx0;
		for (i = 0; i < nsx; i++, x++) {
			if (signBits[(y * sx + x) / 32] & (1 << ((y * sx + x) % 32)))
				*ns |= mask;
			if (!(mask <<= 1)) {
				ns++;
				mask = 1;
...
void MLshow(int n) {
	curGMap->gotoxy(
		MLTable[n]->table[0].x0 + MLTable[n]->table[0].sx / 2,
		MLTable[n]->table[0].y0 + MLTable[n]->table[0].sy / 2
	);
}
...
const char *getVLname(void) {
	return vlNames[VLmode];
}
BaseValoc *getVLTail(void) {
	switch (VLmode) {
	case 0:
		return TntValoc::tail;
	case 1:
		return MLCloneValoc::tail;
	case 2:
		return SensorValoc::tail;
	case 3:
		return DangerValoc::tail;
	}
	return NULL;
}
...
BaseValoc *vlSeek(int x, int y) {
	...
	BaseValoc *p = getVLTail();
	while (p) {
		if (abs(getDistX(x, p->x)) <= E && abs(getDistY(y, p->y)) <= E)
			return p;
		p = p->next;
	}
	return NULL;
...
void VLadd(int x, int y) {
	...
	case 0:
		...
		*((TntValoc *)p) = TntVLpattern;
		if (TntZall == -1)
			p->z =
				GET_UP_ALT(vMap->lineT[y] + x + H_SIZE, *(vMap->lineT[y] + x), vMap->lineT[y], x);
		else
			p->z = TntZall;
		break;
	case 1:
...
void BaseValoc::show(void) {
	...
	if (abs(xd) < curGMap->xside - D && abs(yd) < curGMap->yside - D)
		XGR_Rectangle(
			curGMap->xc + xd - D, curGMap->yc + yd - D, 2 * D + 1, 2 * D + 1, COL1, COL1, XGR_FILLED
		);
}
...
void VLnext(void) {
	if (!curVL)
		...
		curVL = curVL->next;
	if (curVL)
		curGMap->gotoxy(curVL->x, curVL->y);
}
