...
int CurrentTerrain = -1, ActiveTerrain[TERRAIN_MAX] = {1,
							 1,
							 1,
							 1,
							 1,
...
ibmFile ibmObj = {1024, 1024, 320, 240, 800, 600, 250, 200, 0, nullptr, 0, 0, 0};
...
void sqStateSave(XStream &ff) {
	ff < LBM_mode[0] < LBM_mode[1] < FloodTerrainType < FloodLevel < DriftTerrainType < DriftLevel <
		ToolzerRadius < ToolzerSmooth < ToolzerDelta < ToolzerSMode < ToolzerEquDelta <
		GeonetRadius < GeonetLevel < GeonetDelta < CurrentTerrain;
	for (int i = 0; i < TERRAIN_MAX; i++)
		ff < ActiveTerrain[i];
	ff < MiniLevel < MaxiLevel < MIN_WIDE < H_MAIN < WATERLINE < ROAD_MATERIAL < ROAD_FORCING <
		DriftPower < DriftNoise < TunnelHeight < ExclusiveLayerMode < SecondLayerPriority;

	ff < S3Dcopt < S3Dmode < S3Dlevel < S3Dinverse < S3DnoiseLevel < S3DnoiseAmp < S3Dside <
		mosaicBMP.mode < mosaicBMP.force < mosaicBMP.copt < mosaicBMP.xoffset < mosaicBMP.yoffset <
		TunnelProof;
	for (int i = 0; i < 8; i++)
		ff < MosaicTypes[i];
}
...
PrmFormat PrmItem0[PRMMAX0] = {{-1, "Current Terrain ", &CurrentTerrain, -1, TERRAIN_MAX - 1},
...
PrmFormat PrmItem1[PRMMAX1] = {{-2, "Lift The Branch ", NULL, 0, 0},
...
static const char *MModeStr[LBM_MAX] = {"FLOOD", "DRIFT", "TOOLZER", "GEONET"};
...
iPrmMenu::iPrmMenu(sqElem *_owner, int _x, int _y): sqInputBox(_owner, _x, _y, 10, 10, &sysfont) {
	x0 = _x;
	y0 = _y;
	...
	*this + (menu = new sqPopupMenu(this, 4, 4, vprm, &sysfont, 0, 2));
	...
	for (i = 0; i < prmmax; i++) {
		...
		*buf < PrmItemPtr[i].string;
		if (PrmItemPtr[i].prm)
			*buf < " " <= *(PrmItemPtr[i].prm) < " ";
		*menu *new sqMenuBar(
			(uchar *)((*buf).GetBuf()), menu, LBM_mode[PrmToggle] == PrmItemPtr[i].mode
		);
	}
...
void iPrmMenu::message(int code, sqElem *object) {
	...
	case M_SETOPTION:
		...
		if (PrmItemPtr[copt].mode >= 0) {
			...
			if (!PrmToggle && LBM_mode[0] == FLOOD_TERRAIN)
				curGMap->setFloodLvl();
		}
		if (!mosaicBMP.data)
			MosaicToggle = 0;
		if (PrmItemPtr[copt].mode == -2) {
			...
			case TM_LIFT:
				if (cBranch)
					Track.lift_branch(cBranch);
				break;
			case TM_INTERPOLATE:
				if (cBranch)
					Track.recalc_height_branch(cBranch);
				break;
			}
		}
		close();
...
void iPrmMenu::keytrap(int key) {
	...
	case '9':
		...
		if (*(PrmItemPtr[n].prm) < PrmItemPtr[n].min)
			*(PrmItemPtr[n].prm) = PrmItemPtr[n].min;
		if (*(PrmItemPtr[n].prm) > PrmItemPtr[n].max)
			*(PrmItemPtr[n].prm) = PrmItemPtr[n].max;
		buf->init();
	...
	case SDLK_LEFT:
		...
		if (PrmItemPtr[n].prm) {
			buf->init();
			if (--(*(PrmItemPtr[n].prm)) == PrmItemPtr[n].min - 1)
				*(PrmItemPtr[n].prm) = PrmItemPtr[n].max;
			(*buf) < PrmItemPtr[n].string <= *(PrmItemPtr[n].prm);
			menu->pointer->replace(menu, (uchar *)((*buf).GetBuf()));
			draw();
	...
	case SDLK_RIGHT:
		...
		if (PrmItemPtr[n].prm) {
			buf->init();
			if (++(*(PrmItemPtr[n].prm)) == PrmItemPtr[n].max + 1)
				*(PrmItemPtr[n].prm) = PrmItemPtr[n].min;
			(*buf) < PrmItemPtr[n].string <= *(PrmItemPtr[n].prm);
			menu->pointer->replace(menu, (uchar *)((*buf).GetBuf()));
			draw();
...
iInputForm::iInputForm(sqElem *_owner, int _x, int _y, int _mode)
	...
	case E_PLACEBMPFORM:
		...
		*this + (menu = new sqPopupMenu(this, 7, 22, NITEMS, &sysfont, 0, 3));
		...
		do {
			i++;
			*menu *new sqMenuBar((uchar *)fname, menu);
		} while ((fname = win32_findnext()) != NULL);
		if (i > NITEMS)
			i = NITEMS;
		ady = 22 + (yadd = (i - 1) * 16);
		yadd += 5 * 24;
		*this + (params[0] = new sqField(
					 this,
					 "FORCE (0/1):",
					 4,
					 ady + 1 * 24,
					 0,
					 &sysfont,
					 (uchar *)itoa(placeBMP.force, tmpstr, 10),
					 1,
					 T_NUMERIC
				 ));
		*this + (params[1] = new sqField(
					 this,
					 "MAP/MAX/MIN/MEAN/ADD (0/1/2/3/4):",
					 4,
					 ady + 2 * 24,
					 0,
					 &sysfont,
					 (uchar *)itoa(placeBMP.mode, tmpstr, 10),
					 1,
					 T_NUMERIC
				 ));
		*this + (params[2] = new sqField(
					 this,
					 "Border: ",
					 4,
					 ady + 3 * 24,
					 0,
					 &sysfont,
					 (uchar *)itoa(placeBMP.border, tmpstr, 10),
					 2,
					 T_NUMERIC
				 ));
		*this + (params[3] = new sqField(
					 this,
					 "Level: ",
					 4,
					 ady + 4 * 24,
					 0,
					 &sysfont,
					 (uchar *)itoa(placeBMP.level, tmpstr, 10),
					 3,
					 T_NUMERIC
				 ));
		*this + (params[4] = new sqField(
					 this,
					 "Size: ",
					 4,
					 ady + 5 * 24,
					 0,
					 &sysfont,
					 (uchar *)itoa(placeBMP.size, tmpstr, 10),
					 4,
					 T_NUMERIC
				 ));
		break;
	case E_MOSAICFORM:
		...
		*this + (menu = new sqPopupMenu(this, 7, 22, NITEMS, &sysfont, 0, 3));
		...
		if (mosaicBMP.copt < i)
			menu->setpointer(menu->getbar(mosaicBMP.copt), 0);
		if (i > NITEMS)
			i = NITEMS;
		...
		*this + (params[1] = new sqField(
					 this,
					 "Relative/Additive/Terrain (0/1/2):",
					 4,
					 ady + 1 * 24,
					 0,
					 &sysfont,
					 (uchar *)itoa(mosaicBMP.mode, tmpstr, 10),
					 1,
					 T_NUMERIC
				 ));
		*this + (params[2] = new sqField(
					 this,
					 "Additive amplitude (-255 - 255):",
					 4,
					 ady + 2 * 24,
					 0,
					 &sysfont,
					 (uchar *)itoa(mosaicBMP.force, tmpstr, 10),
					 4,
					 T_NUMERIC
				 ));
		*this + (params[3] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(mosaicBMP.xoffset, tmpstr, 10),
					 4,
					 T_NUMERIC
				 ));
		*this + (params[4] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(mosaicBMP.yoffset, tmpstr, 10),
					 4,
					 T_NUMERIC
				 ));
		for (i = 0; i < 8; i++) {
			...
			*this + (params[5 + i] = new sqField(
						 ...
						 0,
						 &sysfont,
						 (uchar *)itoa(MosaicTypes[i], tmpstr, 10),
						 2,
						 T_NUMERIC
					 ));
		}
	...
	case E_IBMFORM:
		...
		*this + (params[0] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(ibmObj.mapx_center, tmpstr, 10),
					 5,
					 T_NUMERIC
				 ));
		*this + (params[1] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(ibmObj.mapy_center, tmpstr, 10),
					 5,
					 T_NUMERIC
				 ));
		*this + (params[2] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(ibmObj.x_center, tmpstr, 10),
					 4,
					 T_NUMERIC
				 ));
		*this + (params[3] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(ibmObj.y_center, tmpstr, 10),
					 4,
					 T_NUMERIC
				 ));
		*this + (params[4] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(ibmObj.x_size, tmpstr, 10),
					 4,
					 T_NUMERIC
				 ));
		*this + (params[5] = new sqField(
					 this,
					 "Y Size:",
					 4,
					 22 + 6 * 24,
					 0,
					 &sysfont,
					 (uchar *)itoa(ibmObj.y_size, tmpstr, 10),
					 4,
					 T_NUMERIC
				 ));
		*this + (params[6] = new sqField(
					 this,
					 "X Side:",
					 4,
					 22 + 7 * 24,
					 0,
					 &sysfont,
					 (uchar *)itoa(ibmObj.x_side, tmpstr, 10),
					 4,
					 T_NUMERIC
				 ));
		*this + (params[7] = new sqField(
					 this,
					 "Y Side:",
					 4,
					 22 + 8 * 24,
					 0,
					 &sysfont,
					 (uchar *)itoa(ibmObj.y_side, tmpstr, 10),
					 4,
					 T_NUMERIC
				 ));
		*this + (params[8] = new sqField(
					 this,
					 "Level:",
					 4,
					 22 + 9 * 24,
					 0,
					 &sysfont,
					 (uchar *)itoa(ibmObj.level, tmpstr, 10),
					 3,
					 T_NUMERIC
				 ));
		*this + (params[9] = new sqField(
					 this,
					 "Shot Counter:",
					 4,
					 22 + 10 * 24,
					 0,
					 &sysfont,
					 (uchar *)itoa(ibmObj.counter, tmpstr, 10),
					 3,
					 T_NUMERIC
				 ));
		break;
	case E_IMPCHOICE:
		...
		*this + (menu = new sqPopupMenu(this, 7, 22, IMP_MAX, &sysfont, 0, 3));
		for (i = 0; i < IMP_MAX; i++)
			*menu *new sqMenuBar((uchar *)ImpItem[i], menu);
		if (copt >= IMP_MAX)
			copt = IMP_MAX - 1;
		menu->setpointer(menu->getbar(copt), 0);
		xadd = menu->len * 8 + 16;
	...
	case E_IMPSPHERE:
		...
		*this + (params[0] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(ImpPattern[curImpIndex]->sx, tmpstr, 10),
					 3,
					 T_NUMERIC
				 ));
		*this + (params[1] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(ImpPattern[curImpIndex]->sy, tmpstr, 10),
					 3,
					 T_NUMERIC
				 ));
		*this + (params[2] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(ImpPattern[curImpIndex]->force, tmpstr, 10),
					 1,
					 T_NUMERIC
				 ));
		*this + (params[3] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(ImpPattern[curImpIndex]->mode, tmpstr, 10),
					 1,
					 T_NUMERIC
				 ));
		*this + (params[4] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(ImpPattern[curImpIndex]->level, tmpstr, 10),
					 3,
					 T_NUMERIC
				 ));
		*this + (params[5] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(ImpPattern[curImpIndex]->radInt, tmpstr, 10),
					 3,
					 T_NUMERIC
				 ));
		*this + (params[6] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(ImpPattern[curImpIndex]->h, tmpstr, 10),
					 3,
					 T_NUMERIC
				 ));
		*this + (params[7] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(ImpPattern[curImpIndex]->inverse, tmpstr, 10),
					 1,
					 T_NUMERIC
				 ));
		*this + (params[8] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(ImpPattern[curImpIndex]->plain, tmpstr, 10),
					 1,
					 T_NUMERIC
				 ));
		*this + (params[9] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(ImpPattern[curImpIndex]->noiseLevel, tmpstr, 10),
					 3,
					 T_NUMERIC
				 ));
		*this + (params[10] = new sqField(
					 this,
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(ImpPattern[curImpIndex]->noiseAmp, tmpstr, 10),
					 3,
					 T_NUMERIC
				 ));
		break;
	case E_IMPPOLYGON:
		...
		*this + (params[9] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(ImpPattern[curImpIndex]->n, tmpstr, 10),
					 3,
					 T_NUMERIC
				 ));
		*this + (params[10] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(ImpPattern[curImpIndex]->phase, tmpstr, 10),
					 4,
					 T_NUMERIC
				 ));
		*this + (params[11] = new sqField(
					 this,
					 "NOISE LEVEL (0 - 100)  :",
					 4,
					 22 + 11 * 24,
	...
	case E_TRACKFORM:
		...
		*this + (params[0] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(GroupToggle, tmpstr, 10),
					 1,
					 T_NUMERIC
				 ));
		*this + (params[1] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(NodeHeight, tmpstr, 10),
					 3,
					 T_NUMERIC
				 ));
		*this + (params[2] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(NodeSlope, tmpstr, 10),
					 4,
					 T_NUMERIC
				 ));
		*this + (params[3] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(NoiseLevel, tmpstr, 10),
					 3,
					 T_NUMERIC
				 ));
		*this + (params[4] = new sqField(
		...
		*this + (params[5] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(ShapeNumber, tmpstr, 10),
					 2,
					 T_NUMERIC
				 ));
		*this + (params[6] = new sqField(
		...
		*this + (params[7] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(TerrainMode, tmpstr, 10),
					 1,
					 T_NUMERIC
				 ));
		*this + (params[8] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(ColorBase, tmpstr, 10),
					 1,
					 T_NUMERIC
				 ));
		*this + (params[9] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(RenderMode, tmpstr, 10),
					 1,
					 T_NUMERIC
				 ));
		*this + (params[10] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(ReBuildWide, tmpstr, 10),
					 3,
					 T_NUMERIC
				 ));
		*this + (params[11] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(MaterSection, tmpstr, 10),
					 3,
					 T_NUMERIC
				 ));
		*this + (params[12] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(WideSection, tmpstr, 10),
					 3,
					 T_NUMERIC
				 ));
		break;
	case E_GOTOFORM:
		...
		*this + (params[0] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(curGMap->CX, tmpstr, 10),
					 5,
					 T_NUMERIC
				 ));
		tb.init();
		...
		*this + (params[1] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(curGMap->CY, tmpstr, 10),
					 5,
					 T_NUMERIC
				 ));
		break;
	case E_FORM3D:
		...
		*this + (menu = new sqPopupMenu(this, 7, 22, NITEMS, &sysfont, 0, 3));
		...
		if (S3Dcopt < i)
			menu->setpointer(menu->getbar(S3Dcopt), 0);
		if (i > NITEMS)
			i = NITEMS;
		ady = 22 + (yadd = (i - 1) * 16);
		yadd += 6 * 24;
		...
		*this + (params[3] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(S3Dinverse, tmpstr, 10),
					 1,
					 T_NUMERIC
				 ));
		*this + (params[4] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(S3DnoiseLevel, tmpstr, 10),
					 3,
					 T_NUMERIC
				 ));
		*this + (params[5] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(S3DnoiseAmp, tmpstr, 10),
					 3,
					 T_NUMERIC
				 ));
		*this + (params[6] = new sqField(
	...
	case E_MOBILOCMENU:
		...
		*this + (menu = new sqPopupMenu(this, 7, 22, i, &sysfont, 0, 3));
		...
		case 0:
			...
			*menu *new sqMenuBar(
				(uchar *)RUS("Покадровое редактирование существующих ML-объектов..."), menu
			);
			*menu *new sqMenuBar((uchar *)RUS("Удаление ML-объектов..."), menu);
			*menu *new sqMenuBar(
				(uchar *)RUS("Редактирование параметров у существующих ML-объектов..."), menu
			);
			*menu *new sqMenuBar(
				(uchar *)RUS("Показать один из существующих ML-объектов..."), menu
			);
			break;
		case 1:
			*menu *new sqMenuBar((uchar *)RUS("Добавить кадр"), menu);
			*menu *new sqMenuBar(
				(uchar *)RUS("Закончить создание ML-объекта и записать его"), menu
			);
			*menu *new sqMenuBar(
				(uchar *)RUS("Прекратить создание ML-объекта без сохранения результатов"), menu
			);
			break;
		case 2:
			*menu *new sqMenuBar(
				(uchar *)RUS("[G] Перейти на один из существующих кадров..."), menu
			);
			*menu *new sqMenuBar((uchar *)RUS("[A] Запомнить изменения текущего кадра"), menu);
			*menu *new sqMenuBar(
				(uchar *)RUS("[Q] Восстановить кадр в его начальное состояние"), menu
			);
			...
			*menu *new sqMenuBar(
				(uchar *)RUS("Закончить редактирование ML-объекта и сохранить все изменения"), menu
			);
			*menu *new sqMenuBar(
				(uchar *)RUS("Прекратить редактирование без сохранения всех изменений"), menu
			);
			break;
		}
		if (copt >= 6)
	...
	case E_VALOCMENU:
		...
		*menu *new sqMenuBar((uchar *)"Save Results", menu);
		...
		break;
	case E_MOBILOCFORM:
		*this + (name = new sqField(
					 this, "Name: ", 4, 22, 0, &sysfont, (uchar *)mlobj->name, MLNAMELEN
				 ));
		...
		*this + (params[0] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(mlobj->DryTerrain, tmpstr, 10),
					 1,
					 T_NUMERIC
				 ));
		*this + (params[1] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(mlobj->Impulse, tmpstr, 10),
					 3,
					 T_NUMERIC
				 ));
		*this + (params[2] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(mlobj->KeyPhase[1], tmpstr, 10),
					 3,
					 T_NUMERIC
				 ));
		for (i = 0; i < MAX_KEYPHASE - 2; i++) {
			...
			*this + (params[3 + i] = new sqField(
						 ...
						 0,
						 &sysfont,
						 (uchar *)itoa(mlobj->KeyPhase[i + 2], tmpstr, 10),
						 3,
						 T_NUMERIC
					 ));
		}
		...
		for (i = 0, j = 0; i < max; i++, j++) {
			...
			*this + (params[5 + i] = new sqField(
						 ...
						 0,
						 &sysfont,
						 (uchar *)itoa(mlobj->table[i].period, tmpstr, 10),
						 3,
						 T_NUMERIC
					 ));
		}
		for (i = 0, j = 0; i < max; i++, j++) {
			...
			tb.init();
			tb < "Srf[" <= i < "]:";
			*this + (params[5 + max + i] = new sqField(
						 this,
						 tb.GetBuf(),
						 i / M * 100 + 4 + W / 2,
						 22 + (2 + j) * 24,
						 0,
						 &sysfont,
						 (uchar *)itoa(mlobj->table[i].surfType, tmpstr, 10),
						 2,
						 T_NUMERIC
					 ));
		}
	...
	case E_WORLDCHOICE:
		...
		*this + (menu = new sqPopupMenu(this, 7, 22, NITEMS, &sysfont, 0, 3));
		for (i = 0; i < vMap->maxWorld; i++)
			*menu *new sqMenuBar((uchar *)vMap->wTable[i].name, menu);
		xadd = menu->len * 8 + 16;
		break;
	case E_VLTNTFORM:
		yadd = 3 * 24;
		*this + (params[0] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(TntVLobj->z, tmpstr, 10),
					 3,
					 T_NUMERIC
				 ));
		*this + (params[1] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(TntVLobj->value, tmpstr, 10),
					 6,
					 T_NUMERIC
				 ));
		*this + (params[2] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(TntVLobj->radius, tmpstr, 10),
					 6,
					 T_NUMERIC
				 ));
		*this + (params[3] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(TntVLobj->type, tmpstr, 10),
					 6,
					 T_NUMERIC
				 ));
	...
	case E_VLMLCFORM:
		...
		*this + (params[0] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(MLCVLobj->z, tmpstr, 10),
					 3,
					 T_NUMERIC
				 ));
		*this + (params[1] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(MLCVLobj->value, tmpstr, 10),
					 6,
					 T_NUMERIC
				 ));
		break;
	case E_VLSNSFORM:
		...
		*this + (params[0] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(SnsVLobj->z, tmpstr, 10),
					 3,
					 T_NUMERIC
				 ));
		*this + (params[1] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(SnsVLobj->id, tmpstr, 10),
					 9,
					 T_NUMERIC
				 ));
		*this + (params[2] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(SnsVLobj->radius, tmpstr, 10),
					 6,
					 T_NUMERIC
				 ));
		*this + (name = new sqField(
					 this, "Name: ", 4, 22 + 3 * 24, 0, &sysfont, (uchar *)SnsVLobj->name, 32
				 ));
		...
		*this + (params[4] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(SnsVLobj->z0, tmpstr, 10),
					 3,
					 T_NUMERIC
				 ));
		*this + (params[5] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(SnsVLobj->data0, tmpstr, 10),
					 3,
					 T_NUMERIC
				 ));
		*this + (params[6] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(SnsVLobj->data1, tmpstr, 10),
					 3,
					 T_NUMERIC
				 ));
		*this + (params[7] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(SnsVLobj->data2, tmpstr, 10),
					 3,
					 T_NUMERIC
				 ));
		*this + (params[8] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(SnsVLobj->data3, tmpstr, 10),
					 3,
					 T_NUMERIC
				 ));
		*this + (params[9] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(SnsVLobj->data4, tmpstr, 10),
					 3,
					 T_NUMERIC
				 ));
		*this + (params[10] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(SnsVLobj->data5, tmpstr, 10),
					 3,
					 T_NUMERIC
				 ));
		*this + (params[11] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(SnsVLobj->data6, tmpstr, 10),
					 3,
					 T_NUMERIC
				 ));
		break;
	case E_VLDNGFORM:
		...
		*this + (params[0] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(DngVLobj->z, tmpstr, 10),
					 3,
					 T_NUMERIC
				 ));
		*this + (params[1] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(DngVLobj->type, tmpstr, 10),
					 6,
					 T_NUMERIC
				 ));
		*this + (params[2] = new sqField(
					 ...
					 0,
					 &sysfont,
					 (uchar *)itoa(DngVLobj->radius, tmpstr, 10),
					 6,
					 T_NUMERIC
				 ));
...
void iInputForm::message(int code, sqElem *object) {
	...
	case M_SETOPTION:
		...
		case E_PLACEBMPFORM:
			placeBMP.place(
				(char *)menu->getbar(menu->getptr(menu->pointer))->original_data,
				atoi(params[0]->getstr()),
				atoi(params[1]->getstr()),
				atoi(params[2]->getstr()),
				atoi(params[3]->getstr()),
				atoi(params[4]->getstr())
			);
			break;
		case E_IBMFORM:
			...
			ibmObj.save(
				...
				atoi(params[3]->getstr()),
				atoi(params[4]->getstr()),
				atoi(params[5]->getstr()),
				atoi(params[6]->getstr()),
				atoi(params[7]->getstr()),
				atoi(params[8]->getstr()),
				atoi(params[9]->getstr())
			);
			break;
		case E_MOSAICFORM:
		...
		case E_IMPPOLYGON:
			ImpPattern[curImpIndex]->change(
				...
				atoi(params[7]->getstr()),
				atoi(params[8]->getstr()),
				atoi(params[11]->getstr()),
				atoi(params[12]->getstr())
			);
			break;
		case E_TRACKFORM:
			...
			if (cBranch) {
				...
				if (LastBranch)
					Track.lift_branch(cBranch);
				if (ReBuildWide)
					Track.recalc_wide_branch(cBranch, ReBuildWide);
				ReBuildWide = 0;
				LastBranch = 0;
			} else
				Track.set_node_parametr(
					cNode,
					GroupToggle,
					NoiseLevel,
					NoiseAmp,
		...
		case E_MOBILOCMENU:
			...
			case 0:
				...
				case 2:
					if (MLTableSize)
						sqE->put(E_MOBILOCCHOICE, E_COMMON, XGR_MAXX / 2, XGR_MAXY / 2);
					break;
				case 3:
					if (MLTableSize)
						sqE->put(E_MOBILOCDELETECHOICE, E_COMMON, XGR_MAXX / 2, XGR_MAXY / 2);
					break;
				case 4:
					if (MLTableSize)
						sqE->put(E_MOBILOCEDITCHOICE, E_COMMON, XGR_MAXX / 2, XGR_MAXY / 2);
					break;
				case 5:
					if (MLTableSize)
						sqE->put(E_MOBILOCSHOWCHOICE, E_COMMON, XGR_MAXX / 2, XGR_MAXY / 2);
					break;
				}
				break;
			case 1:
		...
		case E_MOBILOCSHOWCHOICE:
			if (code == M_SETOPTION) {
				...
				if (menu->pointer->status)
					*menu->pointer->data = 4;
				else
					*menu->pointer->data = ' ';
				MLTable[menu->getptr(menu->pointer)]->inUse =
					1 - MLTable[menu->getptr(menu->pointer)]->inUse;
		...
		case E_VLDNGFORM:
			DngVLobj->accept(
				atoi(params[0]->getstr()), atoi(params[1]->getstr()), atoi(params[2]->getstr())
			);
...
static int checkStage(int *y, int n, int stage) {
	...
	if (minY > ((stage + 1) << WPART_POWER) || maxY < (stage << WPART_POWER))
		return 0;
	return 1;
}
...
const int LAYMAX = 9;
const char *LayEditStr[2] = {"Begin editing the second layer", "Accept the created layer"};

LayFormat LayItem[PRMMAX0] = {{-1, LayEditStr[0], NULL, 0, 0},
...
iLayerMenu::iLayerMenu(sqElem *_owner, int _x, int _y)
	...
	if (LayerStatus == UP_LAYER)
		LayItem[0].string = LayEditStr[0];
	else
		LayItem[0].string = LayEditStr[1];
...
void iLayerMenu::message(int code, sqElem *object) {
	...
	case M_SETOPTION:
		copt = menu->getptr(menu->pointer);
		if (LayItem[copt].mode >= 0)
			LayerMode = LayItem[copt].mode;
		switch (menu->getptr(menu->pointer)) {
		case 0:
			if (LayerStatus == UP_LAYER)
...
void iLayerMenu::keytrap(int key) {
	...
	case '9':
		...
		if (*(LayItem[n].prm) < LayItem[n].min)
			*(LayItem[n].prm) = LayItem[n].min;
		if (*(LayItem[n].prm) > LayItem[n].max)
			*(LayItem[n].prm) = LayItem[n].max;
		buf->init();
	...
	case SDLK_LEFT:
		...
		if (LayItem[n].prm) {
			buf->init();
			if (--(*(LayItem[n].prm)) == LayItem[n].min - 1)
				*(LayItem[n].prm) = LayItem[n].max;
			(*buf) < LayItem[n].string <= *(LayItem[n].prm);
			menu->pointer->replace(menu, (uchar *)((*buf).GetBuf()));
			draw();
	...
	case SDLK_RIGHT:
		...
		if (LayItem[n].prm) {
			buf->init();
			if (++(*(LayItem[n].prm)) == LayItem[n].max + 1)
				*(LayItem[n].prm) = LayItem[n].min;
			(*buf) < LayItem[n].string <= *(LayItem[n].prm);
			menu->pointer->replace(menu, (uchar *)((*buf).GetBuf()));
			draw();
...
void ibmFile::save(
	...
	mapx_center = _mapx_center;
	mapy_center = _mapy_center;
	x_center = _x_center;
	y_center = _y_center;
	x_size = _x_size;
	y_size = _y_size;
	...
	if (_level == -1) {
		...
		for (int i = y0; i < y0 + y_size; i++)
			fff.write(vMap->lineTcolor[i] + x0, x_size);
		fff.close();
		return;
	}
	...
	vMap->change(y0, y0 + y_size);
	register int i, j;
	for (i = y0; i < y0 + y_size; i++)
		ff.write(vMap->lineTcolor[i] + x0, x_size);
