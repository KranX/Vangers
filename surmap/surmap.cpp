...
int RestoreLog, CGenLog, BorderLog, MobilityLog, InitLog, SSSLog, ROLog, EncodeLog, DirectLog,
	ExclusiveLog = 1, ShotLog, GlassLog, CutvmpLog, WHLog;
...
int xtInitApplication(void) {
	...
	if (!win32_SingleCheck())
		ErrH.Abort("Only one uniVang Instance available...");
...
void ComlineAnalyze(int argc, char **argv) {
	...
	if (RestoreLog && CGenLog)
		ErrH.Abort("Disable concurrent parameters");
}
...
void KeyCenter(int key) {
	if (key == SDLK_ESCAPE) {
		...
		sqKey->put(key);
	} else if (key != SDLK_LSHIFT && key != SDLK_LCTRL)
		sqKey->put(key);
...
#ifndef SCREENSHOT
	for (int i = 0; i < 256; i++)
		XGR_LineTo(5 + i, XGR_MAXY - 16, 14, 3, i);
#endif
...
void iGameMap::keytrap(int key) {
	...
	case SDLK_DELETE:
		if (!MLstatus)
			if (XKey.Pressed(SDLK_LSHIFT))
				CX--;
			else if (XKey.Pressed(SDLK_LCTRL))
				CX -= 10;
			else
				CX -= DELTA;
		else {
	...
	case SDLK_SPACE:
		if (TrackStatus)
			...
			ImpPattern[curImpIndex]->put(
				(xm - xc + CX) & clip_mask_x, (ym - yc + CY) & clip_mask_y
			);
		}
		break;
	case 'S':
...
iGameMap::iGameMap(sqElem *_owner, int _x, int _y, int _xside, int _yside, char *_title)
	...
	*this +
		(knobTrack = new sqTextButton(this, sx - 50 - 6 * 8, 24 + 2 * 24, "[Track OFF]", &sysfont));
...
void iGameMap::draw(int self) {
	...
	if (prmFlag & PRM_INFO) {
		...
		status < GetMMode();
		sysfont.draw(
			xc - xside + 3, yc - yside + 3 + 20, (unsigned char *)(status.GetBuf()), COL1, -1
		);
		int off = 0;
		if (catch_log) {
			status.init();
			...
			sysfont.draw(
				xc - xside + 3, yc - yside + 3 + 40, (unsigned char *)(status.GetBuf()), COL1, -1
			);
			off += 20;
		}
		if (mlobj && (!MLstatus || MLstatus == 2)) {
			status.init();
			status < "ML<" < mlobj->name < ">, Phase:" <= mlobj->getCurPhase() < " Step: " <=
				mlobj->steps[mlobj->cFrame] - 1 <
				/*" Stage: " <= mlobj -> getStage() < */ " Frame: " <= mlobj->cFrame;
			sysfont.draw(
				xc - xside + 3,
				yc - yside + 3 + 40 + off,
				(unsigned char *)(status.GetBuf()),
...
void iGameMap::quant(void) {
	...
	if (xm < xc - xside + 8)
		dx = -DELTA;
	else if (xm > xc + xside - 8)
		dx = DELTA;
	if (ym < yc - yside + 8)
		dy = -DELTA;
	else if (ym > yc + yside - 8)
		dy = DELTA;
	...
	case M_SETOBJ:
		...
		if (object == knobTrack) {
			...
			if (TrackStatus) {
				...
				*this + (knobSpline = new sqTextButton(
							 this, sx - 50 - 4 * 8, 24 + 3 * 24, "[Splines]", &sysfont
						 ));
				*this + (knobBuild = new sqTextButton(
							 this, sx - 50 - 2 * 8, sy - 30 - 3 * 24, "(Build)", &sysfont
						 ));
				*this + (knobBuildAll = new sqTextButton(
							 this, sx - 50 - 6 * 8, sy - 30 - 2 * 24, "(Build All)", &sysfont
						 ));
				*this + (knobLinking = new sqTextButton(
							 this, sx - 50 - 4 * 8, sy - 30 - 1 * 24, "(Linking)", &sysfont
						 ));
			} else {
		...
		if (object == knobSpline)
			Track.build_all_spline();
		if (object == knobBuild)
			Track.render(1);
		if (object == knobBuildAll)
			Track.render(0);
		if (object == knobLinking)
			Track.linking();
		break;
	}
