...
inline void
downMainStage(BYTE *pa0, int &hC, BYTE *pc0, int SizeX, int &x, BYTE *&grid, int &MaxAlt) {
	BYTE *pa = pa0 + x;
	BYTE *pf = pa + H_SIZE;
	BYTE *pc = pc0 + x;
	...
	for (i = 0; i < SizeX; i += 2) {
		if (*pf & DOUBLE_LEVEL) {
			...
			if (*(grid + level)) {
				...
				*(pf - 1) |= SHADOW_MASK;
				*(pc - 1) = *pc = palCLRlast
					[256 + (lightCLRlast[255 - (lxVal - rxVal)] - (255 - h >> H_CORRECTION) >> 1)];
			} else {
				*pf &= ~SHADOW_MASK;
				*(pf - 1) &= ~SHADOW_MASK;
				*(pc - 1) = *pc = palCLRlast
			...
			while (bNeedScan && MaxPossibleAlt >= 0) {
				...
				if (*pf & DOUBLE_LEVEL) {
					...
					if (*(grid + level)) {
						...
						*(pc - 1) = *pc = palCLRlast
							[256 +
								(lightCLRlast[255 - (lxVal - rxVal)] - (255 - h >> H_CORRECTION) >>
									1)];
					} else {
						*pf &= ~SHADOW_MASK;
						*(pf - 1) &= ~SHADOW_MASK;
