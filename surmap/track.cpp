...
double f3(double t, double d) {
	...
	if (t < 1.0 / 3)
		z = sqrt(0.17 * 0.17 - (t - 0.17) * (t - 0.17));
	else if (t < 2.0 / 3)
		z = -sqrt(0.17 * 0.17 - (t - 0.5) * (t - 0.5));
	else
		z = sqrt(0.17 * 0.17 - (t - 0.83) * (t - 0.83));
	return z * 6 * d;
}
...
double f5(double t, double d) {
	if (t < 0.05)
		return t * 20 * d;
	if (t < 0.15)
		return d + (0.05 - t) * 20 * d;
	if (t < 0.25)
		return d + (t - 0.25) * 20 * d;
	if (t < 0.35)
		return d + (0.25 - t) * 20 * d;
	if (t < 0.45)
		return d + (t - 0.45) * 20 * d;
	if (t < 0.55)
		return d + (0.45 - t) * 20 * d;
	if (t < 0.65)
		return d + (t - 0.65) * 20 * d;
	if (t < 0.75)
		return d + (0.65 - t) * 20 * d;
	if (t < 0.85)
		return d + (t - 0.85) * 20 * d;
	if (t < 0.95)
		return (1.0 - t) * 20 * d;
	return 0.0;
}
...
void do_left_buttion_off(int mX, int mY) {
	...
	case SET_HEIGHT_NODE:
		...
		WideSection = 0;
		RenderMode = 0;
		Track.get_node_parametr(
			cNode, NoiseLevel, NoiseAmp, NodeHeight, ColorBase, MaterSection, TerrainMode
		);
		cBranch = 0;
		sqE->put(E_TRACKFORM, E_COMMON, XGR_MAXX / 2, XGR_MAXY / 2);
		TrackBuild = WAIT;
...
void line(int x1, int y1, int x2, int y2, int color) {
	...
	if (xx1 > ((int)clip_mask_x / 2))
		xx1 = xx1 - clip_mask_x - 1;
	if (xx1 < (-(int)clip_mask_x / 2))
		xx1 = xx1 + clip_mask_x + 1;

	if (xx2 > ((int)clip_mask_x / 2))
		xx2 = xx2 - clip_mask_x - 1;
	if (xx2 < (-(int)clip_mask_x / 2))
		xx2 = xx2 + clip_mask_x + 1;
...
void eBranch::show(int all) {
	...
	if (all_on) {
		...
		//		  for( i = 0; i < n_point; i++ ){
		//			  line(data_all[i].x-1, data_all[i].y-1, data_all[i].x+1, data_all[i].y-1,
		// COLOR_TRACK); 			  line(data_all[i].x-1, data_all[i].y, data_all[i].x+1,
		// data_all[i].y, COLOR_TRACK); 			  line(data_all[i].x-1, data_all[i].y+1,
		// data_all[i].x+1, data_all[i].y+1, COLOR_TRACK);
		//		  }
	} else {
...
void sTrack::add_branch_to_node(int cNode, int cBranch) {
	...
	if (i == n_branch)
		ErrH.Abort("Dont Find Branch in add_branch_to_node");
	i = 0;
	...
	if (i == n_node)
		ErrH.Abort("Dont Find Node in add_branch_to_node");

	if (abs(l_branch->data_base[0].x - l_node->x) < delta &&
		abs(l_branch->data_base[0].y - l_node->y) < delta) {
...
int sTrack::add_point_to_branch(int pos, int X, int Y, int WIDE) {
	...
	if (i == n_branch)
		ErrH.Abort("Dont Find Branch in add_point_to_");
...
void sTrack::set_point_in_branch(int pos1, int pos2, int X, int Y, int hh) {
	...
	if (i == n_branch)
		ErrH.Abort("Dont Find Branch in set_point_in");
...
void sTrack::set_node(int cNode, int Mode, int X, int Y) {
	...
	if (i == n_node)
		ErrH.Abort("Dont Find Node in set_node");
	...
	if (Mode)
		for (i = 0; i < l_node->n_branch; i++) {
			...
			while (l_branch->index != l_node->branches[i])
				l_branch = l_branch->r;

			if (l_branch->beg_node == cNode)
				l_branch->set_section(1, X, Y, l_node->h);

			register int pos = l_branch->n_section;

			if (l_branch->end_node == cNode)
				l_branch->set_section(pos, X, Y, l_node->h);
		}
	else
		for (i = 0; i < l_node->n_branch; i++) {
			eBranch *l_branch = branch;
			...
			if (l_branch->beg_node == cNode)
				l_branch->set_section(1, X, Y, -1);

			register int pos = l_branch->n_section;

			if (l_branch->end_node == cNode)
				l_branch->set_section(pos, X, Y, -1);
		}
...
int eBranch::find_point(int &cPoint, int &X, int &Y) {
	...
	for (i = 0; i < n_section; i++) {
		...
		if ((data_base[i].x - X) > (int)clip_mask_x)
			dop = clip_mask_x + 1;
		else if ((data_base[i].x - X) < -(int)clip_mask_x)
			dop = -((int)clip_mask_x + 1);

		_x += dop;

		dop = 0;
		if ((data_base[i].y - Y) > (int)clip_mask_y)
			dop = clip_mask_y + 1;
		else if ((data_base[i].y - Y) < -(int)clip_mask_y)
			dop = -((int)clip_mask_y + 1);
...
void sTrack::concate_branch(int cwBranch, int cBranch) {
	...
	if (i == n_branch)
		ErrH.Abort("Dont Find cBranch ..");

	int pos = lw_branch->n_section - 1;
	int pos2 = ls_branch->n_section - 1;

	if (abs(lw_branch->data_base[0].x - ls_branch->data_base[pos2].x) < 2 * delta &&
		abs(lw_branch->data_base[0].y - ls_branch->data_base[pos2].y) < 2 * delta) {
...
int sTrack::decompose_branch(int cwBranch, int cwPoint, int &cNode) {
	...
	if (i == n_branch)
		ErrH.Abort("Dont find branch in decompose_branch");

	if ((cwPoint == 1) || (cwPoint == l_branch->n_section))
		return 0;

	loc = add_branch(
		l_branch->data_base[cwPoint - 1].x,
		l_branch->data_base[cwPoint - 1].y,
		pos,
		l_branch->data_base[cwPoint - 1].wide
	);
	...
	if (i == n_branch)
		ErrH.Abort("Dont find newbranch in decompose_branch");
	...
	cNode = add_node(
		l_branch->data_base[cwPoint - 1].x,
		l_branch->data_base[cwPoint - 1].y,
		l_branch->data_base[cwPoint - 1].h
	);
...
void sTrack::delete_branch_from_node(int cNode, int cBranch) {
	...
	if (i == n_node)
		ErrH.Abort("Dont Find Node in delete branch..");

	for (i = 0; l_node->branches[i] != cBranch && i < l_node->n_branch; i++)
		;

	if (i == l_node->n_branch)
		ErrH.Abort("Dont Find Branch in Node");
...
void sTrack::delete_branch(int cwBranch) {
	...
	if (cwBranch < 1)
		ErrH.Abort("Error in struct Track");
	...
	if (i == n_branch)
		ErrH.Abort("Dont Find deleted Branch");
...
int sTrack::delete_node(int cNode) {
	...
	if (cNode < 1)
		ErrH.Abort("Error in struct Node");
	...
	if (l_node->n_branch > 2)
		...
		if (i == n_branch)
			ErrH.Abort(" Dont Find branch for Node..");
...
void sTrack::prepare_branch_concate(int cBranch, int cNode) {
	...
	if (cNode < 1)
		ErrH.Abort("This Node dont exist");
	...
	if (i == n_branch)
		ErrH.Abort("Dont Find Branch");

	if (cNode == l_branch->end_node)
		return;
	if (cNode != l_branch->beg_node)
		ErrH.Abort("Error in Struct");
...
int eBranch::insert_point(int X, int Y) {
	...
	for (i = 0; i < n_section - 1; i++) {
		...
		if (x1 - x2 > (int)clip_mask_x)
			dop = clip_mask_x + 1;
		else if (x1 - x2 < -(int)clip_mask_x)
			dop = -((int)clip_mask_x + 1);

		x2 += dop;
		dop = 0;

		if (x1 - X > (int)clip_mask_x)
			dop = clip_mask_x + 1;
		else if (x1 - X < -(int)clip_mask_x)
			dop = -((int)clip_mask_x + 1);
		X += dop;
		...
		if ((minx - delta) <= X && (maxx + delta) >= X && (miny - delta) <= Y &&
			(maxy + delta) >= Y) {
			int yy = 0;
...
void calc_normal_vector(eSection *data, int pos, int &xv, int &yv) {
	...
	if (data[pos - 1].x - data[pos].x + XGR_MAXX > (int)clip_mask_x)
		dop = clip_mask_x + 1;
	if (data[pos - 1].x - data[pos].x - XGR_MAXX < -(int)clip_mask_x)
		dop = -((int)clip_mask_x + 1);
...
double eNode::get_alpha(eBranch *branch, int cBranch) {
	...
	while (branch->index != cBranch)
		branch = branch->r;

	if (branch->beg_node == index) {
		dop = 0;

		if (x - branch->data_base[1].x + XGR_MAXX > (int)clip_mask_x)
			dop = clip_mask_x + 1;
		if (x - branch->data_base[1].x - XGR_MAXX < -(int)clip_mask_x)
			dop = -((int)clip_mask_x + 1);
		...
		if (x - branch->data_base[pos].x + XGR_MAXX > (int)clip_mask_x)
			dop = clip_mask_x + 1;
		if (x - branch->data_base[pos].x - XGR_MAXX < -(int)clip_mask_x)
			dop = -((int)clip_mask_x + 1);
...
void eNode::find_cross_point(eBranch *&branch, int ls, int rs, int pos) {
	...
	if (x - ll.x + XGR_MAXX > (int)clip_mask_x)
		dop = clip_mask_x + 1;
	if (x - ll.x - XGR_MAXX < -(int)clip_mask_x)
		dop = -((int)clip_mask_x + 1);
	...
	if (x - rl.x + XGR_MAXX > (int)clip_mask_x)
		dop = clip_mask_x + 1;
	if (x - rl.x - XGR_MAXX < -(int)clip_mask_x)
		dop = -((int)clip_mask_x + 1);
	...
	if (l_br->beg_node == index) {
		...
		if (xx - l_br->data_base[0].xl + XGR_MAXX > (int)clip_mask_x)
			dop = clip_mask_x + 1;
		if (xx - l_br->data_base[0].xl - XGR_MAXX < -(int)clip_mask_x)
			dop = -((int)clip_mask_x + 1);
		...
		if (xx - l_br->data_base[p].xr + XGR_MAXX > (int)clip_mask_x)
			dop = clip_mask_x + 1;
		if (xx - l_br->data_base[p].xr - XGR_MAXX < -(int)clip_mask_x)
			dop = -((int)clip_mask_x + 1);
	...
	if (r_br->beg_node == index) {
		...
		if (xx - r_br->data_base[0].xr + XGR_MAXX > (int)clip_mask_x)
			dop = clip_mask_x + 1;
		if (xx - r_br->data_base[0].xr - XGR_MAXX < -(int)clip_mask_x)
			dop = -((int)clip_mask_x + 1);
		...
		if (xx - r_br->data_base[p].xl + XGR_MAXX > (int)clip_mask_x)
			dop = clip_mask_x + 1;
		if (xx - r_br->data_base[p].xl - XGR_MAXX < -(int)clip_mask_x)
			dop = -((int)clip_mask_x + 1);
...
void sTrack::render(int what) {
	...
	if (linkingLog) {
		...
		int m =
			((((curGMap->CY + curGMap->yside) & clip_mask_y) >> WPART_POWER) + 1) & (PART_MAX - 1);
		eNode *l_node = node;
		eBranch *l_branch = branch;
		for (i = 0; i < n_branch; i++) {
			l_branch->builded = 0;
...
void eNode::render(int what) {
	...
	for (i = 0; i < n_branch; i++) {
		while (l_branch->index != branches[i])
			l_branch = l_branch->r;
...
void find_point_on_line(gLine ll, gLine rl, int &xx, int &yy) {
	if (ll.a * rl.b == ll.b * rl.a) {
		...
		double t =
			double((rl.x - ll.x) * rl.b - (rl.y - ll.y) * rl.a) / (ll.a * rl.b - rl.a * ll.b);
		xx = (int)(ll.x + t * ll.a);
		yy = (int)(ll.y + t * ll.b);
	}
}

#define DIVISION_PLUS(a, b, c) \
	if (c)                     \
		a = (b << 16) / c;     \
	else                       \
		a = (b << 16);
#define DIVISION_MINUS(a, b, c) \
	if (c)                      \
		a = -(b << 16) / c;     \
	else                        \
		a = -(b << 16);
...
void drawpoly(
	...
	uchar **lt = vMap->lineT;
	if ((!lt[minY] || !lt[maxY]) && what)
		return;
	if ((maxX < curGMap->CX - curGMap->xside || minX > curGMap->CX + curGMap->xside) && what)
		return;
		...
		while (d > 0) {
			...
			for (j = 0, xx = xll; j < max; j++, xx++) {
				if (xmode) {
					...
					height = hl;
				if (noise)
					if ((int)realRND(100) < noise)
						height += dnoise - realRND((dnoise << 1) + 1);
...
void m_line(int x1, int y1, int x2, int y2, int color) {
	if ((x1 > UcutLeft) && (x1 < UcutRight) && (x2 > UcutLeft) && (x2 < UcutRight) &&
		(y1 > VcutUp) && (y1 < VcutDown) && (y2 > VcutUp) && (y2 < VcutDown))
		...
		if (tmaxu > 1.0)
			tmaxu = 1.0;
		if ((tminu >= tmaxu) || (tminu >= 1.0) || (tmaxu <= 0.0))
			return;
...
void set_point_from_base(eSection *base, rSection &P, int pos, int pos0) {
	...
	if (base[pos0].xl - base[pos].xl + XGR_MAXX > (int)clip_mask_x)
		dop = clip_mask_x + 1;
	if (base[pos0].xl - base[pos].xl - XGR_MAXX < -(int)clip_mask_x)
		dop = -((int)clip_mask_x + 1);
...
void spline(eSection *&base, rSection *all, int k, int n_section, int &n_point) {
	...
	double lenl =
		sqrt((double)(P2.xl - P3.xl) * (P2.xl - P3.xl) + (P2.yl - P3.yl) * (P2.yl - P3.yl));
	double lenr =
		sqrt((double)(P2.xr - P3.xr) * (P2.xr - P3.xr) + (P2.yr - P3.yr) * (P2.yr - P3.yr));
	double len;
	...
	if (n_point + n > MAX_ALL)
		ErrH.Abort("Out of all section");
...
void eBranch::set_wide_in_section(int pos, int X, int Y) {
	...
	len = (int)sqrt(
		(double)(data_base[pos].x - X) * (data_base[pos].x - X) +
		(data_base[pos].y - Y) * (data_base[pos].y - Y)
	);
...
int eBranch::find_bound_point(int &cPoint, int X, int Y) {
	...
	for (i = 0; i < n_section; i++) {
		...
		if ((data_base[i].xr - X) > (int)clip_mask_x)
			dop = clip_mask_x + 1;
		else if ((data_base[i].xr - X) < -(int)clip_mask_x)
			dop = -((int)clip_mask_x + 1);
		...
		dop = 0;
		if ((data_base[i].xl - X) > (int)clip_mask_x)
			dop = clip_mask_x + 1;
		else if ((data_base[i].xl - X) < -(int)clip_mask_x)
			dop = -((int)clip_mask_x + 1);
...
void sTrack::show_branch_height(int cBranch, int cPoint) {
	...
	if (l_branch->data_base[cPoint].h < 0)
		l_branch->data_base[cPoint].h -= DeltamH;
	if (l_branch->data_base[cPoint].h > 255)
		l_branch->data_base[cPoint].h -= DeltamH;
	...
	line(
		...
		v2 - 2,
		(u2 + u1) / 2 - 20,
		v2 - (l_branch->data_base[cPoint].hl / 4),
		COLOR_TRACK
	);
	line(
		(u1 + u2) / 2 + 20,
		v2 - 2,
		(u2 + u1) / 2 + 20,
		v2 - (l_branch->data_base[cPoint].hr / 4),
		COLOR_TRACK
	);
	line(
		(u2 + u1) / 2 - 20,
		v2 - (l_branch->data_base[cPoint].hl / 4),
		(u2 + u1) / 2,
		v2 - (l_branch->data_base[cPoint].h / 4),
		COLOR_TRACK
	);
	line(
		(u2 + u1) / 2 + 20,
	...
	line(
		(u1 + u2) / 2, v2 - 2, (u2 + u1) / 2, v2 - (l_branch->data_base[cPoint].h / 4), COLOR_TRACK
	);
	...
	if (cPoint > 0) {
		...
		line(
			u1 + 2,
			v2 - (l_branch->data_base[cPoint - 1].h / 4),
			(u2 + u1) / 2,
			v2 - (l_branch->data_base[cPoint].h / 4),
			COLOR_TRACK
		);
...
void sTrack::get_section_parametr(
	int cBranch,
	int cPoint,
	int &NoiseLevel,
	int &profil,
	...
	if (i == n_branch)
		ErrH.Abort("Dont Find Branchs parametor");
...
void sTrack::set_section_parametr(
	int cBranch,
	int cPoint,
	int NoiseLevel,
	int profil,
...
void sTrack::get_node_parametr(
	...
	if (i == n_node)
		ErrH.Abort("Dont Find nodes parametor");
...
void eBranch::saveKron(XStream &fout) {
	...
	for (i = 0; i < n_section; i++)
		if (data_base[i].dnoise > max_noise)
			max_noise = data_base[i].dnoise;
...
void sTrack::set_height_in_branch_from_ground(eBranch *l_branch, int cSection, int what) {
	if (l_branch->data_base[cSection].mode < 2)
		return;
	...
	if ((!vMap->lineT[yyr] || !vMap->lineT[yyl]) && what)
		return;
	...
	l_branch->data_base[cSection].h =
		(l_branch->data_base[cSection].hr + l_branch->data_base[cSection].hl) >> 1;
}
...
void sTrack::lift_branch(int cBranch) {
	...
	if (i == n_branch)
		ErrH.Abort("Dont Find Branch in lift.");
...
void sTrack::recalc_height_branch(int cBranch) {
	...
	if (i == n_branch)
		ErrH.Abort("Dont Find Branch in recalc_heght.");
...
void sTrack::recalc_wide_branch(int cBranch, int new_wide) {
	...
	if (i == n_branch)
		ErrH.Abort("Dont Find Branch in recalc new_wide.");
...
int eBranch::find_next_point_in_spline(int n, int w) {
	...
	for (i = n + 2; ((i < n_point) && (sum / (i - n + 1) < DD)); i++) {
		if (w == 0) {
			...
			for (j = n + 1; j < i; j++) {
				t = double(
						data_all[j].yl * b + data_all[j].xl * a - data_all[n].yl * b -
						data_all[n].xl * a
					) /
					(a * a + b * b);

				sum += sqrt(
					sqr(data_all[n].xl + a * t - data_all[j].xl) +
					sqr(data_all[n].yl + b * t - data_all[j].yl)
				);
			}
		} else {
			...
			for (j = n + 1; j < i; j++) {
				t = double(
						data_all[j].yr * b + data_all[j].xr * a - data_all[n].yr * b -
						data_all[n].xr * a
					) /
					(a * a + b * b);

				sum += sqrt(
					sqr(data_all[n].xr + a * t - data_all[j].xr) +
					sqr(data_all[n].yr + b * t - data_all[j].yr)
				);
			}
		} //  end if
	} // end for i
