...
void Model::draw_big() {
	...
	A_convert =
		A_g2s * A_shape * DBM(shape_scale_x, 0, 0, 0, shape_scale_y, 0, 0, 0, shape_scale_z);
...
#define NEXT_RIGHT(p) ((p) == end ? beg : (p) + 1)
	...
	if (y_down < VcutUp || y_up > VcutDown || x_right < UcutLeft || x_left > UcutRight)
		return;
	...
	while (1) {
		...
		while (d-- > 0) {
			...
			if (x1 > x2)
				return;
			if (x1 < x2) {
				if (Y >= VcutUp && Y < VcutDown && x2 >= UcutLeft && x1 < UcutRight) {
					len = x2 - x1;
					cf = cl;
					cfk = (cr - cl) / len;
					if (x1 < UcutLeft) {
...
void Model::height_project() {
	...
	A_convert = A_shape * DBM(shape_scale_x, 0, 0, 0, shape_scale_y, 0, 0, 0, shape_scale_z);
...
//	  c_putspr(shape_x,shape_y - shape_size/2 - 2,1 <<
// shape_shift,shape_size,upper_buffer,BLACK_FON); 	  c_putspr(shape_x,shape_y + shape_size/2 + 2,1
// << shape_shift,shape_size,lower_buffer,BLACK_FON);
#endif
}
