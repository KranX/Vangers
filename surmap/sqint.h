...
struct sqFont {
	...
	void draw(int x, int y, char *s, int fore = SYSCOLOR, int back = -1) {
		draw(x, y, (unsigned char *)s, fore, back);
	}
...
struct sqEventQueue {
	...
	sqEventQueue(int _max) {
		queue = new sqEvent[max = _max];
		top = bottom = 0;
	}
...
struct sqElem {
	...
	int getX(void) {
		if (owner)
			return owner->getX() + owner->xl - owner->dxl;
		return 0;
	}
	int getY(void) {
		if (owner)
			return owner->getY() + owner->yl - owner->dyl;
		return 0;
	}
	int getRX(void) {
		if (owner)
			return owner->getX() + owner->xr + owner->dxr;
		return 0;
	}
	int getRY(void) {
		if (owner)
			return owner->getY() + owner->yr + owner->dyr;
		return 0;
	}
};
