...
static uchar PeakData[PEAK_SIZE * PEAK_SIZE] = {10,
	20,
	50,
	80,
	80,
...
#ifdef _SURMAP_
int curImpIndex;
const char *ImpItem[IMP_MAX] = {"SPHERE", "POLYGON", "PEAK"};
#endif
...
void ImpassType::read(XStream &ff) {
	ff > sx > sy > level > force > mode > radInt > h > n > phase > inverse > plain > noiseLevel >
		noiseAmp;
}
			...
			if (force || *p != level) {
				...
				if (lt[yy]) {
					...
					if (noiseLevel && noiseAmp)
						if ((int)realRND(100) < noiseLevel)
							vv += noiseAmp - realRND((noiseAmp << 1) + 1);
					pixSet(xx, yy, vv - v);
				}
			}
...
void ImpPolygon::build(void) {
	...
	for (yy = 1.0, y = 0; y < sy; yy -= dy, y++)
		for (xx = -1.0, x = 0; x < sx; xx += dx, x++, p++) {
			...
			//			  zzz[1] = plane(xx,yy, -rad,-rad,0.0, rad,-rad,0.0,
			// 0.0,0.0,(double)h/256.); 			  zzz[2] = plane(xx,yy, -rad,rad,0.0,
			// -rad,-rad,0.0, 0.0,0.0,(double)h/256.); 			  zzz[3] = plane(xx,yy, rad,rad,0.0,
			// rad,-rad,0.0, 0.0,0.0,(double)h/256.);
