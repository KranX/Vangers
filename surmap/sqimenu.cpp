...
sqPopupMenu &sqPopupMenu::operator*(sqMenuBar *obj) {
	...
	insert = obj;
	maxbars++;
	sy = MIN(maxbars, visibars) * font->sy;
	set(height);
	return *this;
}
...
void sqPopupMenu::draw(int self) {
	...
	while (p && j < visibars) {
		if (p == pointer)
			font->draw(
				getX() + x, getY() + i, p->data, SQ_SYSCOLOR, SQ_SYSCOLOR + 6 + (2 - height)
			);
		else
...
void sqPopupMenu::keytrap(int key) {
	...
	case SDLK_DOWN:
		if (!pointer->next) {
			...
			if (getptr(pointer) - topbar + 1 == visibars)
				topbar++;
	...
	case SDLK_KP_MINUS:
		...
		break;
	default:
		if (CLOCK() - seekcounter > SEEKDELAY)
			seeklen = 0;
		if (seeklen <= MAXSEEK) {
			seekbuf[seeklen++] = key; // !
			seek(pointer);
...
void sqPopupMenu::handler(sqEvent *e) {
	...
	case E_LBMPRESS:
		...
		//				XGR_MouseObj.SetClip(getX() + x + XGR_MouseObj.SizeX/2, getY() + y +
		// XGR_MouseObj.SizeY/2, getX() + x + sx - XGR_MouseObj.SizeX/2, getY() + y + sy -
		// XGR_MouseObj.SizeY/2); 				  XGR_MouseObj.left = getX() + x +
		// XGR_MouseObj.SizeX/2; 				  XGR_MouseObj.top = getY() + y +
		// XGR_MouseObj.SizeY/2;
		...
		} else if (XGR_MouseObj.LastPosY < XGR_MouseObj.PosY && topbar == getptr(pointer)) {
			if (pointer->prev) {
				topbar--;
				pointer = pointer->prev;
			}
		...
		break;
	case E_LBMUNPRESS:
		//			XGR_MouseObj.SetClip(sqScr -> x + XGR_MouseObj.SizeX/2, sqScr -> y +
		// XGR_MouseObj.SizeY/2, sqScr -> x + sqScr -> sx - XGR_MouseObj.SizeX/2, sqScr -> y + sqScr
		//-> sy - XGR_MouseObj.SizeY/2); 			  XGR_MouseObj.left = sqScr -> x +
		// XGR_MouseObj.SizeX/2; 			  XGR_MouseObj.right = sqScr -> x + sqScr -> sx -
		// XGR_MouseObj.SizeX/2; 			  XGR_MouseObj.top = sqScr -> y + XGR_MouseObj.SizeY/2;
...
void sqPopupMenu::setpointer(sqMenuBar *p, int force) {
	...
	if (topbar < 0)
		...
		topbar = MIN(topbar, maxbars - MIN(maxbars, visibars));
	if (force)
		draw();
}
...
sqMultiMenu::sqMultiMenu(
	...
	set(0);
	*this + (edit = new sqInputString(
				 this, 2, 0, _font->sx * (_maxlen + 1), _font, (unsigned char *)NULL, _maxlen
			 ));
...
void sqMultiMenu::message(int code, sqElem *object) {
	...
	case M_DELETE:
		if (list->maxbars > 1) {
			if (list->pointer->next)
				list->insert = list->pointer->next;
			else
				list->insert = list->pointer->prev;
			list->remove();
