...
static void r_net_init(void) {
	...
	for (x = 0; x < POWER; x++)
		r_preRNDVAL[x] = realRND(0xFFFFFFFF);
	...
	for (y = 0; y < map_size_y / QUANT; y++) {
		...
		for (x = 0; x < map_size_x / QUANT; x++)
			...
			case 0:
				...
				break;
			case 1:
				//					  R_NET_MAP(y,x) = (uchar)((max_alt - 1)*(x + y)/(map_size_x +
				// map_size_y));
				R_NET_MAP(y, x) = (uchar)realRND(max_alt - 1);
				break;
			case 2:
				//					  R_NET_MAP(y,x) = (uchar)((max_alt - 1)*((x +
				// y)*QUANT)/abs(x*QUANT - map_size_x/2 - 1));
				R_NET_MAP(y, x) = (uchar)realRND(max_alt - 1);
				break;
			case 3:
				//					  R_NET_MAP(y,x) = (uchar)((max_alt - 1)*((x +
...
static void m_net_init(void) {
	...
	for (x = 0; x < POWER; x++)
		m_preRNDVAL[x] = realRND(0xFFFFFFFF);
	for (y = 0; y < PART_MAX; y++)
		for (x = 0; x < POWER; x++)
...
static void rough_init(void) {
	...
	for (y = 0, j = Stage * (part_map_size_y / QUANT); y <= part_map_size_y + QUANT;
		y += QUANT, j++) {
		if (j == PART_MAX * (part_map_size_y / QUANT))
			j = 0;
...
static void generate_roughness_map(void) {
	...
	for (ind = 0, square_size = QUANT; square_size > MINSQUARE; square_size >>= 1, ind++) {
		...
		for (y1 = 0; y1 <= (int)part_map_size_y; y1 += square_size) {
			if (y1 == square_size)
				RNDVAL = r_cycleRNDVAL[Stage][ind];
			else if (y1 == (int)part_map_size_y)
				RNDVAL = r_preRNDVAL[ind];
...
static void generate_alt_map(void) {
	...
	for (ind = 0, square_size = QUANT; square_size > 1; square_size >>= 1, ind++) {
		...
		for (y1 = 0; y1 <= (int)part_map_size_y; y1 += square_size) {
			if (y1 == square_size)
				RNDVAL = m_cycleRNDVAL[Stage][ind];
			else if (y1 == (int)part_map_size_y)
				RNDVAL = m_preRNDVAL[ind];
			for (x1 = 0; x1 < map_size_x; x1 += square_size) {
				x2 = (x1 + square_size) & clip_mask_x;
	...
	if (!which)
		memcpy(alt_map, color_map, part_map_size);
}
	...
	while (log) {
		...
		for (j = 0, i = 0; i < num[index]; i++) {
			...
			if (pa) {
				...
				if (ti)
					na += (level - h) * DriftPower / 16 + realRND(DriftNoise);
				t = GET_REAL_TERRAIN_TYPE(pf, x);
				if (h < level && t != ttype && j < N - 4 && na >= MiniLevel && na <= MaxiLevel &&
					ActiveTerrain[t >> TERRAIN_OFFSET] && h >= MiniLevel && h <= MaxiLevel) {
					if (SET_REAL_TERRAIN(pf, ttype, x))
						log = 1;
	...
	while (log) {
		...
		for (j = 0, i = 0; i < num[index]; i++) {
			...
			if (pa) {
				...
				if (h < level && t != ttype && j < N - 4 && na >= MiniLevel && na <= MaxiLevel &&
					ActiveTerrain[t >> TERRAIN_OFFSET] && h >= MiniLevel && h <= MaxiLevel) {
					if (SET_REAL_DOWNTERRAIN(pf, ttype, x))
						log = 1;
		...
		for (x = map_size_x - 1; x >= 0; x--, pa--, pc--, pf--) {
			...
			if (*pf & DOUBLE_LEVEL) {
				...
				//					исправление 0-полоски
									if(x == map_size_x - 1 && (*(pf - 2) & DOUBLE_LEVEL) && (*(pf0 +
				1) & DOUBLE_LEVEL)){ *pa = (uchar)((int)*(pa - 2) + (int)rxVal >> 1); type = *(pf -
				2) & TERRAIN_MASK; SET_TERRAIN(*pf,type); type >>= TERRAIN_OFFSET;
									}
				*/
				h = *(pa - dnD);
				hd = *(pa - 1);
				...
				if (grid[h]) {
					if (isColor)
						*(pc - 1) = *pc = palCLR[type]
												[256 + (lightCLR[type][255 - (lxVal - rxVal)] -
															   (255 - h >> H_CORRECTION) >>
														   1)];
					*pf |= SHADOW_MASK;
					*(pf - 1) |= SHADOW_MASK;
				...
				if (!dnD)
					memset(grid + h - SS_WIDTH, 2, SS_WIDTH + 1);
				if (h > hM)
					hM = h;
				else
					hM -= 3;
		...
		for(y = 0;y < (int)part_map_size_y;y++){
			...
			for(x = 0,i = 0;i < map_size_x || state == 2;x = XCYCL(x + 2),pa += 2,pf += 2,i += 2){
				...
				if(state == 2 && (*pf & DOUBLE_LEVEL)){
					state = 1;
					if(cnt <= CLUSTER_SIZE){
						v = (int)*(pa + 1) - (h = (int)*((uchar*)alt_map + (offset = y*map_size_x +
	   XCYCL(x - 2*cnt - 1)))); if(abs(v) > CLUSTER_MAXDELTA) continue; x = XCYCL(x - 2*cnt); pa =
	   (uchar*)alt_map + y*map_size_x + x; pf = (uchar*)surf_map + y*map_size_x + x; h += v/2; h -=
	   CLUSTER_HOLE; dl1 = GET_DELTA(*((uchar*)surf_map + offset - 1)); dl2 =
	   GET_DELTA(*((uchar*)surf_map + offset)); while(cnt){ SET_TERRAIN(*(pf + 1),GLASS_TERRAIN);
			...
			for(x = 0,i = 0;x < map_size_x;x += 2,pa += 2,pf += 2,i++){
				...
				if(stateP[i] == 2 && (*pf & DOUBLE_LEVEL)){
					...
					if(cntP[i] <= CLUSTER_SIZE){
						v = (int)*(pa + 1) - (h = (int)*((uchar*)alt_map + (offset = (y - cntP[i] -
	   1)*map_size_x + XCYCL(x + 1)))); if(abs(v) > CLUSTER_MAXDELTA) continue; y -= cntP[i]; pa =
	   (uchar*)alt_map + y*map_size_x + x; pf = (uchar*)surf_map + y*map_size_x + x; h += v/2; h -=
	   CLUSTER_HOLE; dl1 = GET_DELTA(*((uchar*)surf_map + offset - 1)); dl2 =
	   GET_DELTA(*((uchar*)surf_map + offset)); while(cntP[i]){ SET_TERRAIN(*(pf +
	   1),GLASS_TERRAIN); if(h < 0) h = 0; else if(h > 255) h = 255;
							*(pa + 1) = h;
							*pf |= DOUBLE_LEVEL;
							*(pf + 1) |= DOUBLE_LEVEL;
							SET_DELTA(*pf,dl1);
...
void LoadVPR(int ind) {
	...
	if (ff.size() != 2 * 4 + (1 + 4 + 4) * 4 + 2 * (int)net_size + 2 * POWER * 4 +
						 2 * (int)PART_MAX * POWER * 4 + (int)PART_MAX * 4)
...
void buildWorld(void) {
	...
	if (InitLog || (!CGenLog && !MobilityLog && !ShotLog && !GlassLog && !WHLog)) {
		for (i = 0; i < PART_MAX; i++)
			FloodLvl[i] = WaterPrm == -1 ? FLOOD_INIT : WaterPrm;
		r_net_init();
		m_net_init();
