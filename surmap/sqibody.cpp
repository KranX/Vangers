...
void sqElem::remove(void) {
	if (owner)
		owner->restore(
			getX() + xl - height, getY() + yl, xr - xl + height + 1, yr - yl + height + 1
		);
}
...
int sqElem::ishere(int x, int y) {
	if (x >= getX() + xl && x <= getX() + xr && y >= getY() + yl && y <= getY() + yr)
		return 1;
	return 0;
}
...
void sqInputBox::draw(int self) {
	...
	if (self && title) {
		...
		font->draw(
			getX() + x + 1 + (sx - 2 - font->sx * strlen(title)) / 2,
			getY() + y + 1,
			(unsigned char *)title,
			SQ_SYSCOLOR + 6,
			SQ_SYSCOLOR + 2
	...
	if (last != t) {
		if (!(t % 2)) {
			if (!(type & T_FIELD)) {
				...
				cdy = insert ? 1 : 9;
				XGR_Rectangle(
					getX() + x + 2 + offset + index * font->sx,
					getY() + y + 2 + font->sy - cdy,
					font->sx,
					cdy + 1,
					c,
					c,
...
void sqInputString::delcursor(void) {
	if (!(type & T_FIELD)) {
		...
		font->drawchar(
			getX() + x + index * font->sx, getY() + y, str[index], SQ_SYSCOLOR, SQ_SYSCOLOR + 6
		);
	}
}
...
void sqInputString::keytrap(int key) {
		...
		if (index) {
			...
			if (newindex && XKey.Pressed(SDLK_LCTRL)) {
				if (!isalnum(str[newindex]))
					while (newindex && !isalnum(str[newindex]))
						newindex--;
				while (newindex && isalnum(str[newindex]))
					newindex--;
				if (newindex)
					newindex++;
			} else if (dp && newindex == dp)
				newindex--;
		}
		break;
	case SDLK_RIGHT:
		if (index < len - 1) {
			...
			if (newindex < len - 1 && XKey.Pressed(SDLK_LCTRL)) {
				if (isalnum(str[newindex]))
					while (newindex < len - 1 && isalnum(str[newindex]))
						newindex++;
				while (newindex < len - 1 && !isalnum(str[newindex]))
					newindex++;
			} else if (newindex == dp)
	...
	case SDLK_BACKSPACE:
		if (XKey.Pressed(SDLK_LCTRL)) {
			...
			if (!dp) {
				...
				font->draw(
					getX() + x + xadd, getY() + y + yadd, str, SQ_SYSCOLOR, SQ_SYSCOLOR + col
				);
			}
		}
		break;
	case SDLK_DELETE:
		if (!dp) {
			...
			if (index < dp) {
				if (index < dp - 1)
					memmove(str + index, str + index + 1, dp - index - 1);
				str[dp - 1] = ' ';
			}
			if (index > dp) {
				if (index < len - 1)
	...
	default:
		if (fstatus && !index) {
			...
			index = 0;
			if (dp)
				sqset2null(str, len, dec);
			clen = len;
			draw();
		}
		c = key; // !
		if (c == SDLK_MINUS || c == SDLK_KP_MINUS)
			c = '-';
		if (!dp) {
			if ((type & T_STRING) ||
				((type & T_NUMERIC) && !dec && ((isdigit(c) || c == ' ' || c == '-')))) {
				if (index < len - 1) {
					newindex = index + 1;
					if (insert && index < len - 2)
						memmove(str + newindex, str + index, len - index - 1);
				}
				str[index] = c;
				if (insert && index < len - 2)
					font->draw(
						getX() + x + xadd + index * font->sx,
						getY() + y + yadd,
						str + index,
						SQ_SYSCOLOR,
						SQ_SYSCOLOR + col
			...
			if (isdigit(c) || c == '.' || c == ' ' || c == '-') {
				if (c == '.') {
					newindex = dp + 1;
					if (index && index < dp)
						memmove(str + dp - index, str, index), memset(str, ' ', index);
					if (!index || str[dp - 1] == ' ')
						memset(str, ' ', dp - 1), str[dp - 1] = '0';
				} else {
					if (index < dp - 1 && insert)
						memmove(str + index + 1, str + index, dp - index - 1);
					if (index < dp)
						newindex = index + 1;
					if (newindex == dp)
						newindex++;
					if (index > dp && insert && index < len - 1)
...
sqField::sqField(
	...
	: sqInputString(
		  _owner,
		  _x + strlen(_prompt) * _font->sx + 3,
		  _y,
		  _size - strlen(_prompt) * _font->sx - 3,
		  _font,
		  _str,
		  _len,
		  _type,
...
void sqTextButton::draw(int self) {
	...
	//	  c_putspr(getX() + x - 6,getY() + y - 3,ButElem[ind] -> x_size,ButElem[ind] ->
	// y_size,ButElem[4*(2 - height) + ind] -> data,BLACK_FON); 	  c_putspr(getX() + x + sx -
	// 2,getY() + y - 3,ButElem[ind + 1] -> x_size,ButElem[ind + 1] -> y_size,ButElem[4*(2 - height)
	// + ind + 1]
	//-> data,BLACK_FON);

	for (int i = 0; i < 23; i++)
		XGR_LineTo(
			getX() + x + 2, getY() + y - 3 + i, sx - 4, 2, SQ_SYSCOLOR + mask[23 * (height - 1) + i]
		);
	if (!ind)
