...
void SoundPlay(void *lpDSB, int channel, int priority, int cropos, int flags) {
	if (XSoundInitFlag) {
		...
		// if (channel==2)
		//	std::cout<<"SoundPlay:"<<source->name<<" channel:"<<channel<<" cropos:"<<cropos<<"
		// pr1:"<<channels[channel].priority<<" pr2:"<<priority<<"
		// playing:"<<clunk_object->playing(source->name)<<std::endl;
		if (cropos == 255) {
			if (!clunk_object->playing(source->name) || channels[channel].priority > priority) {
				...
				clunk_object->play(
					source->name,
					new clunk::Source(
						source, false, clunk::v3<float>(), 1, 1, channels[channel].pan * 0.0001
					)
				);
				if (flags & DS_LOOPING) {
			...
			if (channels[channel].sound) {
				if (clunk_object->playing(channels[channel].sound->name) &&
		...
		if(channels[channel].sound)
			...
			if(channels[channel].priority < priority)
				...
				clunk_object->play(channels[channel].sound->name, new
		clunk::Source(channels[channel].sound, false)); if(flags & DS_LOOPING){
...
void *GetSound(int channel) {
	// std::cout<<"GetSound:"<<((clunk::Sample *)channels[channel].sound)->name<<"
...
void SoundStop(int channel) {
	if (XSoundInitFlag) {
		...
		if (channels[channel].sound) {
			...
			channels[channel].priority = channels[channel].flags = channels[channel].cropos =
