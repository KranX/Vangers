...
XZIP_FileHeader::XZIP_FileHeader(
	char *fname,
	unsigned offs,
	unsigned size,
	void *ext_ptr,
...
void XZIP_Resource::LoadHeaders(void) {
	...
	while (!fl) {
		...
		case local_file_header_signature:
			...
			p = new XZIP_FileHeader(
				fname, file.tell(), lhdr.uncompressed_size, &lhdr, sizeof(local_file_header)
			);
			fileList.append(p);
...
int XZIP_Resource::open(char *fname, XStream &fh) {
