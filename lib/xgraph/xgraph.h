...
enum XGR_LINEDIR { XGR_LEFT, XGR_TOP, XGR_RIGHT, XGR_BOTTOM };
...
typedef void (*XGR_OutTextHandler)(
	int x,
	int y,
	int col,
	void *text,
...
struct XGR_Screen {
  ...
  private:
	...
	// @caiiycuk TODO: TBD: refactor this methods to accept video buffer as argument (to get rid
	// from usage of XGR_VIDEOBUF)
	...
	friend void
	Draw3DPlane(int x, int y, int size, int shift, unsigned char *buffer, int image, int y_offset);
	friend void smart_putspr(
		unsigned char *data,
		int Xcenter,
		int Ycenter,
		int XsizeB,
	...
	// @caiiycuk TODO: TBD: refactor structures below to use get_default_render_buffer() instead of
	// XGR_VIDEOBUFF
...
struct XGR_MouseButton {
	...
	void SetPress(XGR_MOUSE_HANDLER p) {
		Press = p;
	}
	void SetUnPress(XGR_MOUSE_HANDLER p) {
		UnPress = p;
	}
	void SetDblClick(XGR_MOUSE_HANDLER p) {
		DblClick = p;
	}

	XGR_MouseButton(void) {
		Press = UnPress = DblClick = NULL;
		Pressed = 0;
	}
};
...
struct XGR_Mouse {
	...
	void SetClip(int l, int t, int r, int b) {
		ClipCoords[XGR_LEFT] = l;
		ClipCoords[XGR_TOP] = t;
		ClipCoords[XGR_RIGHT] = r;
		ClipCoords[XGR_BOTTOM] = b;
	}
	...
	void DisablePrompt(void) {
		flags |= XGM_DISABLE_PROMPT;
	}
	void EnablePrompt(void) {
		flags &= ~XGM_DISABLE_PROMPT;
	}

	void SetPromptData(XGR_MousePromptScreen *p) {
		promptData = p;
	}
...
#define XGR_Rectangle(x, y, sx, sy, outcol, incol, mode) \
...
#	define XGR_Rectangle16(x, y, sx, sy, outcol, incol, mode) \
		XGR_Obj.rectangle(x, y, sx, sy, outcol, incol, mode)
#else
...
#	define XGR_PutSpr16a(x, y, sx, sy, p, mode, alpha) \
		XGR_Obj.putspr16a(x, y, sx, sy, p, mode, alpha)
#	define XGR_PutSpr16ap(x, y, sx, sy, p, mode, alpha) \
...
#define XGR_VIDEOBUF XGR_Obj.ScreenBuf
enum XGR_CustomEvents { CursorAnimationEvent };
...
inline uint8_t *XGR_GetVideoLine(int y) {
	return (XGR_Obj.ScreenBuf + XGR_Obj.yOffsTable[y]);
}

inline void XGR_SetPixelFast(int x, int y, int col) {
	XGR_Obj.ScreenBuf[XGR_Obj.yOffsTable[y] + x] = col;
}
