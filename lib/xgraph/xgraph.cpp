...
#define DD_STATE(a)                                                                        \
	{                                                                                      \
		DDrawState = a;                                                                    \
		if (DDrawState != DD_OK)                                                           \
			ErrH.Abort("DirectDraw error...", XERR_USER, DDrawState, ddError(DDrawState)); \
	}
	...
	for (i = 0; i < _sy; i++) {
		for (j = 0; j < _sx; j++) {
			if (s_pBuf[j]) {
				col1 = s_pBuf[j];
				col0 = s_scrBuf[j];
				r = (((col0 & XGR_MASK_R) * alpha + (col1 & XGR_MASK_R) * alpha1)) &
					(XGR_MASK_R << 8);
				g = (((col0 & XGR_MASK_G) * alpha + (col1 & XGR_MASK_G) * alpha1)) &
					(XGR_MASK_G << 8);
				b = (((col0 & XGR_MASK_B) * alpha + (col1 & XGR_MASK_B) * alpha1)) &
					(XGR_MASK_B << 8);
				s_scrBuf[j] = (r + g + b) >> 8;
			}
		}
...
void XGR_Screen::blitRgba(
	uint32_t *dstRgba,
	uint8_t *screenIndexes,
	uint32_t *screen2DRgba,
	uint8_t *screen2DIndexes
) {
	int x, y;
...
void XGR_Screen::flip() {
	if (flags & XGR_INIT) {
		...
		blitRgba(
			...
			get_2d_rgba_render_buffer(),
			get_2d_render_buffer()
		);
...
void XGR_Screen::setpal(void *ptr, int start, int count) {
	...
	{ averageColorPalette.r = 255; if (averageColorPalette.b+averageColorPalette.g>200) {
	...
	// std::cout<<"Set averageColorPalette r:"<<(int)averageColorPalette.r<<"
	// g:"<<(int)averageColorPalette.g<<" b:"<<(int)averageColorPalette.b<<std::endl;
	//}
}
...
static unsigned char XGR_MouseDefFrame[240] = {0x00,
	0x00,
	0x00,
	0x00,
	0x00,
...
static unsigned short XGR_MouseDefFrameHC[240] = {0x000,
	0x000,
	0x000,
	0x000,
	0x000,
		...
		if (flags & XGM_HICOLOR) {
			if (AlphaData) {
				XGR_PutSpr16ap(
					...
					SizeX,
					SizeY,
					FrameBuf + SizeX * SizeY * CurFrame * 2,
					XGR_HIDDEN_FON | XGR_NOCLIP,
					AlphaData + SizeX * SizeY * CurFrame
				);
			} else {
				if (!CurAlpha)
					XGR_PutSpr16(
...
void XGR_Mouse::PutFon(void) {
	...
	if (PosX + SizeX < XGR_MAXX && PosY + SizeY < XGR_MAXY) {
		...
		// std::cout<<"XGR_Mouse::PutFon sx:"<<sx<<" sy:"<<sy<<" SizeX:"<<SizeX<<" SizeY:"<<SizeY<<"
		// PosY:"<<PosY<<std::endl;
		if (sx > SizeX)
			sx = SizeX;
		if (sy > SizeY)
...
int XGR_Mouse::CheckPromptRedraw(int x, int y, int sx, int sy) {
	...
	if (!(flags & XGM_PROMPT_ACTIVE) || !Visible() || (flags & XGM_DISABLE_PROMPT) || !promptData)
		return 0;
...
int XGR_Mouse::NextFrame(void) {
	...
	if (SeqMode != XGM_NONE) {
		...
		if (SeqMode == XGM_PLAY_ONCE && CurFrame == EndSeqFrame)
			SeqMode = XGM_NONE;
	...
	if (!AlphaData && AlphaSeqMode != XGM_NONE) {
		...
		if (AlphaSeqMode == XGM_PLAY_ONCE && CurAlpha == EndSeqAlpha)
			AlphaSeqMode = XGM_NONE;
...
void XGR_Mouse::InitPrompt(void) {
	...
	if (XGR_Obj.get_is_scaled_renderer()) {
		if (prompt_sx > XGR_SCALED_RENDER_SOURCE_X)
			prompt_sx = XGR_SCALED_RENDER_SOURCE_X;
		if (prompt_sy > XGR_SCALED_RENDER_SOURCE_Y)
			prompt_sy = XGR_SCALED_RENDER_SOURCE_Y;
	}
...
void XGR_Mouse::PutPrompt(void) {
	if (promptData && promptData->curText) {
		if (XGR_TextOutFnc)
			(*(XGR_TextOutFnc))(
				PromptX, PromptY, PromptColor, promptData->curText, promptData->curFont, 1, 1
			);
		else
...
void XGR_Mouse::PutPromptFon(void) {
	if (flags & XGM_HICOLOR)
		XGR_PutSpr16(
			PromptX, PromptY, PromptSizeX, PromptSizeY, PromptFon, XGR_BLACK_FON | XGR_NOCLIP
		);
	else
		XGR_PutSpr(
			PromptX, PromptY, PromptSizeX, PromptSizeY, PromptFon, XGR_BLACK_FON | XGR_NOCLIP
...
void XGR_MouseFnc(SDL_Event *p) {
	...
	if (p->type == SDL_EVENT_MOUSE_MOTION) {
		...
		case SDL_BUTTON_LEFT:
			flag = XGM_LEFT_BUTTON;
			break;
		case SDL_BUTTON_MIDDLE:
			flag = XGM_MIDDLE_BUTTON;
			break;
		case SDL_BUTTON_RIGHT:
			flag = XGM_RIGHT_BUTTON;
			break;
		}
