...
std::pair<const char *, const char *> getSideNames() {
	...
	if (currentRto == 5 /*RTO_MAIN_MENU_ID*/) {
		...
		return std::make_pair<>(
			"resource/actint/hd/side/menu_left.png",
			"resource/actint/hd/side/menu_right.png");
	} else if (
		...
		case 0:
			return std::make_pair<>(
				"resource/actint/hd/side/fostral_left.png",
				"resource/actint/hd/side/fostral_right.png");
		case 1:
			return std::make_pair<>(
				"resource/actint/hd/side/glorx_left.png",
				"resource/actint/hd/side/glorx_right.png");
		case 2:
			return std::make_pair<>(
				"resource/actint/hd/side/necross_left.png",
				"resource/actint/hd/side/necross_right.png");
		case 3:
			return std::make_pair<>(
				"resource/actint/hd/side/parafin_left.png",
				"resource/actint/hd/side/parafin_right.png");
		default:;
		}
...
void XGR_RenderSides(SDL_Renderer *renderer) {
	...
	if (sideNames.first != activeSides.first) {
		...
		HDLeftSideTexture =
			sideNames.first ? PNG_CreateTexture(sideNames.first, renderer) : nullptr;
	...
	if (sideNames.second != activeSides.second) {
		...
		HDRightSideTexture =
			sideNames.second ? PNG_CreateTexture(sideNames.second, renderer) : nullptr;
